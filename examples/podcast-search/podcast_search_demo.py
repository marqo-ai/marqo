...
def load_data(file: str, number_data: int) -> dict:
    podcast_data = (
        pd.read_csv(file).head(number_data)[["name", "description"]].to_dict("records")
    )
...
for data in podcast_data:
    ...
    with open(path, "r") as f:
        ...
        data["_id"] = str(
            id_counter
        )  # _id is a special key which is unique to every document
    id_counter += 1
...
mq.index(index_name).add_documents(
    podcast_data,
    tensor_fields=["name", "description", "transcript"],
...
print("Result 5 -", end=" ")
pprint.pprint(
    results["hits"][0]["_highlights"]
)  # [_highlights] will return an empty list if using lexical search
