...
if __name__ == "__main__":
    ...
    documents = data[
        ["s3_http", "_id", "price", "blip_large_caption", "aesthetic_score"]
    ...
    res = client.index(index_name).add_documents(
        documents, client_batch_size=64, tensor_fields=["s3_http"], device=device
    )
    ...
    score_modifiers = {
        "add_to_score": [{"field_name": "aesthetic_score", "weight": 0.02}]
    }

    res = client.index(index_name).search(
        query, device=device, limit=10, score_modifiers=score_modifiers
    )
    ...
    # index the document
    res = client.index(index_name_context).add_documents(
        [document1], tensor_fields=["multimodal"], device=device, mappings=mappings1
    )

    # index the other using a different mappings
    res = client.index(index_name_context).add_documents(
        [document2], tensor_fields=["multimodal"], device=device, mappings=mappings2
    )

    # retrieve the embedding to use as a context for search
    indexed_documents = client.index(index_name_context).get_documents(
        [document1["_id"], document2["_id"]], expose_facets=True
    )
    ...
    context1 = {"tensor": [{"vector": context_vector1, "weight": 0.50}]}

    # create the context for the search
    context2 = {"tensor": [{"vector": context_vector2, "weight": 0.50}]}

    # now search
    query = {"backpack": 1.0}
    res1 = client.index(index_name).search(
        query, device=device, limit=10, context=context1
    )

    res2 = client.index(index_name).search(
        query, device=device, limit=10, context=context2
    )
    ...
    # now index
    res = client.index(index_name_mm_objects).add_documents(
        documents,
