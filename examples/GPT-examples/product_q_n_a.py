...
if __name__ == "__main__":
    ...
    # index the documents
    indexing = mq.index(index_name).add_documents(
        documents, tensor_fields=["cleaned_text"], client_batch_size=64
    )
    ...
    docs = [
        Document(page_content=f"Source [{ind}]:" + t) for ind, t in enumerate(texts)
    ]
    llm = OpenAI(temperature=0.9)
    chain_qa = LLMChain(llm=llm, prompt=qna_prompt())
    llm_results = chain_qa.invoke(
        {"summaries": docs, "question": results["query"]}, return_only_outputs=True
    )
    ...
    references = [
        (str(np.round(scores[i], 2)), texts[i])
        for i in inds[:top_k]
