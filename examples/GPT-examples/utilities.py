...
def truncate_text(text, token_limit, highlight=None):
    ...
    ind_left, ind_right = get_token_indices(
        text, token_limit, method=method, offset=center_ind
    )
...
def test_truncate():
    ...
    text_en = [
        "hello this is a test sentence. this is another one? i think so. maybe throw in some more letteryz....xo"
    ]
    text_rm = [
        "".join(random.choices(string.ascii_uppercase + string.digits, k=10000))
        for _ in range(10)
    ]
...
def test_find_highlight_in_text():
    ...
    for text in texts:
        for _ in range(n_highlights):
            highlight_ind = sorted(
                np.random.choice(list(range(len(text))), 2, replace=False)
            )
...
def test_check_highlights_field():
    ...
    assert all(
        not check_highlights_field(hit, highlight=ResultsFields.highlights)
        for hit in results_lexical[ResultsFields.hits]
    )
    ...
    assert all(
        check_highlights_field(hit, highlight=ResultsFields.highlights)
        for hit in results_tensor[ResultsFields.hits]
    )
