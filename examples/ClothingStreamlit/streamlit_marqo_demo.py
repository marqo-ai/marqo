    ...
    try:
        ...
        with st.spinner("Creating Index..."):
            mq.index("demo-search-index").add_documents(
                shirt_data, tensor_fields=["image", "label"]
            )
...
def main():
    ...
    with st.expander("Search Settings"):
        ...
        with attr_col:
            searchable_attr = st.multiselect(
                "Searchable Attributes", ["Image", "Label"], default=["Label"]
            )
    ...
    if (
        ...
        if search_text != "" and search_text != None:
            ...
            results = mq.index("demo-search-index").search(
                search_image_url, filter_string=create_filter_str(filtering), limit=30
            )
            ...
            results = mq.index("demo-search-index").search(
                uploaded_img_path, filter_string=create_filter_str(filtering), limit=30
            )
    ...
    if st.session_state["results"] != {}:
        if st.session_state["results"]["hits"]:
            ...
            for img in enumerate(st.session_state["results"]["hits"]):
                ...
                if pic is not None:
                    if (img[0] >= st.session_state["page"] * 10) and (
                        img[0] < (st.session_state["page"] * 10 + 10)
                    ):
                        with col[(img[0] % 10) // 2]:
                            if pic.startswith("http://host.docker.internal:8222/"):
                                pic_url = (
                                    "http://localhost:8222/"
                                    + pic.split("http://host.docker.internal:8222/")[1]
                                )
                                image = Image.open(
                                    requests.get(pic_url, stream=True).raw
                                )
