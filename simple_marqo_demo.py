...
def load_index(index_name: str, number_data: int) -> None:
    #    Create an index with a specified name and add data to it.

    #This function first reads a CSV file and converts it into a dictionary.
    #It then modifies the 'image' field to include the full path of each image.
    #Next, it creates an index with the specified name and settings.
    #Finally, it adds the data to the index.

    #:param index_name: The name of the index to be created.
    #:param number_data: The number of records to be added to the index.
    try:
...
def search_index_image(index_name: str, image_name: str):
    # make sure the image is located inside the directory in which the python http server is running
        #Search an index using an image.

    #:param index_name: The name of the index to be searched.
    #:param image_name: The name of the image to be searched in the index.
...
#    Get statistics of an index.:param index_name: The name of the index to get statistics for.
