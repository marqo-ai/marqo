...
- Opt-in `lexicalOperand` parameter for hybrid search ([#1405](https://github.com/marqo-ai/marqo/pull/1405)). You can now explicitly control the lexical query operator (`or`, `and`, `weakAnd`) in hybrid search via the new `lexicalOperand` parameter in `hybridParameters`. This is supported only for semi-structured indexes.
