...
class TestEmbed(BaseCompatibilityTestCase):
    ...
    def prepare(self):
        ...
        self.logger.debug(
            f"Ran prepare method for {self.indexes_to_test_on} inside test class {self.__class__.__name__}")
    ...
    def _compare_embed_results(self, expected_result, actual_result):
        ...
        expected_embeddings = np.array(expected_result.get("embeddings")).reshape(1, -1)
        actual_embeddings = np.array(actual_result.get("embeddings")).reshape(1, -1)

        expected_embeddings = expected_embeddings / np.linalg.norm(expected_embeddings)
        actual_embeddings = actual_embeddings / np.linalg.norm(actual_embeddings)

        if expected_embeddings.shape != actual_embeddings.shape:
            raise ValueError(
                f"The two embeddings must have the same shape. "
                f"Expected shape: {expected_embeddings.shape}, Actual shape: {actual_embeddings.shape}"
            )
        normalised_mean_abs_error = np.mean(np.abs(expected_embeddings - actual_embeddings))

        self.assertLessEqual(
            normalised_mean_abs_error,
            1e-3,
            f"Embeddings do not match within the acceptable threshold. Embedding difference: {normalised_mean_abs_error}"
        )
        self.assertEqual(expected_result.get("content"), actual_result.get("content"),
                         f"Expected results don't match actual results. Expected {expected_result.get('content')} but got {actual_result.get('content')}")
