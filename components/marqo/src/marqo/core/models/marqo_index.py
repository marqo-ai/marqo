...
class SemiStructuredMarqoIndex(UnstructuredMarqoIndex):
    ...
    @property
    def index_supports_custom_score_rerank(self) -> bool:
        """
        Check if the index Vespa schema supports custom score rerankers (marqo__score_*).
        Custom score rerankers require schema template version 2.26.0 or later.
        """
        return self._cache_or_get(
            "index_supports_custom_score_rerank",
            lambda: self.parsed_schema_template_version() >= constants.
