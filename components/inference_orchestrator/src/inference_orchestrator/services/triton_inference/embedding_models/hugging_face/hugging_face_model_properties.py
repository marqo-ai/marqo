...
class HFTritonModelProperties(TritonModelProperties):
    ...
    def _validate_model_input(cls, v: list):
        if len(v) != 3:
            raise ValueError(f"Hugging Face models must have exactly 3 inputs. Received {len(v)} inputs.")
    ...
    def _validate_model_output(cls, v: list):
        if len(v) != 1:
            raise ValueError(f"Hugging Face models must have exactly 1 output. Received {len(v)} inputs.")
        names = [inp.name for inp in v]
