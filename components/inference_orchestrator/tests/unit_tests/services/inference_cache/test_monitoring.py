...
class TestOTELCacheStatsCollector(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        """Set up a single meter provider for all tests in this class."""
    ...
    def tearDownClass(cls):
        """Clean up the meter provider."""
        cls.provider.shutdown()

    def setUp(self):
        # Each test creates its own stats collector
        pass

    def _create_stats_collector(self):
        return OTELCacheStatsCollector(
            curr_size_fn=lambda: 42,
            max_size_fn=lambda: 99,
        )

    def test_otel_stats(self):
        stats_collector = self._create_stats_collector()
        stats_collector.record_get(True, 0.0009)
        stats_collector.record_get(True, 0.0008)
        stats_collector.record_get(False, 0.0004)
        stats_collector.record_set(100, 0.001)
        stats_collector.record_set(200, 0.0005)

        self.reader.force_flush()

        metrics_data = self.reader.get_metrics_data()
        self.assertEqual(
            metrics_data.resource_metrics[0].scope_metrics[0].scope.name,
            "inference_cache_stats",
        )
        ...
        self._assert_histogram(
            metrics[1], "cache_get_latency", bucket_counts={500.0: 1, 1000.0: 2}
        )
        self._assert_histogram(
            metrics[4], "cache_set_latency", bucket_counts={500.0: 1, 1000.0: 1}
        )
    ...
    def _assert_histogram(
        self, metric: Metric, name: str, bucket_counts: Dict[float, int]
    ):
        self.assertEqual(metric.name, name)
        ...
        for bucket in bucket_counts.keys():
            if bucket not in explicit_bounds:
                self.fail(
                    f"histogram {name} bucket {bucket} is not in metrics: {explicit_bounds}"
                )
