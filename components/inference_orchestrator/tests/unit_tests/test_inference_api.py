...
class TestInferenceAPI(unittest.TestCase):
    ...
    def test_vectorise_success(self):
        ...
        inference_request = InferenceRequest(
            ...
            model_config_=ModelConfig(model_name='random'),
            preprocessing_config=TextPreprocessingConfig()
        )
        packed_data = msgpack.packb(inference_request.model_dump(by_alias=True), use_bin_type=True)
    ...
    def test_vectorise_validation_error(self):
        ...
        self.assertIn("detail", unpacked_response)
        self.assertEquals(
            {'type': 'missing', 'loc': ['modality'], 'msg': 'Field required',
             'input': {'invalid_field': 'value'}, 'url': 'https://errors.pydantic.dev/2.11/v/missing'},
    ...
    def test_lifespan_integration_bootstrap_and_shutdown_otel(self, mock_bootstrap_otel):
        ...
        with TestClient(app) as _:
            ...
            mock_bootstrap_otel.assert_called_once_with(app, service_name='marqo-inference')
