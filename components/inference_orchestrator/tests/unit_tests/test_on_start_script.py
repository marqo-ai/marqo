...
from inference_orchestrator.errors.common_errors import StartupSanityCheckError, EnvironmentVariableParsingError
...
class TestOnStartScript(unittest.TestCase):
    ...
    def test_preload_registry_models(self):
        environ_expected_models = [
            ...
            ({"MARQO_MODELS_TO_PRELOAD": '["Marqo/marqo-fashionSigLIP"]'}, ["Marqo/marqo-fashionSigLIP"]),
        ]
        for mock_environ, expected in environ_expected_models:
            ...
            with mock.patch.dict(os.environ, mock_environ, clear=True):
                ...
                loaded_models = {args[0].model_config_.model_name for args, _ in
                                 self.mock_inference.vectorise.call_args_list}
    ...
    def test_preload_custom_models(self):
        ...
        with mock.patch.dict(os.environ, {"MARQO_MODELS_TO_PRELOAD": json.dumps(models_to_preload)}, clear=True):
            self.mock_inference.reset_mock()
            self._help_reload_envs()
            model_caching_script = on_start_script.CacheModels(self.mock_config)
            model_caching_script.run()
            loaded_models = self.mock_inference.vectorise.call_args_list[0].args[0].model_config_.model_name
            loaded_model_properties = self.mock_inference.vectorise.call_args_list[0].args[
                0].model_config_.model_properties
    ...
    def test_missing_punkt_downloaded(self):
        ...
        with mock.patch("inference_orchestrator.on_start_script.nltk.data.find") as mock_find, \
                mock.patch("inference_orchestrator.on_start_script.nltk.download") as mock_nltk_download:
