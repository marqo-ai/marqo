...
class TestUpdateDocumentsUnstructured2_16(BaseCompatibilityTestCase):
    ...
    def prepare(self):
        self.logger.debug(
            f"Creating indexes {self.indexes_to_test_on} in test case: {self.__class__.__name__}"
        )
        ...
        for index in self.indexes_to_test_on:
            ...
            for docs in self.text_docs:
                with self.subTest(indexName=index_name, doc_id=docs["_id"]):
                    get_docs_result = self.client.index(index_name).get_document(
    ...
    def _assert_updates_have_happened(self, result, partial_update_test_case):
        ...
        """
        self.logger.debug(
            f"Verifying updates for document {partial_update_test_case.get('_id')}"
        )
            ...
            if isinstance(expected_value, dict):
                ...
                for key, value in expected_value.items():
                    ...
                    if result.get(nested_field) != value:
                        self.fail(
                            f"Nested dictionary field {nested_field} does not match expected value. "
        ...
        self.logger.debug(
            f"All updates verified successfully for document {partial_update_test_case.get('_id')}"
        )
