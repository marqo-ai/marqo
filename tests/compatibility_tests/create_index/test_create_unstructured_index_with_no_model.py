    ...
    def prepare(self):
        ...
        try:
            ...
            self.client.create_index(
                index_name=self.index_name, settings_dict=self.settings
            )
            all_results[self.index_name] = self.client.index(
                self.index_name
            ).get_settings()
            self.save_results_to_file(all_results)
        except Exception as e:
            raise Exception(
                f"Exception when creating index with name {self.index_name}"
            ) from e
