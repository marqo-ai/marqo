...
class TestEmbed(BaseCompatibilityTestCase):
    indexes_to_test_on = [
        {"indexName": "test_embed_api_index", "model": "hf/e5-base-v2"}
    ]
    ...
    def prepare(self):
        ...
        for index in self.indexes_to_test_on:
            try:
                all_results[index["indexName"]] = self.client.index(
    ...
    def test_embed(self):
        ...
        for index in self.indexes_to_test_on:
            ...
            try:
                ...
                actual_result = self.client.index(index_name).embed(
                    content=[
                        "Men shoes brown",
                        {
                            "Large grey hat": 0.7,
    ...
    def _compare_embed_results(self, expected_result, actual_result):
        self.assertEqual(
            expected_result.get("embeddings"),
            actual_result.get("embeddings"),
            f"Expected embeddings don't match actual embeddings. Expected {expected_result.get('embeddings')} but got {actual_result.get('embeddings')}",
        )
        self.assertEqual(
            expected_result.get("content"),
