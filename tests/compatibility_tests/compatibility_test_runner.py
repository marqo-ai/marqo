...
def run_prepare_mode(version_to_test_against: str):
    logger.info(
        "===================================== RUN PREPARE MODE BEGINS ================================================="
    )
    version_to_test_against = semver.VersionInfo.parse(version_to_test_against)
    logger.debug(
        f"Printing all test cases defined under tests/compatibility_tests/: {BaseCompatibilityTestCase.__subclasses__()}"
    )
    errors = []
    ...
    for test_class in BaseCompatibilityTestCase.__subclasses__():
        if test_class.__name__ in seen_classes:
            logger.info(
                f"Skipping duplicate test class {test_class.__name__} as it has already been processed"
            )
            continue

        # Log to confirm no duplicates
        logger.info(
            f"{test_class.__name__} has NOT been processed yet. Processed classes: {seen_classes}. Processing now."
        )
        ...
        marqo_version_marker = next(  # Checks what version a compatibility test is marked with (ex: @pytest.mark.marqo_version('2.11.0')). If no version is marked, it will skip the test
            (marker for marker in markers if marker.name == "marqo_version"), None
        )
        skip_marker = (
            next(  # Checks if a compatibility test is marked with @pytest.mark.skip
                (marker for marker in markers if marker.name == "skip"), None
            )
        )
        skip_marqo_version_marker = (
            next(  # Checks if a compatibility test is marked with @pytest.mark.skip
        ...
        if not marqo_version_marker:
            logger.info(
                f"No marqo_version marker detected for class {test_class.__name__}, skipping prepare mode for this test class"
            )
            continue

        if skip_marker:
            logger.info(
                f"Detected 'skip' marker for class {test_class.__name__}, skipping prepare mode for this test class"
            )
            continue

        if skip_marqo_version_marker and (
            str(version_to_test_against) in skip_marqo_version_marker.args
        ):
            logger.info(
        ...
        logger.info(
            f"Detected marqo_version '{marqo_version}' for testcase: {test_class.__name__}"
        )
        try:
            if (
                semver.VersionInfo.parse(marqo_version).compare(version_to_test_against)
                <= 0
            ):
                ...
                logger.info(
                    f"Skipping testcase {test_class.__name__} as {marqo_version} > {version_to_test_against}"
                )
        except Exception as e:
            logger.error(
                f"Failed to run prepare mode on testcase: {test_class.__name__}, when test mode runs on this test case, it is expected to fail. The exception was {e}",
                exc_info=True,
            )
            errors.append(
                f"Failed to run prepare mode on testcase: {test_class.__name__}, when test mode runs on this test case, it is expected to fail. Search the class name in the logs to find the exact error."
            )
        logger.info(
            "##################################################################################################"
        )
    ...
    if errors:
        raise RuntimeError(
            f"Some errors occurred while running prepare mode on test cases: {errors}"
        )
...
def run_test_mode(version_to_test_against):
    logger.info(
        f"Beginning test mode on all test cases for version: {version_to_test_against}"
    )
    pytest_args = construct_pytest_arguments(version_to_test_against)
    pytest_result = pytest.main(pytest_args)

    if pytest_result == 0:
        logger.info("Successfully ran test mode on all test cases")
    elif pytest_result == 1:
        raise RuntimeError(
            "Failed to run test mode on some test cases. Check pyTest output for exactly which test cases failed"
        )


def trigger_rollback_endpoint():
    logger.info("Triggering rollback endpoint.")
...
def backwards_compatibility_test(
    from_version: str, to_version: str, to_version_image: str
):
    """
    ...
    try:
        ...
        logger.info(
            f"Starting backwards compatibility tests with from_version: {from_version}, to_version: {to_version}, to_version_image: {to_version_image}"
        )

        # Generate a volume name to be used with the "from_version" Marqo container for state transfer.
        from_version_volume = docker_manager.get_volume_name_from_marqo_version(
            from_version
        )
        ...
        try:
            ...
            raise RuntimeError(
                f"Error running tests in 'prepare' mode across versions on from_version: {from_version}"
            ) from e
        ...
        logger.debug(
            f"Starting Marqo to_version: {to_version} container by transferring state from version {from_version} to {to_version}"
        )
        docker_manager.start_marqo_container_by_transferring_state(
            to_version, from_version, from_version_volume, to_version_image, "ECR"
        )

        logger.info(
            f"Started Marqo to_version: {to_version} container by transferring state"
        )
        # Step 4: Run tests
        try:
            ...
            raise RuntimeError(
                f"Error running tests across versions in 'test' mode on from_version: {from_version}"
            ) from e
        logger.info(
            "Finished running tests in Test mode. THIS MARKS THE END OF BACKWARDS COMPATIBILITY TESTS ACROSS TWO CONTAINERS WITH DIFFERENT VERSIONS"
        )
        ...
        try:
            ...
            raise RuntimeError(
                f"Error running tests in full test run, on to_version: {to_version}."
            ) from e
    except Exception as e:
        raise RuntimeError(
            f"An error occurred while executing backwards compatibility tests, on from_version: {from_version}, to_version: {to_version}, to_version_image: {to_version_image}"
        ) from e
    finally:
        # Stop the to_version container (but don't remove it yet)
        logger.info(
            f"Stopping Marqo to_version ({to_version}) container " + str(to_version)
        )
...
def rollback_test(to_version: str, from_version: str, to_version_image: str):
    ...
    """
    logger.info(
        f"Starting Marqo rollback tests with from_version: {from_version}, to_version: {to_version}, to_version_image: {to_version_image}"
    )
    try:
        load_all_subclasses("tests.compatibility_tests")
        ...
        logger.info(
            f"Generated volume name: {from_version_volume} for from_version: {from_version}"
        )
        ...
        logger.info(
            f"Step 4: Starting Marqo to_version: {to_version} container by transferring state from version: "
        ...
        logger.info(
            f"Step 6: Going back to marqo from_version."
        ...
        prepare_volume_for_rollback(
            target_version=from_version,
            source_volume=from_version_volume,
            source="docker",
        )
        docker_manager.start_marqo_container_by_transferring_state(
            target_version=from_version,
        ...
        logger.info(
            f"Step 7: Running tests in test mode on from_version: {from_version}"
        )
        run_test_mode(
            from_version
        )  # This will validate results from the older indexes added as part of the PREPARE mode above.

        # Step 8: Run prepare and test mode again, on the from_version container.
        logger.info(
            f"Step 8: Running prepare and test mode on the same from_version: {from_version} container"
        )
        run_prepare_mode(from_version)
        run_test_mode(
            from_version
        )  # This will validate results by creating newer indexes and adding documents to them. This is required just so that we know that even after transferring state from an older version, we are able to create new indexes in the older state seamlessly.
        ...
        if semver.VersionInfo.parse(from_version) >= semver.VersionInfo.parse("2.13.0"):
            ...
            try:
                logger.info(
                    f"Running full test suite with from_version: {from_version}"
                )
                run_prepare_mode(from_version)
                run_test_mode(from_version)
            except Exception as e:
                raise RuntimeError(
                    ...
                    f"with from_version: {from_version}, to_version: {to_version}"
                ) from e
...
def prepare_volume_for_rollback(
    ...
    if (
        source == "docker"
    ):  # In case the source is docker, we will directly pull the image using version (ex: marqoai/marqo:2.13.0)
    ...
    logger.info(
        f"Running this command: {' '.join(cmd)} to prepare volume for rollback using from_version: {target_version}"
    )
    try:
        subprocess.run(cmd, check=True)
    except subprocess.CalledProcessError as e:
        raise RuntimeError(
            f"Failed to run command: {' '.join(cmd)} when preparing volume for rollback: {e}"
        ) from e
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode", choices=["backwards_compatibility", "rollback"], required=True
    )
    ...
    try:
        ...
        if from_version >= to_version:
            ...
            raise ValueError(
                f"from_version: {from_version} should be less than to_version: {to_version}"
            )
        ...
        if from_version.major != to_version.major:
            logger.error(
                f"from_version {from_version} & to_version {to_version} cannot "
    ...
    try:
        if args.mode == "backwards_compatibility":
            backwards_compatibility_test(
                args.from_version, args.to_version, args.to_image
            )
        ...
        logger.exception(
            f"Encountered an exception: {e} while running tests in mode {args.mode}, exiting",
            exc_info=True,
        )
        sys.exit(1)
