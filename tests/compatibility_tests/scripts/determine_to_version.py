...
def determine_to_version(run_commit_hash: str, current_marqo_version: str):
    ...
    tag = (
        subprocess.check_output(
            ["git", "tag", "--list", f"{current_marqo_version}"], text=True
        ).splitlines()
    )  # Determine if tags exist for current_marqo_version picked from version.py file
    if tag:  # If tag already exists for the current_marqo_version, it means that this version is already released and we are working towards the next version release, thus we need to treat this commit as commit of the next version release.
        try:
            tag_commit_hash = (
                subprocess.check_output(  # Determining commit hash of the tag
                    ["git", "rev-list", "-n", "1", tag[0]], text=True
                ).strip()
            )
            if (
                tag_commit_hash != run_commit_hash
            ):  # If commit hashes don't match, it means that this commit is for the next version, thus we need to set to_version to version.bump_patch().
                to_version = semver.VersionInfo.parse(
                    current_marqo_version
                ).bump_patch()
                return str(to_version)
            elif (
                tag_commit_hash == run_commit_hash
            ):  # If the commit hashes are the same - it means that this could be a manual re-run, in that case no need to set to_version to version.bump_patch().
