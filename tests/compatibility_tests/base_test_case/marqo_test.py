...
class MarqoTestCase(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        local_marqo_settings = {"url": cls._MARQO_URL}
    ...
    def create_indexes(cls, index_settings_with_name: List[Dict]):
        ...
        r = requests.post(
            f"{cls._MARQO_URL}/batch/indexes/create",
            data=json.dumps(index_settings_with_name),
        )
    ...
    def delete_indexes(cls, index_names: List[str]):
        r = requests.post(
            f"{cls._MARQO_URL}/batch/indexes/delete", data=json.dumps(index_names)
        )
    ...
    def clear_indexes(cls, index_names: List[str]):
        for index_name in index_names:
            r = requests.delete(
                f"{cls._MARQO_URL}/indexes/{index_name}/documents/delete-all"
            )
            try:
                r.raise_for_status()
            except requests.exceptions.HTTPError as e:
    ...
    def removeAllModels(cls) -> None:
        ...
        index_names_list: List[str] = [
            item["indexName"] for item in client.get_indexes()["results"]
        ]
        for index_name in index_names_list:
            loaded_models = (
                client.index(index_name).get_loaded_models().get("models", [])
            )
            for model in loaded_models:
                try:
                    client.index(index_name).eject_model(
