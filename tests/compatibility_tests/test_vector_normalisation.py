...
class CompatibilityTestVectorNormalisation(BaseCompatibilityTestCase):
    ...
    def prepare(self):
        ...
        # Add documents
        self.logger.debug(
            f"Creating indexes {self.text_index_with_normalize_embeddings_true}"
        )
        ...
        try:
            ...
            self.logger.debug(
                f"Ran prepare mode test for {self.text_index_with_normalize_embeddings_true} inside test class {self.__class__.__name__}"
            )
        except Exception:
            self.logger.error(
                f"Exception occurred while adding documents / getting documents to / from index {self.text_index_with_normalize_embeddings_true}. When the corresponding test runs, it is expected to fail."
            ...
            with self.subTest(index=index_name):
                ...
                try:
                    with self.subTest(index=index_name):
                        ...
                        self.assertEqual(
                            doc_res_normalized["custom_vector_field_1"],
                            "custom vector text",
                        )
                        self.assertEqual(
                            doc_res_normalized["_tensor_facets"][0][
                                "custom_vector_field_1"
                            ],
                            "custom vector text",
                        )
                        self._compare_results(
                            result_from_prepare_mode, doc_res_normalized
                        )
                except Exception:
    ...
    def _compare_results(self, expected_result, actual_result):
        ...
        self.assertEqual(
            expected_result,
            actual_result,
            f"Results do not match. Expected: {expected_result}, Got: {actual_result}",
        )
