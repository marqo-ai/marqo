        ...
        try:
            ...
            for index in self.indexes_to_test_on:
                if index.get("type") is not None and index.get("type") == "structured":
                    ...
                    self.client.index(index["indexName"]).add_documents(
                        documents=self.text_docs, tensor_fields=["Description"]
                    )

            self.logger.debug(
                f"Finished running prepare method for test case: {self.__class__.__name__}"
            )
        except Exception as e:
            raise Exception(
                f"Exception occurred while adding documents to indexes {e}"
            ) from e
