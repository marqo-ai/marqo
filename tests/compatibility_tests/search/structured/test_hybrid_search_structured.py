...
@pytest.mark.marqo_version("2.10.0")
class TestHybridSearchStructured(BaseCompatibilityTestCase):
    ...
    searchable_attributes = {
        "TENSOR": ["image_field", "multimodal_field"],
        "LEXICAL": ["text_field"],
    }
    ...
    def prepare(self):
        ...
        for index in self.indexes_to_test_on:
            try:
                if index.get("type") is not None and index.get("type") == "structured":
                    self.client.index(index_name=index["indexName"]).add_documents(
                        documents=self.docs
                    )
                else:
                    self.client.index(index_name=index["indexName"]).add_documents(
        ...
        for index in self.indexes_to_test_on:
            ...
            for query, search_method, result_key in zip(
                self.queries, self.search_methods, self.result_keys
            ):
                try:
                    if (
                        index.get("type") is not None
                        and index.get("type") == "structured"
                    ):
                        if search_method == "HYBRID":
                            result = self.client.index(index_name).search(
                                q=query,
                                search_method=search_method,
                                hybrid_parameters=self.hybrid_search_params,
                            )
                        else:
                            result = self.client.index(index_name).search(
                                q=query,
                        ...
                        result = self.client.index(index_name).search(
                            q=query, search_method=search_method
                        )
                    all_results[index_name][result_key] = result
                except Exception:
                    search_errors.append(
                        (query, search_method, index_name, traceback.format_exc())
                    )
        if add_docs_errors:
            failure_message = "\n".join(
                [f"Failure in index {idx}, {error}" for idx, error in add_docs_errors]
            )
            self.logger.error(
                f"Failed to add documents to index: {failure_message}. When the corresponding test runs for this index, it is expected to fail"
            )
        if search_errors:
            failure_message = "\n".join(
                [
        ...
        for index in self.indexes_to_test_on:
            ...
            for query, search_method, result_key in zip(
                ...
                try:
                    ...
                    self._compare_search_results(
                        stored_results[index_name][result_key], result
                    )
    ...
    def _compare_search_results(self, expected_result, actual_result):
        ...
        self.assertEqual(
            expected_result.get("hits"),
            actual_result.get("hits"),
