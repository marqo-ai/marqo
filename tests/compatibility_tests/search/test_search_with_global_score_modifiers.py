...
class TestSearchWithGlobalScoreModifiers(BaseCompatibilityTestCase):
    ...
    hybrid_test_cases = [("disjunction", "rrf")]
        ...
        for index in self.indexes_to_test_on:
            try:
                if index.get("type") is not None and index.get("type") == "structured":
                    ...
                    self.client.index(index_name=index["indexName"]).add_documents(
                        documents=self.docs, tensor_fields=self.tensor_fields
                    )
            except Exception:
            ...
            for retrieval_method, ranking_method in self.hybrid_test_cases:
                try:
                    ...
                    result = self.client.index(index_name).search(
                        ...
                        score_modifiers={
                            "multiply_score_by": [
                                {"field_name": "multiply_1", "weight": 2}
                            ],
                            "add_to_score": [{"field_name": "add_1", "weight": 3}],
                        },
                        rerank_depth=2,  # To show not all results are reranked
                    )
                    all_results[index_name][retrieval_method][ranking_method] = result
                    self.logger.debug(
                        f"Result for {index_name} with retrieval method {retrieval_method} "
        ...
        for index in self.indexes_to_test_on:
            ...
            for retrieval_method, ranking_method in self.hybrid_test_cases:
                try:
                    ...
                    self.assertEqual(
                        stored_results[index_name][retrieval_method][
                            ranking_method
                        ].get("hits"),
                        result.get("hits"),
                    )
