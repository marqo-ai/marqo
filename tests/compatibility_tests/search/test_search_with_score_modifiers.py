...
class TestSearchWithScoreModifiers(BaseCompatibilityTestCase):
    ...
    docs_with_double_and_long_score_modifiers = [
        {
            ...
            "long_score_mods": 3 * 1**39,
        },
        {
            "_id": "2",
            "text_field": "a photo of a cat",
            "double_score_mods": 4.5 * 1**39,
            "long_score_mods": 4 * 1**39,
        },
        {
            "_id": "3",
            "text_field": "a photo of a cat",
            "double_score_mods": 5.5 * 1**39,
            "long_score_mods": 5 * 1**39,
        },
    ...
    def prepare(self):
        ...
        for index in self.indexes_to_test_on:
            ...
            try:
                if index.get("type") is not None and index.get("type") == "structured":
                    ...
                    self.client.index(index_name=index["indexName"]).add_documents(
                        documents=self.docs, tensor_fields=tensor_fields
                    )
            except Exception:
            ...
            for index in self.indexes_to_test_on:
                ...
                try:
                    result = self.client.index(index_name).search(
                        ...
                        score_modifiers={
                            "add_to_score": [
                                {"field_name": type_of_score_modifier, "weight": 2}
                            ]
                        },
                    )
        ...
        if errors:
            failure_message = "\n".join(
                [f"Failure in idx: {idx} : {error}" for idx, error in errors]
            )
            self.logger.error(
    ...
    def test_search_with_double_score_modifier(self):
        ...
        for index in self.indexes_to_test_on:
            ...
            try:
                ...
                self.assertEqual(
                    stored_results[index_name]["double_score_mods"].get("hits"),
                    result.get("hits"),
                )
            except Exception:
    ...
    def test_search_with_long_score_modifier(self):
        ...
        for index in self.indexes_to_test_on:
            ...
            try:
                result = self.client.index(index_name).search(
                    ...
                    score_modifiers={
                        "add_to_score": [
                            {"field_name": "long_score_mods", "weight": 2}
                        ],
                    },
                )
                self.assertEqual(
                    stored_results[index_name]["long_score_mods"].get("hits"),
                    result.get("hits"),
                )
            except Exception:
                test_failures.append((index_name, traceback.format_exc()))
        ...
        if test_failures:
            failure_message = "\n".join(
                [f"Failure in idx: {idx} : {error}" for idx, error in test_failures]
            )
            self.fail(f"Some subtests failed:\n{failure_message}")
    ...
    def test_search_with_int_score_modifier(self):
        ...
        for index in self.indexes_to_test_on:
            ...
            try:
                ...
                self.assertEqual(
                    stored_results[index_name]["rating"].get("hits"), result.get("hits")
                )
            except Exception:
                test_failures.append((index_name, traceback.format_exc()))
    ...
    def test_search_with_float_score_modifier(self):
        ...
        for index in self.indexes_to_test_on:
            ...
            try:
                ...
                self.assertEqual(
                    stored_results[index_name]["popularity"].get("hits"),
                    result.get("hits"),
                )
            except Exception:
                test_failures.append((index_name, traceback.format_exc()))
