        ...
        for method in dir(cls):
            if method.startswith("test"):
                setattr(
                    cls,
                    method,
                    (decorator(allowed_configurations))(getattr(cls, method)),
                )
        return cls
...
def rerun_marqo_with_env_vars(env_vars: list = [], calling_class: str = ""):
    ...
    if test_config == "CPU_LOCAL_MARQO":
        ...
        raise RuntimeError(
            f"Invalid testing configuration: {test_config}. "
    ...
    full_script_path = (
        f"{os.environ['MARQO_API_TESTS_ROOT']}/scripts/{start_script_name}"
    )
...
def attach_docker_logs(
    container_name: str, log_collection: typing.List, start_time: str = None
) -> None:
...
def retrieve_docker_logs(
    ...
    if docker_log_failure_message in log_collection[0]:
        raise RuntimeError(
            f"{docker_log_fetcher.__name__} encountered an "
