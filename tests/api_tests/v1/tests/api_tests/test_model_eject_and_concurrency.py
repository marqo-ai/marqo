    ...
    def test_sequentially_search(self):
        ...
        for index_name in list(self.index_model_object):
            self.client.index(index_name).search(
                q="What is the best outfit to wear on the moon?", device=self.device
            )
            time.sleep(5)
...
class TestConcurrencyRequestsBlock(MarqoTestCase):
    ...
    def normal_search(self, index_name, q):
        ...
        try:
            res = self.client.index(index_name).search(
                "what is best to wear on the moon?", device=self.device
            )
            if len(res["hits"]) == 2:
    ...
    def racing_search(self, index_name, q):
        ...
        try:
            ...
            if (
                "Request rejected, as this request attempted to update the model cache,"
                in str(e)
            ):
    ...
    def test_concurrent_search_with_cache(self):
        ...
        for i in range(2):
            t = threading.Thread(
                target=self.normal_search, args=(self.index_name, normal_search_queue)
            )
            threads.append(t)
            t.start()
    ...
    def test_concurrent_search_without_cache(self):
        ...
        for i in range(2):
            t = threading.Thread(
                target=self.racing_search, args=(self.index_name, racing_search_queue)
            )
            threads.append(t)
            t.start()
        ...
        while not racing_search_queue.empty():
            assert (
                racing_search_queue.get()
                == "racing search get blocked with correct error"
            )
