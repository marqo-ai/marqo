...
class TestLanguage(MarqoTestCase):
    ...
    def test_hybrid_rrf(self):
        ...
        self.populate_index()
        res = self.client.index(self.multilingual_index_name).search(
            q="canine and feline", search_method="HYBRID"
        )
        self.assertGreater(
            len(res["hits"]), 0, "Should find matches for 'dress' in hybrid default"
        )
    ...
    def test_field_language_override(self):
        ...
        self.assertFalse(
            add_res["errors"], "Should not have errors when adding documents"
        )

        res_pt = self.client.index(self.multilingual_index_name).search(
            q="mole", search_method="LEXICAL", language="pt"
        )

        res_en = self.client.index(self.multilingual_index_name).search(
            q="mole", search_method="LEXICAL", language="en"
        )
        ...
        for language, matches in cases:
            with self.subTest(
                ...
                res = self.client.index(self.multilingual_index_name).search(
                    ...
                    facets={"fields": {"size": {"type": "string"}}},
                    relevance_cutoff={
                        "method": "mean_std_dev",
    ...
    def test_tensor_search_with_language_error(self):
        ...
        with self.assertRaises(MarqoWebError) as e:
            self.client.index(self.multilingual_index_name).search(
                q="dress", search_method="TENSOR", language="en"
            )
