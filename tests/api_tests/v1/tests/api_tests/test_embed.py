...
class TestEmbed(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.unstructured_index_non_e5 = "unstructured_non_e5_" + str(
            uuid.uuid4()
        ).replace("-", "")

        cls.structured_image_index_name = "structured_image_index" + str(
            uuid.uuid4()
        ).replace("-", "")
        cls.unstructured_image_index_name = "unstructured_image_index" + str(
            uuid.uuid4()
        ).replace("-", "")
    ...
    def test_embed_single_string(self):
        ...
        test_cases = [
            ...
            self.unstructured_index_non_e5,
        ]
        ...
        for test_index_name in test_cases:
            with self.subTest(test_index_name):
                ...
                d1 = {"_id": "doc1", "text_field_1": "Jimmy Butler is the GOAT."}
                res = self.client.index(test_index_name).add_documents(
                    [d1], tensor_fields=tensor_fields
                )
                ...
                if test_index_name == self.unstructured_index_non_e5:
                    embed_res = self.client.index(test_index_name).embed(
                        "Jimmy Butler is the GOAT.", device="cpu"
                    )
                else:
                    embed_res = self.client.index(test_index_name).embed(
                ...
                self.assertTrue(
                    np.allclose(
                        ...
                        retrieved_d1["_tensor_facets"][0]["_embedding"],
                        atol=1e-6,
                    )
                )
    ...
    def test_embed_with_device(self):
        ...
        for test_index_name in test_cases:
            with self.subTest(test_index_name):
                ...
                if test_index_name == self.unstructured_index_non_e5:
                    embed_res = self.client.index(test_index_name).embed(
                        content="Jimmy Butler is the GOAT.", device="cpu"
                    )
                else:
                    embed_res = self.client.index(test_index_name).embed(
                        content="Jimmy Butler is the GOAT.",
                        device="cpu",
                        content_type="document",
                    )
    ...
    def test_embed_single_dict(self):
        ...
        for test_index_name in test_cases:
            with self.subTest(test_index_name):
                ...
                if test_index_name == self.unstructured_index_non_e5:
                    embed_res = self.client.index(test_index_name).embed(
                        content={"Jimmy Butler is the GOAT.": 1}
                    )
                else:
                    embed_res = self.client.index(test_index_name).embed(
                        content={"Jimmy Butler is the GOAT.": 1},
    ...
    def test_embed_list_content(self):
        ...
        for test_index_name in test_cases:
            with self.subTest(test_index_name):
                ...
                tensor_fields = ["text_field_1"] if "unstr" in test_index_name else None
                d1 = {"_id": "doc1", "text_field_1": "Jimmy Butler is the GOAT."}
                d2 = {"_id": "doc2", "text_field_1": "Alex Caruso is the GOAT."}
                res = self.client.index(test_index_name).add_documents(
                    [d1, d2], tensor_fields=tensor_fields
                )
                ...
                self.assertEqual(
                    embed_res["content"],
                    [{"Jimmy Butler is the GOAT.": 1}, "Alex Caruso is the GOAT."],
                )
                self.assertTrue(
                    np.allclose(
                        embed_res["embeddings"][0],
                        retrieved_docs["results"][0]["_tensor_facets"][0]["_embedding"],
                        atol=1e-6,
                    )
                )
                self.assertTrue(
                    np.allclose(
                        embed_res["embeddings"][1],
                        retrieved_docs["results"][1]["_tensor_facets"][0]["_embedding"],
                        atol=1e-6,
                    )
                )
        ...
        for index_name in [
            ...
            for kwargs in kwargs_list:
                with self.subTest(f"{index_name} - {kwargs}"):
                    res = self.client.index(index_name).embed(content, **kwargs)
