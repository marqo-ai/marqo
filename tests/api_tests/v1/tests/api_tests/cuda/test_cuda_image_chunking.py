...
def generate_structured_index_settings_dict(
    index_name, image_preprocessing_method
) -> Dict:
    return {
...
def generate_unstructured_index_settings_dict(
    index_name, image_preprocessing_method
) -> Dict:
    return {
        "indexName": index_name,
...
class TestUnstructuredImageChunking(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.unstructured_no_image_processing_index_name = (
            "unstructured_no_image_processing_index_name"
            + str(uuid.uuid4()).replace("-", "")
        )
        cls.unstructured_simple_image_processing_index_name = (
            "unstructured_simple_image_processing_index_name"
            + str(uuid.uuid4()).replace("-", "")
        )
        cls.unstructured_frcnn_image_processing_index_name = (
            "unstructured_frcnn_image_processing_index_name"
            + str(uuid.uuid4()).replace("-", "")
        )
        cls.unstructured_dino_v1_image_processing_index_name = (
            "unstructured_dino_v1_image_processing_index_name"
            + str(uuid.uuid4()).replace("-", "")
        )
        cls.unstructured_dino_v2_image_processing_index_name = (
            "unstructured_dino_v2_image_processing_index_name"
            + str(uuid.uuid4()).replace("-", "")
        )
        cls.unstructured_marqo_yolo_image_processing_index_name = (
            "unstructured_marqo_yolo_image_processing_index_name"
            + str(uuid.uuid4()).replace("-", "")
        )
        ...
        cls.structured_simple_image_processing_index_name = (
            "structured_simple_image_processing_index_name"
            + str(uuid.uuid4()).replace("-", "")
        )
        cls.structured_frcnn_image_processing_index_name = (
            "structured_frcnn_image_processing_index_name"
            + str(uuid.uuid4()).replace("-", "")
        )
        cls.structured_dino_v1_image_processing_index_name = (
            "structured_dino_v1_image_processing_index_name"
            + str(uuid.uuid4()).replace("-", "")
        )
        cls.structured_dino_v2_image_processing_index_name = (
            "structured_dino_v2_image_processing_index_name"
            + str(uuid.uuid4()).replace("-", "")
        )
        cls.structured_marqo_yolo_image_processing_index_name = (
            "structured_marqo_yolo_image_processing_index_name"
            + str(uuid.uuid4()).replace("-", "")
        )
        ...
        cls.create_indexes(
            [
                ...
                generate_structured_index_settings_dict(
                    cls.structured_simple_image_processing_index_name, "simple"
                ),
                generate_structured_index_settings_dict(
                    cls.structured_frcnn_image_processing_index_name, "frcnn"
                ),
                generate_structured_index_settings_dict(
                    cls.structured_dino_v1_image_processing_index_name, "dino-v1"
                ),
                generate_structured_index_settings_dict(
                    cls.structured_dino_v2_image_processing_index_name, "dino-v2"
                ),
                generate_structured_index_settings_dict(
                    cls.structured_marqo_yolo_image_processing_index_name, "marqo-yolo"
                ),
            ]
        )
        ...
        cls.create_indexes(
            [
                generate_unstructured_index_settings_dict(
                    cls.unstructured_no_image_processing_index_name, None
                ),
                generate_unstructured_index_settings_dict(
                    cls.unstructured_simple_image_processing_index_name, "simple"
                ),
                generate_unstructured_index_settings_dict(
                    cls.unstructured_frcnn_image_processing_index_name, "frcnn"
                ),
                generate_unstructured_index_settings_dict(
                    cls.unstructured_dino_v1_image_processing_index_name, "dino-v1"
                ),
                generate_unstructured_index_settings_dict(
                    cls.unstructured_dino_v2_image_processing_index_name, "dino-v2"
                ),
                generate_unstructured_index_settings_dict(
                    cls.unstructured_marqo_yolo_image_processing_index_name,
                    "marqo-yolo",
    ...
    def test_image_no_chunking(self):
        ...
        test_case = [
            (
                self.unstructured_no_image_processing_index_name,
                {"tensor_fields": ["image_content"]},
                {"searchable_attributes": ["image_content"]},
        ...
        for index_name, add_docs_call, search_call, msg in test_case:
            with self.subTest(msg):
                self.client.index(index_name).add_documents(
                    [document], device="cuda", **add_docs_call
                )

                # test the search works
                results = self.client.index(index_name).search(
                    "a", device="cuda", **search_call
                )
                self.assertEqual(temp_file_name, results["hits"][0]["image_content"])
                # the highlight should be the location
                self.assertEqual(
                    temp_file_name,
                    results["hits"][0]["_highlights"][0]["image_content"],
                )
    ...
    def test_image_simple_chunking(self):
        ...
        test_case = [
            (
                self.unstructured_simple_image_processing_index_name,
                {"tensor_fields": ["image_content"]},
                {"searchable_attributes": ["image_content"]},
                "unstructured_index, simple image preprocessing",
            ),
            (
                self.structured_simple_image_processing_index_name,
                {},
                {"searchable_attributes": ["image_content"]},
    ...
    def test_image_frcnn_chunking(self):
        ...
        test_case = [
            (
                self.unstructured_frcnn_image_processing_index_name,
                {"tensor_fields": ["image_content"]},
                {"searchable_attributes": ["image_content"]},
                "unstructured_index, frcnn image preprocessing",
            ),
            (
                self.structured_frcnn_image_processing_index_name,
                {},
                {"searchable_attributes": ["image_content"]},
    ...
    def test_image_dino_v1_chunking(self):
        ...
        test_case = [
            (
                self.unstructured_dino_v1_image_processing_index_name,
                {"tensor_fields": ["image_content"]},
                {"searchable_attributes": ["image_content"]},
                "unstructured_index, dino_v1 image preprocessing",
            ),
            (
                self.structured_dino_v1_image_processing_index_name,
                {},
                {"searchable_attributes": ["image_content"]},
    ...
    def test_image_dino_v2_chunking(self):
        ...
        test_case = [
            (
                self.unstructured_dino_v2_image_processing_index_name,
                {"tensor_fields": ["image_content"]},
                {"searchable_attributes": ["image_content"]},
                "unstructured_index, dino_v2 image preprocessing",
            ),
            (
                self.structured_dino_v2_image_processing_index_name,
                {},
                {"searchable_attributes": ["image_content"]},
    ...
    def test_image_marqo_yolo_chunking(self):
        ...
        test_case = [
            (
                self.unstructured_marqo_yolo_image_processing_index_name,
                {"tensor_fields": ["image_content"]},
                {"searchable_attributes": ["image_content"]},
                "unstructured_index, marqo_yolo image preprocessing",
            ),
            (
                self.structured_marqo_yolo_image_processing_index_name,
                {},
                {"searchable_attributes": ["image_content"]},
