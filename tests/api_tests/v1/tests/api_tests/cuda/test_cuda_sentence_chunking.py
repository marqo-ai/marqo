...
class TestCudaSentenceChunking(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.large_structured_index_name = "structured_large" + str(
            uuid.uuid4()
        ).replace("-", "")
        ...
        cls.overlap_structured_index_name = "structured_overlap" + str(
            uuid.uuid4()
        ).replace("-", "")

        cls.large_unstructured_index_name = "unstructured_large" + str(
            uuid.uuid4()
        ).replace("-", "")
        cls.standard_unstructured_index_name = "unstructured_standard" + str(
            uuid.uuid4()
        ).replace("-", "")
        cls.overlap_unstructured_index_name = "unstructured_overlap" + str(
            uuid.uuid4()
        ).replace("-", "")
    ...
    def test_sentence_no_chunking(self):
        ...
        for index_name in [
            self.large_structured_index_name,
            self.large_structured_index_name,
        ]:
            with self.subTest(index_name):
                self.client.index(index_name).add_documents(
                    [document],
                    tensor_fields=unstructured_tensor_fields
                ...
                results = self.client.index(index_name).search(
                    "hello how are you", device="cuda"
                )
                self.assertEqual(
                    document["text_field_1"], results["hits"][0]["text_field_1"]
                )
                self.assertEqual(
                    document["text_field_2"], results["hits"][0]["text_field_2"]
                )
                self.assertEqual(
                    document["text_field_3"], results["hits"][0]["text_field_3"]
                )
                self.assertEqual(
                    document["text_field_1"],
                    results["hits"][0]["_highlights"][0]["text_field_1"],
                )
    ...
    def test_sentence_chunking_no_overlap(self):
        test_cases = [
            ...
            (
                "sasasasaifjfnonfqeno asadsdljknjdfln",
                "sasasasaifjfnonfqeno asadsdljknjdfln",
            ),
            ...
            for search_term, expected_highlights_chunk in test_cases:
                with self.subTest(f"{search_term}, {index_name}"):
                    ...
                    res = self.client.index(index_name).search(
                        search_term, device="cuda"
                    )
                    returned_highlights = list(
                        res["hits"][0]["_highlights"][0].values()
                    )[0]
    ...
    def test_sentence_chunking_overlap(self):
        ...
        for index_name in [
            self.overlap_unstructured_index_name,
            self.overlap_unstructured_index_name,
        ]:
            for search_term, expected_highlights_chunk in test_cases:
                with self.subTest(f"{search_term}, {index_name}"):
