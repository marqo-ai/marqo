...
class TestCudaStructuredSearch(MarqoTestCase):
    ...
    filter_test_index_name = "api_test_structured_filter_index_filter" + str(
        uuid.uuid4()
    ).replace("-", "")
    ...
    def test_search_single_doc(self):
        ...
        add_doc_res = self.client.index(self.text_index_name).add_documents(
            [d1], device="cuda"
        )
        ...
        assert ("title" in search_res["hits"][0]["_highlights"][0]) or (
            "content" in search_res["hits"][0]["_highlights"][0]
        )
    ...
    def test_search_multi_docs(self):
        ...
        assert (
            search_res["hits"][0]["_highlights"][0]["content"] == "this is a solid doc"
        )
    ...
    def test_select_lexical(self):
        ...
        search_res = self.client.index(self.text_index_name).search(
            ...
            search_method=enums.SearchMethods.TENSOR,
            device="cuda",
        )
        assert d2 == self.strip_marqo_fields(search_res["hits"][0], strip_id=False)
        assert search_res["hits"][0]["_highlights"][0]["title"].startswith(
            "The captain bravely lead her followers"
        )
        ...
        assert (
            self.client.index(self.text_index_name).search(
                "captain", device="cuda", search_method=marqo.SearchMethods.LEXICAL
            )["hits"][0]["_id"]
            == "123456"
        )
    ...
    def test_filter_string_and_searchable_attributes(self):
        ...
        res = self.client.index(
            self.filter_test_index_name,
        ).add_documents(docs, device="cuda")
        ...
        for case in test_cases:
            ...
            with self.subTest(
                query=query, filter_string=filter_string, expected=expected
            ):
                search_res = self.client.index(
                    self.filter_test_index_name,
                ).search(query, filter_string=filter_string, device="cuda")
    ...
    def test_custom_search_results(self):
        ...
        query = {"What are the best pets": 1}
        context = {
            "tensor": [
                {
                    "vector": [
        ...
        original_res = self.client.index(self.image_index_name).search(
            q=query, device="cuda"
        )
        custom_res = self.client.index(self.image_index_name).search(
            q=query, context=context, device="cuda"
        )
...
class TestCudaUnstructuredSearch(MarqoTestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            cls.delete_indexes(
                ["api_test_unstructured_index", "api_test_unstructured_image_index"]
            )
        except Exception:
            pass
        ...
        cls.image_index_name = "api_test_unstructured_image_index" + str(
            uuid.uuid4()
        ).replace("-", "")
    ...
    def test_search_single_doc(self):
        ...
        add_doc_res = self.client.index(self.text_index_name).add_documents(
            [d1], tensor_fields=["Title", "Description"], device="cuda"
        )
        search_res = self.client.index(self.text_index_name).search(
            "title about some doc", device="cuda"
        )
    ...
    def test_search_multi_docs(self):
        ...
        res = self.client.index(self.text_index_name).add_documents(
            [d1, d2], tensor_fields=["doc_title", "field_1", "field_X"], device="cuda"
        )
        search_res = self.client.index(self.text_index_name).search(
            "this is a solid doc"
        )
        assert d2 == self.strip_marqo_fields(search_res["hits"][0], strip_id=False)
        assert (
            search_res["hits"][0]["_highlights"][0]["field_X"] == "this is a solid doc"
        )
    ...
    def test_select_lexical(self):
        ...
        res = self.client.index(self.text_index_name).add_documents(
            [d1, d2], tensor_fields=["doc_title", "field_1", "field_X"]
        )
        ...
        assert d2 == self.strip_marqo_fields(search_res["hits"][0], strip_id=False)
        assert search_res["hits"][0]["_highlights"][0]["doc_title"].startswith(
            "The captain bravely lead her followers"
        )
    ...
    def test_filter_string_and_searchable_attributes(self):
        ...
        res = self.client.index(self.text_index_name).add_documents(
            docs, tensor_fields=["field_a", "field_b"], device="cuda"
        )
    ...
    def test_escaped_non_tensor_field(self):
        ...
        """
        docs = [
            {"dont_tensorise_Me": "Dog", "tensorise_me": "quarterly earnings report"}
        ]
        self.client.index(index_name=self.text_index_name).add_documents(
            docs, tensor_fields=["tensorise_me"], device="cuda"
        )
        search_res = self.client.index(index_name=self.text_index_name).search(
            "Dog", device="cuda"
        )
