...
class TestCudaUnstructuredAddDocuments(MarqoTestCase):
    ...
    def test_add_documents_with_ids(self):
        ...
        res = self.client.index(self.text_index_name).add_documents(
            [d1, d2], tensor_fields=["doc_title"], device="cuda"
        )
    ...
    def test_add_batched_documents(self):
        ...
        ix.add_documents(
            ...
            tensor_fields=["Title", "Generic_text"],
            device="cuda",
        )
        for _id in [0, 19, 20, 99]:
            original_doc = docs[_id].copy()
    ...
    def test_add_documents_with_ids_twice(self):
        ...
        self.client.index(self.text_index_name).add_documents(
            [d1], tensor_fields=["doc_title"], device="cuda"
        )
        assert d1 == self.client.index(self.text_index_name).get_document("56")
        d2 = {
            "_id": "56",
        ...
        self.client.index(self.text_index_name).add_documents(
            [d2], tensor_fields=["doc_title"], device="cuda"
        )
        assert d2 == self.client.index(self.text_index_name).get_document("56")
    ...
    def test_add_documents_missing_index_fails(self):
        with self.assertRaises(MarqoWebError) as ex:
            self.client.index("a void index").add_documents(
                [{"abd": "efg"}], tensor_fields=[], device="cuda"
            )
        assert "index_not_found" in str(ex.exception.message)
    ...
    def test_add_documents_empty(self):
        ...
        with self.assertRaises(MarqoWebError) as e:
            self.client.index(self.text_index_name).add_documents(
                documents=[], tensor_fields=[], device="cuda"
            )
        assert "bad_request" in str(e.exception.message)
    ...
    def test_add_documents_call_tensor_fields(self):
        ...
        for tensor_fields, msg in test_cases:
            with self.subTest(msg):
                with self.assertRaises(MarqoWebError) as e:
                    self.client.index(self.text_index_name).add_documents(
                        documents=[{"some": "data"}], device="cuda", **tensor_fields
                    )
                assert "bad_request" in str(e.exception.message)
    ...
    def test_add_documents_with_unsupported_modalities(self):
        ...
        with self.subTest("Unsupported modality in add_documents"):
            with self.assertRaises(MarqoWebError) as e:
                self.client.index(
                    self.unstructured_languagebind_index_name_with_limited_supported_modalities
                ).add_documents(
                    documents=documents, tensor_fields=["text_field_1", "image_field_1"]
                )
            self.assertIn(
                "The model does not support the requested modality",
