...
class TestUnstructuredAddDocuments(MarqoTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.text_index_with_normalize_embeddings_true = (
            "api_test_unstructured_index_with_normalize_embeddings_true"
    ...
    def test_add_documents_without_ids(self):
        ...
        d1 = {"doc_title": "Cool Document 1", "field_1": "some extra info"}
        d2 = {"doc_title": "Just Your Average Doc", "field_X": "this is a solid doc"}
        res = self.client.index(self.text_index_name).add_documents(
            [d1, d2], tensor_fields=["doc_title"]
        )
    ...
    def test_add_batched_documents(self):
        ...
        ix.add_documents(
            docs, client_batch_size=10, tensor_fields=["Title", "Generic_text"]
        )
    ...
    def test_add_documents_missing_index_fails(self):
        with self.assertRaises(MarqoWebError) as ex:
            self.client.index("a void index").add_documents(
                [{"abd": "efg"}], tensor_fields=[]
            )
    ...
    def test_add_documents_no_device(self):
        ...
        def run():
            temp_client.index(self.image_index_name).add_documents(
                documents=[{"d1": "blah"}, {"d2", "some data"}], tensor_fields=[]
            )
            return True
    ...
    def test_add_document_multimodal(self):
        ...
        documents = [
            {"title": "test-1", "image_content": image_content, "non_tensor": "test"},
            {"title": "test-2", "image_content": image_content, "content": "test"},
        ]
        ...
        test_cases = [
            ...
            (
                ...
                ["my_multimodal_field", "title", "content"],
                2,
                5,
                "multimodal field with other tensor fields",
            ),
            (
                {
                    "my_multimodal_field": {
            ...
            (
                ...
                ["my_multimodal_field_1", "my_multimodal_field_2"],
                2,
                3,
                "multiple multimodal fields",
            ),
        ]
    ...
    def test_add_documents_call_tensor_fields(self):
        ...
        for tensor_fields, msg in test_cases:
            with self.subTest(msg):
                with self.assertRaises(MarqoWebError) as e:
                    self.client.index(self.text_index_name).add_documents(
                        documents=[{"some": "data"}], **tensor_fields
                    )
                assert "bad_request" in str(e.exception.message)
    ...
    def test_add_docs_with_large_integers_and_floats(self):
        test_documents = [
            ...
            (
                {"long_field_1": 1002321423542},
                False,
            ),  # large positive integer that can't be handled by int
            (
                {"long_field_1": -9232172132345},
                False,
            ),  # large positive integer that can't be handled by int
            (
    ...
    def test_custom_vector_doc_in_normalized_embedding_true(self):
        ...
        add_docs_res_normalized = self.client.index(
            ...
            mappings={"custom_vector_field_1": {"type": "custom_vector"}},
        )
