...
class TestUnstructuredSearch(MarqoTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.text_index_2_name = "api_test_unstructured_index_2_" + str(
            uuid.uuid4()
        ).replace("-", "")
    ...
    def test_search_single_doc(self):
        ...
        add_doc_res = self.client.index(self.text_index_name).add_documents(
            [d1], tensor_fields=["title", "description"]
        )
    ...
    def test_select_lexical(self):
        ...
        assert (
            self.client.index(self.text_index_name).search(
                "captain", search_method=marqo.SearchMethods.LEXICAL
            )["hits"][0]["_id"]
            == "123456"
        )
    ...
    def test_search_with_no_device(self):
        ...
        def run():
            ...
            temp_client.index(self.text_index_name).search(
                q="my search term", device="cuda:2"
            )
            return True
    ...
    def test_filter_string_and_searchable_attributes(self):
        ...
        res = self.client.index(self.text_index_name).add_documents(
            docs, tensor_fields=["field_a", "field_b"]
        )
    ...
    def test_filter_on_large_integer_and_float(self):
        ...
        for document in valid_documents:
            for search_method in [SearchMethods.LEXICAL, SearchMethods.TENSOR]:
                ...
                numeric_value = (
                    document[numeric_field]
                    if isinstance(document[numeric_field], (int, float))
                ...
                with self.subTest(
                    ...
                    res = self.client.index(self.text_index_name).search(
                        q="some text",
                        filter_string=filter_string,
                        search_method=SearchMethods.LEXICAL,
                    )
    ...
    def test_filter_on_id(self):
        ...
        test_case = [
            ...
            (
                "_id:e197e580-039 OR _id:123456",
                ["e197e580-039", "123456"],
                "multiple _id filter with OR",
            ),
            ("_id:e197e580-039 AND _id:123456", [], "multiple _id filter with AND"),
            (
                "_id:e197e580-039 AND title:(Cool Document 1)",
                ["e197e580-039"],
        ...
        for search_method in ["TENSOR", "LEXICAL"]:
            for filter_string, expected, msg in test_case:
                with self.subTest(f"{search_method} - {msg}"):
                    search_res = self.client.index(self.text_index_name).search(
                        q="*", search_method=search_method, filter_string=filter_string
                    )
                    actual_ids = set([hit["_id"] for hit in search_res["hits"]])
                    self.assertEqual(
                        len(search_res["hits"]),
    ...
    def test_searchable_attributes(self):
        ...
        self.client.index(self.text_index_2_name).add_documents(
            docs_batch_1, tensor_fields=["title", "content"]
        )

        docs_batch_2 = [
            {"desc": "Cool Document 2", "content": "some extra info", "_id": "3"},
            {
        ...
        self.client.index(self.text_index_2_name).add_documents(
            docs_batch_2, tensor_fields=["desc", "content"]
        )
        ...
        self.client.index(self.text_index_2_name).add_documents(
            docs_batch_3, tensor_fields=["content"]
        )
