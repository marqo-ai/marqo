...
class TestUpdateDocumentsInUnstructuredIndex(MarqoTestCase):
    ...
    def test_update_document_with_ids(self):
        ...
        add_docs_response = self.client.index(self.text_index_name).add_documents(
            documents=text_docs, mappings=mappings, tensor_fields=tensor_fields
        )
        ...
        get_docs_response = self.client.index(self.text_index_name).get_document(
            document_id="1"
        )
    ...
    def test_add_new_fields_with_update_documents_api(self):
        ...
        add_docs_response = self.client.index(self.text_index_name).add_documents(
            documents=initial_doc, tensor_fields=["tensor_field"]
        )
        self.assertFalse(add_docs_response["errors"])
        ...
        update_doc = {
            ...
            "new_bool_field": True,
            "new_int_map": {"key1": 100, "key2": 200},
            "new_float_map": {"pi": 3.14, "e": 2.718},
        }
        update_docs_response = self.client.index(self.text_index_name).update_documents(
            [update_doc]
        )
    ...
    def test_add_new_tensor_field_with_update_documents_api(self):
        ...
        update_docs_response = self.client.index(self.text_index_name).update_documents(
            [{"_id": "tensor_update_doc", "new_tensor_field": "This should fail"}]
        )

        # Verify that errors were returned
        self.assertTrue(
            update_docs_response["errors"],
        ...
        self.assertIn(
            "new_tensor_field of type str does not exist in the original document. "
    ...
    def test_add_new_string_array_with_update_documents_api(self):
        ...
        update_docs_response = self.client.index(self.text_index_name).update_documents(
            [{"_id": "string_array_update_doc", "new_string_array": ["item1", "item2"]}]
        )
    ...
    def test_add_new_string_field_with_update_documents_api(self):
        ...
        # Assert the error message
        self.assertIn(
            "new_string_field of type str does not exist in the original document. "
            "Marqo does not support adding new lexical fields in partial updates",
            error_details["error"],
        )
    ...
    def test_update_document_and_change_field_type(self):
        ...
        type_change_scenarios = [
            ...
            },
            {
                "field": "int_map",
                "new_value": {"a": 2.5},
                "original_type": "int_map",
                "new_type": "float_map",
            },
            {
                "field": "float_map",
                "new_value": {"c": 3},
                "original_type": "float_map",
                "new_type": "int_map",
            },
            {
                "field": "string_array",
                "new_value": 123,
                "original_type": "string_array",
                "new_type": "int",
            },
        ...
        for scenario in type_change_scenarios:
            with self.subTest(
                f"Changing {scenario['field']} from {scenario['original_type']} to {scenario['new_type']}"
            ):
                update_payload = {"_id": "1", scenario["field"]: scenario["new_value"]}

                update_docs_response = self.client.index(
                    self.text_index_name
                ).update_documents([update_payload])
                ...
                if scenario["original_type"] == "bool":
                    self.assertIn(
                        "bool_field of type str does not exist in the original document. Marqo does not support adding new lexical fields in partial updates",
    ...
    def test_concurrent_partial_update_requests_on_maps(self):
        ...
        def update_rank_thread(index_name, rank_values):
            for i, new_rank in enumerate(rank_values):
                ...
                print(
                    f"[{timestamp}] Rank update {i + 1}/{len(rank_values)}: Setting rank to {new_rank}"
                )
                r = self.client.index(index_name).update_documents(
                    [{"_id": "3", "score_map": {"rank": new_rank}}]
                )
                timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        ...
        def update_popularity_thread(index_name, popularity_values):
            for i, new_pop in enumerate(popularity_values):
                ...
                r = self.client.index(index_name).update_documents(
                    [{"_id": "3", "score_map": {"popularity": new_pop}}]
                )
                timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                # We can't be sure that the response will be error-free due to concurrent updates. So we will not check that here
                print(
                    f"[{timestamp}] Popularity update {i + 1} complete. Response: {r}"
                )
        ...
        rank_thread = threading.Thread(
            target=update_rank_thread, args=(self.text_index_name, rank_values)
        )
        pop_thread = threading.Thread(
            target=update_popularity_thread,
            args=(self.text_index_name, popularity_values),
        )
        ...
        updated_doc = self.client.index(self.text_index_name).get_document(
            document_id="3"
        )
        ...
        self.assertTrue(
            has_rank or has_popularity, "Neither rank nor popularity field is present"
        )
        self.assertTrue(
            has_rank != has_popularity,
            "Both rank and popularity fields are present. Only one value should be present",
        )

        # If rank is present, verify it's one of the rank values
        if has_rank:
            self.assertIn(
                updated_doc["score_map.rank"],
                rank_values,
        ...
        if has_popularity:
            self.assertIn(
                updated_doc["score_map.popularity"],
                popularity_values,
        ...
        self.assertEqual(
            ...
            "This document will be updated by multiple threads",
        )
        ...
        base_search_result = self.client.index(self.text_index_name).search(
            "concurrent update"
        )
        base_score = base_search_result["hits"][0]["_score"]

        if has_rank:
            search_result = self.client.index(self.text_index_name).search(
                "concurrent update",
                score_modifiers={
            ...
            self.assertAlmostEqual(
                hit["_score"], base_score + 1 * updated_doc["score_map.rank"], places=5
            )
        ...
        if has_popularity:
            search_result = self.client.index(self.text_index_name).search(
                ...
                score_modifiers={
                    "add_to_score": [
                        {"field_name": "score_map.popularity", "weight": 1}
                    ]
                },
            )
            ...
            self.assertAlmostEqual(
                hit["_score"],
                base_score + 1 * updated_doc["score_map.popularity"],
                places=5,
            )
    ...
    def test_concurrent_partial_update_requests_on_numeric_fields(self):
        ...
        # Either rank or popularity should be present, but not both
        self.assertTrue(
            has_rank and has_popularity, "Neither rank nor popularity field is present"
        )
        ...
        search_result = self.client.index(self.text_index_name).search(
            "concurrent update",
            score_modifiers={"add_to_score": [{"field_name": "rank", "weight": 1}]},
        )

        self.assertTrue(len(search_result["hits"]) > 0, "No search results found")
        hit = search_result["hits"][0]
        self.assertAlmostEqual(
            hit["_score"], base_score + 1 * updated_doc["rank"], places=5
        )
        ...
        hit = search_result["hits"][0]
        self.assertAlmostEqual(
            hit["_score"], base_score + 1 * updated_doc["popularity"], places=5
        )
    ...
    def test_update_document_with_changes_in_score_modifiers(self):
        ...
        search_result_weight_0 = self.client.index(self.text_index_name).search(
            "title",
            score_modifiers={
        ...
        self.assertAlmostEqual(
            search_result_weight_0["hits"][0]["_score"], base_score, places=5
        )

        # Search with score modifier weight=1 (should add int_map.d value to score)
        search_result_weight_1 = self.client.index(self.text_index_name).search(
            "title",
            score_modifiers={
        ...
        self.assertAlmostEqual(
            search_result_weight_1["hits"][0]["_score"], base_score + 5, places=5
        )
        ...
        update_docs_response_2 = self.client.index(
            self.text_index_name
        ).update_documents(
            [
                {
        ...
        search_result_after_update = self.client.index(self.text_index_name).search(
            "title",
            score_modifiers={
                "add_to_score": [{"field_name": "int_map.d", "weight": 1}]
            },
        ...
        self.assertAlmostEqual(
            search_result_after_update["hits"][0]["_score"], base_score + 10, places=5
        )
    ...
    def test_concurrent_mixed_add_documents_and_update_documents_requests(self):
        ...
        def update_documents_thread(index_name):
            ...
            for i in range(10):
                timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                print(
                    f"[{timestamp}] update_documents thread - iteration {i + 1}/10: Updating int_field and metadata"
                )
                r = self.client.index(index_name).update_documents(
                    [
                        {
                ...
                self.assertTrue(
                    r["errors"]
                )  # We can be sure that the response will be error-free due to concurrent updates and the updates sent by the other add documents
                # thread
                timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                print(
                    f"[{timestamp}] update_documents thread - iteration {i + 1} complete. Response: {r}"
                )
        ...
        def add_documents_thread(index_name):
            ...
            for i in range(10):
                timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                print(
                    f"[{timestamp}] add_documents thread - iteration {i + 1}/10: Replacing document"
                )
                ...
                timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                print(
                    f"[{timestamp}] add_documents thread - iteration {i + 1} complete. Response: {r}"
                )
                time.sleep(0.5)  # Delay between updates

        # Create and start threads
        update_thread = threading.Thread(
            target=update_documents_thread, args=(self.text_index_name,)
        )
        add_thread = threading.Thread(
            target=add_documents_thread, args=(self.text_index_name,)
        )
        ...
        final_doc = self.client.index(self.text_index_name).get_document(
            document_id="4"
        )
        ...
        self.assertIn(
            "This document was replaced in iteration 10 inside an add_documents thread",
    ...
    def test_partial_update_new_map_field(self):
        ...
        add_docs_response = self.client.index(self.text_index_name).add_documents(
            documents=text_docs, mappings={}, tensor_fields=["text"]
        )
        ...
        get_docs_result = self.client.index(
            index_name=self.text_index_name
        ).get_document(document_id="4")
        self.assertIsNone(get_docs_result.get("metadata.key1"))
        self.assertEqual(get_docs_result["metadata.key2"], 100.5)

        base_search_result = self.client.index(index_name=self.text_index_name).search(
            "test document"
        )
        base_score = base_search_result["hits"][0]["_score"]

        search_result_with_non_existent_score_modifier = self.client.index(
            index_name=self.text_index_name
        ).search(
            "test document",
            score_modifiers={
        ...
        score_with_non_existent_score_modifier = (
            search_result_with_non_existent_score_modifier["hits"][0]["_score"]
        )
        ...
        search_with_existing_score_modifier = self.client.index(
            index_name=self.text_index_name
        ).search(
            "test document",
            score_modifiers={
        ...
        score_with_existing_score_modifier = search_with_existing_score_modifier[
            "hits"
        ][0]["_score"]
        self.assertAlmostEqual(
            score_with_existing_score_modifier, base_score + 1 * 100.5, 5
        )
