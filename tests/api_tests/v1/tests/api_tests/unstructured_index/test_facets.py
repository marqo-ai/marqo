...
from tests.marqo_test import EXAMPLE_FASHION_DOCUMENTS, MarqoTestCase
...
class TestFacets(MarqoTestCase):
    ...
    def setUp(self):
        ...
        self.client.index(self.unstructured_text_index_name).add_documents(
            EXAMPLE_FASHION_DOCUMENTS, tensor_fields=["title", "description"]
        )
    ...
    def test_facets_structured_index_fails(self):
        ...
        self.assertIn(
            "Facets are only supported for unstructured indexes", str(e.exception)
        )
    ...
    def test_facets_non_hybrid_search_fails(self):
        ...
        for search_method in ["lexical", "tensor"]:
            ...
            self.assertIn(
                "Facets can only be provided for 'HYBRID' search", str(e.exception)
            )
            ...
            self.client.index(self.unstructured_text_index_name).search(
                ...
                facets={
                    "fields": {
                        "price": {
                            "type": "number",
                            "ranges": [{"from": 0, "to": 60}, {"from": 50, "to": 100}],
                        }
                    }
                },
            )
    ...
    def test_non_number_facet_with_ranges_fails(self):
        ...
        for facet_type in ["string", "array"]:
            ...
            self.assertIn(
                "Ranges can only be used for 'number' facets", str(e.exception)
            )
    ...
    def test_facets_with_filter_exclusions(self):
        ...
        res = self.client.index(self.unstructured_text_index_name).search(
            ...
            facets={
                "fields": {
                    "color": {"type": "string", "excludeTerms": ["price:[100 TO 200]"]}
                }
            },
        ...
        for param in invalid_params:
            with self.assertRaises(MarqoWebError) as e:
                ...
                if param.startswith("fields."):
                    ...
                    facets[param] = 100
                self.client.index(self.unstructured_text_index_name).search(
                    "shirt", search_method="HYBRID", facets=facets
                )
    ...
    def test_track_total_hits_enabled(self):
        ...
        res = self.client.index(self.unstructured_text_index_name).search(
            "shirt", search_method="HYBRID", track_total_hits=True
        )
    ...
    def test_track_total_hits_with_filter(self):
        ...
        filtered_res = self.client.index(self.unstructured_text_index_name).search(
            "shirt", search_method="HYBRID", filter_string="price:[100 TO 200]"
        )
    ...
    def test_non_existing_array_field_returns_empty_value(self):
        ...
        res = self.client.index(self.unstructured_text_index_name).search(
            "shirt",
            search_method="HYBRID",
            facets={"fields": {"non_existing_field": {"type": "array"}}},
        )
        ...
        docs_to_add = [
            {
                ...
                "tags": [
                    f"color:{existing_doc['color']}",
                    f"brand:{existing_doc['brand']}",
                    f"style:{existing_doc['style']}",
                ],
            }
            for existing_doc in EXAMPLE_FASHION_DOCUMENTS
        ]
        self.client.index(self.unstructured_text_index_name).add_documents(
            docs_to_add, tensor_fields=["title", "description"]
        )
    ...
    def test_numeric_facet_with_ranges_returns_exact_value(self):
        ...
        res = self.client.index(self.unstructured_text_index_name).search(
            ...
            facets={
                "fields": {
                    "price": {"type": "number", "ranges": [{"to": 50}, {"from": 50}]}
                }
            },
        )
