...
class TestStructuredUpdateDocuments(MarqoTestCase):
    ...
    large_score_modifier_index_name = "update_doc_api_test_score_modifier_index" + str(
        uuid.uuid4()
    ).replace("-", "")
    ...
    def setUpClass(cls):
                    ...
                    "allFields": [
                        {
                            ...
                            "type": "float",
                            "features": ["score_modifier", "filter"],
                        }
                        for i in range(100)
                    ]
                    + [{"name": "text_field_tensor", "type": "text"}],
        ...
        cls.indexes_to_delete = [
            cls.update_doc_index_name,
            cls.large_score_modifier_index_name,
        ]
    ...
    def set_up_for_text_field_test(self):
        ...
        self.client.index(self.update_doc_index_name).add_documents(
            documents=[original_doc]
        )
        self.assertEqual(
            1,
            self.client.index(self.update_doc_index_name).get_stats()[
                "numberOfDocuments"
            ],
        )
    ...
    def test_update_text_field_filter(self):
        ...
        r = self.client.index(self.update_doc_index_name).update_documents(
            documents=[updated_doc]
        )

        # Retrieve the updated document using the API client
        updated_doc = self.client.index(self.update_doc_index_name).get_document(
            updated_doc["_id"]
        )
        ...
        search_result = self.client.index(self.update_doc_index_name).search(
            q="test", filter_string="text_field_filter:(updated text field filter)"
        )
        self.assertEqual(1, len(search_result["hits"]))

        # Ensure the document cannot be found using the old text field filter
        search_result = self.client.index(self.update_doc_index_name).search(
            q="test", filter_string="text_field_filter:(text field filter)"
        )
    ...
    def test_update_text_field_add(self):
        ...
        lexical_search_result = self.client.index(self.update_doc_index_name).search(
            ...
            searchable_attributes=[
                "text_field_add"
            ],  # Assuming you meant to search the new field
            search_method="LEXICAL",
        )
        self.assertEqual(1, len(lexical_search_result["hits"]))
        ...
        filter_search_result = self.client.index(self.update_doc_index_name).search(
            q="test", filter_string="text_field_add:(I am a new field)"
        )
    ...
    def test_update_int_field_filter(self):
        ...
        search_result = self.client.index(self.update_doc_index_name).search(
            q="test", filter_string="int_field_filter:(22)"
        )
        self.assertEqual(1, len(search_result["hits"]))

        # Ensure the document cannot be found using the old integer value as a filter
        search_result = self.client.index(self.update_doc_index_name).search(
            q="test", filter_string="int_field_filter:(2)"
        )
        self.assertEqual(0, len(search_result["hits"]))
    ...
    def test_update_int_field_score_modifier(self):
        ...
        search_result = self.client.index(
            self.update_doc_index_name
        ).search(
            q="test",
    ...
    def test_update_float_field_filter(self):
        ...
        # Perform a search to verify the document can be found with the updated filter value
        search_result = self.client.index(self.update_doc_index_name).search(
            q="test", filter_string="float_field_filter:(22.2)"
        )
        self.assertEqual(1, len(search_result["hits"]))

        # Ensure the document cannot be found using the old float value as a filter
        search_result = self.client.index(self.update_doc_index_name).search(
            q="test", filter_string="float_field_filter:(2.2)"
        )
        self.assertEqual(0, len(search_result["hits"]))
    ...
    def test_update_bool_field_filter(self):
        ...
        search_result = self.client.index(self.update_doc_index_name).search(
            q="test", filter_string="bool_field_filter:(false)"
        )
        self.assertEqual(1, len(search_result["hits"]))

        # Ensure the document cannot be found using the old boolean value as a filter
        search_result = self.client.index(self.update_doc_index_name).search(
            q="test", filter_string="bool_field_filter:(true)"
        )
        self.assertEqual(0, len(search_result["hits"]))
    ...
    def test_update_array_text_field_filter(self):
        ...
        search_result = self.client.index(self.update_doc_index_name).search(
            q="test", filter_string="array_text_field:(text3)"
        )
        self.assertEqual(1, len(search_result["hits"]))

        # Ensure the document cannot be found using the old array text values as a filter
        search_result = self.client.index(self.update_doc_index_name).search(
            q="test", filter_string="array_text_field:(text1)"
        )
        self.assertEqual(0, len(search_result["hits"]))
    ...
    def test_update_with_incorrect_field_value(self):
        ...
        for updated_doc, expected_error, msg in test_cases:
            ...
            with self.subTest(f"{updated_doc} - {msg}"):
                ...
                self.assertEqual(
                    expected_error, r["errors"], msg=f"Failed on test case: {msg}"
                )
                if expected_error:
                    self.assertTrue(
                        r["errors"], msg=f"Expected an error for test case: {msg}"
                    )
                    self.assertTrue(
                        r["items"][0]["status"] >= 400,
    ...
    def test_multi_threading_update(self):
        ...
        def randomly_update_document(number_of_updates: int = 50):
            ...
            for _ in range(number_of_updates):
                ...
                self.client.index(self.update_doc_index_name).update_documents(
                    documents=[updated_doc]
                )
        ...
        threads = [
            threading.Thread(
                target=randomly_update_document, args=(updates_per_thread,)
            )
    ...
    def test_multi_threading_update_for_large_score_modifier_fields(self):
        ...
        # Add the original document to the index
        self.client.index(self.large_score_modifier_index_name).add_documents(
            documents=[original_document]
        )
        self.assertEqual(
            1,
            self.client.index(self.large_score_modifier_index_name).get_stats()[
                "numberOfDocuments"
            ],
        )
        ...
        def randomly_update_document(number_of_updates: int = 50):
            ...
            for _ in range(number_of_updates):
                ...
                ).update_documents(documents=[updated_doc])
        ...
        final_doc["_id"] = "1"
        r = self.client.index(self.large_score_modifier_index_name).update_documents(
            documents=[final_doc]
        )

        original_score = self.client.index(self.large_score_modifier_index_name).search(
            q="test"
        )["hits"][0]["_score"]
        ...
        for i in range(100):
            ...
            modified_score = self.client.index(
                self.large_score_modifier_index_name
            ).search(q="test", score_modifiers=score_modifiers)["hits"][0]["_score"]
    ...
    def test_batch_update_document_requests(self):
        ...
        r = self.client.index(self.update_doc_index_name).update_documents(
            documents=updated_doc, client_batch_size=10
        )

        self.assertEqual(10, len(r))  # 10 batches
        self.assertTrue(
            all([len(batch["items"]) == 10 for batch in r]), True
        )  # 10 items in each batch

        for i in range(100):
            document = self.client.index(self.update_doc_index_name).get_document(
                f"{i}"
            )
    ...
    def test_incorrect_update_document_body(self):
        ...
        cases = [
            ...
            (
                ...
                "Body is missing the 'documents' key",
            ),
        ]
    ...
    def test_too_many_documents_exceeds_max_batch_size(self):
        ...
        documents = [
            {"_id": str(i), "text_field": f"updated text field {i}"} for i in range(129)
        ]
        ...
        self.assertIn(
            "Number of docs in update_documents request (129) exceeds", str(e.exception)
        )
