...
class TestStructuredHybridSearch(MarqoTestCase):
    ...
    image_index_name = "api_test_structured_image_index_image" + str(
        uuid.uuid4()
    ).replace("-", "")
    unstructured_text_index_name = "api_test_unstructured_index_text" + str(
        uuid.uuid4()
    ).replace("-", "")
    unstructured_image_index_name = "api_test_unstructured_image_index_image" + str(
        uuid.uuid4()
    ).replace("-", "")
    ...
    def test_hybrid_search_with_custom_vector_query(self):
        ...
        for index_name in [self.text_index_name, self.unstructured_text_index_name]:
            with self.subTest(index=index_name):
                ...
                self.assertEqual(
                    len(res_custom_vector["hits"]), len(res_context["hits"])
                )
                for i in range(len(res_custom_vector["hits"])):
                    self.assertEqual(
                        res_custom_vector["hits"][i]["_id"],
                        res_context["hits"][i]["_id"],
                    )
        ...
        for index_name in [self.text_index_name, self.unstructured_text_index_name]:
            with self.subTest(index=index_name):
                ...
                lexical_res = self.client.index(index_name).search(
                    "dogs", search_method="LEXICAL", limit=10
                )
        ...
        for index_name in [self.text_index_name, self.unstructured_text_index_name]:
            with self.subTest(index=index_name):
                ...
                tensor_res = self.client.index(index_name).search(
                    "dogs", search_method="TENSOR", limit=10
                )
            ...
            with self.subTest(index=index_name):
                ...
                with self.subTest("retrieval: lexical, ranking: tensor"):
                    ...
                    self.assertAlmostEqual(
                        hybrid_res["hits"][0]["_score"], 30.0, places=4
                    )
                    self.assertEqual(
                        hybrid_res["hits"][1]["_id"], "doc8"
                    )  # (score*10*2)
                    self.assertAlmostEqual(
                        hybrid_res["hits"][1]["_score"], 20.0, places=4
                    )
                    self.assertEqual(
                        hybrid_res["hits"][2]["_id"], "doc7"
                    )  # (score + 5*1)
                    self.assertAlmostEqual(
                        hybrid_res["hits"][2]["_score"], 6.0, places=4
                    )
                    self.assertEqual(hybrid_res["hits"][3]["_id"], "doc6")  # (score)
                    self.assertAlmostEqual(
                        hybrid_res["hits"][3]["_score"], 1.0, places=4
                    )
                    self.assertEqual(
                        hybrid_res["hits"][-1]["_id"], "doc10"
                    )  # lowest score (score*-10*3)
                    self.assertAlmostEqual(
                        hybrid_res["hits"][-1]["_score"], -30.0, places=4
                    )
                ...
                with self.subTest("retrieval: disjunction, ranking: rrf"):
                    ...
                    self.assertAlmostEqual(
                        ...
                        base_lexical_score * 10 * 3,
                    )
                    self.assertEqual(
                        hybrid_res["hits"][0]["_tensor_score"],
                        base_tensor_score * 10 * 3,
                    )
                    ...
                    self.assertAlmostEqual(
                        ...
                        base_lexical_score * 10 * 2,
                    )
                    self.assertAlmostEqual(
                        hybrid_res["hits"][1]["_tensor_score"],
                        base_tensor_score * 10 * 2,
                    )
                    ...
                    self.assertAlmostEqual(
                        hybrid_res["hits"][2]["_lexical_score"],
                        base_lexical_score + 5 * 1,
                    )
                    self.assertAlmostEqual(
                        hybrid_res["hits"][2]["_tensor_score"],
                        base_tensor_score + 5 * 1,
                    )
    ...
    def test_hybrid_search_same_retrieval_and_ranking_matches_original_method(self):
        ...
        for index_name in [self.text_index_name, self.unstructured_text_index_name]:
            with self.subTest(index=index_name):
                ...
                test_cases = [("lexical", "lexical"), ("tensor", "tensor")]
        ...
        for index_name in [self.text_index_name, self.unstructured_text_index_name]:
            with self.subTest(index=index_name):
                ...
                default_hybrid_res = self.client.index(index_name).search(
                    "dogs", search_method="HYBRID", limit=10
                )
                ...
                self.assertEqual(
                    len(default_hybrid_res["hits"]), len(disjunction_res["hits"])
                )
                for i in range(len(default_hybrid_res["hits"])):
                    self.assertEqual(
                        default_hybrid_res["hits"][i]["_id"],
                        disjunction_res["hits"][i]["_id"],
                    )
                    self.assertEqual(
                        default_hybrid_res["hits"][i]["_score"],
                        disjunction_res["hits"][i]["_score"],
                    )
