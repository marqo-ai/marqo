...
class TestStructuredHybridSearch(MarqoTestCase):
    ...
    def test_hybrid_search_with_facets(self):
        self.client.index(self.unstructured_text_index_name).add_documents(
            EXAMPLE_FASHION_DOCUMENTS,
            tensor_fields=["title", "description"]
        )

        with self.subTest("Try getting facets for structured index should fail"):
            with self.assertRaises(MarqoWebError) as e:
        ...
        with self.subTest("Try getting facets with search_method != 'HYBRID' should fail"):
        ...
        with self.subTest("Try getting 1 facet"):
            res = self.client.index(self.unstructured_text_index_name).search(
                "shirt",
                search_method="HYBRID",
                facets={"fields": {"color": {"type": "string"}}}
        ...
        with self.subTest("Try getting 3 facets"):
            res = self.client.index(self.unstructured_text_index_name).search(
                "shirt",
                search_method="HYBRID",
                facets={
        ...
        with self.subTest("Try getting number facet"):
            res = self.client.index(self.unstructured_text_index_name).search(
                "shirt",
                search_method="HYBRID",
        ...
        with self.subTest("Try getting number facet with ranges"):
            res = self.client.index(self.unstructured_text_index_name).search(
                "shirt",
                search_method="HYBRID",
                facets={
        ...
        with self.subTest("Try getting number facet with overlapping ranges raises an error"):
            with self.assertRaises(MarqoWebError) as e:
        ...
        with self.subTest("Try getting string/array facet with ranges raises an error"):
        ...
        with self.subTest("Try getting facets with filter terms exclusions"):
            res = self.client.index(self.unstructured_text_index_name).search(
                "shirt",
                search_method="HYBRID",
                facets={
        ...
        with self.subTest("Try getting facets with filter terms exclusions and no filter raises an error"):
            with self.assertRaises(MarqoWebError) as e:
                self.client.index(self.unstructured_text_index_name).search(
                    "shirt",
                    search_method="HYBRID",
        ...
        with self.subTest("Try getting facets with filter terms exclusions not present in filter raises an error"):
            with self.assertRaises(MarqoWebError) as e:
                self.client.index(self.unstructured_text_index_name).search(
                    "shirt",
                    search_method="HYBRID",
        ...
        with self.subTest("Try getting facets with invalid parameters raises error"):
