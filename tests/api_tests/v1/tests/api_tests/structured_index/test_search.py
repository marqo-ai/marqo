...
class TestStructuredSearch(MarqoTestCase):
    ...
    def test_filter_on_large_integer_and_float(self):
        valid_documents = [
            ...
            # large float array
            {
                "array_double_field_1": [1e10, 1e10 + 0.123249357987123],
                "_id": "8",
                "field_a": "some text",
            },
            # large integer array
            {
                "array_long_field_1": [1002321423, -4923217213, 12390809],
                "_id": "9",
                "field_a": "some text",
            },
        ]
        ...
        self.assertEqual(
            len(valid_documents),
            self.client.index(self.filter_test_index_name).get_stats()[
                "numberOfDocuments"
            ],
        )
    ...
    def test_filter_on_id(self):
        ...
        test_case = [
            ...
            (
                "_id in (e197e580-039, 123456)",
                ["e197e580-039", "123456"],
                "IN, multiple IDs",
            ),
            ("_id in (e197e580-039)", ["e197e580-039"], "IN, single ID"),
            (
                "_id in (e197e580-039) OR title:(Just Your Average Doc)",
                ["e197e580-039", "123456"],
                "IN, single ID with OR",
            ),
        ]
        for search_method in ["TENSOR", "LEXICAL"]:
            for filter_string, expected, msg in test_case:
                with self.subTest(f"{search_method} - {msg}"):
                    search_res = self.client.index(self.text_index_name).search(
                        q="title", filter_string=filter_string
                    )
                    actual_ids = set([hit["_id"] for hit in search_res["hits"]])
                    self.assertEqual(
                        len(search_res["hits"]),
    ...
    def test_filter_on_id_error_message(self):
        ...
        for filter_string, msg in test_case:
            with self.subTest(filter_string=filter_string):
                ...
                self.assertIn(
                    "Available filterable fields are", str(cm.exception.message)
                )
