...
class TestStructuredAddDocuments(MarqoTestCase):
    ...
    image_index_name = "add_doc_api_test_structured_image_index" + str(
        uuid.uuid4()
    ).replace("-", "")
    structured_languagebind_index_name = (
        "add_doc_api_test_structured_languagebind_index"
        + str(uuid.uuid4()).replace("-", "")
    )
    text_index_with_normalize_embeddings_true = (
        "add_doc_api_test_structured_index_with_normalize_embeddings_true"
        + str(uuid.uuid4()).replace("-", "")
    )
    ...
    def test_add_documents_with_ids(self):
        ...
        retrieved_d2 = self.client.index(self.text_index_name).get_document(
            document_id="123456"
        )
    ...
    def test_add_documents_without_ids(self):
        ...
        d1 = {"title": "Cool Document 1", "content": "some extra info"}
        d2 = {"title": "Just Your Average Doc", "content": "this is a solid doc"}
        res = self.client.index(self.text_index_name).add_documents([d1, d2])
    ...
    def test_add_documents_with_ids_twice(self):
        ...
        d2 = {"_id": "56", "title": "different doc.", "content": "this is a solid doc"}
    ...
    def test_add_documents_with_device(self):
        ...
        def run():
            temp_client.index(self.image_index_name).add_documents(
                documents=[{"title": "blah"}, {"title", "some data"}], device="cuda:45"
            )
            return True
    ...
    def test_add_documents_no_device(self):
        ...
        def run():
            temp_client.index(self.image_index_name).add_documents(
                documents=[{"title": "blah"}, {"title", "some data"}]
            )
            return True
    ...
    def test_add_long_double_numeric_values(self):
        ...
        test_case = [
            (
                {"_id": "1", "int_field_1": 2147483647},
                False,
                "maximum positive integer that can be handled by int",
            ),
            (
                {"_id": "2", "int_field_1": -2147483647},
                False,
                "maximum negative integer that can be handled by int",
            ),
            (
                {"_id": "3", "int_field_1": 2147483648},
                True,
                "integer slightly larger than boundary so can't be handled by int",
            ),
            (
                {"_id": "4", "long_field_1": 2147483648},
                False,
                "integer slightly larger than boundary can be handled by long",
            ),
            (
                {"_id": "5", "int_field_1": -2147483648},
                True,
                "integer slightly smaller than boundary so can't be handled by int",
            ),
            (
                {"_id": "6", "long_field_1": -2147483648},
                False,
                "integer slightly larger than boundary can be handled by long",
            ),
            (
                {"_id": "7", "float_field_1": 3.4028235e38},
                False,
                "maximum positive float that can be handled by float",
            ),
            (
                {"_id": "8", "float_field_1": -3.4028235e38},
                False,
                "maximum negative float that can be handled by float",
            ),
            (
                {"_id": "9", "float_field_1": 3.4028235e40},
                True,
                "float slightly larger than boundary can't be handled by float",
            ),
            (
                {"_id": "10", "double_field_1": 3.4028235e40},
                False,
            ...
            with self.subTest(msg):
                ...
                if error:
                    ...
                    returned_doc = self.client.index(self.text_index_name).get_document(
                        document_id=document_id, expose_facets=False
                    )
    ...
    def test_custom_vector_doc(self):
        ...
        tensor_res = self.client.index(self.text_index_name).search(
            ...
            context={
                "tensor": [
                    {"vector": [1.0 for _ in range(DEFAULT_DIMENSIONS)], "weight": 1}
                ]
            },
        )
        ...
        assert (
            doc_res["_tensor_facets"][0]["custom_vector_field_1"]
            == "custom vector text"
        )
        assert doc_res["_tensor_facets"][0]["_embedding"] == [
            1.0 for _ in range(DEFAULT_DIMENSIONS)
        ]
    ...
    def test_add_multimodal_single_documents(self):
        ...
        res = self.client.index(self.structured_languagebind_index_name).add_documents(
            documents
        )
        for item in res["items"]:
            self.assertEqual(200, item["status"])

        get_res = self.client.index(
            self.structured_languagebind_index_name
        ).get_documents(document_ids=["1", "2", "3", "4"], expose_facets=True)
    ...
    def test_add_documents_with_invalid_media_fields(self):
        documents = [
            {"video_field_3": "This is text, not a video URL", "_id": "1"},
            {"audio_field_2": "This is text, not an audio URL", "_id": "2"},
            {
                "image_field_2": "https://raw.githubusercontent.com/marqo-ai/marqo-api-tests/mainline/assets/ai_hippo_realistic.png",
                "_id": "3",
            },
            {"text_field_3": "This is a valid text field", "_id": "4"},
        ]
        ...
        get_res = self.client.index(
            self.structured_languagebind_index_name
        ).get_documents(document_ids=["3", "4"], expose_facets=True)
        ...
        with self.assertRaises(MarqoWebError):
            self.client.index(self.structured_languagebind_index_name).get_document(
                document_id="1"
            )
    ...
    def test_custom_vector_doc_in_normalized_embedding_true(self):
        ...
        expected_custom_vector_after_normalization = [
            0.05103103816509247 for _ in range(DEFAULT_DIMENSIONS)
        ]

        add_docs_res_normalized = self.client.index(
            index_name=self.text_index_with_normalize_embeddings_true
        ).add_documents(
            documents=[
                {
                    "custom_vector_field_1": {
        ...
        doc_res_normalized = self.client.index(
            self.text_index_with_normalize_embeddings_true
        ).get_document(document_id="doc1", expose_facets=True)

        assert doc_res_normalized["custom_vector_field_1"] == "custom vector text"
        assert (
            doc_res_normalized["_tensor_facets"][0]["custom_vector_field_1"]
            == "custom vector text"
        )
        assert (
            doc_res_normalized["_tensor_facets"][0]["_embedding"]
            == expected_custom_vector_after_normalization
        )
    ...
    def test_custom_zero_vector_doc_in_normalized_embedding_true(self):
        ...
        self.assertIn(
            "Field custom_vector_field_1 has zero magnitude vector, cannot normalize.",
            add_docs_res_normalized["items"][0]["message"],
        )
        self.assertEqual(
            add_docs_res_normalized["items"][0]["code"], "invalid_argument"
        )
