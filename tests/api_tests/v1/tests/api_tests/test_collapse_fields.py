...
class TestCollapseFields(MarqoTestCase):
            ...
            [
                {
                    ...
                    "collapseFields": [{"name": "parent_id", "minGroups": 100}],
                },
            ]
        )
    ...
    def test_collapse_fields_is_in_index_settings(self):
        ...
        self.assertEqual(
            index_settings["collapseFields"][0], {"name": "parent_id", "minGroups": 100}
        )
    ...
    def test_partial_update_of_collapse_field_does_not_work(self):
        ...
        res = self.client.index(index_name).add_documents(docs, tensor_fields=[])
    ...
    def test_search_with_valid_collapse_field_succeeds(self):
        ...
        self.client.index(index_name).add_documents(docs, tensor_fields=["title"])
        ...
        for retrieval_method, ranking_method in test_cases:
            with self.subTest(
                ...
                self.assertEqual(
                    set([f"group_{g}" for g in range(5)]),
                    set([hit["parent_id"] for hit in res["hits"]]),
                )
                ...
                res = self.client.index(index_name).search(
                    ...
                    facets={
                        "fields": {
                            "price": {
                                "type": "number",
                                "ranges": [{"from": 0, "to": 2}, {"from": 2, "to": 4}],
                            },
                            "color": {"type": "string"},
                        }
                    },
    ...
    def test_score_modifiers(self):
        ...
        score_modifiers = {"multiply_score_by": [{"field_name": "rating", "weight": 1}]}
            ...
            with self.subTest(
                ...
                res = self.client.index(index_name).search(
                    ...
                    hybrid_parameters={
                        ...
                        if ranking_method != "tensor" or retrieval_method != "tensor"
                        else None,
                    },
                    collapse_fields=[{"name": "parent_id"}],
                    limit=6,
    ...
    def test_filter_by_collapse_field(self):
        ...
        lexical_res = self.client.index(index_name).search(
            q="*", search_method="LEXICAL", filter_string="parent_id:group_1", limit=10
        )
