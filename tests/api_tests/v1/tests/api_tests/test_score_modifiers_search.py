...
class TestScoreModifierSearch(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.unstructured_score_modifier_index_name = (
            "unstructured_score_modifier" + str(uuid.uuid4()).replace("-", "")
        )
        cls.structured_score_modifier_index_name = "structured_score_modifier" + str(
            uuid.uuid4()
        ).replace("-", "")
        ...
        cls.indexes_to_delete = [
            cls.structured_score_modifier_index_name,
            cls.unstructured_score_modifier_index_name,
        ]

    def test_score_modifier_search_results(self):
        for index_name in [
            self.unstructured_score_modifier_index_name,
            ...
            for _ in range(10):
                ...
                msg = (
                    f"{'unstructured' if index_name.startswith('un') else 'structured'}, doc = {doc}, "
                ...
                with self.subTest(msg):
                    ...
                    self.assertEqual(
                        1,
                        self.client.index(index_name).get_stats()["numberOfDocuments"],
                    )

                    original_res = self.client.index(index_name).search(
                        q="test_search", score_modifiers=None
                    )
                    modifiers_res = self.client.index(index_name).search(
                        q="test_search", score_modifiers=score_modifiers
                    )
                    ...
                    expected_score = (
                        ...
                        * multiply_2_value
                        * multiply_2_weight
                        + add_1_value * add_1_weight
                        + add_2_value * add_2_weight
                    )
            ...
            for valid_score_modifiers in valid_score_modifiers_list:
                with self.subTest(f"{index_name} - {valid_score_modifiers}"):
                    self.client.index(index_name).search(
                        "test", score_modifiers=valid_score_modifiers
                    )
    ...
    def test_hybrid_search_rrf_score_modifiers_with_rerank_depth(self):
        ...
        docs_list = [
            ...
            {
                "_id": "tensor2",
                "text_field": "random words",
                "multiply_1": 3,
                "add_1": 3,
            },  # LOW tensor
        ]

        for test_index_name in [
            self.unstructured_score_modifier_index_name,
            self.structured_score_modifier_index_name,
        ]:
            with self.subTest(index=test_index_name):
                ...
                unmodified_results = self.client.index(test_index_name).search(
                    q="dogs", search_method="HYBRID", limit=3
                )
                unmodified_scores = {
                    hit["_id"]: hit["_score"] for hit in unmodified_results["hits"]
                }
                self.assertEqual(
                    ["both1", "tensor1", "tensor2"],
                    [hit["_id"] for hit in unmodified_results["hits"]],
                )

                # Get modified scores (rank all 3)
                # Modified result order should be: tensor2, tensor1, both1
                score_modifiers = {
                    "multiply_score_by": [{"field_name": "multiply_1", "weight": 1}],
                    "add_to_score": [{"field_name": "add_1", "weight": 1}],
                }
                with self.subTest("Case 1: limit == rerankCount == hits.size()"):
                    ...
                    self.assertEqual(
                        ...
                        [hit["_id"] for hit in modified_results["hits"]],
                    )
                    self.assertAlmostEqual(
                        modified_results["hits"][0]["_score"],
                        3 * unmodified_scores["tensor2"] + 3,
                    )
                    self.assertAlmostEqual(
                        modified_results["hits"][1]["_score"],
                        2 * unmodified_scores["tensor1"] + 2,
                    )
                    self.assertAlmostEqual(
                        modified_results["hits"][2]["_score"],
                        -1 * unmodified_scores["both1"] - 1,
                    )
                    ...
                    self.assertEqual(
                        ["tensor1", "tensor2", "both1"],
                        [hit["_id"] for hit in modified_results["hits"]],
                    )
                    self.assertAlmostEqual(
                        modified_results["hits"][0]["_score"],
                        unmodified_scores["tensor1"],
                    )  # unmodified
                    self.assertAlmostEqual(
                        modified_results["hits"][1]["_score"],
                        unmodified_scores["tensor2"],
                    )  # unmodified
                    self.assertAlmostEqual(
                        modified_results["hits"][2]["_score"],
                        -1 * unmodified_scores["both1"] - 1,
                ...
                with self.subTest("Case 2: limit < rerankCount < hits.size()"):
                    modified_results = self.client.index(test_index_name).search(
                        q="dogs",
                        search_method="HYBRID",
                        limit=1,
                    ...
                    self.assertEqual(
                        ["both1"], [hit["_id"] for hit in modified_results["hits"]]
                    )
                    self.assertAlmostEqual(
                        modified_results["hits"][0]["_score"],
                        -1 * unmodified_scores["both1"] - 1,
                ...
                with self.subTest("Case 3: limit == rerankCount < hits.size()"):
                ...
                with self.subTest("Case 4: limit < hits.size() < rerankCount"):
                    modified_results = self.client.index(test_index_name).search(
                        q="dogs",
                        search_method="HYBRID",
                        limit=2,
                ...
                with self.subTest("Case 5: rerankCount < hits.size() < limit"):
                ...
                with self.subTest("Case 6: rerankCount == 0"):
                    modified_results = self.client.index(test_index_name).search(
                        q="dogs",
                        search_method="HYBRID",
                        hybrid_parameters={"verbose": True},
                        limit=3,
                        rerank_depth=0,
                        score_modifiers=score_modifiers,
                    )
    ...
    def test_negative_rerank_depth_fails(self):
        ...
        for test_index_name in [
            ...
            with self.assertRaises(MarqoWebError) as e:
                with self.subTest(index=test_index_name):
                    self.client.index(test_index_name).search(
                        q="dogs", search_method="HYBRID", rerank_depth=-5
                    )
            self.assertEqual(422, e.exception.status_code)
            self.assertIn("rerankDepth cannot be negative", str(e.exception.message))
