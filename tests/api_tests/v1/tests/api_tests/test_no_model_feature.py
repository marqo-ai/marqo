...
class TestNoModelFeature(MarqoTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.unstructured_no_model_index_name = "unstructured_no_model" + str(
            uuid.uuid4()
        ).replace("-", "")
        cls.structured_no_model_index_name = "structured_no_model" + str(
            uuid.uuid4()
        ).replace("-", "")
        cls.DIMENSION = 128
        ...
        cls.indexes_to_delete = [
            cls.unstructured_no_model_index_name,
            cls.structured_no_model_index_name,
        ]
    ...
    def test_create_index_with_incorrect_model_properties(self):
        ...
        test_cases = [
            (
                "no_model",
                {"dimensions": 0, "type": "no_model"},
                "invalid dimensions value, can't be 0",
            ),
            (
                "no_model",
                {"dimensions": -123, "type": "no_model"},
                "invalid dimensions value, can't be negative",
            ),
            (
                "no_model",
                {"dimensions": 213.213, "type": "no_model"},
                "invalid dimensions value, can't be float",
            ),
            (
                "no_model",
                {"dimensions": "512", "type": "no_model"},
                "invalid dimensions value, can't be string",
            ),
            (
                "no_model",
                {"dimensions": 234, "type": "No_model"},
                "invalid model type, should be 'no_model'",
            ),
            ...
            (
                "no_model",
                {"dimensions": 512, "type": "open_clip"},
                "invalid model properties type",
            ),
        ]
    ...
    def test_no_model_in_add_documents_error(self):
        ...
        for index_name in [
            self.structured_no_model_index_name,
            self.unstructured_no_model_index_name,
        ]:
            with self.subTest(index_name=index_name):
                tensor_fields = (
                    ["text_field_1", "custom_field_1"]
                    if index_name == self.unstructured_no_model_index_name
                    else None
                )
                mappings = (
                    {"custom_field_1": {"type": "custom_vector"}}
                    if index_name == self.unstructured_no_model_index_name
                    else None
                )
                r = self.client.index(index_name).add_documents(
                    documents=documents, tensor_fields=tensor_fields, mappings=mappings
                )

                self.assertEqual(r["errors"], True)
                self.assertIn(
                    "Cannot vectorise anything with 'no_model'", r["items"][0]["error"]
                )
                ...
                self.assertEqual(
                    1, self.client.index(index_name).get_stats()["numberOfDocuments"]
                )
                self.assertEqual(
                    1, self.client.index(index_name).get_stats()["numberOfVectors"]
                )
    ...
    def test_no_model_raise_error_if_query_in_search(self):
        ...
        for index_name in [
            ...
            with self.subTest(index_name=index_name):
                ...
                self.assertIn(
                    "Cannot vectorise anything with 'no_model'", str(e.exception)
                )
    ...
    def test_no_model_work_with_context_vectors_in_search(self):
        ...
        custom_vector = (
            np.array(custom_vector) / np.linalg.norm(custom_vector)
        ).tolist()
        ...
        for index_name in [
            ...
            with self.subTest(index_name=index_name):
                ...
                r = self.client.index(index_name).add_documents(
                    documents=docs, tensor_fields=tensor_fields, mappings=mappings
                )
                r = self.client.index(index_name).search(
                    q=None,
                    context={
