...
class TestSpecialCharacterInValue(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        super().setUpClass()
        cls.standard_structured_index_name = "structured_standard" + str(
            uuid.uuid4()
        ).replace("-", "")
        cls.standard_unstructured_index_name = "unstructured_standard" + str(
            uuid.uuid4()
        ).replace("-", "")
    ...
    def test_filtering_on_content_with_special_chars_tensor(self):
        for index_name in [
            self.standard_structured_index_name,
            self.standard_unstructured_index_name,
        ]:
            for special_str in self.supported_special_str_sequences:
                docs = [
                    {
                        "_id": "doc_0",
                        "filteringField": f"str_at_back_{special_str}",
                        "searchField": "hello",
                    },
                    {
                        "_id": "doc_1",
                        "filteringField": f"{special_str}_str_at_front",
                        "searchField": "hello",
                    },
                    {
                        "_id": "doc_2",
                        "filteringField": f"str_{special_str}_mid",
                        "searchField": "hello",
                    },
                    {
                        "_id": "doc_3",
                        "filteringField": f"{special_str}",
                        "searchField": "hello",
                    },
                ]
                for expected_document in docs:
                    str_for_filtering = expected_document["filteringField"].replace(
                        special_str, f"\\{special_str}"
                    )

                    with self.subTest(
                        f"Tensor Search, {index_name}, "
                        ...
                        self.client.index(index_name).add_documents(
                            docs,
                            tensor_fields=["searchField"]
                            if index_name.startswith("un")
                            else None,
                        )
                        ...
                        self.assertEqual(
                            res["hits"][0]["_id"], expected_document["_id"]
                        )
        ...
        for index_name in [
            ...
            for special_str in self.supported_special_str_sequences:
                ...
                for expected_document in docs:
                    ...
                    with self.subTest(
                        f"Lexical search, {index_name}, str for filtering "
    ...
    def test_unsupported_special_string_errors(self):
        bad_documents = [
            {"_id": f"doc_{i}", "searchField": f"{bad_string}"}
            for i, bad_string in enumerate(self.unsupported_special_str_sequences)
        ]
        for index_name in [
            self.standard_structured_index_name,
            self.standard_unstructured_index_name,
            ...
            with self.subTest(index_name):
                ...
                for item in res["items"]:
                    ...
                    self.assertIn(
                        "The document contains invalid characters in the fields.",
                        item["message"],
                    )
...
class TestSpecialCharsFieldNamesUnstructured(MarqoTestCase):
    ...
    def test_supported_field_name_in_fieldnames_unstructured(self):
        ...
        docs = [
            {"_id": f"special_{i}", f"field_{special_str}": "hello"}
            for i, special_str in enumerate(self.supported_field_name)
        ]

        # Ensure these special chars can be used in field names
        tensor_fields = [
            f"field_{special_str}" for special_str in self.supported_field_name
        ]
        res = self.client.index(index_name).add_documents(
        ...
        for special_str in self.supported_field_name:
            with self.subTest(f"Supported field name: {repr(special_str)}"):
                escaped_field_name = f"field_{special_str}".replace(
                    special_str, f"{special_str}"
                )
                res = self.client.index(index_name).search(
                    q="hello", filter_string=f"{escaped_field_name}:hello"
                )
                self.assertEqual(
                    f"special_{self.supported_field_name.index(special_str)}",
                    res["hits"][0]["_id"],
                )
    ...
    def test_illegal_field_name_unstructured(self):
        ...
        illegal_field_names = (
            self.illegal_field_name + self.illegal_for_unstructured_index
        )
