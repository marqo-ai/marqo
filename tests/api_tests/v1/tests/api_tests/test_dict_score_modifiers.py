        ...
        for test_index_name in test_cases:
            with self.subTest(test_index_name):
                ...
                res = self.client.index(test_index_name).add_documents(
                    documents=docs, tensor_fields=tensor_fields
                )
                ...
                docs = [
                    {
                        "_id": "1",
                        "text_field": "a photo of a cat",
                        "long_score_mods": 2**34,
                    },
                    {
                        "_id": "2",
                        "text_field": "a photo of a cat",
                        "long_score_mods": 2**35,
                    },
                    {
                        "_id": "3",
                        "text_field": "a photo of a cat",
                        "long_score_mods": 2**36,
                    },
                    {"_id": "4", "text_field": "a photo of a cat"},
                ]
                tensor_fields = ["text_field"] if "unstr" in test_index_name else None
    ...
    def test_multiple_map_values_score_modifiers(self):
        ...
        for index_name in [self.structured_index_name, self.unstructured_index_name]:
            with self.subTest(index_name=index_name):
                docs = [
                    {
                        "_id": "1",
                        "text_field": "test",
                        "map_score_mods": {"a": 1.5, "b": 2, "c": 5},
                    },
                    {
                        "_id": "2",
                        "text_field": "test",
                        "map_score_mods": {"a": 1.5, "b": 2},
                    },
                ...
                tolerance = 0.1
                self.assertTrue(
                    abs(res["hits"][0]["_score"] - expected_score_doc1) <= tolerance
                )
                self.assertTrue(
                    abs(res["hits"][1]["_score"] - expected_score_doc2) <= tolerance
                )
                self.assertTrue(
                    abs(res["hits"][2]["_score"] - expected_score_doc3) <= tolerance
                )
    ...
    def test_combination_score_modifiers(self):
        ...
        for index_name in [self.structured_index_name, self.unstructured_index_name]:
            with self.subTest(index_name=index_name):
                ...
                tolerance = 0.1
                self.assertTrue(
                    abs(res["hits"][0]["_score"] - expected_score_doc2) <= tolerance
                )
                self.assertTrue(
                    abs(res["hits"][1]["_score"] - expected_score_doc1) <= tolerance
                )
        ...
        for index_name in [self.structured_index_name, self.unstructured_index_name]:
            with self.subTest(index_name=index_name):
                ...
                self.assertAlmostEqual(
                    res["hits"][0]["_score"], expected_score_doc1, places=2
                )
                self.assertAlmostEqual(
                    res["hits"][1]["_score"], expected_score_doc2, places=2
                )
                self.assertNotEqual(
                    res["hits"][2]["_score"], 0
                )  # Make sure that doc 3 has a non-zero score
    ...
    def test_empty_map_score_modifier(self):
        ...
        for index_name in [self.structured_index_name, self.unstructured_index_name]:
            with self.subTest(index_name=index_name):
                ...
                self.assertAlmostEqual(
                    res["hits"][0]["_score"], expected_score_doc2, places=2
                )
            ...
            with self.subTest(index_name=index_name):
                ...
                try:
                    res = self.client.index(index_name).search(
                        ...
                        score_modifiers={
                            "multiply_score_by": [
                                {"field_name": "nonexistent_field", "weight": 2}
                            ]
                        },
                    )
                    ...
                    self.assertIn(
                        "has no score modifier field nonexistent_field", str(e)
                    )
    ...
    def test_score_modifier_with_negative_value(self):
        ...
        for index_name in [self.structured_index_name, self.unstructured_index_name]:
            with self.subTest(index_name=index_name):
                ...
                self.assertAlmostEqual(
                    res["hits"][1]["_score"], expected_score_doc1, places=2
                )
                self.assertGreater(res["hits"][0]["_score"], res["hits"][1]["_score"])
    ...
    def test_score_modifier_with_large_values(self):
        ...
        for index_name in [self.structured_index_name, self.unstructured_index_name]:
            with self.subTest(index_name=index_name):
                ...
                self.assertGreater(res["hits"][0]["_score"], res["hits"][1]["_score"])
                self.assertAlmostEqual(
                    math.log10(score_ratio), math.log10(expected_ratio), places=1
                )
