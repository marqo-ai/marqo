...
class TestSearchCommon(MarqoTestCase):
    ...
    structured_filter_index_name = "structured_filter_index" + str(
        uuid.uuid4()
    ).replace("-", "")

    unstructured_text_index_name = "unstructured_index_text" + str(
        uuid.uuid4()
    ).replace("-", "")
    unstructured_image_index_name = "unstructured_image_index" + str(
        uuid.uuid4()
    ).replace("-", "")
    ...
    def setUpClass(cls):
            ...
            [
                ...
                },
                {
                    ...
                    "allFields": [
                        ...
                        {
                            "name": "str_for_filtering",
                            "type": "text",
                            "features": ["filter"],
                        },
                        {
                            "name": "int_for_filtering",
                            "type": "int",
                            "features": ["filter"],
                        },
                        {
        ...
        cls.indexes_to_delete = [
            cls.structured_image_index_name,
            cls.structured_filter_index_name,
            cls.structured_text_index_name,
    ...
    def test_lexical_query_can_not_be_none(self):
        ...
        for index_name in [
            self.structured_text_index_name,
                ...
                with self.subTest(f"{index_name} - {msg}"):
                    with self.assertRaises(MarqoWebError) as e:
                        res = self.client.index(index_name).search(
                            q=None, context=context, search_method="LEXICAL"
                        )
                    self.assertIn(
                        "Query(q) is required for lexical search",
    ...
    def test_add_document_and_search_for_private_images(self):
        ...
        for index_name in [
            ...
            tensor_fields = (
                ["image_field_1"]
                if (index_name == self.unstructured_image_index_name)
                else None
            )
    ...
    def test_proper_error_if_both_imageDownloadHeaders_and_mediaDownloadHeaders_are_provided(
        self,
    ):
    ...
    def test_rerank_depth(self):
        ...
        for index_name in [
            ...
            with self.subTest(index=index_name):
                docs = [
                    {"title": f"Doc {i}", "content": "some extra info", "_id": str(i)}
                    for i in range(10)
                ]
                tensor_fields = (
                    ["title", "content"]
                ...
                add_res = self.client.index(index_name).add_documents(
    ...
    def test_hybrid_search_validations(self):
        # Add docs
        docs = [
            {"title": "Cool Document 1", "content": "some extra info", "_id": "1"},
            {
        ...
        for index_name in [
            ...
            with self.subTest(index_name):
                ...
                with self.subTest(
                    ...
                    with self.assertRaises(MarqoWebError) as e:
                        self.client.index(index_name).search(
                            search_method="HYBRID", hybrid_parameters={}
                        )
                    assert e.exception.status_code == 422
                    assert (
                        "One of Query(q), context, hybridParameters.queryTensor, or hybridParameters.queryTensor is required for HYBRID search but all are missing"
                ...
                with self.subTest(
                    "Hybrid search with query and queryTensor/queryLexical should raise an error"
                ):
                    with self.assertRaises(MarqoWebError) as e:
                        self.client.index(index_name).search(
                            q="Cool",
                ...
                with self.subTest(
                    ...
                    assert "Either 'hybridParameters.queryLexical' or just 'q'" in str(
                        e.exception
                    )
                    with self.assertRaises(MarqoWebError) as e:
                        self.client.index(index_name).search(
            ...
            with self.subTest(index_name=index_name):
                ...
                for search_method in search_methods:
                    ...
                    for threshold in invalid_thresholds:
                        with self.subTest(
                            f"Invalid {search_method} threshold {threshold}"
                        ):
                            with self.assertRaises(MarqoWebError) as e:
                                self.client.index(index_name).search(
                                    q="test",
                ...
                with self.subTest("LEXICAL search with threshold should fail"):
                    with self.assertRaises(MarqoWebError) as e:
                        self.client.index(index_name).search(
                            q="test", search_method="LEXICAL", approximate_threshold=0.5
                        )
                    self.assertEqual(e.exception.status_code, 422)
        ...
        for index_name in [
            ...
            with self.subTest(index=index_name):
                ...
                for (
                    ...
                    with self.subTest(
                        search_method=search_method, description=description
                    ):
                            ...
                            with self.subTest(query_type=query_type):
                                if query_type == "string":
                                    ...
                                    queries_to_test = [
                                        ...
                                        (
                                            base64_dict_query,
                                            {"data:image/[omitted]": 0.8, "text": 0.2},
                                            "base64_dict",
                                        ),
                                        (
                                            mixed_dict_query,
                                            {
                                                "data:image/[omitted]": 0.5,
                                                "sample": 0.3,
                                                "text": 0.2,
                                            },
                                            "mixed_dict",
                                ...
                                for (
                                    ...
                                    with self.subTest(query_desc=query_desc):
                                        ...
                                        if (
                                            ...
                                            and query_desc.startswith("base64")
                                        ):
                                            continue
        ...
        for index_name in [
            ...
            with self.subTest(index=index_name):
                ...
                with self.subTest("Lexical with context fails"):
                    with self.assertRaises(MarqoWebError) as cm:
                        self.client.index(index_name).search(
                            q="apple", search_method="LEXICAL", context=context
                        )
                    self.assertEqual(cm.exception.status_code, 422)
                    self.assertIn(
                        "Context is not supported for lexical search", str(cm.exception)
                    )
                ...
                with self.subTest("Tensor with context works"):
                    result = self.client.index(index_name).search(
                        q={
                            "apple": 1.0
                        },  # Use dict format for tensor search with context
                        search_method="TENSOR",
                        context=context,
                    )
    ...
    def test_search_context_allow_missing_documents_true(self):
        ...
        for index_name in [
            ...
            for search_method in ["TENSOR", "HYBRID"]:
                ...
                with self.subTest(
                    f"index_name={index_name}, search_method={search_method}"
                ):
                    tensor_fields = (
                        ["title", "content"]
                        if index_name == self.unstructured_text_index_name
                    ...
                    if add_docs_results["errors"]:
                        raise Exception(
                            f"Failed to add documents to index {index_name}"
                        )
