...
class TestRelevanceCutoffFeature(MarqoTestCase):
    ...
    def test_relevance_cutoff_is_blocked_by_structured_index(self):
        """
        Tests that relevance cutoff feature is blocked for structured indexes.
        """
        ...
        self.assertIn(
            "are only supported for unstructured indexes created with Marqo version 2.21.0",
            str(cm.exception)
        )
    ...
    def test_relevance_cutoff_basic_relative_max_score(self):
        ...
        docs = [
            {"_id": "h1", "content": "Machine learning algorithms in artificial intelligence enable systems to adapt", "sort_value": 8.1},
            {"_id": "h2", "content": "Artificial intelligence relies on machine learning algorithms to build models", "sort_value": 9.2},
            {"_id": "h3", "content": "Researchers develop artificial intelligence machine learning algorithms", "sort_value": 7.4},
            {"_id": "m1", "content": "Machine learning algorithms process financial data", "sort_value": 6.4},
            {"_id": "l1", "content": "Engineers use machine tools for cutting", "sort_value": 6.5},
            {"_id": "l2", "content": "Bright morning sunlight streamed through the room", "sort_value": 2.1},
        ]
    ...
    def test_relevance_cutoff_with_pagination(self):
        ...
        """
        docs = [
            {"_id": f"h{i}", "content": f"Machine learning artificial intelligence algorithms doc {i}", "sort_value": 10-i}
            for i in range(1, 11)
        ]
        self.client.index(self.unstructured_index_name).add_documents(
            docs, tensor_fields=["content"]
    ...
    def test_relevance_cutoff_without_parameters_for_gap_detection(self):
        """
        Tests that gap_detection method works without parameters.
        """
        docs = [
            {"_id": "h1", "content": "Machine learning algorithms in artificial intelligence", "score": 1.0},
            {"_id": "h2", "content": "Artificial intelligence relies on machine learning", "score": 0.9},
            {"_id": "m1", "content": "Machine learning processes", "score": 0.3},
            {"_id": "l1", "content": "Engineers use tools", "score": 0.1},
        ]
        self.client.index(self.unstructured_index_name).add_documents(
            docs, tensor_fields=["content"]
        ...
        # gap_detection should work without parameters
        response = self.client.index(self.unstructured_index_name).search(
            q="machine learning artificial intelligence",
            search_method="HYBRID",
            relevance_cutoff={
