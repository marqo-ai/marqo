...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "cpu_only_test: mark test as cpu_only_test to skip"
    )
...
def pytest_collection_modifyitems(items):
    ...
    if os.environ["TESTING_CONFIGURATION"] in ["CUDA_DOCKER_MARQO"]:
        ...
        skip_cpu_only_test = pytest.mark.skip(
            reason="need to not set 'TESTING_CONFIGURATION=CUDA_DIND_MARQO_OS' to run"
        )
        for item in items:
        ...
        skip_cuda_test = pytest.mark.skip(
            reason="need to setEnvVars 'TESTING_CONFIGURATION=CUDA_DOCKER_MARQO' to run"
        )
        for item in items:
