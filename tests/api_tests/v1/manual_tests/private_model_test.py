...
def run_s3_test():
    ...
    mq.create_index(
        index_name=index_name,
        settings_dict=_get_s3_settings(),
    )
    print(
...
def run_s3_test_search():
    ...
    mq.index(index_name=index_name).search(
        q="Hehehe",
        model_auth={
            "s3": {"aws_access_key_id": acc_key, "aws_secret_access_key": sec_acc_key}
        },
    )
...
def run_hf_test():
    ...
    mq.create_index(
        index_name=index_name,
        settings_dict=_get_hf_settings(),
    )
    ar = mq.index(index_name=index_name).add_documents(
        auto_refresh=True,
        documents=[{"title": "apples"}, {"title": "office politik"}],
        model_auth={"hf": {"token": hf_token}},
    )
    print(ar)
    sr = mq.index(index_name=index_name).search(
        q="what is healthy fruit I can eat", model_auth={"hf": {"token": hf_token}}
    )
    print(sr)
