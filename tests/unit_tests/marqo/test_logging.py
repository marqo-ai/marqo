...
from marqo.logging import LogFormat, LogLevel
...
class TestLoggingConfig(unittest.TestCase):
    ...
    def test_valid_log_levels(self):
        ...
        for level in LogLevel:
            with patch.dict(
                os.environ, {"MARQO_LOG_LEVEL": level, "MARQO_LOG_FORMAT": "plain"}
            ):
                # Should not raise an exception
                importlib.reload(
                    marqo_logging
                )  # Need to reload module to re-execute the config

        # Test invalid level
        with patch.dict(
            os.environ, {"MARQO_LOG_LEVEL": "invalid", "MARQO_LOG_FORMAT": "plain"}
        ):
            with self.assertRaises(EnvVarError) as context:
                importlib.reload(marqo_logging)
            self.assertIn(
                "`MARQO_LOG_LEVEL` = `invalid` is not supported", str(context.exception)
            )
    ...
    def test_valid_log_formats(self):
        ...
        for fmt in LogFormat:
            with patch.dict(
                os.environ, {"MARQO_LOG_FORMAT": fmt, "MARQO_LOG_LEVEL": "info"}
            ):
                # Should not raise an exception
                importlib.reload(marqo_logging)
        ...
        # Test invalid format
        with patch.dict(
            os.environ, {"MARQO_LOG_FORMAT": "invalid", "MARQO_LOG_LEVEL": "info"}
        ):
            with self.assertRaises(EnvVarError) as context:
                importlib.reload(marqo_logging)
            self.assertIn(
                "`MARQO_LOG_FORMAT` = `invalid` is not supported",
    ...
    def test_config_structure_plain(self):
        ...
        with patch.dict(
            os.environ, {"MARQO_LOG_FORMAT": "plain", "MARQO_LOG_LEVEL": "debug"}
        ):
            importlib.reload(marqo_logging)
            from marqo.logging import LOGGING_CONFIG

            self.assertEqual(
                LOGGING_CONFIG["handlers"]["default"]["formatter"], "default-plain"
            )
            self.assertEqual(
                LOGGING_CONFIG["handlers"]["access"]["formatter"], "access-plain"
            )

    def test_config_structure_json(self):
        """Test the configuration structure with json format"""
        with patch.dict(
            os.environ, {"MARQO_LOG_FORMAT": "json", "MARQO_LOG_LEVEL": "debug"}
        ):
            importlib.reload(marqo_logging)
            from marqo.logging import LOGGING_CONFIG

            self.assertEqual(
                LOGGING_CONFIG["handlers"]["default"]["formatter"], "default-json"
            )
            self.assertEqual(
                LOGGING_CONFIG["handlers"]["access"]["formatter"], "access-json"
            )
    ...
    def test_log_level_propagation(self):
        ...
        for input_level, expected_level in test_cases:
            with patch.dict(
                os.environ,
                {"MARQO_LOG_LEVEL": input_level, "MARQO_LOG_FORMAT": "plain"},
            ):
                importlib.reload(marqo_logging)
                from marqo.logging import LOGGING_CONFIG
                ...
                self.assertEqual(
                    LOGGING_CONFIG["loggers"]["uvicorn"]["level"], expected_level
                )
    ...
    def test_httpx_log_level(self):
        ...
        for input_level, expected_level in test_cases:
            with patch.dict(
                ...
                self.assertEqual(
                    LOGGING_CONFIG["loggers"]["httpx"]["level"], expected_level
                )
    ...
    def test_access_log_level_changes_with_root_log_level(self):
        ...
        for level in ["debug", "info", "warning", "error"]:
            with patch.dict(
                ...
                self.assertEqual(
                    level.upper(), LOGGING_CONFIG["loggers"]["uvicorn.access"]["level"]
                )
    ...
    def test_plain_format_output(self):
        ...
        with patch.dict(
            "os.environ", {"MARQO_LOG_LEVEL": "info", "MARQO_LOG_FORMAT": "plain"}
        ):
            importlib.reload(marqo_logging)
    ...
    def test_json_format_output(self):
        ...
        with patch.dict(
            "os.environ", {"MARQO_LOG_LEVEL": "info", "MARQO_LOG_FORMAT": "json"}
        ):
            importlib.reload(marqo_logging)
    ...
    def test_access_log_json_format(self):
        ...
        with patch.dict(
            ...
            try:
                ...
                self.assertEqual(
                    log_data["request_line"], "GET /api/v1/search HTTP/1.1"
                )
    ...
    def test_exception_traceback_is_logged_as_string_array_with_json_formatter(self):
        ...
        with patch.dict(
            ...
            try:
                ...
                self.assertEqual(
                    "Traceback (most recent call last):", log_data["exc_info"][0]
                )
            except json.JSONDecodeError:
                self.fail("Output is not valid JSON")
