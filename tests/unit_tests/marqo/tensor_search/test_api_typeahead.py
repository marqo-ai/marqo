...
class TestTypeaheadAPIWithTestClient(unittest.TestCase):
    ...
    def test_get_suggestions_valid_request(self):
        ...
        mock_suggestions = [
            TypeaheadSuggestion(
                suggestion="test query", score=0.95, metadata={"key": 2.0}
            )
        ]
        mock_response = TypeaheadResponse(
            suggestions=mock_suggestions, processing_time_ms=50
        )
        ...
        response = self.client.post(
            "/indexes/test_index/suggestions", json=request_data
        )
    ...
    def test_index_queries_valid_request(self):
        ...
        # Create mock response from typeahead service
        mock_response = TypeaheadIndexingResponse(
            indexed=2, errors=[], processing_time_ms=100
        )
        ...
        response = self.client.post(
            "/indexes/test_index/suggestions/queries", json=request_data
        )
    ...
    def test_index_queries_with_errors_response(self):
        ...
        errors = [
            TypeaheadIndexingError(query="bad query", message="Invalid query", code=400)
        ]
        mock_response = TypeaheadIndexingResponse(
            indexed=1, errors=errors, processing_time_ms=80
        )
        self.mock_typeahead.index_queries.return_value = mock_response
        ...
        response_data = response.json()
        self.assertEqual(
            "Value error, query is required and must not be an empty string",
    ...
    def test_delete_all_queries_success(self):
        ...
        try:
            ...
            response = self.client.delete(
                "/indexes/test_index/suggestions/queries/delete-all"
            )
    ...
    def test_delete_queries_success(self):
        ...
        response = self.client.request(
            "DELETE", "/indexes/test_index/suggestions/queries", json=queries_list
        )

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), "Queries deleted successfully")
        self.mock_typeahead.delete_queries.assert_called_once_with(
            "test_index", ["query1", "query2"]
        )
    ...
    def test_get_queries_success(self):
        ...
        response = self.client.request(
            "GET", "/indexes/test_index/suggestions/queries", json=queries_list
        )
        ...
        self.mock_typeahead.get_queries.assert_called_once_with(
            "test_index", queries_list
        )
    ...
    def test_get_queries_empty_list(self):
        ...
        response = self.client.request(
            "GET", "/indexes/test_index/suggestions/queries", json=[]
        )
    ...
    def test_get_queries_invalid_request(self):
        ...
        response = self.client.request(
            "GET", "/indexes/test_index/suggestions/queries", json="123"
        )
