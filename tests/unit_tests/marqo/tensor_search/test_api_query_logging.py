...
class TestAPIQueryLogging(MarqoTestCase):
    def setUp(self):
        ...
        self.search_query = {"q": "test query", "searchMethod": "TENSOR", "limit": 10}

        self.mock_tensor_search_patcher = patch(
    ...
    @patch.dict(os.environ, {EnvVars.MARQO_LOG_QUERY_DETAILS: "TRUE"})
    ...
    def test_slow_query_logging_enabled_default_env_vars(self, mock_time):
        ...
        with patch(
            "marqo.core.search.query_logger.marqo_query_logger"
        ) as mock_marqo_query_logger:
            ...
            response = self.client.post(
                f"/indexes/{self.index_name}/search", json=self.search_query
            )
    ...
    @patch("marqo.tensor_search.telemetry.time")
    def test_validation_error_when_parsing_request_body_are_not_logged(
        self, mock_time, mock_parse_request
    ):
        ...
        with patch(
            ...
            response = self.client.post(
                f"/indexes/{self.index_name}/search", json=search_query
            )
    ...
    def test_do_not_log_vectors(self, mock_time):
        ...
        with patch(
            ...
            search_query = {
                "q": {"customVector": {"content": "abc", "vector": [0.1] * 768}},
                "searchMethod": "TENSOR",
                "limit": 10,
                "context": {
                    "tensor": [
            ...
            expected_query = {
                "q": {"customVector": {"content": "abc", "vector": []}},
                "searchMethod": "TENSOR",
                "limit": 10,
                "context": {
                    "tensor": [
        ...
        with patch(
            ...
            search_query = {
                ...
                "image_download_headers": {
                    "Authorization": "<BEARER TOKEN TOP SECRET>"
                },
            }
            ...
            search_query = {
                ...
                "hybridParameters": {
                    ...
                    "scoreModifiersLexical": {
                        "add_to_score": [
                            {"field_name": "epoch_timestamp", "weight": 0.01}
                        ]
                    },
