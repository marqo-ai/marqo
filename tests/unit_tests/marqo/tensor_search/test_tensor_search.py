...
from marqo.vespa.models.query_result import Child, Coverage, Root
...
class TestTensorSearch(MarqoTestCase):
    ...
    def setUp(self):
        ...
        self.mock_hit = Child(
            id="doc1", relevance=0.95, fields={"field1": "value1", "field2": "value2"}
        )
    ...
    def _assert_basic_search_response(
        self, result, expected_query, expected_limit=10, expected_offset=0
    ):
    ...
    def test_search_lexical_method(
        self, mock_metrics, mock_parse_query, mock_vespa_factory, mock_get_index
    ):
        """Test search with lexical method returns expected results."""
        # Setup
        mock_vespa_index = self._setup_common_mocks(
            mock_metrics, mock_vespa_factory, mock_get_index
        )
    ...
    @patch("marqo.tensor_search.tensor_search.RequestMetricsStore")
    def test_search_tensor_method(
        self, mock_metrics, mock_vectorise, mock_vespa_factory, mock_get_index
    ):
    ...
    def test_select_attributes_without_flattened_fields(self):
        ...
        expected = {"_id": "doc1", "_score": 0.95, "title": "test title"}
    ...
    def test_get_content_vector_not_found_error(self):
        ...
        self.assertIn(
            "could not find corresponding vector for content", str(cm.exception)
        )
    ...
    def test_get_content_vector_found_in_job(self):
        ...
        result = tensor_search.get_content_vector(
            possible_jobs, job_to_vectors, content
        )
    ...
    def test_get_query_vectors_string_query_with_context_fails(self):
        ...
        mock_index.dict.return_value = {"name": "test-index", "type": "semi_structured"}

        # Create a query with string q and context (this should fail)
        context = SearchContext(
            tensor=[SearchContextTensor(vector=[0.1, 0.2, 0.3], weight=1.0)]
        )
        ...
        self.assertIn(
            "Cannot use 'context' for a search with a string 'q'", str(cm.exception)
        )
        self.assertIn("test string query", str(cm.exception))
        self.assertIn(
            "provide a dictionary or a CustomVectorQuery object", str(cm.exception)
        )
    ...
    @patch("marqo.tensor_search.tensor_search.run_vectorise_pipeline")
    @patch("marqo.tensor_search.tensor_search.RequestMetricsStore")
    def test_search_with_base64_query_omitted_in_response(
        self, mock_metrics, mock_vectorise, mock_vespa_factory, mock_get_index
    ):
        ...
        test_cases = [
            ...
            },
            {
                ...
                "expected": {"data:image/[omitted]": 0.8, "regular_field": 0.2},
            },
        ]
...
class TestTensorSearchValidation(MarqoTestCase):
    ...
    def test_search_result_count_validation_negative_fails(self):
        ...
        with self.assertRaises(api_exceptions.IllegalRequestedDocCount) as cm:
            tensor_search.search(
                config=self.config, index_name="test", text="query", result_count=-1
            )
        self.assertIn(
            "result_count must be an integer greater than 0", str(cm.exception)
        )
    ...
    def test_search_result_count_validation_zero_fails(self):
        ...
        with self.assertRaises(api_exceptions.IllegalRequestedDocCount) as cm:
            tensor_search.search(
                config=self.config, index_name="test", text="query", result_count=0
            )
        self.assertIn(
            "result_count must be an integer greater than 0", str(cm.exception)
        )
    ...
    def test_search_result_count_validation_non_integer_fails(self):
        ...
        with self.assertRaises(api_exceptions.IllegalRequestedDocCount) as cm:
            tensor_search.search(
                config=self.config, index_name="test", text="query", result_count=5.5
            )
        self.assertIn(
            "result_count must be an integer greater than 0", str(cm.exception)
        )
    ...
    def test_search_max_docs_limit_validation_fails(self):
        ...
        self.assertIn(
            "search result limit + offset must be less than or equal", str(cm.exception)
        )
    ...
    def test_search_max_search_limit_validation_fails(self):
        ...
        self.assertIn(
            "search result limit must be less than or equal to the MARQO_MAX_SEARCH_LIMIT",
            str(cm.exception),
        )
    ...
    def test_search_max_search_offset_validation_fails(self):
        ...
        self.assertIn(
            "search result offset must be less than or equal to the MARQO_MAX_SEARCH_OFFSET",
            str(cm.exception),
        )
    ...
    def test_search_max_context_docs_validation_fails(self, mock_get_index):
        ...
        context = SearchContext(
            documents=SearchContextDocuments(ids={"doc1": 1, "doc2": 1, "doc3": 1})
        )
    ...
    def test_lexical_search_ef_search_invalid_arg_error(self, mock_get_index):
        ...
        self.assertIn(
            "efSearch is not a valid argument for lexical search", str(cm.exception)
        )
    ...
    def test_lexical_search_approximate_invalid_arg_error(self, mock_get_index):
        ...
        self.assertIn(
            "approximate is not a valid argument for lexical search", str(cm.exception)
        )
    ...
    def test_get_documents_by_ids_empty_collection_fails(self):
        ...
        with self.assertRaises(api_exceptions.InvalidArgError) as cm:
            tensor_search.get_documents_by_ids(
                config=self.config, index_name="test", document_ids=[]
            )
    ...
    def test_get_documents_by_ids_max_docs_limit_fails(self):
        ...
        self.assertIn(
            "documents were requested, which is more than the allowed limit",
            str(cm.exception),
        )
        ...
        test_cases = [
            (
                ...
                self.semi_structured_marqo_index(
                    name="old_index", marqo_version="2.22.0"
                ),
            ...
            (
                "index without collapseFields",
                self.semi_structured_marqo_index(name="new_index"),
                api_exceptions.InvalidArgError,
                "Field 'variant_id' is not configured as a collapse field for this index",
            ),
            (
                "index with different collapseFields",
                self.semi_structured_marqo_index(
...
class TestGetQueryVectorFromJobs(unittest.TestCase):
    def test_get_query_vector_from_jobs_fails_if_no_vector_is_collected(self):
        ...
        with self.assertRaises(api_exceptions.InvalidArgError) as cm:
            with patch(
                "marqo.tensor_search.telemetry.RequestMetricsStore.for_request"
            ) as mock_telemetry:
        ...
        self.assertIn(
            "Marqo could not collect any vectors from the search query but the retrieval "
            "or ranking method requires at least one valid vector",
            str(cm.exception),
        )
