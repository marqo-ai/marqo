...
class TestVectoriseJobs(unittest.TestCase):
    ...
    def test_vectorise_jobs_should_construct_inference_requests_for_all_modalities(
        self,
    ):
        test_cases = [
            ...
            (
                Modality.IMAGE,
                ImagePreprocessingConfig(
                    download_header={"header1": "value1"}, download_thread_count=1
                ),
            ),
            (
                Modality.AUDIO,
        ...
        for modality, expected_preprocessing in test_cases:
            with self.subTest(modality=modality):
                ...
                inference_request: InferenceRequest = (
                    self.inference.vectorise.call_args_list[0][0][0]
                )

                self.assertEqual(job.modality, inference_request.modality)
                self.assertEqual(job.content, inference_request.contents)
                self.assertEqual(
                    job.model_name, inference_request.model_config.model_name
                )
                self.assertEqual(
                    job.model_properties,
                    inference_request.model_config.model_properties,
                )
                self.assertEqual(
                    job.model_auth, inference_request.model_config.model_auth
                )
                self.assertEqual(
                    job.normalize_embeddings,
                    inference_request.model_config.normalize_embeddings,
                )
                ...
                self.assertEqual(
                    expected_preprocessing, inference_request.preprocessing_config
                )
    ...
    def test_vectorise_jobs_should_handle_multiple_jobs(self):
        ...
        self.inference.vectorise.side_effect = [
            InferenceResult(
                result=[[("a", np.array([1.0, 2.3]))], [("b", np.array([1.0, 2.3]))]]
            ),
        ...
        self.assertEqual(
            job1.modality, self.inference.vectorise.call_args_list[0][0][0].modality
        )
        self.assertEqual(
            job2.modality, self.inference.vectorise.call_args_list[1][0][0].modality
        )

    def test_vectorise_jobs_should_raise_internal_error_when_embedding_list_does_not_match_content(
        self,
    ):
        ...
        self.assertEqual(
            "Inference result contains embeddings for 1 query items, but 2 is expected",
            str(context.exception),
        )

    def test_vectorise_jobs_should_raise_internal_error_when_individual_errors_are_returned(
        self,
    ):
        job = vectorise_job(Modality.TEXT, ["a", "b"])
        self.inference.vectorise.side_effect = [
        ...
        with self.assertRaises(InternalError) as context:
            vectorise_jobs(self.inference, [job])
        self.assertEqual(
            "Individual errors returned when vectorising query string: ['a: an error']",
            str(context.exception),
        )

    def test_vectorise_jobs_should_raise_internal_error_when_inference_result_is_chunked(
        self,
    ):
        job = vectorise_job(Modality.TEXT, ["a", "b"])
        self.inference.vectorise.side_effect = [
            InferenceResult(
                result=[
                    [
                        ("a_chunk1", np.array([1.0, 2.3])),
                        ("a_chunk2", np.array([1.0, 2.3])),
                    ],
                    [("b", np.array([1.0, 2.3]))],
                ]
            )
        ...
        with self.assertRaises(InternalError) as context:
            vectorise_jobs(self.inference, [job])
        self.assertEqual(
            "Tensor query string should not be chunked but some query items "
            "have multiple chunks: [('a', 2)]",
            str(context.exception),
        )

    def test_vectorise_jobs_should_raise_bad_request_error_when_model_error_is_raised(
        self,
    ):
        job = vectorise_job(Modality.TEXT, ["a", "b"])
        ...
        with self.assertRaises(BadRequestError) as context:
            vectorise_jobs(self.inference, [job])
        self.assertEqual(
            "BadRequestError: Problem vectorising query. Reason: model error",
            str(context.exception),
        )

    def test_vectorise_jobs_should_raise_invalid_arg_error_when_other_inference_error_is_raised(
        self,
    ):
        job = vectorise_job(Modality.TEXT, ["a", "b"])
        ...
        with self.assertRaises(InvalidArgError) as context:
            vectorise_jobs(self.inference, [job])
        self.assertEqual(
            "InvalidArgError: Error vectorising content: ['a', 'b']. Message: inference error",
            str(context.exception),
        )
