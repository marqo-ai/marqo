...
from scripts.vespa_local.vespa_local import VespaLocalMultiNode, VespaLocalSingleNode
    ...
    def test_generate_docker_compose(self, patched_open):
        ...
        for number_of_shards, number_of_replicas in self.test_cases:
            with self.subTest(
                ...
                test_vespa_local = VespaLocalMultiNode(
        ...
        for number_of_shards, number_of_replicas in self.test_cases:
            with self.subTest(
                ...
                actual_services_xml_content = (
                    test_vespa_local.get_services_xml_content()
                )
        ...
        for number_of_shards, number_of_replicas in self.test_cases:
            with self.subTest(
                ...
                schema_file = os.path.join(
                    "vespa_dummy_application_package", "schemas", "test_vespa_client.sd"
                )
                ...
                services_file = os.path.join(
                    "vespa_dummy_application_package", "services.xml"
                )
                hosts_file = os.path.join(
                    "vespa_dummy_application_package", "hosts.xml"
                )
