...
class TestAddDocumentsResponseCollector(unittest.TestCase):
    ...
    def test_collect_error_response_should_capture_add_document_error_with_default_values(
        self,
    ):
        collector = AddDocumentsResponseCollector()
        collector.collect_error_response(
            "doc_id1", AddDocumentsError("error message"), loc=1
        )
    ...
    def test_collect_error_response_should_capture_add_document_error_with_custom_values(
        self,
    ):
        collector = AddDocumentsResponseCollector()
        collector.collect_error_response(
            "doc_id1",
            AddDocumentsError(
                "error message 2", error_code="err_code", status_code=403
            ),
            loc=1,
        )
        self.assertTrue(collector.errors)
    ...
    def test_collect_marqo_error_response_should_set_loc_to_none_if_doc_id_is_not_available(
        self,
    ):
        """
    ...
    def test_collect_marqo_error_response_should_set_id_as_empty_if_original_id_is_none(
        self,
    ):
        """
    ...
    def test_collect_marqo_error_response_should_set_doc_visited_if_original_id_is_present(
        self,
    ):
        """
    ...
    def test_collect_final_responses(self, mock_timer):
        ...
        collector.collect_error_response(
            "doc_id4", AddDocumentsError("error message 4"), loc=4
        )
        collector.collect_error_response(None, AddDocumentsError("error message 1"))
        collector.collect_error_response(
            "gen_doc_id2", AddDocumentsError("error message 2")
        )
        ...
        )  # doc_id4 is the original doc_id
        self.assertEqual("error message 4", response.items[0].message)
        self.assertEqual(
            "doc_id3", response.items[1].id
        )  # doc_id3 should be returned since it's persisted
        self.assertEqual(
            "", response.items[2].id
        )  # gen_doc_id2 is generated, should not be returned for error
        self.assertEqual("error message 2", response.items[2].message)
        self.assertEqual(
            "", response.items[3].id
        )  # doc_id1 error message does not contain id, this came last
