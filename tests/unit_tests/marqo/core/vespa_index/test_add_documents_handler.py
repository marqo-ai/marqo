...
class TestAddDocumentHandler(MarqoTestCase):
    def setUp(self):
        ...
        self.vespa_client.translate_vespa_document_response.side_effect = (
            VespaClient.translate_vespa_document_response
        )
        self.inference = MagicMock(spec=Inference)

        def vectorise_side_effect(request: InferenceRequest) -> InferenceResult:
            return InferenceResult(
                result=[[("chunk", np.array([1.0, 2.0]))] for _ in request.contents]
            )

        self.inference.vectorise.side_effect = vectorise_side_effect

    def test_add_documents_main_workflow_happy_path(self):
        self.vespa_client.get_batch.side_effect = [
            GetBatchResponse(
                errors=True,
                responses=[
                    GetBatchDocumentResponse(
                        id="id:index1:index1::1",
                        pathId="path_id1",
                        document=Document(
                            id="id:index1:index1:1", fields={"marqo__id": "1"}
                        ),
                        status=200,
                    ),
                    GetBatchDocumentResponse(
                        id="id:index1:index1::2", pathId="path_id2", status=404
                    ),
                    GetBatchDocumentResponse(
                        id="id:index1:index1::3", pathId="path_id3", status=404
                    ),
                ],
            )
        ]
        self.vespa_client.feed_batch.side_effect = [
            FeedBatchResponse(
                errors=False,
                responses=[
                    FeedBatchDocumentResponse(
                        id="id:index1:index1::1", pathId="path_id1", status=200
                    ),
                    FeedBatchDocumentResponse(
                        id="id:index1:index1::2", pathId="path_id2", status=200
                    ),
                    FeedBatchDocumentResponse(
                        id="id:index1:index1::3", pathId="path_id3", status=200
                    ),
                ],
            )
        ]
            ...
            add_docs_params=AddDocsParams(
                ...
                docs=[
                    ...
                    },
                    {
                        "_id": "3",
                        "field2": ["de"],
                        "field5": {"content": "a", "vector": [0.1] * 32},
                    },
                ],
            ),
        )
        ...
        self.assertEqual(
            [Document(id="id:index1:index1:1", fields={"marqo__id": "1"})],
            handler.existing_vespa_docs,
        )  # only the doc with 200 status code is passed to the method
        ...
        self.assertEqual(
            {
                ...
                (
                    "1",
                    "field5",
                ),  # the second doc with the same id overrides the first one
            },
            set(handler.handled_fields),
        )
        self.assertEqual(1, handler.to_vespa_doc_call_count)

    def test_add_documents_should_skip_duplicate_documents_even_when_the_latter_one_errors_out(
        self,
    ):
        self.vespa_client.feed_batch.side_effect = [
            FeedBatchResponse(responses=[], errors=False)
        ]
        ...
        self.assertEqual(
            0, self.vespa_client.feed_batch.call_count
        )  # feed_batch should not be called when no valid docs remain
        ...
        self.vespa_client.feed_batch.side_effect = [
            FeedBatchResponse(
                ...
                responses=[
                    ...
                    FeedBatchDocumentResponse(
                        id="id:index1:index1::2",
                        pathId="path_id2",
                        status=429,
                        message="vespa error2",
                    ),
                    FeedBatchDocumentResponse(
                        id="id:index1:index1::3",
                        pathId="path_id3",
                        status=507,
        ...
        handler = DummyAddDocumentsHandler(
            ...
            add_docs_params=AddDocsParams(
                ...
                docs=[
                    ...
                    {
                        "bad_field": "bad_content"
                    },  # error out when converting to vespa doc
                    {"_id": [5], "field4": ["de"]},  # doc with invalid id
                    {
                        "field4": ["de"],
                        "field5": "a very large string object" * 10000,
                    },  # doc too large
                    {},  # empty doc
        ...
        self.assertEqual(
            [
                ...
                MarqoAddDocumentsItem(
                    status=400,
                    id="",
                    message="MarqoDocumentParsingError",
                    error="MarqoDocumentParsingError",
                ...
                MarqoAddDocumentsItem(
                    status=400,
                    id="",
                    message="Can't index an empty dict.",
                    error="Can't index an empty dict.",
                    code="invalid_argument",
                ),
                MarqoAddDocumentsItem(
                    status=400,
                    id="",
                    message="Docs must be dicts",
    ...
    def test_preprocessing_config_for_text_modality(self):
        ...
        for_top_level_field = handler._get_preprocessing_config(
            Modality.TEXT, for_top_level_field=True
        )
        self.assertEqual(for_top_level_field.text_prefix, "prefix1:")
        self.assertTrue(for_top_level_field.should_chunk)
        self.assertEqual(
            for_top_level_field.chunk_config,
            TextChunkConfig(split_length=100, split_overlap=10, split_method="word"),
        )

        for_subfield = handler._get_preprocessing_config(
            Modality.TEXT, for_top_level_field=False
        )
    ...
    def test_preprocessing_config_for_image_modality_without_patch_method(self):
        ...
        for_top_level_field = handler._get_preprocessing_config(
            Modality.IMAGE, for_top_level_field=True
        )
        ...
        for_subfield = handler._get_preprocessing_config(
            Modality.IMAGE, for_top_level_field=False
        )
        self.assertFalse(for_subfield.should_chunk)
    ...
    def test_preprocessing_config_for_audio_modality(self):
        handler = DummyAddDocumentsHandler(
            ...
            marqo_index=fixture.unstructured_marqo_index(
                ...
                audio_preprocessing=AudioPreProcessing(
                    split_length=25, split_overlap=5
                ),
            ),
            add_docs_params=AddDocsParams(
                index_name="index1",
        ...
        for_top_level_field = handler._get_preprocessing_config(
            Modality.AUDIO, for_top_level_field=True
        )
        self.assertTrue(for_top_level_field.should_chunk)
        self.assertEqual(
            for_top_level_field.chunk_config,
            ChunkConfig(split_length=25, split_overlap=5),
        )
        self.assertEqual(for_top_level_field.download_header, {"a": "b"})
        self.assertEqual(for_top_level_field.download_thread_count, 4)

        for_subfield = handler._get_preprocessing_config(
            Modality.AUDIO, for_top_level_field=False
        )
    ...
    def test_preprocessing_config_for_video_modality(self):
        ...
        for_top_level_field = handler._get_preprocessing_config(
            Modality.VIDEO, for_top_level_field=True
        )
        self.assertTrue(for_top_level_field.should_chunk)
        self.assertEqual(
            for_top_level_field.chunk_config,
        ...
        for_subfield = handler._get_preprocessing_config(
            Modality.VIDEO, for_top_level_field=False
        )
        self.assertTrue(for_subfield.should_chunk)
        self.assertEqual(
            for_top_level_field.chunk_config,
