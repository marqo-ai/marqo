...
class TestStructuredAddDocumentsHandler(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.handler = StructuredAddDocumentsHandler(
            ...
            marqo_index=cls.structured_marqo_index(
                "index1", "index1", fields=[], tensor_fields=[]
            ),
    ...
    def test_infer_modality_should_return_modality_based_on_field_type(self):
        test_cases = [
            (
                "Hello World",
                FieldType.Text,
                "string should be inferred as text",
                Modality.TEXT,
            ),
            (
                self.AUDIO_URL,
                FieldType.AudioPointer,
                "audio url should be inferred as audio",
                Modality.AUDIO,
            ),
            (
                self.VIDEO_URL,
                FieldType.VideoPointer,
                "video url should be inferred as video",
                Modality.VIDEO,
            ),
            (
                self.IMAGE_URL,
                FieldType.ImagePointer,
                "image url should be inferred as image",
                Modality.IMAGE,
            ),
        ]
    ...
    def test_infer_modality_should_ignore_content(self):
        test_cases = [
            (
                self.AUDIO_URL,
                FieldType.Text,
                "audio url should be inferred as text",
                Modality.TEXT,
            ),
            (
                self.IMAGE_URL,
                FieldType.Text,
                "image url should be inferred as text",
                Modality.TEXT,
            ),
            (
                self.VIDEO_URL,
            ...
            (
                self.IMAGE_URL,
                FieldType.AudioPointer,
                "image url should be inferred as audio",
                Modality.AUDIO,
            ),
            (
                self.VIDEO_URL,
                FieldType.AudioPointer,
                "video url should be inferred as audio",
                Modality.AUDIO,
            ),
            (
                self.IMAGE_URL,
                FieldType.VideoPointer,
                "image url should be inferred as video",
                Modality.VIDEO,
            ),
            (
                self.AUDIO_URL,
                FieldType.VideoPointer,
                "audio url should be inferred as video",
                Modality.VIDEO,
            ),
            (
                self.VIDEO_URL,
                FieldType.ImagePointer,
                "video url should be inferred as image",
                Modality.IMAGE,
            ),
            (
                self.AUDIO_URL,
                FieldType.ImagePointer,
                "audio url should be inferred as image",
                Modality.IMAGE,
            ),
        ]
        for content, field_type, test_case, expected_modality in test_cases:
    ...
    def test_infer_modality_should_raise_error_for_unsupported_field_type(self):
        ...
        test_cases = [
            field_type for field_type in FieldType if field_type not in supported_types
        ] + [None]
        ...
        for unsupported_type in test_cases:
            with self.subTest(msg=unsupported_type):
                ...
                self.assertIn(
                    f"Error processing field1, tensor field type {unsupported_type} "
                    f"is not supported",
