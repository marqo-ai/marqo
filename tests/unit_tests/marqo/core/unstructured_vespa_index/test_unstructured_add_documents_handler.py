...
from marqo.core.models.marqo_index import ImagePreProcessing, PatchMethod
...
class TestUnstructuredAddDocumentsHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher = patch(
            "marqo.core.unstructured_vespa_index.unstructured_add_document_handler.infer_modality"
        )
        ...
        with self.assertRaises(AddDocumentsError) as context:
            handler._infer_modality(
                TensorField(
                    doc_id="id",
                    field_name="field1",
                    field_content=self.INVALID_URL,
                    is_top_level_tensor_field=True,
                )
            )
        self.assertIn(
            f"Error processing field1: Error downloading media file {self.INVALID_URL}",
            str(context.exception),
        )

    def test_vectorise_tensor_fields_should_call_vectorise_with_all_modalities(self):
        handler = self._get_handler(
            treat_as_images=True,
            treat_as_media=True,
            add_docs_params=AddDocsParams(
                ...
                docs=[
                    {"_id": "1", "text_field": "hello", "image_field": self.IMAGE_URL},
                    {
                        "_id": "2",
                        "text_field": "hello2",
                        "audio_field": self.AUDIO_URL,
                        "video_field": self.VIDEO_URL,
                    },
                ],
            ),
        )
        ...
        modality_collected = set(
            [
                self.inference.vectorise.call_args_list[i][0][0].modality
                for i in range(4)
            ]
        )
        self.assertSetEqual(
    ...
    def test_vectorise_tensor_fields_should_group_contents_for_the_same_modality_in_a_batch(
        self,
    ):
        handler = self._get_handler(
        ...
        self.assertSetEqual(
            set(contents),
            {"hello", "hello2", self.IMAGE_URL, self.AUDIO_URL, self.VIDEO_URL},
        )
    ...
    def test_vectorise_tensor_fields_should_skip_docs_with_issue_infer_modality(self):
        ...
        self.assertEqual(self.inference.vectorise.call_count, 1)
        self.assertEqual(
            self.inference.vectorise.call_args_list[0][0][0].contents, ["hello2"]
        )
        ...
        self.assertEqual(
            res.items[0].message,
            f"Error processing image_field: "
    ...
    def test_vectorise_tensor_fields_should_collect_individual_inference_error(self):
        handler = self._get_handler(
            ...
            add_docs_params=AddDocsParams(
                index_name="index1",
                tensor_fields=["text_field", "text_field2"],
                docs=[
    ...
    def test_vectorise_tensor_fields_should_populate_chunks_and_embeddings(self):
        ...
        vespa_doc: VespaDocument = self.vespa_client.feed_batch.call_args_list[0][0][0][
            0
        ]
        self.assertEqual(
            vespa_doc.fields["marqo__chunks"],
            ["text_field::chunk1", "text_field::chunk2"],
        )
        self.assertEqual(
            vespa_doc.fields["marqo__embeddings"], {"0": [1.0, 2.0], "1": [2.0, 4.0]}
        )

    def test_vectorise_tensor_fields_should_call_vectorise_for_multimodal_subfields(
        self,
    ):
        handler = self._get_handler(
            treat_as_images=True,
    ...
    def test_vectorise_tensor_fields_should_call_vectorise_for_top_level_tensor_fields_and_subfields_image_text(
        self,
    ):
        handler = self._get_handler(
            treat_as_images=True,
            ...
            add_docs_params=AddDocsParams(
                index_name="index1",
                tensor_fields=["combo_field", "text_field", "image_field"],
                mappings={
                    "captioned_image": {
                        "type": "multimodal_combination",
                        "weights": {"text_field": 0.3, "image_field": 0.7},
            ...
            patch_method=PatchMethod.Simple,
        )  # we also chunk image
        ...
        requests: List[InferenceRequest] = [
            self.inference.vectorise.call_args_list[i][0][0] for i in range(4)
        ]
        self.assertTrue(
            any(
                [
                    r.modality == Modality.TEXT and r.preprocessing_config.should_chunk
                    for r in requests
                ]
            )
        )
        self.assertTrue(
            any(
                [
                    r.modality == Modality.TEXT
                    and not r.preprocessing_config.should_chunk
                    for r in requests
                ]
            )
        )
        self.assertTrue(
            any(
                [
                    r.modality == Modality.IMAGE and r.preprocessing_config.should_chunk
                    for r in requests
                ]
            )
        )
    ...
    def test_vectorise_tensor_fields_should_call_vectorise_for_top_level_tensor_fields_and_subfields_audio_video(
        self,
    ):
        handler = self._get_handler(
            treat_as_images=True,
            ...
            add_docs_params=AddDocsParams(
                index_name="index1",
                tensor_fields=["combo_field", "video_field", "audio_field"],
                mappings={
                    "captioned_image": {
                        "type": "multimodal_combination",
        ...
        self.assertEqual(self.inference.vectorise.call_count, 2)
        requests: List[InferenceRequest] = [
            self.inference.vectorise.call_args_list[i][0][0] for i in range(2)
        ]
        self.assertTrue(
            any(
                [
                    r.modality == Modality.AUDIO and r.preprocessing_config.should_chunk
                    for r in requests
                ]
            )
        )
        self.assertTrue(
            any(
                [
                    r.modality == Modality.VIDEO and r.preprocessing_config.should_chunk
                    for r in requests
                ]
            )
        )

    def test_vectorise_tensor_fields_should_propagate_inference_errors_for_the_batch(
        self,
    ):
    ...
    def test_vectorise_tensor_fields_should_raise_inference_error_if_result_size_does_not_match_contents(
        self,
    ):
        self.inference.vectorise.side_effect = [
            InferenceResult(result=[[("chunk", np.array([1.0, 2.0]))]])
        ]
        ...
        with self.assertRaises(InternalError) as context:
            handler.add_documents()
        self.assertEqual(
            "Inference result contains chunks and embeddings for 1 fields, but 2 are expected",
            str(context.exception),
        )
