...
class TestSemiStructuredVespaDocument(MarqoTestCase):
    ...
    def test_from_marqo_document_with_collapse_field(self):
        ...
        semi_structured_doc = SemiStructuredVespaDocument.from_marqo_document(
            marqo_doc, marqo_index
        )
        ...
        self.assertEqual(
            "Test Title Content",
            semi_structured_doc.text_fields["marqo__lexical_title"],
        )
    ...
    def test_from_marqo_document_non_collapse_field_not_in_field_map_raises_error(self):
        ...
        self.assertIn(
            "Field unknown_field is not in index test_index", str(cm.exception)
        )
