...
class TestSemiStructuredVespaIndex(MarqoTestCase):
    def setUp(self):
        self.vespa_index = SemiStructuredVespaIndex(
        ...
        self.vespa_index_prior_to_216 = SemiStructuredVespaIndex(
            self.semi_structured_marqo_index(
                name="index1",
                lexical_field_names=["title", "desc"],
                tensor_field_names=["title"],
    ...
    def test_get_filter_string_escaped_characters(self):
        ...
        test_cases = [
            ...
            ("title:hello", 'key contains "title", value contains "hello"'),
            # Unescaped backslash gets ignored (double quote does not need to be escaped by user)
            ('title:hel"l\\o', 'key contains "title", value contains "hel\\"lo"'),
    ...
    @patch(
        "marqo.core.semi_structured_vespa_index.semi_structured_vespa_index.SemiStructuredVespaIndex.get_marqo_index"
    )
    def test_get_filter_string_collapse_field(self, mock_get_marqo_index):
        ...
        mock_index.is_collapse_field.side_effect = lambda field: field in [
            "parent_id",
            "variant_id",
        ]
        ...
        test_cases = [
            ...
            (
                "color:red",
                '((marqo__short_string_fields contains sameElement(key contains "color", value contains "red")))',
            ),
        ]
    ...
    def test_vespa_to_marqo_conversion_should_handle_all_fields_from_search_result(
        self,
    ):
        vespa_doc = {
    ...
    def test_vespa_to_marqo_conversion_should_handle_highlights(self):
        ...
        marqo_doc = self.vespa_index.to_marqo_document(
            vespa_doc, return_highlights=True
        )
        self.assertEqual(
            {
                "_id": "123",
    ...
    def test_vespa_to_marqo_conversion_should_convert_all_fields_from_get_document_result(
        self,
    ):
        vespa_doc = {
            "id": "index:index1/1/123",
            "fields": {
                ...
                "marqo__score_modifiers": {
                    "cells": {"int_field1": 1, "int_map1.a": 3, "int_map1.b": 4},
                    "type": "tensor(p{})",
                },
                "marqo__multimodal_params": {
                    "multi_modal": '{"weights": {"title": 1.0, "desc": 0.5}, "type": "multimodal_combination"}'
                },
                "marqo__chunks_multi_modal": [
                    '{"title": "Test document 2", "desc": "desc1"}'
                ],
        ...
        self.assertEqual(
            {
                ...
                "desc": "some awesome product",
                "marqo__tensors": {
                    "title": {"chunks": ["some product"], "embeddings": [[1.0, 2.0]]}
                },
                "multimodal_params": {
                    "multi_modal": {
    ...
    def test_vespa_to_marqo_conversion_should_handle_combined_string_array_fields(self):
        ...
        self.assertEqual(
            {"_id": "123", "tags": ["foo", "bar"], "markets": ["fr", "au"]}, marqo_doc
        )

    @patch(
        "marqo.core.semi_structured_vespa_index.semi_structured_document.generate_uuid_str"
    )
