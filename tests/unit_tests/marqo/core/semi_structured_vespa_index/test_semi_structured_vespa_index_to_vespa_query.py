        ...
        for threshold in threshold_values:
            with self.subTest(approximate_threshold=threshold):
                ...
                if threshold is not None:
                    ...
                    self.assertEqual(
                        vespa_query["ranking.matching.approximateThreshold"], threshold
                    )
                else:
                    # When threshold is None, it should not be included in the query
                    self.assertNotIn(
                        "ranking.matching.approximateThreshold", vespa_query
                    )
        ...
        for test_case in test_cases:
            with self.subTest(case=test_case["name"]):
                ...
                if "expected_query_features" in test_case:
                    self.assertEqual(
                        test_case["expected_query_features"],
                        vespa_query["query_features"],
                    )
                    self.assertEqual(
                        test_case["expected_language"], vespa_query["language"]
                    )
                    self.assertEqual(test_case["expected_hits"], vespa_query["hits"])
                    self.assertEqual(
                        test_case["expected_offset"], vespa_query["offset"]
                    )
...
class TestSemiStructuredIndexToVespaQuerySortBy(TestCase):
    ...
    def test_sort_by_single_field_no_optional(self):
        ...
        self.hybrid_query.sort_by = SortByModel(
            fields=[{"field_name": "title", "order": "asc"}], minSortCandidates=30
        )
    ...
    def test_sort_by_with_missing_first(self):
        ...
        self.hybrid_query.sort_by = SortByModel(
            fields=[{"field_name": "description", "order": "asc", "missing": "first"}],
            sortDepth=2,
    ...
    def test_query_features_sort_field_weights_3_fields(self):
        ...
        for _ in range(20):
            ...
            self.hybrid_query.sort_by = SortByModel(fields=test_fields)
            ...
            for i, field in enumerate(self.hybrid_query.sort_by.fields):
                ...
                self.assertIn(
                    field_name, query_features[f"marqo__sort_field_weights_{i}"]
                )
                self.assertEqual(
                    1, query_features[f"marqo__sort_field_weights_{i}"][field_name]
                )
...
class TestSemiStructuredIndexToVespaQueryRelevanceCutoff(TestCase):
    ...
    def test_relative_max_score_default_probeDepth(self):
        ...
        self.assertAlmostEqual(
            0.8, r["marqo__hybrid.relevanceCutoff.parameters.relativeScoreFactor"]
        )
    ...
    def test_mean_std_dev_default_probeDepth(self):
        ...
        self.assertEqual(
            RelevanceCutoffMethod.MeanStdDev, r["marqo__hybrid.relevanceCutoff.method"]
        )
        self.assertAlmostEqual(
            2.5, r["marqo__hybrid.relevanceCutoff.parameters.stdDevFactor"]
        )
        self.assertEqual(1000, r["marqo__hybrid.relevanceCutoff.probeDepth"])
        # no relativeScoreFactor for this method
        self.assertNotIn(
            "marqo__hybrid.relevanceCutoff.parameters.relativeScoreFactor", r
        )
    ...
    def test_mean_std_dev_custom_probeDepth(self):
        ...
        self.hybrid_query.relevance_cutoff = RelevanceCutoffModel(
            method=RelevanceCutoffMethod.MeanStdDev, parameters=params, probe_depth=7
        )
    ...
    def test_gap_detection_default_and_custom_probeDepth(self):
        ...
        r = self.index.to_vespa_query(self.hybrid_query)
        self.assertEqual(
            RelevanceCutoffMethod.GapDetection,
            r["marqo__hybrid.relevanceCutoff.method"],
        )
        self.assertEqual(1000, r["marqo__hybrid.relevanceCutoff.probeDepth"])
        self.assertNotIn(
        ...
        # custom probeDepth
        self.hybrid_query.relevance_cutoff = RelevanceCutoffModel(
            method=RelevanceCutoffMethod.GapDetection, probe_depth=42
        )
    ...
    def test_relevance_cutoff_edge_case_values(self):
        ...
        r = self.index.to_vespa_query(self.hybrid_query)
        self.assertEqual(
            0.001, r["marqo__hybrid.relevanceCutoff.parameters.relativeScoreFactor"]
        )
        ...
        r = self.index.to_vespa_query(self.hybrid_query)
        self.assertAlmostEqual(
            1.0, r["marqo__hybrid.relevanceCutoff.parameters.relativeScoreFactor"]
        )
    ...
    def test_relevance_cutoff_std_dev_edge_cases(self):
        ...
        self.hybrid_query.relevance_cutoff = RelevanceCutoffModel(
            method=RelevanceCutoffMethod.MeanStdDev, parameters=params, probe_depth=50
        )
        ...
        self.assertAlmostEqual(
            0.1, r["marqo__hybrid.relevanceCutoff.parameters.stdDevFactor"]
        )
        ...
        r = self.index.to_vespa_query(self.hybrid_query)
        self.assertAlmostEqual(
            10.0, r["marqo__hybrid.relevanceCutoff.parameters.stdDevFactor"]
        )
...
class TestSemiStructuredVespaIndexToVespaQueryCollapseFields(MarqoTestCase):
    def setUp(self):
        marqo_index = self.semi_structured_marqo_index(
            "test_index", collapse_fields=[CollapseField(name="parent_id")]
        )
    ...
    def test_hybrid_query_with_collapse_fields(self):
        ...
        self.assertEqual(
            common.RANK_PROFILE_BM25 + "_diversity",
            vespa_query["marqo__ranking.lexical.lexical"],
        )
        self.assertEqual(
            common.RANK_PROFILE_EMBEDDING_SIMILARITY + "_diversity",
            vespa_query["marqo__ranking.tensor.tensor"],
        )
        self.assertEqual(
            common.RANK_PROFILE_HYBRID_BM25_THEN_EMBEDDING_SIMILARITY + "_diversity",
            vespa_query["marqo__ranking.lexical.tensor"],
        )
        self.assertEqual(
            common.RANK_PROFILE_HYBRID_EMBEDDING_SIMILARITY_THEN_BM25 + "_diversity",
            vespa_query["marqo__ranking.tensor.lexical"],
        )

        # assert facets query has an extra grouping
        self.assertEqual(
            "select * from test_index where (false OR False) limit 0 | all(group(1.1) "
    ...
    def test_hybrid_query_without_collapse_fields(self):
        ...
        self.assertEqual(
            common.RANK_PROFILE_BM25, vespa_query["marqo__ranking.lexical.lexical"]
        )
        self.assertEqual(
        ...
        self.assertEqual(
            common.RANK_PROFILE_HYBRID_BM25_THEN_EMBEDDING_SIMILARITY,
            vespa_query["marqo__ranking.lexical.tensor"],
        )
        self.assertEqual(
            common.RANK_PROFILE_HYBRID_EMBEDDING_SIMILARITY_THEN_BM25,
            vespa_query["marqo__ranking.tensor.lexical"],
        )
...
class TestSemiStructuredVespaIndexToVespaQueryFacets(MarqoTestCase):
    ...
    def test_facets_query_with_multiple_or_phrases_and_filter_for_lexical_retriever(
        self,
    ):
        test_cases = [
            ("lexical", "lexical"),
        ...
        for retrieval_method, ranking_method in test_cases:
            ...
            vespa_query = self.vespa_index.to_vespa_query(marqo_query)
            self.assertEqual(
                'select * from test_index where (default contains "hello" OR default contains '
...
class TestSemiStructuredVespaIndexCollapseFieldAttributesToRetrieve(MarqoTestCase):
    ...
    def test_to_vespa_query_does_not_duplicate_collapse_field_in_attributes_to_retrieve(
        self,
    ):
        ...
        hybrid_query = MarqoHybridQuery(
            ...
            attributes_to_retrieve=[
                "title",
                "parent_id",
            ],  # collapse field already present
            collapse_field_name="parent_id",
            limit=10,
            offset=0,
        )
    ...
    def test_to_vespa_query_does_not_add_collapse_field_to_attributes_if_not_provided(
        self,
    ):
    ...
    def test_to_vespa_query_does_not_add_collapse_field_to_attributes_if_attributes_to_retrieve_is_none(
        self,
    ):
        """Test that collapse field is not added to attributes_to_retrieve if not provided"""
        hybrid_query = MarqoHybridQuery(
