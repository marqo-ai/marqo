...
class TestHybridSearch(TestCase):
    ...
    def test_search_creates_correct_marqo_hybrid_query(
        ...
        marqo_index.parsed_marqo_version.return_value = semver.VersionInfo.parse(
            "2.21.0"
        )
        ...
        mock_vespa_index.gather_facets_from_response.return_value = {
            "facets": {"test_field": {}}
        }
        ...
        with patch(
            "marqo.core.search.hybrid_search.MarqoHybridQuery"
        ) as mock_marqo_query:
            ...
            score_modifiers_lexical.to_marqo_score_modifiers.return_value = [
                ScoreModifier(
                    field="field2", weight=2.0, type=ScoreModifierType.Multiply
                )
            ]
    ...
    def test_search_custom_vector_query_with_existing_context_tensor_none(
        ...
        custom_query = CustomVectorQuery(
            customVector=CustomVectorQuery.CustomVector(
                content="test content", vector=[0.5, 0.6, 0.7]
            )
        )
