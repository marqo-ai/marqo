...
from marqo.core.models.marqo_index import IndexType, StructuredMarqoIndex
...
class TestRecommenderGetDocVectorsFromIds(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.recommender = Recommender(
            self.mock_vespa_client, self.mock_index_management, self.mock_inference
        )
    ...
    def test_successful_get_vectors_list_ids(
        self, mock_config_class, mock_get_vectors, mock_get_index
    ):
        ...
        mock_get_vectors.return_value = {
            ...
            "doc2": {"title": [[1.1, 1.2, 1.3]], "description": [[1.7, 1.8, 1.9]]},
        }

        # Call the method
        result = self.recommender.get_doc_vectors_from_ids(
            index_name="test_index", documents=["doc1", "doc2"]
        )
    ...
    @patch("marqo.tensor_search.tensor_search.get_doc_vectors_per_tensor_field_by_ids")
    @patch("marqo.config.Config")
    def test_successful_get_vectors_dict_ids(
        self, mock_config_class, mock_get_vectors, mock_get_index
    ):
    ...
    @patch("marqo.tensor_search.tensor_search.get_doc_vectors_per_tensor_field_by_ids")
    @patch("marqo.config.Config")
    def test_specific_tensor_fields(
        self, mock_config_class, mock_get_vectors, mock_get_index
    ):
    ...
    @patch("marqo.tensor_search.tensor_search.get_doc_vectors_per_tensor_field_by_ids")
    @patch("marqo.config.Config")
    def test_document_without_vectors(
        self, mock_config_class, mock_get_vectors, mock_get_index
    ):
    ...
    def test_empty_documents_list(self):
        ...
        with self.assertRaises(InvalidArgumentError) as cm:
            self.recommender.get_doc_vectors_from_ids(
                index_name="test_index", documents=[]
            )
    ...
    def test_non_string_ids_fails(self):
        ...
        with self.assertRaises(InvalidDocumentIdError) as cm:
            self.recommender.get_doc_vectors_from_ids(
                index_name="test_index", documents=[123, 456]
            )
    ...
    def test_none_documents(self):
        ...
        with self.assertRaises(InvalidArgumentError) as cm:
            self.recommender.get_doc_vectors_from_ids(
                index_name="test_index", documents=None
            )
    ...
    def test_all_zero_weight_documents(self):
        ...
        with self.assertRaises(InvalidArgumentError) as cm:
            self.recommender.get_doc_vectors_from_ids(
                index_name="test_index", documents={"doc1": 0.0, "doc2": 0.0}
            )
    ...
    def test_recommend_with_interpolation_method_none(
        self, mock_search, mock_config_class, mock_get_index
    ):
    ...
    @patch("marqo.config.Config")
    @patch("marqo.tensor_search.tensor_search.search")
    def test_recommend_with_dict_documents_filtering(
        self, mock_search, mock_config_class, mock_get_index
    ):
        ...
        with patch.object(
            ...
            with patch.object(
                self.recommender, "get_exclusion_filter"
            ) as mock_get_filter:
    ...
    @patch("marqo.config.Config")
    @patch("marqo.tensor_search.tensor_search.search")
    def test_recommend_slerp_all_zero_weights_error(
        self, mock_search, mock_config_class, mock_get_index
    ):
        ...
        with patch.object(
            ...
            with patch(
                "marqo.core.utils.vector_interpolation.from_interpolation_method"
            ) as mock_from_interp:
                mock_interpolation = Mock()
                mock_interpolation.interpolate.side_effect = AllZeroWeightsError(
                ...
                self.assertIn(
                    "Cannot interpolate vectors with all zero weights",
                    str(cm.exception),
                )
    ...
    def test_get_doc_vectors_from_ids_with_invalid_document_ids_fails(self):
        ...
        with self.assertRaises(
            Exception
        ):  # Specific exception depends on validation layer
            self.recommender.get_doc_vectors_from_ids(
                index_name="test_index",
    ...
    @patch("marqo.tensor_search.index_meta_cache.get_index")
    def test_get_doc_vectors_from_ids_with_nonexistent_index_fails(
        self, mock_get_index
    ):
        ...
        with self.assertRaises(IndexNotFoundError):
            self.recommender.get_doc_vectors_from_ids(
                index_name="nonexistent_index", documents=["doc1"]
            )

    @patch("marqo.tensor_search.index_meta_cache.get_index")
    def test_get_doc_vectors_from_ids_with_invalid_tensor_fields_fails(
        self, mock_get_index
    ):
    ...
    @patch("marqo.tensor_search.tensor_search.get_doc_vectors_per_tensor_field_by_ids")
    @patch("marqo.config.Config")
    def test_get_doc_vectors_from_ids_with_missing_documents_fails(
        self, mock_config_class, mock_get_vectors, mock_get_index
    ):
        ...
        self.assertIn(
            "The following document IDs were not found: doc2", str(cm.exception)
        )

    @patch("marqo.tensor_search.index_meta_cache.get_index")
    @patch("marqo.tensor_search.tensor_search.get_doc_vectors_per_tensor_field_by_ids")
    @patch("marqo.config.Config")
    def test_get_doc_vectors_from_ids_with_documents_without_embeddings_fails(
        self, mock_config_class, mock_get_vectors, mock_get_index
    ):
        ...
        self.assertIn(
            "The following documents do not have embeddings: doc1", str(cm.exception)
        )

    @patch("marqo.tensor_search.index_meta_cache.get_index")
    @patch("marqo.config.Config")
    @patch("marqo.tensor_search.tensor_search.search")
    def test_recommend_with_lerp_all_zero_weights_error(
        self, mock_search, mock_config_class, mock_get_index
    ):
    ...
    def test_get_exclusion_filter_with_user_filter(self):
        ...
        result = self.recommender.get_exclusion_filter(
            mock_index, ["doc1"], "category:books"
        )
    ...
    def test_single_duplicate_document_id_in_list_fails(self):
        ...
        with self.assertRaises(InvalidArgumentError) as cm:
            self.recommender.get_doc_vectors_from_ids(
                index_name="test_index", documents=["doc1", "doc2", "doc3", "doc1"]
            )
    ...
    def test_recommend_with_duplicate_document_ids_fails(self):
        ...
        with self.assertRaises(InvalidArgumentError) as cm:
            self.recommender.recommend(
                index_name="test_index", documents=["doc1", "doc2", "doc1"]
            )
    ...
    @patch("marqo.tensor_search.tensor_search.get_doc_vectors_per_tensor_field_by_ids")
    @patch("marqo.config.Config")
    def test_get_doc_vectors_allow_missing_documents_true(
        self, mock_config_class, mock_get_vectors, mock_get_index
    ):
        ...
        expected = {"doc1": [[0.1, 0.2, 0.3]], "doc3": [[0.7, 0.8, 0.9]]}
        self.assertEqual(result, expected)
    ...
    @patch("marqo.tensor_search.tensor_search.get_doc_vectors_per_tensor_field_by_ids")
    @patch("marqo.config.Config")
    def test_get_doc_vectors_allow_missing_embeddings_true(
        self, mock_config_class, mock_get_vectors, mock_get_index
    ):
        ...
        self.assertEqual({"doc1": [[0.1, 0.2, 0.3]]}, r)
    ...
    @patch("marqo.tensor_search.tensor_search.get_doc_vectors_per_tensor_field_by_ids")
    @patch("marqo.config.Config")
    def test_get_doc_vectors_allow_both_missing_parameters_true(
        self, mock_config_class, mock_get_vectors, mock_get_index
    ):
