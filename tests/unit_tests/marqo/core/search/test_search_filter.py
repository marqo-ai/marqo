...
class TestMarqoFilterStringParser(MarqoTestCase):
    def test_parse_successful(self):
        test_cases = [
            ("a:b", SearchFilter(root=EqualityTerm("a", "b", "a:b")), "single term"),
            (
                "(a:b)",
            ...
            (
                "a:(hello world)",
                SearchFilter(EqualityTerm("a", "hello world", "a:(hello world)")),
                "grouped text",
            ),
            # After colon, parenthesis does NOT define an IN list.
            (
                "a:(1, 2, 3)",
                SearchFilter(EqualityTerm("a", "1, 2, 3", "a:(1, 2, 3)")),
                "After colon, parenthesis does NOT define an IN list",
            ),
            # IN term divider tests
            (
                "a IN (1, 2, 3)",
                SearchFilter(InTerm("a", ["1", "2", "3"], "a IN (1,2,3)")),
                "basic int IN term",
            ),
            (
                "a in (1, 2, 3)",
            ...
            (
                "a IN (hello,goodbye)",
                SearchFilter(InTerm("a", ["hello", "goodbye"], "a IN (hello,goodbye)")),
            ...
            (
                "NOT a IN (1, 2, 3)",
                SearchFilter(
                    root=Not(InTerm("a", ["1", "2", "3"], "a IN (1,2,3)")),
                ),
                "NOT IN term",
            ),
            (
                ...
                SearchFilter(
                    InTerm(
                        "a", ["value 1", "2", "value 3"], "a IN ((value 1),2,(value 3))"
                    )
                ),
                "grouped IN term",
            ),
            (
                "a IN (hi)",
                SearchFilter(InTerm("a", ["hi"], "a IN (hi)")),
    ...
    def test_parse_malformedString_fails(self):
        ...
        test_cases = [
            ...
            (
                "a:1 AND (b:2 OR c:3 NOT) OR e:5",
                "Unexpected modifier 'NOT'",
                "expression ends with a modifier",
            ),
            (
                "a:1 AND b:2 OR NOT NOT c:3",
                "Unexpected modifier 'NOT'",
            ...
            (
                "(a:1 AND b:2)(c:3 AND d:4)",
                "Unexpected expression ending",
            ...
            (
                "a:1 AND b:2 OR (c:3 AND (d:4 OR e:5))) OR d:6",
                "Unexpected )",
                "imbalanced parentheses, not opened",
            ),
            # In term tests
            ...
            (
                "a IN (1, 2, 3, [0 TO 1])",
                "Unexpected [ after IN operator",
                "RANGE in IN term",
            ),
            ("a IN (1, 2, 3))", "Unexpected )", "extra parenthesis in IN term"),
            (
                "a IN (val1, val 2, val3)",
                "Unexpected white space",
                "ungrouped space in IN term",
            ),
    ...
    def test_equality(self):
        ...
        tree1 = Or(
            left=And(
                left=EqualityTerm("a", "1", "a:1"), right=EqualityTerm("b", "2", "b:2")
            ),
            right=EqualityTerm("c", "3", "c:3"),
        )
        # Set one left node to None
        tree2 = Or(
            left=And(left=EqualityTerm("b", "2", "b:2"), right=None),
            right=EqualityTerm("c", "3", "c:3"),
        )
        ...
        for i in range(len(trees)):
            for j in range(i + 1, len(trees)):
                self.assertFalse(
                    trees[i] == trees[j],
                    f"Equality returned True for unequal trees {i} and {j}",
                )
