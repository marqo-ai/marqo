...
from marqo.vespa.models.query_result import Child, Coverage, Root, RootFields
...
class SearchTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.metrics_store_patcher = patch(
        ...
        cls.structured_index = StructuredMarqoIndex(
            ...
            text_preprocessing=TextPreProcessing(
                split_length=5, split_overlap=2, split_method="word"
            ),
            ...
            fields=[
                Field(
                    ...
                    lexical_field_name="text_field_1",
                ...
                Field(
                    ...
                    type=FieldType.Text,
                    features=[FieldFeature.LexicalSearch, FieldFeature.Filter],
                    lexical_field_name="text_field_2",
                    filter_field_name="text_field_2",
                ),
                Field(
                    name="int_field_1",
                    type=FieldType.Int,
                ...
                Field(
                    ...
                    dependent_fields={"text_field_1": 1.0, "text_field_2": 2.0},
                ),
                Field(
            ...
            tensor_fields=[
                TensorField(
                    name="text_field_1",
                    chunk_field_name="text_field_1",
                    embeddings_field_name="text_field_1",
                ),
                TensorField(
                    name="text_field_2",
                    chunk_field_name="text_field_2",
                    embeddings_field_name="text_field_2",
                ),
                TensorField(
                    name="multimodal_combo_field",
        ...
        cls.legacy_unstructured_index = UnstructuredMarqoIndex(
            name="legacy_unstructured_index",
            schema_name="legacy_test_schema",
            model=cls.model,
            normalize_embeddings=True,
            text_preprocessing=TextPreProcessing(
                split_length=5, split_overlap=2, split_method="word"
        ...
        cls.unstructured_index = SemiStructuredMarqoIndex(
            name="unstructured_index",
            schema_name="unstructured_test_schema",
            model=cls.model,
            normalize_embeddings=True,
            text_preprocessing=TextPreProcessing(
                split_length=5, split_overlap=2, split_method="word"
        ...
        def make_mock_vectorise_result(contents):
            return InferenceResult(
                result=[
                    [("chunk", np.random.rand(5))]
                    for _ in contents  # assuming 5-dim vector per content
                ]
            )

        cls.inference_mock.vectorise.side_effect = (
            lambda req: make_mock_vectorise_result(req.contents)
        )
        ...
        cls.get_index_patcher = patch(
            ...
            return_value=cls.structured_index,
        )
    ...
    def get_expected_tensor_yql_unstructured(
        self, rerank_depth=3, additional_hits=1997, include_select=True
    ):
        yql = ""
        if include_select:
            yql = f"select * from {self.current_index.schema_name} where ("
    ...
    def get_expected_lexical_yql_with_or(self, query, include_select=True):
        ...
        return (
            f"select * from {self.current_index.schema_name} where ({query_string})"
            if include_select
            else query_string
        )
    ...
    def tearDown(self):
        ...
        self.get_index_patcher = patch(
            "marqo.tensor_search.tensor_search.index_meta_cache.get_index",
            return_value=self.structured_index,
        )
    ...
    def test_tensor_search_with_rerank_depth(self):
        tensor_search.search(
            self.config, "index_name", "query", search_method="tensor", rerank_depth=5
        )
        self.vespa_client_mock.query.assert_called_once()
        call_args = self.vespa_client_mock.query.call_args[1]
        self.assertEqual(
    ...
    def test_lexical_search(self):
        tensor_search.search(
            self.config, "index_name", "query", search_method="lexical"
        )
        self.vespa_client_mock.query.assert_called_once()
        call_args = self.vespa_client_mock.query.call_args[1]
        self.assertEqual(call_args["yql"], self.get_expected_lexical_yql("query"))
        self.assertEqual(
            call_args["query_features"], {"text_field_2": 1, "text_field_1": 1}
        )
    ...
    def test_hybrid_search(self):
        ...
        self.vespa_client_mock.query.assert_called_once()
        call_args = self.vespa_client_mock.query.call_args[1]
        self.assertEqual(self.get_expected_tensor_yql(), call_args["marqo__yql.tensor"])
        self.assertEqual(
    ...
    def test_hybrid_search_with_rerank_depth_tensor(self):
        tensor_search.search(
            ...
            hybrid_parameters=HybridParameters(rerankDepthTensor=5),
        )
        self.vespa_client_mock.query.assert_called_once()
        call_args = self.vespa_client_mock.query.call_args[1]
        self.assertEqual(
    ...
    def test_hybrid_search_with_rerank_depth_and_no_rerank_depth_tensor(self):
        tensor_search.search(
            self.config, "index_name", "query", search_method="hybrid", rerank_depth=15
        )
        self.vespa_client_mock.query.assert_called_once()
        call_args = self.vespa_client_mock.query.call_args[1]
        self.assertEqual(
    ...
    def test_ef_search_higher_than_default_ef_search_overrides_it(self):
        tensor_search.search(
            self.config, "index_name", "query", search_method="tensor", ef_search=3000
        )
        self.vespa_client_mock.query.assert_called_once()
        call_args = self.vespa_client_mock.query.call_args[1]
        self.assertEqual(
        ...
        for i in range(len(scenarios)):
            with self.subTest(ef_search=scenarios[i][0], rerank_depth=scenarios[i][1]):
                tensor_search.search(
                    ...
                    "query",
                    search_method="tensor",
                    ef_search=scenarios[i][0],
                    rerank_depth=scenarios[i][1],
                )
                call_args = self.vespa_client_mock.query.call_args[1]
                self.assertEqual(
                    call_args["yql"],
    ...
    def test_legacy_structured_default_ef_search_with_limit(self):
        self.set_index_to_return(self.legacy_unstructured_index)
        tensor_search.search(
            self.config, "index_name", "query", search_method="tensor", result_count=100
        )
        self.vespa_client_mock.query.assert_called_once()
        call_args = self.vespa_client_mock.query.call_args[1]
        self.assertIn(
    ...
    def test_hybrid_search_with_facets_and_exclude_terms(self):
        ...
        self.assertIn(
            '(marqo__short_string_fields contains sameElement(key contains "text_field_1", value contains "test")',
            facet_queries[0],
        )
        self.assertIn(
            "text_field_1", facet_queries[0].split("|")[1]
        )  # getting facets for it
        self.assertNotIn(
            "text_field_2", facet_queries[0].split("|")[1]
        )  # not getting facets for it

        self.assertIn(
            '(marqo__short_string_fields contains sameElement(key contains "text_field_2", value contains "test2")',
            facet_queries[1],
        )
        self.assertIn(
            "text_field_2", facet_queries[1].split("|")[1]
        )  # getting facets for it
        self.assertNotIn(
            "text_field_1", facet_queries[1].split("|")[1]
        )  # not getting facets for it
                ...
                root=Root(
                    ...
                    children=[
                        Child(
                            ...
                            fields={
                                "marqo__field_types": {
                                    "available_sizes": "string_array",
                                    "popularity": "float",
                                    "title": "tensor",
                                },
                                "marqo__float_fields": {
                                    "popularity": 0.010630001972235736
                                },
                                "marqo__int_fields": [{"key": "age"}],
                                "marqo__id": "1",
                                "matchfeatures": {
                                    "global_add_modifier": 0.0,
                                    "global_mult_modifier": 1.0,
                                },
                                "sddocname": "marqo__my_01index",
                                "title": "Red dress",
                            },
                            value=None,
                            coverage=None,
    ...
    def test_search_with_context_documents_fails_for_legacy_unstructured_index(self):
        ...
        self.assertIn(
            "Search context is not supported for unstructured indexes",
            str(cm.exception),
        )
