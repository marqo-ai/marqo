...
class _StubStatsD:
    ...
    def _fmt(
        self, metric: str, value: int, suff: str, tags: Optional[Dict[str, str]]
    ) -> str:
        tag_str = (
            "" if not tags else "|#" + ",".join(f"{k}:{v}" for k, v in tags.items())
        )
...
class TestStatsDMiddleware(unittest.TestCase):
    ...
    def test_search_metrics(self):
        ...
        self.assertTrue(
            any("path:/indexes/foo/search" in m and "method:POST" in m for m in timings)
        )
    ...
    def test_index_docs_metrics_and_headers(self):
        ...
        self.assertTrue(
            any(
                m.startswith("request.duration_ms")
                for k, m in self.stub.sent
                if k == "timing"
            )
        )
    ...
    def test_path_sanitisation(self):
        ...
        timings = _extract(self.stub, "timing", "request.duration_ms")
        self.assertTrue(
            any("path:/indexes/foo/documents/<document_id>" in m for m in timings)
        )
    ...
    def test_patch_docs_malformed_headers(self):
        ...
        self.assertFalse(
            any(
                "batch.success" in m or "batch.failure" in m or "batch.error" in m
                for k, m in self.stub.sent
                if k == "increment"
            )
        )
    ...
    def test_empty_headers_dont_crash(self):
        ...
        self.assertTrue(
            any(
                k == "timing" and m.startswith("request.duration_ms")
                for k, m in self.stub.sent
            )
        )
        self.assertFalse(
            any("batch." in m for k, m in self.stub.sent if k == "increment")
        )
        ...
        for return_enum, expected_str in test_cass:
            with self.subTest(msg=f"status_code tag for {return_enum}"):
                ...
                with TestClient(app) as client:
                    ...
                    self.assertTrue(
                        any(f"status_code:{expected_str}" in m for m in timings),
                    ...
                    for timing in timings:
                        if "status_code:" in timing:
                            status_part = [
                                part
                                for part in timing.split("|#")[1].split(",")
