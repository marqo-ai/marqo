...
class TestMultimodalUtils(unittest.TestCase):
    ...
    def test_infer_modality_url_with_extension(self):
        self.assertEqual(
            infer_modality("https://example.com/image.jpg"), Modality.IMAGE
        )
        self.assertEqual(
            infer_modality("https://example.com/video.mp4"), Modality.VIDEO
        )
        self.assertEqual(
            infer_modality("https://example.com/audio.mp3"), Modality.AUDIO
        )
    ...
    def test_infer_modality_url_without_extension(self, mock_fetch, mock_validate):
        ...
        with patch("magic.from_buffer") as mock_magic:
            ...
            self.assertEqual(
                infer_modality("https://example.com/image"), Modality.IMAGE
            )

            mock_magic.return_value = "video/mp4"
            self.assertEqual(
                infer_modality("https://example.com/video"), Modality.VIDEO
            )

            mock_magic.return_value = "audio/mpeg"
            self.assertEqual(
                infer_modality("https://example.com/audio"), Modality.AUDIO
            )
    ...
    def test_infer_modality_extension_with_query_parameters(self):
        test_cases = [
            ...
            (
                "https://example.com/image.jpg?query=string",
                Modality.IMAGE,
                "Simple image URL with one query param",
            ),
            (
                "https://example.com/video.mp4?foo=bar&baz=qux",
                Modality.VIDEO,
                "Video URL with multiple query params",
            ),
            (
                "https://example.com/audio.mp3?abc=def&123=456",
                Modality.AUDIO,
                "Audio URL with numeric and alpha query params",
            ),
            # Correct cases with more complex URLs
            (
                "https://example.com/photo.jpeg?weirdparam=??&another=##",
                Modality.IMAGE,
            ...
            (
                ...
                Modality.AUDIO,
                "Valid audio with empty query string",
            ),
            (
                "https://example.com/clip.mp4#fragment",
                Modality.VIDEO,
                "Video URL with fragment identifier",
            ),
            # Edge cases: missing or no extension
            (
                "https://example.com/file.unknown?param=test",
                None,
                "Unknown extension should return None",
            ),
            (
                "https://example.com/no_extension?query=data",
                None,
                "URL with no extension should return None",
            ),
            (
                "https://example.com/imagejpg?query=string",
                None,
        ...
        for url, expected_modality, message in test_cases:
            with self.subTest(msg=message, url=url):
                inferred_modality = _infer_modality_based_on_extension(
                    get_url_file_extension(url)
                )
    ...
    def test_infer_modality_no_extension_found(self):
        ...
        with (
            patch(
                "marqo.core.inference.modality_utils.fetch_content_sample"
            ) as mock_fetch,
            patch(
                "marqo.core.inference.modality_utils.magic.from_buffer",
                return_value="audio/mpeg",
            ),
            patch(
                "marqo.core.inference.modality_utils._infer_modality_based_on_mime_type"
            ) as mock_infer_on_mime,
        ):
    ...
    def test_infer_modality_proper_extension_found(self):
        ...
        with (
            patch(
                "marqo.core.inference.modality_utils._infer_modality_based_on_extension"
            ) as mock_infer_on_extension,
            patch(
                "marqo.core.inference.modality_utils.fetch_content_sample"
            ) as mock_fetch,
            patch(
