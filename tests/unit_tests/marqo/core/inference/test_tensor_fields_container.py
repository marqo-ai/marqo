...
class TestTensorFieldsContainer(unittest.TestCase):
    def setUp(self):
        self.container = TensorFieldsContainer(
            tensor_fields=[
                "tensor_field1",
                "tensor_field2",
                "custom_vector_field1",
                "custom_vector_field2",
                "combo_field1",
                "combo_field2",
            ],
    ...
    def test_initialisation(self):
        ...
        self.assertEqual(
            {"subfield1", "tensor_field2"}, self.container.get_multimodal_sub_fields()
        )
        self.assertEqual(
            {"subfield1": 1.0},
            self.container.get_multimodal_field_mapping("combo_field1"),
        )
        self.assertEqual(
            {"subfield1": 2.0, "tensor_field2": 5.0},
    ...
    def test_collect_non_tensor_fields(self):
        ...
        for field_content, field_type in test_cases:
            with self.subTest(
                msg=f"field_content {field_content} of type {field_type}"
            ):
    ...
    def test_collect_custom_vector_field(self):
        content = self.container.collect(
            "doc_id1",
            "custom_vector_field1",
            {"content": "content1", "vector": [1.0, 2.0]},
        )

        self.assertEqual("content1", content)
        self.assertIn("doc_id1", self.container._tensor_field_map)
        self.assertIn(
            "custom_vector_field1", self.container._tensor_field_map["doc_id1"]
        )
        ...
        self.assertEqual(
            [[0.4472135954999579, 0.8944271909999159]], field.embeddings
        )  # normalised
    ...
    def test_collect_custom_vector_field_should_fail_with_zero_magnitude_vector(self):
        with self.assertRaises(AddDocumentsError) as context:
            self.container.collect(
                "doc_id1",
                "custom_vector_field1",
                {"content": "content1", "vector": [0.0, 0.0]},
            )
        self.assertIn(
            "Field custom_vector_field1 has zero magnitude vector, cannot normalize.",
    ...
    def test_collect_multimodal_field_should_raise_error(self):
        ...
        self.assertIn(
            "Field combo_field1 is a multimodal combination field and cannot be assigned a value.",
            str(e.exception),
        )
    ...
    def test_collect_tensor_field_with_non_string_type(self):
        ...
        for field_content, field_type in test_cases:
            with self.subTest(
                ...
                with self.assertRaises(AddDocumentsError) as e:
                    self.container.collect(
                        "doc_id1", "tensor_field1", field_content, field_type
                    )

                self.assertIn(
                    f"Invalid type {type(field_content)} for tensor field tensor_field1",
                    str(e.exception),
                )
    ...
    def test_collect_tensor_field_with_string_type(self):
        for field_type in [
            ...
            with self.subTest(msg=f"field_type {field_type}"):
                content = self.container.collect(
                    "doc_id1", "tensor_field1", "content", field_type
                )
                self.assertEqual("content", content)
                self.assertIn("doc_id1", self.container._tensor_field_map)
                self.assertIn(
                    "tensor_field1", self.container._tensor_field_map["doc_id1"]
                )
    ...
    def test_collect_tensor_field_can_identify_toplevel_or_subfield(self):
        ...
        for field_name, is_tensor_field, is_multimodal_subfield in test_cases:
            with self.subTest(
                msg=f"{field_name}: is_tensor_field={is_tensor_field}, "
                f"is_multimodal_subfield={is_multimodal_subfield}"
            ):
                self.container.collect("doc_id1", field_name, "content")

                tensor_field_content = self.container._tensor_field_map["doc_id1"][
                    field_name
                ]
                self.assertEqual(
                    is_tensor_field, tensor_field_content.is_top_level_tensor_field
                )
                self.assertEqual(
                    is_multimodal_subfield, tensor_field_content.is_multimodal_subfield
                )
    ...
    def test_collect_multimodal_fields_should_return_all(self):
        ...
        self.assertEqual(
            ("combo_field2", {"subfield1": 2.0, "tensor_field2": 5.0}), fields[1]
        )
    ...
    def test_collect_multimodal_fields_should_populate_subfields(self):
        ...
        combo_field1 = cast(
            MultiModalTensorField,
            self.container._tensor_field_map["doc_id1"]["combo_field1"],
        )
        ...
        self.assertEqual(
            {"subfield1": self.container._tensor_field_map["doc_id1"]["subfield1"]},
        ...
        combo_field2 = cast(
            MultiModalTensorField,
            self.container._tensor_field_map["doc_id1"]["combo_field2"],
        )
        self.assertEqual(
            {
                "subfield1": self.container._tensor_field_map["doc_id1"]["subfield1"],
                "tensor_field2": self.container._tensor_field_map["doc_id1"][
                    "tensor_field2"
                ],
            },
    ...
    def test_populate_tensor_from_existing_docs_will_not_populate_if_doc_id_does_not_match(
        self,
    ):
        ...
        self.container.populate_tensor_from_existing_doc(
            {
                ...
                MARQO_DOC_TENSORS: {
                    "tensor_field1": {
                        MARQO_DOC_CHUNKS: ["tensor_field1_content"],
                        MARQO_DOC_EMBEDDINGS: [[1.0, 2.0]],
                    }
                },
            },
            {},
    ...
    def test_populate_tensor_from_existing_docs_will_not_populate_if_content_changes(
        self,
    ):
    ...
    def test_populate_tensor_from_existing_docs_will_not_populate_if_field_does_not_exist(
        self,
    ):
        self.container.collect("doc_id1", "tensor_field1", "tensor_field1_content")
        tensor_field1 = self.container._tensor_field_map["doc_id1"]["tensor_field1"]
    ...
    def test_populate_tensor_from_existing_docs_will_not_populate_if_embedding_does_not_exist(
        self,
    ):
        self.container.collect("doc_id1", "tensor_field1", "tensor_field1_content")
        tensor_field1 = self.container._tensor_field_map["doc_id1"]["tensor_field1"]
    ...
    def test_populate_tensor_from_existing_docs_will_not_populate_if_existing_field_is_multimodal_combo_field(
        self,
    ):
        self.container.collect("doc_id1", "tensor_field1", "tensor_field1_content")
        tensor_field1 = self.container._tensor_field_map["doc_id1"]["tensor_field1"]
        ...
        self.container.populate_tensor_from_existing_doc(
            ...
            {"tensor_field1": {"subfield1": 1.0}},
        )  # tensor_field1 is a multimodal combo field

        self.assertIsNone(tensor_field1.chunks)
        self.assertIsNone(tensor_field1.embeddings)

    def test_populate_tensor_from_existing_docs_will_not_populate_for_custom_vector_field(
        self,
    ):
        self.container.collect(
            "doc_id1",
        ...
        self.container.populate_tensor_from_existing_doc(
            {
                ...
                MARQO_DOC_TENSORS: {
                    "custom_vector_field1": {
                        MARQO_DOC_CHUNKS: ["content2"],
                        MARQO_DOC_EMBEDDINGS: [[3.0, 4.0]],
                    }
        ...
        self.assertEqual(
            [[0.4472135954999579, 0.8944271909999159]], custom_vector_field1.embeddings
        )

    def test_populate_tensor_from_existing_docs_will_not_populate_for_multimodal_field_if_it_does_not_exist(
        self,
    ):
    ...
    def test_populate_tensor_from_existing_docs_will_not_populate_multimodal_field_with_another_type(
        self,
    ):
        combo_field2 = self._get_combo_field2()
        ...
        self.container.populate_tensor_from_existing_doc(
            {
                ...
                MARQO_DOC_TENSORS: {
                    "combo_field2": {
                        MARQO_DOC_CHUNKS: ["combo_field2_content"],
                        MARQO_DOC_EMBEDDINGS: [[1.0, 2.0]],
                    }
    ...
    def test_populate_tensor_from_existing_docs_will_not_populate_multimodal_field_with_different_weight(
        self,
    ):
        combo_field2 = self._get_combo_field2()
        ...
        self.container.populate_tensor_from_existing_doc(
            ...
            {"combo_field2": {"subfield1": 0.5, "tensor_field2": 5.0}},
        )  # weight is different

        self.assertIsNone(combo_field2.chunks)
        self.assertIsNone(combo_field2.embeddings)

    def test_populate_tensor_from_existing_docs_will_not_populate_multimodal_field_with_different_subfields(
        self,
    ):
        combo_field2 = self._get_combo_field2()
    ...
    def test_populate_tensor_from_existing_docs_should_populate_multimodal_field_if_all_conditions_match(
        self,
    ):
        combo_field2 = self._get_combo_field2()
    ...
    def test_select_unresolved_tensor_fields_skips_resolved_fields(self):
        ...
        fields = self.container.select_unresolved_tensor_fields()
        self.assertListEqual(
            fields, [self.container._tensor_field_map["doc_id1"]["subfield1"]]
        )

    def test_select_unresolved_tensor_fields_skips_custom_fields_and_multi_modal_fields(
        self,
    ):
        self.container.collect("doc_id1", "tensor_field1", "tensor_field1_content")
        self.container.collect("doc_id1", "tensor_field2", "tensor_field2_content")
    ...
    def test_select_unresolved_tensor_fields_skips_fields_not_matching_predicates(self):
        ...
        fields = self.container.select_unresolved_tensor_fields(
            lambda f: f.field_name == "tensor_field1"
        )
        self.assertListEqual(
            fields,
            [
    ...
    def test_get_tensor_field_content_for_persisting(self):
        ...
        self.container.collect("doc_id1", "tensor_field2", "tensor_field2_content")
        self.container.collect(
            "doc_id1", "subfield1", "subfield1_content"
        )  # subfield is not persisted
        list(self.container.collect_multi_modal_fields("doc_id1", True))

        self.container._tensor_field_map["doc_id1"][
            "tensor_field1"
        ].populate_chunks_and_embeddings(["hello world"], [[1.0, 1.2]])
        self.container._tensor_field_map["doc_id1"][
            "tensor_field2"
        ].populate_chunks_and_embeddings(["hello world"], [[1.0, 1.2]])
    ...
    def test_get_tensor_field_content_for_persisting_skips_multimodal_field_with_no_subfields(
        self,
    ):
        self.container.collect("doc_id1", "tensor_field1", "tensor_field1_content")
        self.container.collect("doc_id1", "tensor_field2", "tensor_field2_content")
        ...
        self.assertIn(
            "combo_field2", fields
        )  # combo_field2 has tensor_field2 as subfield
        # combo_field1 has subfield1 as the only subfield, since subfield1 is not present, combo_field1 does not
        # have content either

    def test_has_unresolved_parent_field_returns_false_when_field_is_not_a_subfield(
        self,
    ):
        self.container.collect("doc_id1", "tensor_field1", "tensor_field1_content")
    ...
    def test_has_unresolved_parent_field_returns_false_when_field_has_unresolved_parents_fields(
        self,
    ):
    ...
    def test_has_unresolved_parent_field_returns_true_when_all_parents_fields_are_resolved(
        self,
    ):
        self.container.collect("doc_id1", "subfield1", "tensor_field1_content")
        self.container.collect("doc_id1", "tensor_field2", "tensor_field2_content")
