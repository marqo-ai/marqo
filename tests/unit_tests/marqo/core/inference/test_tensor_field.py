...
class TestTensorField(unittest.TestCase):
    def test_is_unresolved_top_level_field(self):
        ...
        for is_top_level_field, embeddings, expected_result in test_cases:
            with self.subTest(
                f"top_level: {is_top_level_field}, embeddings: {embeddings}"
            ):
                field = TensorField(
                    doc_id="id",
                    field_name="field",
    ...
    def test_is_unresolved_multimodal_subfield(self):
        ...
        test_cases = [
            ...
            (
                ...
                None,
                True,
            ),  # text subfield, with chunked embeddings populated
            (
                Modality.TEXT,
                True,
                None,
                [1.0, 2.0],
                False,
            ),  # text subfield, with subfield embeddings populated
            ...
            (
                ...
                None,
                True,
            ),  # image subfield, with chunked embeddings populated
            (
                Modality.IMAGE,
                True,
                None,
                [1.0, 2.0],
                False,
            ),  # image subfield, with subfield embeddings populated
            ...
            (
                ...
                None,
                False,
            ),  # audio subfield, with chunked embeddings populated
            ...
            (
                ...
                None,
                False,
            ),  # video subfield, with chunked embeddings populated
        ...
        for (
            ...
            with self.subTest(
                f"{modality.value}, subfield: {is_subfield}, embeddings: {embeddings}, "
                ...
                field = TensorField(
                    ...
                    embeddings=embeddings,
                    multimodal_subfield_embedding=subfield_embedding,
                )
                self.assertEqual(
                    expected_result, field.is_unresolved_multimodal_subfield()
                )
    ...
    def test_tensor_field_chunk_and_embedding_should_raise_error_if_field_is_not_top_level(
        self,
    ):
        field = TensorField(
            doc_id="id",
        ...
        self.assertIn(
            "field of doc: id is not a top level tensor field", str(context.exception)
        )
    ...
    def test_subfield_chunk_and_embedding_should_raise_error_if_field_is_not_subfield(
        self,
    ):
        field = TensorField(
            doc_id="id",
    ...
    def test_subfield_chunk_and_embedding_for_image_field(self):
        ...
        field = TensorField(
            doc_id="id",
            field_name="field",
            field_content="http://a.com/c.jpg",
    ...
    def test_subfield_chunk_and_embedding_for_audio_field(self):
        ...
        field = TensorField(
            doc_id="id",
            field_name="field",
            field_content="http://a.com/c.mp3",
    ...
    def test_subfield_chunk_and_embedding_for_video_field(self):
        ...
        field = TensorField(
            doc_id="id",
            field_name="field",
            field_content="http://a.com/c.mp4",
...
class TestMultiModalTensorField(unittest.TestCase):
    def test_multimodal_combo_field_with_no_subfield_returns_empty_chunks_and_embeddings(
        self,
    ):
    ...
    def test_multimodal_combo_field_derived_from_one_subfield(self):
        ...
        for normalize_embeddings, expected_embeddings in test_cases:
            with self.subTest(msg=f"Normalize embeddings: {normalize_embeddings}"):
                ...
                subfield1.populate_chunks_and_embeddings(
                    ["hello world!"], [[1.0, 2.0]], for_top_level_field=False
                )
                ...
                self.assertEqual(
                    expected_embeddings, multimodal_field.tensor_field_embeddings
                )
    ...
    def test_multimodal_combo_field_derived_from_multiple_subfields(self):
        ...
        vector_chunk = np.array(
            [(1.0 * 0.3 + 2.0 * 0.7) / 2, (2.0 * 0.3 + 3.0 * 0.7) / 2]
        )
        test_cases = [
            (False, [vector_chunk.tolist()]),
            (True, [(vector_chunk / np.linalg.norm(vector_chunk)).tolist()]),
        ...
        for normalize_embeddings, expected_embeddings in test_cases:
            with self.subTest(msg=f"Normalize embeddings: {normalize_embeddings}"):
                ...
                subfield2.populate_chunks_and_embeddings(
                    ["Hola!"], [[2.0, 3.0]], for_top_level_field=False
                )

                self.assertEqual(
                    ['{"field_1": "hello world!", "field_2": "Hola!"}'],
                    multimodal_field.tensor_field_chunks,
                )
                self.assertEqual(
                    expected_embeddings, multimodal_field.tensor_field_embeddings
...
class TestTensorFieldPopulateChunksAndEmbeddings(unittest.TestCase):
    ...
    def test_populate_chunks_and_embeddings_raises_value_error_when_size_does_not_match(
        self,
    ):
        field = TensorField(
            doc_id="id",
        ...
        with self.assertRaises(ValueError) as context:
            field.populate_chunks_and_embeddings(
                ["hello world!"], [[1.0, 2.0], [2.0, 3.0]]
            )
        self.assertEqual(
            "Chunk and embedding size does not match for doc: id, field: field_1: chunk size: 1, embedding size: 2",
            str(context.exception),
        )
    ...
    def test_populate_chunks_and_embeddings_for_top_level_field_without_modality(self):
        ...
        self.assertTrue(
            field.is_resolved
        )  # resolved since it's not a multimodal subfield
    ...
    def test_populate_chunks_and_embeddings_for_audio_video_field(self):
        ...
        test_cases = [
            ...
            (
                ...
                True,
                True,
            ),  # both top level and subfield, populated as top level field
        ]
        ...
        for modality in [Modality.AUDIO, Modality.VIDEO]:
            for is_top_level_field, is_sub_field, for_top_level_field in test_cases:
                with self.subTest(
                    modality=modality,
                    is_top_level_field=is_top_level_field,
    ...
    def test_populate_chunks_and_embeddings_for_image_text_subfield_only_should_raise_error_when_chunked(
        self,
    ):
        for modality in [Modality.TEXT, Modality.IMAGE]:
            with self.subTest(modality=modality):
                ...
                self.assertIn(
                    "field_1 of doc: id is a subfield and should not be chunked",
                    str(context.exception),
                )

    def test_populate_chunks_and_embeddings_for_image_text_subfield_only_should_success_when_not_chunked(
        self,
    ):
        for modality in [Modality.TEXT, Modality.IMAGE]:
            with self.subTest(modality=modality):
                ...
                field.populate_chunks_and_embeddings(
                    ["text or url"], [[1.0, 2.0]], for_top_level_field=False
                )
                self.assertEqual("text or url", field.subfield_chunk)
                self.assertEqual([1.0, 2.0], field.subfield_embedding)
                self.assertTrue(field.is_resolved)

    def test_populate_chunks_and_embeddings_for_image_text_both_top_level_and_sub_fields_not_chunked(
        self,
    ):
        for modality in [Modality.TEXT, Modality.IMAGE]:
            with self.subTest(modality=modality):
                ...
                field.populate_chunks_and_embeddings(
                    ["text or url"], [[1.0, 2.0]], for_top_level_field=True
                )
    ...
    def test_populate_chunks_and_embeddings_for_image_text_both_top_level_and_sub_fields_chunked(
        self,
    ):
        for modality in [Modality.TEXT, Modality.IMAGE]:
            with self.subTest(modality=modality):
                ...
                self.assertEqual(
                    [[1.0, 2.0], [2.0, 3.0]], field.tensor_field_embeddings
                )
    ...
    def test_populate_chunks_and_embeddings_for_text_image_both_top_level_and_sub_fields_with_single_different_chunk(
        self,
    ):
        """
        ...
        for modality in [Modality.TEXT, Modality.IMAGE]:
            with self.subTest(modality=modality):
                ...
                field.populate_chunks_and_embeddings(
                    ["chunk1"], [[1.0, 2.0]], for_top_level_field=True
                )
