...
class TestModelConfig(unittest.TestCase):
    ...
    def test_missing_required_field(self):
        ...
        with self.assertRaises(ValidationError) as context:
            ModelConfig(
                model_properties={"property1": "value1"}, normalize_embeddings=True
            )
        self.assertIn("modelName", str(context.exception))
        self.assertIn(
            "field required (type=value_error.missing)", str(context.exception)
        )
    ...
    def test_immutability(self):
        ...
        config = ModelConfig(modelName="immutable_model", normalizeEmbeddings=True)
        with self.assertRaises(TypeError) as context:
            config.model_name = "new_model_name"
        self.assertIn(
            '"ModelConfig" is immutable and does not support item assignment',
...
class TestInferenceRequest(unittest.TestCase):
    ...
    def test_valid_inference_request_for_matching_modality(self):
        ...
        for modality, preprocessing_config in [
            ...
            with self.subTest(
                modality=modality, preprocessing_config=preprocessing_config
            ):
    ...
    def test_invalid_inference_request_with_missing_modality(self):
        ...
        self.assertIn(
            "Modality or preprocessing_config is missing", str(context.exception)
        )
    ...
    def test_immutability(self):
        ...
        self.assertIn(
            '"InferenceRequest" is immutable and does not support item assignment',
            str(context.exception),
        )
...
class TestInferenceResult(unittest.TestCase):
    ...
    def test_invalid_union_type(self):
        with self.assertRaises(ValidationError):
            InferenceResult(
                result=[123]
            )  # Invalid type, neither InferenceErrorModel nor list of tuples
