...
class TestChunkConfig(unittest.TestCase):
            ...
            with self.subTest(split_method=split_method):
                try:
                    config = TextChunkConfig(
                        split_length=100, split_overlap=10, split_method=split_method
                    )
                    self.assertEqual(config.split_method, split_method)
                except ValidationError:
                    self.fail(
                        f"TextChunkConfig raised ValidationError unexpectedly for '{split_method}' method!"
                    )

    def test_split_length_greater_than_overlap(self):
        """Test that split_length must be greater than split_overlap."""
        with self.assertRaises(ValidationError):
            TextChunkConfig(split_length=10, split_overlap=20, split_method="word")

    def test_text_chunk_config_invalid_split_method(self):
        """Test TextChunkConfig with invalid split_method."""
        with self.assertRaises(ValidationError):
            TextChunkConfig(
                split_length=100, split_overlap=10, split_method="invalid_method"
            )
    ...
    def test_text_chunk_config_boundary_split_overlap(self):
        ...
        try:
            config = TextChunkConfig(
                split_length=100, split_overlap=0, split_method="sentence"
            )
            self.assertEqual(config.split_overlap, 0)
        except ValidationError:
            self.fail(
                "TextChunkConfig raised ValidationError unexpectedly with split_overlap=0!"
            )
    ...
    def test_chunk_config_aliases(self):
        ...
        try:
            ...
            self.fail(
                "ChunkConfig raised ValidationError unexpectedly when using aliases!"
            )
    ...
    def test_text_chunk_config_aliases(self):
        ...
        try:
            config = TextChunkConfig(
                splitLength=200, splitOverlap=20, splitMethod="passage"
            )
            ...
            self.fail(
                "TextChunkConfig raised ValidationError unexpectedly when using aliases!"
            )
...
class TestTextPreprocessingConfig(unittest.TestCase):
    ...
    def test_valid_configuration_with_chunking(self):
        ...
        chunk_config = TextChunkConfig(
            split_length=100, split_overlap=10, split_method="word"
        )
        config = TextPreprocessingConfig(
            should_chunk=True, text_prefix="PREFIX", chunk_config=chunk_config
        )
    ...
    def test_immutability(self):
        ...
        self.assertIn(
            '"TextPreprocessingConfig" is immutable and does not support item assignment',
            str(context.exception),
        )
...
class TestImagePreprocessingConfig(unittest.TestCase):
    ...
    def test_valid_configuration_with_chunking(self):
        ...
        for patch_method in ["simple", "frcnn", "dino-v1", "dino-v2", "marqo-yolo"]:
            with self.subTest(patch_method=patch_method):
                config = ImagePreprocessingConfig(
                    should_chunk=True, patch_method=patch_method
                )
                self.assertTrue(config.should_chunk)
    ...
    def test_invalid_patch_method(self):
        ...
        with self.assertRaises(ValidationError) as context:
            ImagePreprocessingConfig(should_chunk=True, patch_method="invalid_method")
    ...
    def test_immutability(self):
        ...
        with self.assertRaises(TypeError) as context:
            config.should_chunk = True
        self.assertIn(
            '"ImagePreprocessingConfig" is immutable and does not support item assignment',
            str(context.exception),
        )
...
class TestAudioPreprocessingConfig(unittest.TestCase):
    ...
    def test_valid_configuration_with_chunking(self):
        ...
        self.assertEqual(config.download_thread_count, 4)
        self.assertEqual(
            config.chunk_config, ChunkConfig(split_length=10, split_overlap=2)
        )
    ...
    def test_should_chunk_without_chunk_config(self):
        ...
        """
        with self.assertRaises(ValidationError) as context:
            AudioPreprocessingConfig(should_chunk=True)
        self.assertIn(
            "`chunk_config` must be provided when `should_chunk` is True.",
            str(context.exception),
        )
    ...
    def test_aliases_are_handled_correctly(self):
        ...
        self.assertEqual(config.download_header, {"Content-Type": "application/json"})
        self.assertEqual(
            config.chunk_config, ChunkConfig(splitLength=10, splitOverlap=2)
        )
    ...
    def test_immutability(self):
        ...
        with self.assertRaises(TypeError) as context:
            config.should_chunk = True
        self.assertIn(
            '"AudioPreprocessingConfig" is immutable and does not support item assignment',
            str(context.exception),
        )
...
class TestVideoPreprocessingConfig(unittest.TestCase):
    ...
    def test_should_chunk_without_chunk_config(self):
        ...
        """
        with self.assertRaises(ValidationError) as context:
            VideoPreprocessingConfig(should_chunk=True)
        self.assertIn(
            "`chunk_config` must be provided when `should_chunk` is True.",
            str(context.exception),
        )
    ...
    def test_immutability(self):
        ...
        with self.assertRaises(TypeError) as context:
            config.should_chunk = True
        self.assertIn(
            '"VideoPreprocessingConfig" is immutable and does not support item assignment',
            str(context.exception),
        )
