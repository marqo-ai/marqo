...
class TestModelLoad(TestCase):
    ...
    @patch("marqo.s2_inference.s2_inference._get_model_loader")
    @patch("time.sleep")
    def test_load_model_retries_exhausted_error_raised(
        self, mock_sleep, mock_get_model_loader
    ):
        mock_loader = MagicMock()
        mock_loader.return_value.load.side_effect = ReadTimeoutError(
            pool=None, url=None, message=None
        )
        mock_get_model_loader.return_value = mock_loader
        model_properties = {
            "name": "dummy_model",
        ...
        with self.assertRaises(ReadTimeoutError):
            _load_model(
                "dummy_model", model_properties, "cpu", calling_func="unit_test"
            )
    ...
    @patch("marqo.s2_inference.s2_inference._get_model_loader")
    @patch("time.sleep")
    def test_load_model_unexpected_error_no_retries(
        self, mock_sleep, mock_get_model_loader
    ):
        mock_loader = MagicMock()
