...
<services xmlns:deploy="vespa" xmlns:preprocess="properties">
    ...
    def test_batch_add_index_setting_and_schema_includes_typeahead_schema(self):
        ...
        self.assertListEqual(
            [main_schema_content, "schemas", "marqo__test_index.sd"],
            list(arg_list[0][0]),
        )
        self.assertListEqual(
            [typeahead_schema_content, "schemas", "marqo__test_index_typeahead.sd"],
            list(arg_list[1][0]),
        )
        ...
        self.maxDiff = None
        self.assertListEqual(
            [index_setting_str, "marqo_index_settings.json"], list(arg_list[2][0])
        )
        self.assertListEqual(
            ["{}", "marqo_index_settings_history.json"], list(arg_list[3][0])
        )
        ...
        self.assertIn(
            '<document type="marqo__test_index" mode="index" />',
            services_xml_store_call[0],
        )
        self.assertIn(
            '<document type="marqo__test_index_typeahead" mode="index" />',
            services_xml_store_call[0],
        )
    ...
    def test_batch_add_multiple_indexes_creates_multiple_typeahead_schemas(self):
        ...
        for schema_file, expected_content in expected_schemas.items():
            self.assertIn(
                schema_file, schema_saves, f"Schema file {schema_file} should be saved"
            )
    ...
    def test_batch_delete_index_setting_and_schema_removes_typeahead_schema(self):
        ...
        actual_remove_calls = [
            call
            for call in self.mock_store.remove_file.call_args_list
    ...
    def test_batch_delete_multiple_indexes_removes_all_typeahead_schemas(self):
        ...
        # Execute
        self.vespa_app.batch_delete_index_setting_and_schema(
        ...
        categories_typeahead_call = unittest.mock.call(
            "schemas", "marqo__categories_typeahead.sd"
        )
    ...
    def test_bootstrap_adds_missing_typeahead_schemas_for_2_23_0_indexes(self):
        ...
        self.vespa_app._index_setting_store._index_settings = {
            "test_index_230": index_without_typeahead
        }
        ...
        typeahead_save_calls = [
            ...
            for call in self.mock_store.save_file.call_args_list
            if len(call[0]) >= 3
            and call[0][1] == "schemas"
            and "typeahead" in call[0][2]
        ]
    ...
    def test_bootstrap_skips_typeahead_for_pre_2_23_0_indexes(self):
        ...
        # Mock the index setting store to return this index
        self.vespa_app._index_setting_store.get_all_index_settings = Mock(
    ...
    def test_bootstrap_skips_existing_typeahead_schemas(self):
        ...
        # Mock the index setting store to return this index
        self.vespa_app._index_setting_store.get_all_index_settings = Mock(
            return_value=[index_with_typeahead]
        )
        self.vespa_app._copy_components_jar = Mock()
    ...
    def test_bootstrap_handles_mixed_indexes(self):
        ...
        self.vespa_app._index_setting_store._index_settings = {
            name: self.semi_structured_marqo_index(
                ...
                typeahead_schema_name=f"marqo__{name}_typeahead"
                if has_typeahead_schema
                else None,
