...
class TestIndexManagementUpdateIndex(MarqoTestCase):
    ...
    def _create_semi_structured_index(self, name="test_index"):
        ...
        return self.semi_structured_marqo_index(
            name=name, model=Model(name="hf/e5-small"), version=1
        )
    ...
    @patch(
        "marqo.core.index_management.index_management.SemiStructuredVespaSchema.generate_vespa_schema"
    )
    def test_update_index_success(self, mock_generate_schema):
        ...
        base_existing = self.semi_structured_marqo_index(
            name="test_index", model=Model(name="hf/e5-small")
        )
        ...
        test_cases = [
            ...
            (
                ...
                dict(string_array_field_names=("field1",)),
            ),
        ]

        for case_name, updated_params in test_cases:
            with self.subTest(case=case_name):
                updated_index = self.semi_structured_marqo_index(
                    name="test_index", model=Model(name="hf/e5-small"), **updated_params
                )
                mock_vespa_app = Mock()
                ...
                self.index_management._get_vespa_application = Mock(
                    return_value=mock_vespa_app
                )
                self.index_management._vespa_deployment_lock = Mock(
                    return_value=mock_deployment_lock
                )

                self.index_management.update_index(updated_index)

                mock_generate_schema.assert_called_with(updated_index)
                mock_vespa_app.update_index_setting_and_schema.assert_called_with(
                    updated_index, "mock_schema"
                )
    ...
    def test_update_index_skip_if_nothing_to_update(self):
        ...
        existing_index = base_index.copy(
            update={
    ...
    def test_batch_create_indexes_generates_typeahead_schema(
        self, mock_typeahead_schema_class, mock_vespa_schema_factory
    ):
        ...
        mock_vespa_schema_factory.return_value.generate_schema.return_value = (
            mock_main_schema,
            mock_marqo_index,
        )

        # Setup typeahead schema mocks
        mock_updated_index = mock_marqo_index.copy(
            deep=True, update={"typeahead_schema_name": "marqo__test_index_typeahead"}
        )
        mock_typeahead_schema = "typeahead_schema_content"
        mock_typeahead_instance = Mock()
        mock_typeahead_instance.generate_schema.return_value = (
            mock_typeahead_schema,
            mock_updated_index,
        )
        ...
        mock_vespa_app.batch_add_index_setting_and_schema.assert_called_once_with(
            expected_call
        )
