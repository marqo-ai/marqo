...
from marqo.core.exceptions import (
    CudaDeviceNotAvailableError,
    CudaOutOfMemoryError,
    DeviceError,
)
from marqo.inference.native_inference.device_manager import Device, DeviceManager
...
class TestDeviceManager(unittest.TestCase):
    ...
    def _device_manager_with_multiple_cuda_devices(self, total_memory: int = 1_000_000):
        with (
            ...
            mock.patch(
                "torch.cuda.get_device_name", side_effect=["Tesla T4", "Tesla H200"]
            ),
            mock.patch(
                "torch.cuda.get_device_properties",
                return_value=SimpleNamespace(total_memory=total_memory),
    ...
    def test_init_with_gpu(self):
        ...
        self.assertEqual(
            ...
            [Device.cpu(), Device.cuda(0, "Tesla T4", 1_000_000)],
        )
    ...
    def test_cuda_health_check_should_skip_without_cuda_devices(self):
        ...
        with mock.patch(
            "marqo.inference.native_inference.device_manager.torch"
        ) as mock_cuda:
    ...
    def test_cuda_health_check_should_pass_when_cuda_device_is_healthy(self):
        ...
        with (
            ...
            mock.patch(
                "marqo.inference.native_inference.device_manager.logger"
            ) as mock_logger,
        ):
            device_manager.cuda_device_health_check()
    ...
    def test_cuda_health_check_should_fail_when_cuda_device_becomes_unavailable(self):
        ...
        with mock.patch("torch.cuda.is_available", return_value=False):
            ...
            self.assertEqual(
                str(err.exception), "CUDA device(s) have become unavailable"
            )
    ...
    def test_cuda_health_check_should_fail_when_cuda_device_is_out_of_memory(self):
        ...
        with (
            mock.patch("torch.cuda.is_available", return_value=True),
            mock.patch(
                "torch.randn", side_effect=RuntimeError("CUDA error: out of memory")
            ),
            mock.patch(
                "torch.cuda.memory_stats",
                return_value=self._mem_stats(900_000, 950_000),
            ),
        ):
            ...
            self.assertEqual(
                str(err.exception),
                "CUDA device cuda:0(Tesla T4) is out of memory "
    ...
    def test_cuda_health_check_should_pass_and_log_error_message_when_cuda_calls_encounter_issue_other_than_oom(
        self,
    ):
        device_manager = self._device_manager_with_multiple_cuda_devices()
        ...
        with (
            ...
            mock.patch(
                "torch.cuda.memory_stats",
                side_effect=[
                    RuntimeError("not a memory issue"),
        ...
        self.assertEqual(
            "Encountered issue inspecting CUDA device cuda:0(Tesla T4): not a memory issue",
            mock_logger.mock_calls[0][1][0],
        )

        self.assertEqual("error", mock_logger.mock_calls[1][0])
        self.assertEqual(
            "Encountered issue inspecting CUDA device cuda:1(Tesla H200): random exception",
    ...
    def test_pick_and_validate_device_should_return_device_if_valid(self):
        ...
        for device in ["cuda:0", "cuda:1", "cuda", "cpu"]:
            with self.subTest(device=device):
                self.assertEqual(
                    device, device_manager.pick_and_validate_device(device)
                )

    def test_pick_and_validate_device_should_raise_device_error_if_not_valid(self):
        device_manager = self._device_manager_with_multiple_cuda_devices()

        for device in ["cuda:2", "gpu", "tpu", "dish washer"]:
            with self.subTest(device=device):
                with self.assertRaises(DeviceError) as context:
                    device_manager.pick_and_validate_device(device)
                self.assertEqual(
                    f"`{device}` is not a valid device. Valid devices are [cpu, cuda:0, cuda:1]",
