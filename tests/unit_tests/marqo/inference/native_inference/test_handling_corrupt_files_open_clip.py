...
class TestCorruptFileInOpenCLIP(unittest.TestCase):
    ...
    def test_corrupted_file_handling_runtime_error(
        self, mock_os_remove, mock_create_model_and_transforms
    ):
        """Ensure that a proper error is raised when a corrupted file is encountered. The file should be removed."""
        mock_create_model_and_transforms.side_effect = RuntimeError(
    ...
    @patch("open_clip.create_model", autospec=True)
    @patch("os.remove", autospec=True)
    def test_corrupted_file_handling_unpickling_error(
        self, mock_os_remove, mock_create_model_and_transforms
    ):
        """Ensure that a proper error is raised when a corrupted file is encountered. The file should be removed."""
        mock_create_model_and_transforms.side_effect = UnpicklingError(
            "The file might be corrupted"
        )
        for model_properties in self.dummy_model_properties:
            with patch(
    ...
    @patch("open_clip.create_model", autospec=True)
    @patch("os.remove", autospec=True)
    def test_file_removal_failure_handling(
        self, mock_os_remove, mock_create_model_and_transforms
    ):
        # Setup
        mock_create_model_and_transforms.side_effect = RuntimeError(
        ...
        with patch(
            ...
            for model_properties in self.dummy_model_properties:
                ...
                self.assertIn(
                    "Marqo encountered an error while attempting to delete a corrupted file",
                    str(context.exception),
                )
    ...
    def test_handling_unpickling_error_with_weights_only_true(
        self, mock_download, mock_create_model
    ):
        # Setup
        ...
        for model_properties in self.dummy_model_properties:
            ...
            self.assertNotIn(
                "load_weights_only", mock_create_model.call_args_list[0][1]
            )
            # assert the second create_model call specifies `load_weights_only=False`
            self.assertEqual(
                mock_create_model.call_args_list[1][1]["load_weights_only"], False
            )
    ...
    @patch("open_clip.create_model", autospec=True)
    @patch("os.remove", autospec=True)
    def test_other_errors_handling(
        self, mock_os_remove, mock_create_model_and_transforms
    ):
        # Setup
        ...
        with patch(
            ...
            for model_properties in self.dummy_model_properties:
                ...
                self.assertIn(
                    "Marqo encountered an error when loading custom open_clip model",
                    str(context.exception),
                )
    ...
    @patch("open_clip.create_model", autospec=True)
    @patch("os.remove", autospec=True)
    def test_load_clip_into_open_clip_errors_handling(
        self, mock_os_remove, mock_create_model_and_transforms
    ):
        # Setup
