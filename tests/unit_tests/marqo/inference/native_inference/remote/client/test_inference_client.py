...
from src.marqo.inference.native_inference.remote.client.inference_client import (
...
class TestNativeInferenceClient(unittest.TestCase):
    def setUp(self):
        patcher = patch(
            "src.marqo.inference.native_inference.remote.client.inference_client.httpx.Client"
        )
    ...
    def test_vectorise_http_error_with_invalid_msgpack(self):
        ...
        self.assertIn(
            "Error parsing error message in msgpack format", str(context.exception)
        )
        self.mock_httpx_client.return_value.post.assert_called_once()

    def test_vectorise_timeout_error(self):
        # Simulate a timeout error
        self.mock_httpx_client.return_value.post.side_effect = httpx.TimeoutException(
            "Request timed out"
        )
    ...
    def test_vectorise_connection_error(self):
        ...
        self.mock_httpx_client.return_value.post.side_effect = httpx.ConnectError(
            "Connection failed"
        )
    ...
    @patch(
        "src.marqo.inference.native_inference.remote.client.inference_client.msgpack.unpackb"
    )
    ...
    def test_vectorise_invalid_status_code(self):
        ...
        self.assertIn(
            "HTTP error when calling remote inference service", str(context.exception)
        )
        self.mock_httpx_client.return_value.post.assert_called_once()
