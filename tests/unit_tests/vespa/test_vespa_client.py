...
class TestVespaClient(unittest.TestCase):
    ...
    def test_query_httpx_timeout_configuration_small_vespa_timeout(self):
        ...
        def mock_post(*args, **kwargs):
            ...
            mock_response.text = (
                '{"root": {"id": "test", "relevance": 1.0, "children": []}}'
            )
        ...
        test_cases = [
            ...
            (
                ...
                5.0,
                "1000ms",
                "Vespa timeout None -> Default to 1000 -> httpx timeout 5.0s",
            ),
            (
                0,
                5.0,
                "1000ms",
                "Vespa timeout 0ms -> Default to 1000 -> httpx timeout 5.0s",
            ),
        ]

        for (
            provided_vespa_timeout_ms,
            httpx_read_timeout_second,
            expected_vespa_timeout_ms,
            msg,
        ) in test_cases:
            with self.subTest(msg=msg):
                with patch.object(
                    httpx.Client, "post", side_effect=mock_post
                ) as mock_query:
