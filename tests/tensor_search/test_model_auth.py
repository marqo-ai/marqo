...
class TestModelAuthLoadedS3HuggingFaceModel(MarqoTestCase):
    """Tests for model auth with a huggingface sentence transfomers model loaded from S3"""
    ...
    def setUpClass(cls) -> None:
        ...
        model_properties = {
            #"name": "ViT-B/32",  Note name is not required for HF models
            "dimensions": 384,
            "model_location": {
                "s3": {
                    "Bucket": cls.s3_bucket,
...
class TestModelAuthModelLoadingHF(MarqoTestCase):
    ...
    def test_1_load_model_from_hf_zip_file_with_auth(self):
        """
        Does not yet assert that a file is downloaded
        """
        hf_object = "some_model.pt"
    ...
    def test_2_load_model_from_hf_zip_file_without_auth(self):
        """
        Does not yet assert that a file is downloaded
        """
        hf_object = "some_model.pt"
    ...
    def test_3_load_model_from_s3_zip_file_with_auth(self):
        """The other test load from add_docs, we have to make sure it works for
         search"""
    ...
    def test_4_load_model_from_public_url_zip_file(self):
    ...
    def test_5_load_model_from_public_hf_repo_with_auth(self):
        ...
        mock_automodel_from_pretrained = mock.MagicMock(side_effect=AutoModel.from_pretrained("sentence-transformers/all-MiniLM-L6-v1"))

        with mock.patch('transformers.AutoModel.from_pretrained', new=mock_automodel_from_pretrained):
            res = tensor_search.search(config=self.config, text='hello', index_name=self.index_name_1, model_auth=ModelAuth(hf=HfAuth(token=hf_token)))

        assert len(mock_automodel_from_pretrained.call_args_list) == 1

    def test_6_load_model_from_public_hf_repo_without_auth(self):
