...
class TestRerankingWithModels(unittest.TestCase):
    ...
    def test_reranking_text_sbert(self):
        ...
        assert all(
            isinstance(doc["_score"], (int, float, np.float32))
    ...
    def test_reranking_images_owl_inconsistent_highlights(self):
        ...
        assert (
            sum(doc["_highlights"] == [] for doc in results_lexical_copy["hits"]) == 1
        )

        assert (
            sum(doc["_highlights"] != [] for doc in results_lexical_copy["hits"]) == 2
        )
    ...
    def test_reranking_check_search_has_fields(self):
        ...
        assert any(
            True if in_results_field in r else False for r in results_lexical["hits"]
        )

        assert not any(
            True if not_in_results_field in r else False
            for r in results_lexical["hits"]
        )

        assert rerank._check_searchable_fields_in_results(
            search_results=results_lexical, searchable_fields=None
        )

        assert rerank._check_searchable_fields_in_results(
            search_results=results_lexical, searchable_fields=[in_results_field]
        )

        assert not rerank._check_searchable_fields_in_results(
            search_results=results_lexical, searchable_fields=[not_in_results_field]
        )
