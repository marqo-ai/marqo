...
from marqo.s2_inference.clip_utils import download_image_from_url, encode_url
from marqo.s2_inference.errors import ImageDownloadError
from tests.integ_tests.marqo_test import MockHttpServer
...
class TestImageDownloading(TestCase):
    def setUp(self):
        self.test_cases = [
            ...
            (
                ...
                "http://example.com/test%20url",
                "URL with spaces",
            ),
            (
                "http://example.com/test!@$&*()_+={}[]|\\:;'\"<>,.?/",
                "http://example.com/test!@$&*()_+=%7B%7D[]%7C%5C:;'%22%3C%3E,.?/",
                "URL with special characters",
            ),
            (
                "http://example.com/你好世界",
                "http://example.com/%E4%BD%A0%E5%A5%BD%E4%B8%96%E7%95%8C",
                "URL with non-ASCII characters",
            ),
            (
                "http://example.com/test?name=John Doe&age=30",
                "http://example.com/test?name=John%20Doe&age=30",
                "URL with query parameters",
            ),
            (
                "http://example.com/test#section 1",
                "http://example.com/test#section%201",
                "URL with fragments",
            ),
            (
            ...
            (
                "http://example.com/test url%20example",
                "http://example.com/test%20url%20example",
                "URL with mixed encoded and unencoded characters",
            ),
            (
                "http://example.com/例子.测试.jpg",
                "http://example.com/%E4%BE%8B%E5%AD%90.%E6%B5%8B%E8%AF%95.jpg",
            ...
            ("", "", "Empty URL"),
            (
                "http://example.com/œ∑ł.jpg",
                "http://example.com/%C5%93%E2%88%91%C5%82.jpg",
        ...
        for url, expected, msg in self.test_cases:
            with self.subTest(url=url, expected=expected, msg=msg):
                ...
                self.assertEqual(
                    result,
                    expected,
                    f"Error: for {msg}, expected '{expected}', but got '{result}'",
                )
            ...
            with self.subTest(url=url, expected=expected, msg=msg):
                with self.assertRaises(ImageDownloadError) as cm:
                    download_image_from_url(
                        image_path=url + ".jpg", media_download_headers={}
                    )

    def test_download_image_from_url_handlesUrlRequiringUserAgentHeader(self):
        url_requiring_user_agent_header = (
            "https://docs.marqo.ai/2.0.0/Examples/marqo.jpg"
        )
        try:
            download_image_from_url(
                image_path=url_requiring_user_agent_header, media_download_headers={}
            )
        except Exception as e:
            self.fail(
                f"Exception was raised when downloading {url_requiring_user_agent_header}: {e}"
            )

    @patch("pycurl.Curl")
    def test_download_image_from_url_mergesDefaultHeadersWithCustomHeaders(
        self, mock_curl
    ):
        ...
        test_cases = [
            ...
            (
                {"User-Agent": "Marqobot-Image/1.0"},
                ["User-Agent: Marqobot-Image/1.0"],
                "Headers with override",
            ),
        ]

        for headers, expected_headers, msg in test_cases:
            with self.subTest(
                headers=headers, expected_headers=expected_headers, msg=msg
            ):
                download_image_from_url(
                    "http://example.com/image.jpg", media_download_headers=headers
                )
                mock_curl_instance.setopt.assert_called_with(
                    pycurl.HTTPHEADER, expected_headers
                )
    ...
    def test_download_image_from_url_handlesRedirection(self):
        ...
        app = Starlette(
            routes=[
                Route(
                    "/missing_image.jpg",
                    lambda _: Response(
                        status_code=301, headers={"Location": "/image.jpg"}
                    ),
                ),
                Route(
                    "/image.jpg",
                    lambda _: Response(image_content, media_type="image/png"),
                ),
            ]
        )
        ...
        with MockHttpServer(app).run_in_thread() as base_url:
            result = download_image_from_url(
                f"{base_url}/missing_image.jpg", media_download_headers={}
            )
            self.assertEqual(result.getvalue(), image_content)

    @patch("marqo.s2_inference.clip_utils.pycurl.Curl")
    @patch.dict(
        "os.environ", {"MARQO_MAX_SEARCH_VIDEO_AUDIO_FILE_SIZE": "5000000"}
    )  # 5MB limit
    ...
    def test_video_audio_file_size_check_under_limit(self, mock_curl):
        ...
        mock_curl_instance.setopt.side_effect = simulate_setopt
        mock_curl_instance.getinfo.return_value = (
            200  # Simulate successful HTTP response
        )
        ...
        try:
            ...
            self.fail(
                "ImageDownloadError raised unexpectedly for file under size limit"
            )
    ...
    @patch("marqo.s2_inference.clip_utils.pycurl.Curl")
    @patch(
        "marqo.s2_inference.clip_utils.EnvVars.MARQO_MAX_SEARCH_VIDEO_AUDIO_FILE_SIZE",
        5_000_000,
    )  # 5MB limit
