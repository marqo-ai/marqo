...
from marqo.tensor_search.models.private_models import ModelLocation, S3Auth, S3Location
    ...
    def test_is_image(self):
        test_cases = [
            ...
            (
                ...
                "http://dummy.dummy.com/is/image/dummy/dummy (1)?wid=123&hei=321&qlt=123&fmt=png-alpha",
                True,
            ),
        ]
        ...
        for description, input_value, expected in test_cases:
            with self.subTest(description=description, input=input_value):
                self.assertEqual(
                    clip_utils._is_image(input_value),
                    expected,
...
class TestDownloadFromRepo(unittest.TestCase):
    ...
    def test__download_from_repo_with_auth(
        self,
        mock_download_model,
    ):
        ...
        s3_auth = S3Auth(
            aws_access_key_id="some_key_id", aws_secret_access_key="some_secret"
        )

        model_props = {
            ModelProperties.model_location: location.dict(),
        }
        auth = {"s3": s3_auth.dict()}
    ...
    @patch("marqo.s2_inference.clip_utils.download_model")
    def test__download_from_repo_without_auth(
        self,
        mock_download_model,
    ):
        mock_download_model.return_value = "model.pth"
...
class TestLoad(unittest.TestCase):
    ...
    @patch(
        ...
        return_value="downloaded_model.pth",
    )
    @patch(
        "marqo.s2_inference.clip_utils.clip.load",
        return_value=(mock.Mock(), mock.Mock()),
    ...
    def test_load_with_url(
        self, mock_url_valid, mock_isfile, mock_clip_load, mock_download_model
    ):
    ...
    @patch(
        "marqo.s2_inference.clip_utils.CLIP._download_from_repo",
        return_value="downloaded_model.pth",
    )
    @patch(
        "marqo.s2_inference.clip_utils.clip.load",
    ...
    def test_load_with_model_location(self, mock_clip_load, mock_download_from_repo):
        ...
        clip = CLIP(
            model_properties={ModelProperties.model_location: model_location.dict()},
            device="cpu",
        )
        clip.load()
