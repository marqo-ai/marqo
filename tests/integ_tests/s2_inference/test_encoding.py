...
class TestEncoding(unittest.TestCase):
                ...
                for sentence in sentences:
                    with self.subTest(sentence=sentence):
                        ...
                        if isinstance(sentence, str):
                            with self.subTest(
                                "Hardcoded Python 3.8 Embeddings Comparison"
                            ):
                                try:
                                    self.assertEqual(
                                        np.allclose(
                                        ...
                                        True,
                                        f"Calculated embeddings do not match hardcoded embeddings for model: {name}, sentence: {sentence}. Printing output: {output_m}",
                                    )
                                except KeyError:
                                    raise KeyError(
                        ...
                        with self.subTest("Model encode vs vectorize"):
                            self.assertEqual(
                                np.allclose(output_m, output_v, atol=eps),
                                True,
    ...
    def test_vectorize_normalise(self):
        ...
        for name in names:
            ...
            for sentence in sentences:
                output_v = vectorise(
                    name, sentence, model_properties, device, normalize_embeddings=True
                )
                assert _check_output_type(output_v)
                output_m = model.encode(sentence, normalize=True)
                assert (
                    abs(torch.FloatTensor(output_m) - torch.FloatTensor(output_v)).sum()
                    < eps
                )
                for vector in output_v:
                    assert abs(np.linalg.norm(np.array(vector)) - 1) < 1e-5

                output_v_unnormalised = vectorise(
                    name, sentence, model_properties, device, normalize_embeddings=False
                )
                assert _check_output_type(output_v)
                output_m_unnormalised = model.encode(sentence, normalize=False)
                assert (
                    abs(
                        torch.FloatTensor(output_v_unnormalised)
                        - torch.FloatTensor(output_m_unnormalised)
                    ).sum()
                    < eps
                )
    ...
    def test_cpu_encode_type(self):
        ...
        for name in names:
            ...
            for sentence in sentences:
                output_v = _convert_tensor_to_numpy(
                    model.encode(sentence, normalize=True)
                )
    ...
    def test_compare_onnx_sbert_text_models(self):
        names_sbert_onnx = [
            ("all-MiniLM-L6-v1", "onnx/all-MiniLM-L6-v1"),
        ...
        for name_sbert, name_onnx in names_sbert_onnx:
            for sentence in sentences:
                ...
                model_sbert = _load_model(
                    model_properties_sbert["name"],
                    model_properties=model_properties_sbert,
                    device=device,
                )

                model_properties_onnx = get_model_properties_from_registry(name_onnx)
                model_onnx = _load_model(
                    model_properties_onnx["name"],
                    model_properties=model_properties_onnx,
                    device=device,
                )

                assert (
                    abs(
                        model_onnx.encode(sentence) - model_sbert.encode(sentence)
                    ).sum()
                    < eps
                )
