...
from marqo.s2_inference.types import FloatTensor, Modality
...
class TestOpenClipModelEncoding(unittest.TestCase):
    ...
    def test_load_clip_image_model(self):
        ...
        for name in names:
            ...
            for image in images:
                assert abs(model.encode(image, modality=Modality.IMAGE) - model.encode([image], modality=Modality.IMAGE)).sum() < eps
                assert abs(model.encode_image(image) - model.encode([image], modality=Modality.IMAGE)).sum() < eps
                assert abs(model.encode(image, modality=Modality.IMAGE) - model.encode_image([image])).sum() < eps
