...
class TestReranking(unittest.TestCase):
    ...
    def test_results_to_df(self):
        ...
        assert sorted(
            results_lexical_formatted.results_df._id.apply(str).tolist()
        ) != sorted(results_lexical_formatted.results_df._rerank_id.apply(str).tolist())

        # searchable fields if not specified should be all non marqo fields
        assert results_lexical_formatted.searchable_fields == [
            "attributes",
            "location",
            "other",
        ]
    ...
    def test_formatting_for_model(self):
        ...
        inputs_df = results_lexical_formatted.format_for_model(
            results_lexical_formatted.results_df,
            results_lexical_formatted.searchable_fields,
            query=query,
        )
        ...
        assert len(inputs_df) == (
            len(results_lexical_formatted.searchable_fields)
            * len(results_lexical["hits"])
            - 2
        )
        ...
        assert all(inputs_df["field_content"].notna()) and all(
            inputs_df["field_content"].notnull()
        )

        # check the original field names are part of the available fields
        available_fields = ["attributes", "location", "other"]
        assert sorted(available_fields) == sorted(
            inputs_df["original_field_name"].unique().tolist()
        )
    ...
    def test_reranking_text(self):
        ...
        assert all(
            len(doc["_reranked_highlights"]) >= 1 for doc in results_lexical["hits"]
        )

        assert all(
            isinstance(doc["_reranked_score"], (int, float))
            for doc in results_lexical["hits"]
        )
    ...
    def test_rerank(self):
        ...
        assert all(
            isinstance(doc["_score"], (int, float)) for doc in results_lexical["hits"]
        )
