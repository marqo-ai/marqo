...
class TestVectorise(unittest.TestCase):
    def test_vectorise_in_batches(self):
        ...
        random_model = random_utils.Random(
            model_name="mock_model", embedding_dim=128, device="cpu"
        )
        ...
        mock_available_models = {
            s2_inference._create_model_cache_key(
                model_name="mock_model", device="cpu", model_properties=mock_model_props
            ): {
        ...
        @mock.patch(
            "marqo.s2_inference.s2_inference._available_models", mock_available_models
        )
        @mock.patch(
            "marqo.s2_inference.s2_inference._update_available_models", mock.MagicMock()
        )
        def run():
            s2_inference.vectorise(
                model_name="mock_model",
                content=["just a single content"],
    ...
    def test_vectorise_in_batches_with_different_batch_sizes(self):
        ...
        @mock.patch(
            "marqo.s2_inference.s2_inference.read_env_vars_and_defaults",
            side_effect=[2, 3, 10],
        )
...
class TestVectoriseBatching(unittest.TestCase):
    ...
    def test_vectorise_varying_content_lengths(self):
        ...
        result = s2_inference.vectorise(
            model_name="mock_model",
            content=varying_length_content,
            model_properties=self.mock_model_props,
            device="cpu",
        )
    ...
    @mock.patch(
        "marqo.s2_inference.s2_inference.read_env_vars_and_defaults", return_value=1
    )
