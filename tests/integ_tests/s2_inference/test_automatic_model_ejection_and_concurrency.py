...
class TestAutomaticModelEject(unittest.TestCase):
    ...
    def test_validate_model_into_device(self):
        ...
        with unittest.mock.patch(
            "marqo.s2_inference.s2_inference._validate_model_into_device"
        ) as mock_method:
            for model in small_list_of_models:
                _ = vectorise(model_name=model, content=content, device="cpu")

        checked_models = [
            call_args[0] for call_args, call_kwargs in mock_method.call_args_list
        ]
    ...
    def test_check_memory_threshold_for_model(self):
        ...
        with unittest.mock.patch(
            "marqo.s2_inference.s2_inference._check_memory_threshold_for_model"
        ) as mock_method:
            for model in small_list_of_models:
                _ = vectorise(model_name=model, content=content, device="cpu")
    ...
    def test_load_very_large_model(self):
        ...
        for model in huge_models:
            try:
                _ = vectorise(
                    model_name=model, content="this is a huge model", device="cpu"
                )
    ...
    def test_get_model_size(self):
        ...
        for model_name, size in models_and_sizes.items():
            self.assertEqual(
                s2_inference.get_model_size(
                    model_name, validate_model_properties(model_name, None)
                ),
                size,
                msg=model_name,
            )
        ...
        self.assertEqual(
            s2_inference.get_model_size(
                generic_model["model_name"], generic_model["model_properties_1"]
            ),
            1.53,
        )
        self.assertEqual(
            s2_inference.get_model_size(
                generic_model["model_name"], generic_model["model_properties_2"]
            ),
            1.5,
        )
    ...
    def test_thread_safe_function_call(self):
        ...
        _validate_model_into_device(
            model_name, validated_model_properties, device, calling_func="unit_test"
        )
        ...
        try:
            _validate_model_into_device(
                model_name, validated_model_properties, device, calling_func="void"
            )
            raise AssertionError
        except RuntimeError:
            pass
        ...
        _check_memory_threshold_for_model(
            device, model_size, calling_func="_validate_model_into_device"
        )
    ...
    def test_concurrent_vectorise_call_no_cache(self):
        ...
        q_2 = queue.Queue()
        t = threading.Thread(
            target=normal_vectorise_call, args=(test_model, test_content, q_1)
        )
        threads.append(t)
        t.start()
        ...
        for i in range(num_of_threads):
            t = threading.Thread(
                target=racing_vectorise_call, args=(test_model, test_content, q_2)
            )
            threads.append(t)
            t.start()
    ...
    def test_concurrent_vectorise_call_cached(self):
        ...
        for i in range(num_of_threads):
            t = threading.Thread(
                target=normal_vectorise_call, args=(test_model, test_content, q_2)
            )
            threads.append(t)
            t.start()
    ...
    def test_concurrent_model_loading_and_vectorise(self):
        ...
        q_1 = queue.Queue()
        t = threading.Thread(
            target=normal_vectorise_call, args=(test_model_1, test_content, q_1)
        )
        t.start()
        ...
        t_1 = threading.Thread(
            target=normal_vectorise_call, args=(test_model_2, test_content, q_1)
        )
