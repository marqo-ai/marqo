...
class TestModelRegistry(unittest.TestCase):
    ...
    def test_open_clip_model_configurations_match(self):
        ...
        model_pairs = [
            ...
            (
                "Marqo/ViT-L-14.laion2b_s32b_b82k",
                "open_clip/ViT-L-14/laion2b_s32b_b82k",
            ),
            (
                "Marqo/roberta-ViT-B-32.laion2b_s12b_b32k",
        ...
        for marqo_model, open_clip_model in model_pairs:
            with self.subTest(
                f"Check if {marqo_model} & {open_clip_model} configurations match"
            ):
                self.compare_models(
                    self.open_clip_properties[marqo_model],
                    self.open_clip_properties[open_clip_model],
                )

    def compare_models(
        self, model_properties_with_name_in_marqtune_convention, model_properties
    ):
        self.assertEqual(
            model_properties_with_name_in_marqtune_convention["dimensions"],
            model_properties["dimensions"],
            f"dimensions do not match for {model_properties_with_name_in_marqtune_convention['name']} and {model_properties['name']}",
        )
        self.assertEqual(
            model_properties_with_name_in_marqtune_convention["note"],
            model_properties["note"],
            f"note do not match for {model_properties_with_name_in_marqtune_convention['name']} and {model_properties['name']}",
        )
        self.assertEqual(
            model_properties_with_name_in_marqtune_convention["type"],
            model_properties["type"],
            f"type do not match for {model_properties_with_name_in_marqtune_convention['name']} and {model_properties['name']}",
        )
        self.assertEqual(
            model_properties_with_name_in_marqtune_convention["pretrained"],
            model_properties["pretrained"],
            f"pretrained do not match for {model_properties_with_name_in_marqtune_convention['name']} and {model_properties['name']}",
        )
