...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        action="store_true",
        default=False,
        help="Run tests that have multiple Vespa nodes",
    )
    parser.addoption(
        "--split-n",
        action="store",
    ...
    parser.addoption(
        ...
        type=int,
        default=0,
        help="Run tests for part N (zero-based index)",
    )
    ...
    parser.addoption(
        ...
        type=str,
        default=TESTS_SPLIT,
        help=f"Split tests by {CLASSES_SPLIT} or {TESTS_SPLIT}",
    )
    parser.addoption(
        "--suppress-skips",
        action="store_true",
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "unittest: mark test as unit test, it does not require vespa to run"
    )
    config.addinivalue_line(
        "markers", "skip_for_multinode: mark test to be skipped if multinode"
    )
...
def pytest_collection_modifyitems(config, items):
    ...
    print(
        f"Total tests: {len(filtered_items)}, Splitting into {parts} parts, each with {chunk_size} {split_by}"
    )
    ...
    if split_by == CLASSES_SPLIT:
        items[:] = [
            item
            for item in filtered_items
            if item.parent.name in set(sorted_classes[start_idx:end_idx])
        ]
    else:
