...
MULTILINGUAL_CLIP_TEST_MODELS = ["multilingual-clip/XLM-Roberta-Large-Vit-B-32"]


@parameterized_class(
    [{"model_name": model_name} for model_name in MULTILINGUAL_CLIP_TEST_MODELS]
)
class TestMultilingualCLIPModelEncode(InferenceTestCase):
    ...
    device = "cpu"
    embeddings_test_texts = ["hello", "This is a test sentence"]
    ...
    def setUpClass(cls):
        ...
        json_file = (
            ...
            / "embeddings_multilingual_clip_python_3_9.json"
        )
        ...
        cls.device_patcher = mock.patch.dict(
    ...
    def test_embeddings_regression(self):
        try:
            self.model_embeddings_reference = (
                self.multilingual_clip_embeddings_reference[self.model_name]
            )
        except KeyError:
            raise KeyError(
    ...
    @patch(
        "marqo.inference.native_inference.embedding_models.multilingual_clip_model.torch.cuda.amp.autocast"
    )
