...
@parameterized_class(
    [{"model_name": model_name} for model_name in OPEN_CLIP_PREPROCESSOR_TEST_MODELS]
)
class TestOpenClipModelPreprocessor(InferenceTestCase):
    ...
    def test_to_device_called_on_preprocessor_for_image(self):
        ...
        test_images = [
            load_image_from_path(image_path, media_download_headers=dict())
            for image_path in test_image_path
        ]

        image_outputs = self.preprocessor.preprocess(
            test_images, modality=Modality.IMAGE
        )
