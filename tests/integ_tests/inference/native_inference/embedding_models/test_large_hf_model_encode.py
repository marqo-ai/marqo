...
@parameterized_class(
    [{"model_name": model_name} for model_name in LARGE_HF_TEST_MODELS]
)
class TestLargeHFModelEncode(InferenceTestCase):
    ...
    def setUpClass(cls):
        ...
        files = [
            ...
            / "embeddings_all_models_python_3_8.json",
            target_dir / "embeddings_reference" / "embeddings_large_e5_python_3_8.json",
            target_dir
            / "embeddings_reference"
            / "embeddings_large_multilingual_e5_python_3_8.json",
        ...
        for file in files:
            if not os.path.exists(file):
                raise FileNotFoundError(
                    f"File {file} not found, which is needed to compare embeddings."
                )
    ...
    def test_embeddings_regression(self):
        try:
            self.model_embeddings_reference = self.hf_embeddings_reference[
                self.model_name
            ]
        except KeyError:
            self.skipTest(
                reason=f"Model {self.model_name} not found in the embeddings reference file."
            )
