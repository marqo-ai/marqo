...
class TestMarqoFashionCLIPEncode(InferenceTestCase):
    ...
    FASHIONCLIP_IMAGE_EMBEDDING = np.squeeze(
        np.array(
            [
                [
                    -5.5725e-02,
    ...
    FASHIONCLIP_TEXT_EMBEDDING = np.squeeze(
        np.array(
            [
                [
                    -2.6367e-02,
    ...
    SiGLIP_IMAGE_EMBEDDING = np.squeeze(
        np.array(
            [
                [
                    -2.4643e-02,
    ...
    SiGLIP_TEXT_EMBEDDING = np.squeeze(
        np.array(
            [
                [
                    -2.0950e-02,
    ...
    def test_MarqoFashionSigLIPModel_load(self):
        ...
        marqo_image_embeddings = self.encode_content_helper(
            content=[self.IMAGE_CONTENT], model_name=model_tag, modality="image"
        )[0]
        marqo_text_embeddings = self.encode_content_helper(
            content=[self.TEXT_CONTENT], model_name=model_tag, modality="language"
        )[0]

        image_difference = np.linalg.norm(
            marqo_image_embeddings - self.SiGLIP_IMAGE_EMBEDDING
        ) / len(marqo_image_embeddings)
        text_difference = np.linalg.norm(
            marqo_text_embeddings - self.SiGLIP_TEXT_EMBEDDING
        ) / len(marqo_text_embeddings)

        self.assertLess(
            image_difference,
            1e-4,
            f"Image embeddings are not close enough. "
            f"The average difference is: {image_difference}",
        )
        self.assertLess(
            text_difference,
            1e-4,
            f"Text embeddings are not close enough. "
    ...
    def test_MarqoFashionCLIPModel_load(self):
        ...
        image_difference = np.linalg.norm(
            marqo_image_embeddings - self.FASHIONCLIP_IMAGE_EMBEDDING
        ) / len(marqo_image_embeddings)
        text_difference = np.linalg.norm(
            marqo_text_embeddings - self.FASHIONCLIP_TEXT_EMBEDDING
        ) / len(marqo_text_embeddings)
