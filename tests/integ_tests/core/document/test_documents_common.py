...
class TestDocumentsCommon(MarqoTestCase):
    ...
    def test_marqoAddDocumentsResponseDictFormat(self):
        ...
        item = MarqoAddDocumentsItem(
            status=200, id="id", message="message", error="error", code="code"
        )
        res = MarqoAddDocumentsResponse(
            errors=False, processingTimeMs=0.0, index_name="index_name", items=[item]
        )
    ...
    def test_marqoUpdateDocumentsResponseDictFormat(self):
        ...
        item = MarqoUpdateDocumentsItem(
            id="id", status=200, message="message", error="error"
        )
        res = MarqoUpdateDocumentsResponse(
            errors=False, index_name="index_name", items=[item], processingTimeMs=0.0
        )
        ...
        expected_dict = {
            ...
            "index_name": "index_name",
            "items": [
                {"_id": "id", "status": 200, "message": "message", "error": "error"}
            ],
            "processingTimeMs": 0.0,
        }
        self.assertEqual(expected_dict, returned_dict)
    ...
    def test_marqoGetDocumentsByIdsResponseDictFormat(self):
        ...
        item = MarqoGetDocumentsByIdsItem(
            id="id", status=200, message="message", found=True
        )
        ...
        expected_dict = {
            "errors": False,
            "results": [
                {"_id": "id", "status": 200, "message": "message", "_found": True}
            ],
        }
        self.assertEqual(expected_dict, returned_dict)
