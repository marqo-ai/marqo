...
from marqo.core.models.marqo_update_documents_response import (
    MarqoUpdateDocumentsItem,
...
class TestUpdate(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        structured_index_request_1 = cls.structured_marqo_index_request(
            fields=[
                ...
                FieldRequest(
                    name="text_field_filter",
                    type=FieldType.Text,
                    features=[FieldFeature.Filter],
                ),
                FieldRequest(
                    name="text_field_lexical",
                    type=FieldType.Text,
                ...
                FieldRequest(
                    name="text_field_tensor", type=FieldType.Text
                ),  # A tensor field
                FieldRequest(name="int_field", type=FieldType.Int),
                FieldRequest(
                    name="int_field_filter",
                    type=FieldType.Int,
                    features=[FieldFeature.Filter],
                ),
                FieldRequest(
                    name="int_field_score_modifier",
                    type=FieldType.Int,
                    features=[FieldFeature.ScoreModifier],
                ),
                FieldRequest(name="float_field", type=FieldType.Float),
                FieldRequest(
                    name="float_field_filter",
                    type=FieldType.Float,
                    features=[FieldFeature.Filter],
                ),
                FieldRequest(
                    name="float_field_score_modifier",
                    type=FieldType.Float,
                    features=[FieldFeature.ScoreModifier],
                ),
                FieldRequest(
                    name="bool_field_filter",
                    type=FieldType.Bool,
                    features=[FieldFeature.Filter],
                ),
                ...
                FieldRequest(
                    name="multi_modal_field",
                    type=FieldType.MultimodalCombination,
                    dependent_fields={
                        "dependent_field_1": 1.0,
                        "dependent_field_2": 1.0,
                    },
                ),
                FieldRequest(
                    name="array_text_field",
        ...
        large_score_modifiers_index_fields = [
            FieldRequest(
                name=f"float_field_{i}",
                type=FieldType.Float,
                features=[FieldFeature.Filter, FieldFeature.ScoreModifier],
            )
            for i in range(100)
        ]
    ...
    def _set_up_for_text_field_test(self):
        ...
        self.add_documents(
            ...
            add_docs_params=AddDocsParams(
                index_name=self.structured_index_name, docs=[original_doc]
            ),
        )
        self.assertEqual(
            1,
            self.monitoring.get_index_stats_by_name(
                self.structured_index_name
            ).number_of_documents,
        )
    ...
    def test_update_text_field(self):
        ...
        updated_doc = {"text_field": "updated text field", "_id": "1"}
        r = update_documents(
        ...
        updated_doc = tensor_search.get_document_by_id(
            self.config, self.structured_index_name, updated_doc["_id"]
        )
    ...
    def test_update_text_field_filter(self):
        ...
        updated_doc = {"text_field_filter": "updated text field filter", "_id": "1"}
        r = update_documents(
            body_dict={"documents": [updated_doc]},
            index_name=self.structured_index_name,
            marqo_config=self.config,
        ...
        search_result = tensor_search.search(
            ...
            index_name=self.structured_index_name,
            text="test",
            filter="text_field_filter:(updated text field filter)",
        )
        ...
        search_result = tensor_search.search(
            ...
            index_name=self.structured_index_name,
            text="test",
            filter="text_field_filter:(text field filter)",
        )
        self.assertEqual(0, len(search_result["hits"]))

    def test_update_text_field_lexical(self):
        self._set_up_for_text_field_test()
        updated_doc = {"text_field_lexical": "search me please", "_id": "1"}
        r = update_documents(
            body_dict={"documents": [updated_doc]},
            index_name=self.structured_index_name,
            marqo_config=self.config,
    ...
    def test_update_text_field_tensor(self):
        self._set_up_for_text_field_test()
        updated_doc = {"text_field_tensor": "I can't be updated", "_id": "1"}
        r = self.config.document.partial_update_documents_by_index_name(
            partial_documents=[updated_doc], index_name=self.structured_index_name
    ...
    def test_update_text_field_add(self):
        ...
        self._set_up_for_text_field_test()
        updated_doc = {"text_field_add": "I am a new field", "_id": "1"}
        r = update_documents(
            body_dict={"documents": [updated_doc]},
            index_name=self.structured_index_name,
            marqo_config=self.config,
        ...
        lexical_search_result = tensor_search.search(
            ...
            index_name=self.structured_index_name,
            search_method="LEXICAL",
            text="I am a new field",
            searchable_attributes=["text_field_lexical"],
        )
        self.assertEqual(1, len(lexical_search_result["hits"]))

        filter_search_result = tensor_search.search(
            config=self.config,
            index_name=self.structured_index_name,
            text="test",
            filter="text_field_add:(I am a new field)",
        )
        self.assertEqual(1, len(filter_search_result["hits"]))

    def test_update_int_field(self):
        self._set_up_for_int_field_test()
        updated_doc = {"int_field": 11, "_id": "1"}
    ...
    def test_update_int_field_filter(self):
        self._set_up_for_int_field_test()
        updated_doc = {"int_field_filter": 22, "_id": "1"}
        ...
        search_result = tensor_search.search(
            ...
            index_name=self.structured_index_name,
            text="test",
            filter="int_field_filter:(2)",
        )
        self.assertEqual(0, len(search_result["hits"]))

    def test_update_int_field_score_modifier(self):
        self._set_up_for_int_field_test()
        updated_doc = {"int_field_score_modifier": 33, "_id": "1"}

        score_modifier = ScoreModifierLists(
            **{
                "add_to_score": [
                    {"field_name": "int_field_score_modifier", "weight": 1}
                ]
            }
        )
        ...
        modified_score = tensor_search.search(
            config=self.config,
            index_name=self.structured_index_name,
            text="test",
        ...
        self.assertTrue(
            33 <= modified_score <= 34, f"Modified score is {modified_score}"
        )

    def test_update_float_field(self):
        self._set_up_for_float_field_test()
        updated_doc = {"float_field": 11.1, "_id": "1"}
    ...
    def test_update_float_field_filter(self):
        self._set_up_for_float_field_test()
        updated_doc = {"float_field_filter": 22.2, "_id": "1"}
        ...
        search_result = tensor_search.search(
            ...
            index_name=self.structured_index_name,
            text="test",
            filter="float_field_filter:(22.2)",
        )
        self.assertEqual(1, len(search_result["hits"]))
        ...
        search_result = tensor_search.search(
            ...
            index_name=self.structured_index_name,
            text="test",
            filter="float_field_filter:(2.2)",
        )
        self.assertEqual(0, len(search_result["hits"]))

    def test_update_float_field_score_modifier(self):
        self._set_up_for_float_field_test()
        updated_doc = {"float_field_score_modifier": 33.3, "_id": "1"}

        score_modifier = ScoreModifierLists(
            **{
                "add_to_score": [
                    {"field_name": "float_field_score_modifier", "weight": 1.0}
                ]
            }
        )
        ...
        self.assertTrue(
            33.3 <= modified_score <= 34.3, f"Modified score is {modified_score}"
        )
    ...
    def test_update_bool_field_filter(self):
        ...
        updated_doc = {"bool_field_filter": False, "_id": "1"}
        r = update_documents(
            body_dict={"documents": [updated_doc]},
            index_name=self.structured_index_name,
            marqo_config=self.config,
        ...
        search_result = tensor_search.search(
            ...
            index_name=self.structured_index_name,
            text="test",
            filter="bool_field_filter:(false)",
        )
        self.assertEqual(1, len(search_result["hits"]))
    ...
    def test_update_image_pointer_field(self):
        ...
        updated_doc = {"image_pointer_field": TestImageUrls.IMAGE2.value, "_id": "1"}
        r = update_documents(
            body_dict={"documents": [updated_doc]},
            index_name=self.structured_index_name,
            marqo_config=self.config,
        ...
        self.assertEqual(TestImageUrls.IMAGE2.value, updated_doc["image_pointer_field"])
    ...
    def test_update_multimodal_image_field(self):
        ...
        for index_name in [
            self.structured_image_index_name,
            self.unstructured_image_index_name,
        ]:
            with self.subTest(index_name=index_name):
                ...
                updated_doc = {"_id": "1", "image_field_1": updated_image_url}
    ...
    def test_update_multimodal_dependent_field(self):
        ...
        updated_doc = {"dependent_field_1": "updated dependent field 1", "_id": "1"}
    ...
    def test_update_array_text_field_filter(self):
        ...
        updated_doc = {"array_text_field": ["text3", "text4"], "_id": "1"}
        r = update_documents(
            body_dict={"documents": [updated_doc]},
            index_name=self.structured_index_name,
            marqo_config=self.config,
        ...
        search_result = tensor_search.search(
            ...
            index_name=self.structured_index_name,
            text="test",
            filter="array_text_field:(text1)",
        )
        self.assertEqual(0, len(search_result["hits"]))

    def test_update_a_document_that_does_not_exist(self):
        """"""
        updated_doc = {"_id": "gibberish", "text_field": "some value"}
        r = self.config.document.partial_update_documents_by_index_name(
            partial_documents=[updated_doc], index_name=self.structured_index_name
        ).dict(exclude_none=True, by_alias=True)
    ...
    def test_update_a_document_without_id(self):
        updated_doc = {"text_field": "updated text field"}
        r = self.config.document.partial_update_documents_by_index_name(
            partial_documents=[updated_doc], index_name=self.structured_index_name
        ).dict(exclude_none=True, by_alias=True)
        self.assertEqual(True, r["errors"])
        self.assertIn(
            "'_id' is a required field but it does not exist", r["items"][0]["error"]
        )
    ...
    def test_update_non_existent_field(self):
        self._set_up_for_text_field_test()
        updated_doc = {"_id": "1", "non_existent_field": "some value"}
        r = self.config.document.partial_update_documents_by_index_name(
            partial_documents=[updated_doc], index_name=self.structured_index_name
        ).dict(exclude_none=True, by_alias=True)
        self.assertEqual(True, r["errors"])
    ...
    def test_update_with_incorrect_field_value(self):
        ...
        test_cases = [
            (
                {"int_field_filter": "should be an integer"},
                True,
            ...
            (
                {"array_int_field": [1, "should be an integer", 3]},
                True,
                "This should be a list of integers",
            ),
            (
                {"array_text_field": ["string", 2, "string"]},
                True,
    ...
    def test_multi_threading_update(self):
        ...
        self.add_documents(
            config=self.config,
            add_docs_params=AddDocsParams(
                index_name=self.structured_index_name, docs=[original_document]
            ),
        )
            ...
            for _ in range(number_of_updates):
                ...
                for picked_field in picked_fields:
                    if picked_field.startswith("text_field"):
                        updated_doc[picked_field] = "text field" + str(
        ...
        threads = [
            threading.Thread(
                target=randomly_update_document(updates_per_thread),
                args=[updates_per_thread],
            )
        ...
        updated_doc = tensor_search.get_document_by_id(
            self.config, self.structured_index_name, "1"
        )
        ...
        self.assertEqual(
            updated_doc["text_field_filter"].startswith("text field"), True
        )
        self.assertEqual(
            updated_doc["text_field_lexical"].startswith("text field"), True
        )
        self.assertEqual(
            updated_doc["text_field_tensor"].startswith("text field"), True
        )
    ...
    def test_multi_threading_update_for_large_score_modifier_fields(self):
        ...
        self.assertEqual(
            1,
            self.monitoring.get_index_stats_by_name(
                self.large_score_modifier_index_name
            ).number_of_documents,
        )
        ...
        updated_doc = tensor_search.get_document_by_id(
            self.config, self.large_score_modifier_index_name, "1"
        )
        ...
        final_doc["_id"] = "1"
        r = update_documents(
            body_dict={"documents": [final_doc]},
            index_name=self.large_score_modifier_index_name,
            marqo_config=self.config,
        )

        original_score = tensor_search.search(
            config=self.config,
            index_name=self.large_score_modifier_index_name,
            text="test",
        )["hits"][0]["_score"]
        ...
        for i in range(100):
            score_modifiers = ScoreModifierLists(
                **{
                    "add_to_score": [
                        {"field_name": f"float_field_{i}", "weight": "1.0"}
                    ]
                }
            )
    ...
    def test_duplicate_ids_in_one_batch(self):
        ...
        for i in range(10):
            r = self.config.document.partial_update_documents_by_index_name(
                partial_documents=update_docs, index_name=self.structured_index_name
            ).dict(exclude_none=True, by_alias=True)
    ...
    def test_update_document_response_format(self):
        ...
        test_cases = [
            ...
            (
                ...
                400,
                "",
            ),  # An invalid doc without _id
            (
                [{"text_field": ["1", "1"], "_id": "1"}],
                True,
                400,
                "1",
            ),  # An invalid doc with wrong field type
            (
                [{"text_field": "updated text field", "_id": "2"}],
                True,
                404,
                "2",
            ),  # An invalid doc with non-existent _id
        ]
        for update_docs, expected_error, expected_status, expected_id in test_cases:
            with self.subTest(
                f"{update_docs} - {expected_error} - {expected_status} - {expected_id}"
            ):
                r = self.config.document.partial_update_documents_by_index_name(
                    partial_documents=update_docs, index_name=self.structured_index_name
                ).dict(exclude_none=True, by_alias=True)
    ...
    def test_update_documents_response_successCounts(self):
        ...
        for items, expected_success, expected_failure, expected_error in test_cases:
            with self.subTest(items=items):
                ...
                self.assertEqual(
                    list(update_documents_response.get_header_dict().values()),
