...
class TestPartialUpdate(MarqoTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.doc2 = {  # This document helps us test behavior of partial updating fields when
            ...
            "bool_field": True,
            "bool_field2": False,
            "custom_vector_field": {"content": "abcd", "vector": [1.0] * 32},
        ...
        self.id_to_doc = {"1": self.doc, "2": self.doc2, "3": self.doc3}
    ...
    def _assert_fields_unchanged(self, doc: Dict[str, Any], excluded_fields: List[str]):
        ...
        for field, value in doc_to_compare_against.items():
            if field in excluded_fields:
                ...
                self.assertEqual(
                    value["content"], doc.get(field, None), f"{field} is changed."
                )
                ...
                for k, v in value.items():
                    ...
                    self.assertEqual(
                        v,
                        doc.get(flattened_field_name, None),
                        f"{flattened_field_name} is changed.",
                    )
            else:
                self.assertEqual(value, doc.get(field, None), f"{field} is changed.")

    def _assert_field_types(
        self, id: str, field_type_pairs: List[Tuple[str, MarqoFieldTypes]]
    ):
        """
        ...
        raw_vespa_doc = self.config.vespa_client.get_document(
            id, self.index.schema_name
        )
        ...
        for field_name, field_type in field_type_pairs:
            ...
            self.assertEqual(
                vespa_fields.get("marqo__field_types").get(field_name),
                expected_type,
    ...
    def _assert_field_types_not_changed(
        self,
        id: str,
        vespa_fields: dict,
        ...
        if not is_expected_and_actual_equal:
            ...
            intersection = set(actual_field_types.items()) & set(
                expected_field_types.items()
            )
            # Find fields that are in expected but not in actual
            fields_present_in_expected_but_not_in_actual = (
                set(expected_field_types.items()) - intersection
            )
            # Find fields that are in actual but not in expected
            fields_present_in_actual_but_not_in_expected = (
                set(actual_field_types.items()) - intersection
            )
            # Report any missing fields that should be present
            for field, _ in fields_present_in_expected_but_not_in_actual:
                self.fail(
                    f"Field {field} is present in expected field types but not in actual field types"
                )
            # Report any extra fields that shouldn't be present
            for field, _ in fields_present_in_actual_but_not_in_expected:
                self.fail(
                    f"Field {field} is present in actual field types but not in expected field types"
                )
        ...
        for doc in test_docs:
            with self.subTest(f"Updating document with ID {doc['_id']}"):
                ...
                res = self.config.document.partial_update_documents(
                    [{"_id": id, "bool_field": False}], self.index
                )
                self.assertFalse(
                    res.errors, f"Expected no errors when updating document {id}"
                )
        ...
        for doc in test_docs:
            with self.subTest(f"Verifying document with ID {doc['_id']}"):
                ...
                updated_doc = tensor_search.get_document_by_id(
                    self.config, self.index.name, id
                )
                self.assertFalse(
                    updated_doc["bool_field"],
                    f"Expected bool_field to be False for document {id}",
                )
                self._assert_fields_unchanged(updated_doc, ["bool_field"])

                # Verify field type
                self._assert_field_types(id, [("bool_field", MarqoFieldTypes.BOOL)])
    ...
    def test_partial_update_should_update_int_field_to_int(self):
        ...
        for doc in test_docs:
            with self.subTest(f"Updating document with ID {doc['_id']}"):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [{"_id": id, "int_field": 500}], self.index
                )
                self.assertFalse(
                    res.errors, f"Expected no errors when updating document {id}"
                )
        ...
        for doc in test_docs:
            with self.subTest(f"Verifying document with ID {doc['_id']}"):
                ...
                self.assertEqual(
                    500,
                    updated_doc["int_field"],
                    f"Expected int_field to be 500 for document {id}",
                )
                self._assert_fields_unchanged(updated_doc, ["int_field"])

                # Verify field type
                self._assert_field_types(id, [("int_field", MarqoFieldTypes.INT)])
    ...
    def test_partial_update_to_non_existent_field(self):
        ...
        for doc in test_docs:
            with self.subTest(f"Adding new field to document with ID {doc['_id']}"):
                ...
                self.assertEqual(
                    500,
                    doc["update_field_that_doesnt_exist"],
                ...
                self._assert_field_types(
                    id, [("update_field_that_doesnt_exist", MarqoFieldTypes.INT)]
                )
    ...
    def test_partial_update_should_not_update_int_field_to_float(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Attempting to update int field to float for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [{"_id": id, "int_field": 1.0}], self.index
                )
                self.assertTrue(res.errors)
                self.assertIn(
                    "reference/api/documents/update-documents/#response",
                    res.items[0].error,
                )
                self.assertIn(
    ...
    def test_partial_update_should_update_float_field_to_float(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Updating float field for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [{"_id": id, "float_field": 500.0}], self.index
                )
                self.assertFalse(
                    res.errors, f"Expected no errors when updating document {id}"
                )
                ...
                self.assertEqual(
                    500.0,
                    doc["float_field"],
                    f"Expected float_field to be 500.0 for document {id}",
                )
                self._assert_fields_unchanged(doc, ["float_field"])

                # Verify field type
                self._assert_field_types(id, [("float_field", MarqoFieldTypes.FLOAT)])
    ...
    def test_partial_update_should_update_int_map(self):
        ...
        for doc in test_docs:
            with self.subTest(f"Updating int map for document with ID {doc['_id']}"):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [{"_id": id, "int_map": {"c": 2, "d": 3}}], self.index
                )
                self.assertFalse(
                    res.errors, f"Expected no errors when updating document {id}"
                )
                ...
                doc = tensor_search.get_document_by_id(self.config, self.index.name, id)
                self.assertEqual(
                    doc["int_map.c"], 2, f"Expected int_map.c to be 2 for document {id}"
                )
                self.assertEqual(
                    doc["int_map.d"], 3, f"Expected int_map.d to be 3 for document {id}"
                )
                self.assertEqual(
                    doc.get("int_map.a"),
                    None,
                    f"Expected int_map.a to be None for document {id}",
                )
                self.assertEqual(
                    doc.get("int_map.b"),
                    None,
                    f"Expected int_map.b to be None for document {id}",
                )
    ...
    def test_partial_update_should_replace_int_map(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Replacing int map keys for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [{"_id": id, "int_map": {"f": 2, "g": 3}}], self.index
                )
                self.assertFalse(
                    res.errors, f"Expected no errors when updating document {id}"
                )
                ...
                doc = tensor_search.get_document_by_id(self.config, self.index.name, id)
                self.assertEqual(
                    doc["int_map.f"], 2, f"Expected int_map.f to be 2 for document {id}"
                )
                self.assertEqual(
                    doc["int_map.g"], 3, f"Expected int_map.g to be 3 for document {id}"
                )
                self.assertEqual(
                    doc.get("int_map.a"),
                    None,
    ...
    def test_partial_update_should_update_int_map_with_new_value(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Adding new key to int map for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [{"_id": id, "int_map": {"d": 2}}], self.index
                )
                self.assertFalse(
                    res.errors, f"Expected no errors when updating document {id}"
                )
                ...
                self.assertEqual(
                    doc["int_map.d"], 2, f"Expected int_map.d to be 2 for document {id}"
                )
    ...
    def test_partial_update_should_update_float_map(self):
        ...
        for doc in test_docs:
            with self.subTest(f"Updating float map for document with ID {doc['_id']}"):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [{"_id": id, "float_map": {"c": 2.0, "d": 3.0}}], self.index
                )
                self.assertFalse(
                    res.errors, f"Expected no errors when updating document {id}"
                )
                ...
                self.assertEqual(
                    doc["float_map.c"],
                    2.0,
                    f"Expected float_map.c to be 2.0 for document {id}",
                )
                self.assertEqual(
                    doc["float_map.d"],
                    3.0,
                    f"Expected float_map.d to be 3.0 for document {id}",
                )
    ...
    def test_partial_update_should_allow_changing_multiple_maps_in_same_request(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Updating multiple fields for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [
                ...
                doc = tensor_search.get_document_by_id(self.config, self.index.name, id)
                self.assertEqual(
                    2, doc["int_field"], f"Expected int_field to be 2 for document {id}"
                )
                self.assertFalse(
                    doc["bool_field"],
                    f"Expected bool_field to be False for document {id}",
                ...
                self.assertEqual(
                    doc["int_map.a"], 2, f"Expected int_map.a to be 2 for document {id}"
                )
                self.assertIsNone(
                    doc.get("int_map.b"),
                    f"Expected int_map.b to be None for document {id}",
                ...
                self.assertEqual(
                    doc["float_map.c"],
                    3.0,
                    f"Expected float_map.c to be 3.0 for document {id}",
                )
                self.assertIsNone(
                    doc.get("float_map.d"),
                    f"Expected float_map.d to be None for document {id}",
                )
    ...
    def test_partial_update_should_update_string_array(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Updating string array for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [{"_id": id, "string_array": ["ccc"]}], self.index
                )
                self.assertFalse(
                    res.errors, f"Expected no errors when updating document {id}"
                )
                ...
                self.assertEqual(
                    ["ccc"],
                    doc["string_array"],
                    f"Expected string_array to be ['ccc'] for document {id}",
                )
                self._assert_fields_unchanged(doc, ["string_array"])

                # Verify field type
                self._assert_field_types(
                    id, [("string_array", MarqoFieldTypes.STRING_ARRAY)]
                )
    ...
    def test_partial_update_should_reject_new_string_array_field(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Adding new string array for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [{"_id": id, "string_array3": ["ccc"]}], self.index
                )
                self.assertTrue(
                    res.errors,
                    f"Expected errors when adding new string array to document {id}",
                )
                self.assertEqual(400, res.items[0].status)
                self.assertIn(
                    "Unstructured index updates only support updating existing string array fields",
                    res.items[0].error,
                )
    ...
    def test_partial_update_should_allow_adding_new_string_string_array_field_if_present_in_other_docs_in_same_index(
        ...
        for doc in test_docs:
            with self.subTest(
                f"Adding field from another document to document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [
    ...
    def test_partial_update_should_update_short_string(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Updating short string for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                ...
                doc = tensor_search.get_document_by_id(self.config, self.index.name, id)
                self.assertEqual(
                    "updated_short_string",
                    doc["short_string_field"],
                ...
                # Verify field type
                self._assert_field_types(
                    id, [("short_string_field", MarqoFieldTypes.STRING)]
                )
    ...
    def test_partial_update_should_update_long_string(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Updating long string for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                index = self.config.index_management.get_index(self.index.name)
                res = self.config.document.partial_update_documents(
                    [{"_id": id, "long_string_field": "updated_long_string" * 10}],
                    index,
                )
                self.assertFalse(
                    res.errors, f"Expected no errors when updating document {id}"
                ...
                doc = tensor_search.get_document_by_id(self.config, self.index.name, id)
                self.assertEqual(
                    "updated_long_string" * 10,
                    doc["long_string_field"],
                ...
                # Verify field type
                self._assert_field_types(
                    id, [("long_string_field", MarqoFieldTypes.STRING)]
                )
    ...
    def test_partial_update_should_update_long_string_to_short_string(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Updating long string to short string for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                index = self.config.index_management.get_index(self.index.name)
                res = self.config.document.partial_update_documents(
    ...
    def test_partial_update_should_update_short_string_to_long_string(self):
        ...
        index = self.config.index_management.get_index(self.index.name)
        res = self.config.document.partial_update_documents(
            [{"_id": id, "short_string_field": "verylongstring" * 10}], index
        )
    ...
    def test_partial_update_should_update_score_modifiers_and_version_uuid(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Updating score modifiers for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                ...
                self.assertIsNotNone(
                    doc.get("marqo__version_uuid")
                )  # version_uuid should be present.
                ...
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"]["int_field"],
                    float(int_field_val),
                )
                float_field_val = self.id_to_doc[id].get("float_field", 0)
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"]["float_field"],
                    float(float_field_val),
                )
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"]["int_map.a"], 2.0
                )
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"]["float_map.c"], 3.0
                )

                # assert that the map keys that must've been removed don't exist in marqo__score_modifiers
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"].get("int_map.b", None), None
                )
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"].get("float_map.d", None),
                    None,
                )
                self.assertEqual(doc["marqo__score_modifiers"]["cells"]["new_int"], 1.0)
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"]["new_float"], 2.0
                )
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"]["new_map.a"], 1.0
                )
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"]["new_map.b"], 2.0
                )
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"]["int_map.d"], 5.0
                )
                # Assert that after processing an update request that contains map fields, version uuid changes
                self.assertNotEqual(
                    doc.get("marqo__version_uuid"), version_uuid.get(id)
                )  # version_uuid should change
    ...
    def test_partial_update_should_add_score_modifiers(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Adding score modifiers for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                # Doing a get to set the version_uuid in the version_uuid hashmap, which we'll check later to make sure it has changed after
                # processing an update request that contains maps
                ...
                original_doc = tensor_search.get_document_by_id(
                    self.config, self.index.name, id
                )
                ...
                res = self.config.vespa_client.get_document(
                    id,
                    self.config.index_management.get_index(self.index.name).schema_name,
                )
                ...
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"]["int_map_2.d"], 5.0
                )
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"]["int_map_2.e"], 6.0
                )
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"]["float_map_2.f"], 4.0
                )
                self.assertEqual(doc["marqo__score_modifiers"]["cells"]["new_int"], 1.0)
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"]["new_float"], 2.0
                ...
                if int_map_a_val:
                    self.assertEqual(
                        doc["marqo__score_modifiers"]["cells"]["int_map.a"],
                        float(int_map_a_val),
                    )
                float_map_c_val = self.id_to_doc[id].get("float_map", {}).get("c", 0)
                if float_map_c_val:
                    self.assertEqual(
                        doc["marqo__score_modifiers"]["cells"]["float_map.c"],
                        float(float_map_c_val),
                    )
    ...
    def test_partial_update_only_update_existing_score_modifiers(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Updating existing score modifiers for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                # Doing a get to set the version_uuid in the version_uuid hashmap, which we'll check later to make sure it has changed after
                # processing an update request that contains maps
                ...
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"]["int_map.a"], 3.0
                )
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"]["int_map.b"], 4.0
                )
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"]["float_map.c"], 3.0
                )
                self.assertEqual(
                    doc["marqo__score_modifiers"]["cells"]["float_map.d"], 4.0
                )
        ...
        for doc in test_docs:
            with self.subTest(f"Adding new fields to document with ID {doc['_id']}"):
                ...
                self.assertEqual(
                    500.0,
                    doc["new_float"],
                    f"Expected new_float to be 500.0 for document {id}",
                )
                self.assertEqual(
                    2,
                    doc["new_int_map.a"],
                    f"Expected new_int_map.a to be 2 for document {id}",
                )
                self.assertEqual(
                    500,
                    doc["new_field"],
                    f"Expected new_field to be 500 for document {id}",
                )
                self.assertEqual(
                    True,
                    doc["new_bool_field"],
                    f"Expected new_bool_field to be True for document {id}",
                )
                self.assertEqual(
                    10.0,
                    doc["new_float_field"],
                    f"Expected new_float_field to be 10.0 for document {id}",
                )
    ...
    def test_partial_update_should_reject_tensor_field(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Attempting to update tensor field for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [{"_id": id, "tensor_field": "new_title"}], self.index
                )
                self.assertTrue(res.errors)
                self.assertIn(
                    "reference/api/documents/update-documents/#response",
    ...
    def test_partial_update_should_reject_multi_modal_field_subfield(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Attempting to update tensor subfield for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [{"_id": id, "tensor_subfield": "new_description"}], self.index
                )
                self.assertTrue(res.errors)
                self.assertIn(
                    "reference/api/documents/update-documents/#response",
    ...
    def test_partial_update_should_reject_custom_vector_field(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Attempting to update custom vector field for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [
                ...
                self.assertEqual(400, res.items[0].status)
                self.assertIn(
                    f"Unsupported field type <class 'str'> for field custom_vector_field in doc {id}. "
    ...
    def test_partial_update_should_reject_multimodal_combo_field(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Attempting to update multimodal combo field for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [
                ...
                self.assertTrue(res.errors)
                self.assertIn(
                    f"Unsupported field type <class 'str'> for field multimodal_combo_field in doc {id}",
                    res.items[0].error,
                )
                self.assertEqual(400, res.items[0].status)
    ...
    def test_partial_update_should_reject_numeric_array_field_type(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Attempting to update numeric array field for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [{"_id": id, "int_array": [1, 2, 3]}], self.index
                )
                self.assertTrue(res.errors)
                self.assertIn(
                    "Unstructured index updates only support updating existing string array fields",
    ...
    def test_partial_update_should_reject_new_lexical_field(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Attempting to update new lexical field for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [
                        {
                            "_id": id,
                            "new_lexical_field": "some string that signifies new lexical field",
                        }
                    ],
                    self.index,
                )
    ...
    def test_partial_update_should_handle_mixed_numeric_map_updates(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Updating mixed numeric maps for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [
    ...
    def test_partial_update_should_reject_invalid_map_values(self):
        ...
        for doc in test_docs:
            with self.subTest(
                f"Attempting to update invalid map values for document with ID {doc['_id']}"
            ):
                id = doc["_id"]
                res = self.config.document.partial_update_documents(
                    [
                ...
                self.assertTrue(res.errors)
                self.assertIn(
                    f"Unsupported field type <class 'str'> for field int_map in doc {id}",
                    res.items[0].error,
                )
                self.assertEqual(400, res.items[0].status)

                # Verify original values unchanged
                get_docs = tensor_search.get_document_by_id(
                    self.config, self.index.name, id
                )
                if doc.get(
                    "int_map.a", None
                ):  # Only assert that the original values are unchanged if they exist in the original document
    ...
    def test_partial_update_should_handle_multiple_docs(self):
        ...
        updates = [
            {"_id": "2", "int_field": 1000, "float_map": {"c": 99.9}},
            {"_id": "3", "bool_field": False, "int_map": {"a": 777}},
        ]
    ...
    def test_partial_update_should_handle_duplicate_doc_ids(self):
        ...
        updates = [{"_id": "2", "int_field": 100}, {"_id": "2", "int_field": 200}]
        res = self.config.document.partial_update_documents(updates, self.index)
        self.assertFalse(res.errors)
    ...
    def test_partial_update_should_handle_non_existent_doc_id(self):
        ...
        """
        res = self.config.document.partial_update_documents(
            [{"_id": "non_existent", "int_field": 100}], self.index
        )
        self.assertTrue(res.errors)
        self.assertIn(
            "reference/api/documents/update-documents/#response", res.items[0].error
        )
        self.assertIn(
            "Marqo vector store couldn't update the document. Please see",
            res.items[0].error,
    ...
    def test_partial_update_should_handle_none_id(self):
        ...
        """
        res = self.config.document.partial_update_documents(
            [{"_id": None, "int_field": 100}], self.index
        )
        self.assertTrue(res.errors)
        self.assertIn(
            "document _id must be a string type! received _id none of type `nonetype`",
    ...
    def test_partial_update_should_handle_nested_maps(self):
        ...
        """
        res = self.config.document.partial_update_documents(
            [{"_id": "2", "int_map": {"nested": {"too": "deep"}}}], self.index
        )
        self.assertTrue(res.errors)
        self.assertEqual(400, res.items[0].status)
    ...
    def test_partial_update_should_handle_random_dict_field(self):
        ...
        """
        res = self.config.document.partial_update_documents(
            [{"_id": "2", "random_field": {"content1": "abcd", "content2": "efgh"}}],
            self.index,
        )
        self.assertTrue(res.errors)
    ...
    def test_partial_update_should_handle_random_field_type(self):
        ...
        """
        res = self.config.document.partial_update_documents(
            [{"_id": "2", "random_field": None}], self.index
        )
        self.assertTrue(res.errors)
        self.assertIn("Unsupported field type", res.items[0].error)
    ...
    def test_partial_update_should_handle_empty_dict_field(self):
        ...
        """
        res = self.config.document.partial_update_documents(
            [{"_id": "2", "float_map": {}}], self.index
        )
        self.assertFalse(res.errors)
        updated_doc = tensor_search.get_document_by_id(
            self.config, self.index.name, "2"
        )
        self.assertIsNone(updated_doc.get("float_map.c", None))
        self.assertIsNone(updated_doc.get("float_map.d", None))
        self._assert_field_types(
            "2", [("float_map", None), ("float_map.c", None), ("float_map.d", None)]
        )
    ...
    def test_partial_update_should_reject_updating_dict_to_int_field(self):
        ...
        """
        res = self.config.document.partial_update_documents(
            [{"_id": "2", "float_map": 100}], self.index
        )
        self.assertTrue(res.errors)
        self.assertIn(
            "Marqo vector store couldn't update the document. Please see",
    ...
    def test_updating_int_to_int_map(self):
        ...
        res = self.config.document.partial_update_documents(
            [{"_id": "2", "int_field": {"a": 100}}], self.index
        )
        print(res)
        raw_vespa_doc = self.config.vespa_client.get_document(
            "2", self.index.schema_name
        )
        print(raw_vespa_doc)
        self.assertTrue(res.errors)
        self.assertIn(
    ...
    def test_updating_non_existent_document_with_maps(self):
        ...
        """
        res = self.config.document.partial_update_documents(
            [{"_id": "4", "metadata": {"key1": 2}}], self.index
        )
        self.assertTrue(res.errors)
        self.assertEqual(400, res.items[0].status)
        self.assertIn(
    ...
    def test_partial_update_adding_all_field_types_to_minimal_document(self):
        ...
        minimal_doc = {"_id": "minimal_doc"}

        # Add the minimal document to the index
        self.add_documents(
            self.config,
            add_docs_params=AddDocsParams(
                index_name=self.index.name, docs=[minimal_doc], tensor_fields=[]
            ),
        )

        # Verify the document exists
        doc_before_update = tensor_search.get_document_by_id(
            self.config, self.index.name, "minimal_doc"
        )
        ...
        self.assertEqual(
            "This is a very long string value " * 10, updated_doc["long_string_field"]
        )
