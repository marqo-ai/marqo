...
class TestTypeaheadIntegration(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.old_version_index_request = cls.unstructured_marqo_index_request(
            name="old_version_index", marqo_version="2.22.0"
        )

        cls.indexes = cls.create_indexes(
            [cls.test_index, cls.old_version_index_request]
        )
    ...
    def _create_test_queries(self):
        ...
        return [
            TypeaheadAddQueryRequest(
                query="apple iphone 14", popularity=2.0, metadata={"category": 0.8}
            ),
            TypeaheadAddQueryRequest(
                query="apple macbook pro", popularity=1.5, metadata={"category": 0.9}
            ),
            TypeaheadAddQueryRequest(
                query="apple watch series", popularity=1.0, metadata={"category": 0.7}
            ),
            TypeaheadAddQueryRequest(
                query="samsung galaxy s23", popularity=1.8, metadata={"category": 0.8}
            ),
            TypeaheadAddQueryRequest(
                query="samsung tablet", popularity=0.5, metadata={"category": 0.6}
            ),
            TypeaheadAddQueryRequest(
                query="laptop computer", popularity=1.2, metadata={"category": 0.7}
            ),
        ]
    ...
    def test_index_single_query(self):
        ...
        query = TypeaheadAddQueryRequest(
            query="test query", popularity=1.0, metadata={"test": 0.5}
        )
        ...
        get_result = self.config.typeahead.get_queries(
            self.test_index_name, ["test query"]
        )
        ...
        self.assertEqual(
            "t te tes test q qu que quer query", get_result.queries[0].query_index
        )
    ...
    def test_index_duplicate_queries(self):
        ...
        queries = [
            ...
            TypeaheadAddQueryRequest(
                query="apple iphone", popularity=2.0
            ),  # Should be detected as duplicate
            TypeaheadAddQueryRequest(
                query="  apple   iphone  ", popularity=3.0
            ),  # this is not a duplicate. should it?
        ]
        request = TypeaheadIndexingRequest(queries=queries)
    ...
    def test_get_suggestions_strict_prefix_matching(self):
        ...
        response = self.config.typeahead.get_suggestions(self.test_index_name, request)
        apple_suggestions = [
            s for s in response.suggestions if "apple" in s.suggestion.lower()
        ]
    ...
    def test_get_suggestions_fuzzy_matching_disabled(self):
        ...
        response_no_fuzzy = self.config.typeahead.get_suggestions(
            self.test_index_name, request_no_fuzzy
        )
    ...
    def test_get_suggestions_with_weights(self):
        ...
        request_high_popularity = TypeaheadRequest(
            q="apple macbook", popularity_weight=10.0, bm25_weight=0.1
        )
        response_high_popularity = self.config.typeahead.get_suggestions(
            self.test_index_name, request_high_popularity
        )
        self.assertEqual(
            "apple iphone 14", response_high_popularity.suggestions[0].suggestion
        )

        # Test with high BM25 weight - should favor BM25 scoring over popularity
        request_high_bm25 = TypeaheadRequest(
            q="apple macbook", popularity_weight=0.1, bm25_weight=10.0
        )
        response_high_bm25 = self.config.typeahead.get_suggestions(
            self.test_index_name, request_high_bm25
        )
        self.assertEqual(
            "apple macbook pro", response_high_bm25.suggestions[0].suggestion
        )
    ...
    def test_get_suggestions_limit(self):
        ...
        for limit in [1, 3]:
            with self.subTest(limit=limit):
                ...
                response = self.config.typeahead.get_suggestions(
                    self.test_index_name, request
                )
        ...
        for limit, bm25_weight, popularity_weight in test_cases:
            with self.subTest(
                ...
                self.assertListEqual(
                    [q.query for q in sorted_queries[0:limit]],
    ...
    def test_get_queries_by_strings(self):
        ...
        response = self.config.typeahead.get_queries(
            self.test_index_name, query_strings
        )
        ...
        expected_queries = {
            q.query: q for q in test_queries if q.query in query_strings
        }
    ...
    def test_get_queries_not_found(self):
        ...
        response = self.config.typeahead.get_queries(
            self.test_index_name, ["nonexistent query"]
        )
        self.assertEqual(len(response.queries), 0)

        # Mix of existing and non-existing
        response = self.config.typeahead.get_queries(
            self.test_index_name, ["apple iphone 14", "nonexistent query"]
        )
    ...
    def test_delete_specific_queries(self):
        ...
        # Verify they're gone
        response = self.config.typeahead.get_queries(
            self.test_index_name, queries_to_delete
        )
        self.assertEqual(len(response.queries), 0)
        ...
        self.assertEqual(
            final_stats.indexed_queries,
            initial_stats.indexed_queries - len(queries_to_delete),
        )
    ...
    def test_typeahead_version_check_with_old_version_index(self):
        ...
        self._assert_version_error_message(
            context.exception, self.index_220_name, "2.22.0"
        )
    ...
    def _assert_version_error_message(
        self, exception: UnsupportedFeatureError, index_name: str, version: str
    ):
