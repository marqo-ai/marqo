...
class TestDownloadModelFromHF(unittest.TestCase):
    def setUp(self):
        self.hf_location = HfModelLocation(
    ...
    def test_download_model_from_hf_success(self):
        with patch(
            "marqo.inference.model_download.download_model_from_hf.hf_hub_download",
            return_value="model_path",
        ) as hf_hub_download_mock:
    ...
    def test_download_model_from_hf_no_auth(self):
        ...
        self.assertEqual(result, "model_path")
        hf_hub_download_mock.assert_called_once_with(
            repo_id="test-repo-id", filename="test-filename", cache_dir=None
        )
    ...
    def test_download_model_from_hf_with_download_dir(self):
        with patch(
            ...
            with patch(
                "marqo.inference.model_download.download_model_from_hf.logger.warning"
            ) as logger_warning_mock:
                result = download_model_from_hf(
                    self.hf_location, self.hf_auth, download_dir="custom_download_dir"
                )
        self.assertEqual(result, "model_path")
        hf_hub_download_mock.assert_called_once_with(
            repo_id="test-repo-id",
    ...
    def test_download_model_from_hf_no_auth_with_hf_dir(self):
        with patch(
            ...
            result = download_model_from_hf(
                self.hf_location, download_dir=ModelCache.hf_cache_path
            )
        self.assertEqual(result, "model_path")
        hf_hub_download_mock.assert_called_once_with(
            repo_id="test-repo-id",
    ...
    def test_download_model_from_hf_no_auth_with_openclip_dir(self):
        with patch(
            ...
            result = download_model_from_hf(
                self.hf_location, download_dir=ModelCache.clip_cache_path
            )
        self.assertEqual(result, "model_path")
        hf_hub_download_mock.assert_called_once_with(
            repo_id="test-repo-id",
