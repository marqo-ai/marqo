...
class TestHuggingFaceModel(unittest.TestCase):
    ...
    def test_initialize_huggingface_model_with_invalid_properties(self):
        test_cases = [
            ...
            ({"type": "hf", "pooling_method": "cls"}, "Name is not provided"),
            ({"name": "test-model", "pooling_method": "cls"}, "Type is not provided"),
            (
                {
    ...
    def test_hf_e5_base_v2_embeddings_load_from_hf(self):
        ...
        difference = np.linalg.norm(
            embeddings - self.E5_BASE_V2_MODEL_EMBEDDINGS
        ) / len(embeddings)
        self.assertLess(
            difference,
            1e-4,
            f"Text embeddings for model {model_properties} "
    ...
    @mock.patch(
        ...
        side_effect=OSError("Tokenizer load failed"),
    )
    ...
    def test_trust_remote_code_validation_false(self):
        ...
        for trust_remote_code in [None, False]:
            for model_trust_remote_code, tokenizer_trust_remote_code in [
                (True, False),
                (False, True),
                (True, True),
            ]:
    ...
    def test_sentence_transformers_nli_bert_base_cls_pooling_embeddings(self):
        ...
        difference = np.linalg.norm(
            embeddings - self.NLI_BERT_BASE_CLS_MODEL_EMBEDDINGS
        ) / len(embeddings)
        self.assertLess(
            difference,
            1e-4,
        ...
        difference_for_different_models = np.linalg.norm(
            embeddings - self.E5_BASE_V2_MODEL_EMBEDDINGS
        ) / len(embeddings)

        self.assertGreater(
            difference_for_different_models,
            1,
            "Text embeddings for two different models are too close. "
            "There is a problem with the test data or bug in the code.",
        )
    ...
    def test_instantiate_a_hugging_face_model_with_minimum_model_properties(
    ...
    def test_instantiate_a_hugging_face_model_with_custom_model_properties(
        self, mock_auto_model, mock_auto_tokenizer
    ):
