...
from marqo.inference.native_inference.embedding_models.hugging_face_model_properties import (
...
class TestHuggingFaceModelProperties(unittest.TestCase):
        ...
        for model_properties, msg in test_cases:
            with self.subTest(msg):
                ...
                self.assertEqual(
                    ...
                    ModelLocation(
                        hf=HfModelLocation(
                            repo_id="test-repo-id", filename="test-filename"
                        )
                    ),
                )
                self.assertIsNone(model.note)
    ...
    def test_invalid_type(self):
        with self.assertRaises(ValidationError) as excinfo:
            HuggingFaceModelProperties(
                name="test-model", type="invalid_type", dimensions=768
            )
        self.assertIn("The type of the model should be 'hf'", str(excinfo.exception))

    def test_valid_model_with_url(self):
        model = HuggingFaceModelProperties(
            name="test-model", type="hf", url="http://example.com", dimensions=768
        )
    ...
    def test_valid_model_with_model_location(self):
        model_location = ModelLocation(
            hf=HfModelLocation(repo_id="test-repo-id", filename="test-filename")
        )
        model = HuggingFaceModelProperties(
            name="test-model", type="hf", model_location=model_location, dimensions=768
        )
    ...
    def test_infer_pooling_method(self):
        for pooling_method in (PoolingMethod.Mean, PoolingMethod.CLS):
            with self.subTest(
                f"Pooling method inferred from name with {pooling_method}"
            ):
                with mock.patch(
                    "marqo.inference.native_inference.embedding_models.hugging_face_model_properties."
                    "HuggingFaceModelProperties._infer_pooling_method_from_name",
                    return_value=pooling_method,
                ) as mock_infer:
                    model = HuggingFaceModelProperties(
                        name="model-with-cls", type="hf", dimensions=768
                    )
    ...
    def test_explicit_invalid_pooling_method(self):
        with self.assertRaises(ValidationError) as excinfo:
            _ = HuggingFaceModelProperties(
                name="test-model", type="hf", pooling_method="invalid", dimensions=768
            )
        self.assertIn(
            "value is not a valid enumeration member; permitted: 'mean', 'cls'",
            str(excinfo.exception),
        )
    ...
    def test_valid_model_with_custom_url_and_inferred_pooling(self):
        model = HuggingFaceModelProperties(
            url="http://example.com", type="hf", pooling_method=None, dimensions=768
        )
        self.assertEqual(model.pooling_method, PoolingMethod.Mean)
    ...
    def test_some_pooling_method_infer_on_real_model(self):
        ...
        for model_name, pooling_method in test_cases:
            with self.subTest(f"Pooling method inferred from name with {model_name}"):
                model = HuggingFaceModelProperties(
                    name=model_name, type="hf", dimensions=768
                )
                self.assertEqual(pooling_method, model.pooling_method)
