...
class TestDownloadModel(unittest.TestCase):
    def test_both_location_and_url_provided(self):
        with self.assertRaises(InvalidModelPropertiesError):
            download_model(
                repo_location=ModelLocation(
                    s3=S3Location(Bucket="test_bucket", Key="test_key")
                ),
    ...
    @patch("marqo.inference.model_download.model_download.download_pretrained_from_s3")
    def test_download_from_s3(self, mock_download_s3):
        ...
        mock_download_s3.assert_called_once_with(
            location=repo_location.s3, auth=auth.s3, download_dir=None
        )

    @patch("marqo.inference.model_download.model_download.download_pretrained_from_url")
...
class TestDownloadPretrainedFromS3(unittest.TestCase):
    def setUp(self):
        self.s3_location = S3Location(
            Bucket="test_bucket", Key="remote_path/test_key.pt"
        )
        self.s3_auth = S3Auth(
    ...
    def test_model_exists_locally(self, mock_check_s3_model):
        ...
        with patch(
            "marqo.inference.model_download.model_download.get_s3_model_absolute_cache_path"
        ) as mock_get_abs_path:
            with patch(
                "marqo.inference.model_download.model_download.download_pretrained_from_url"
            ) as mock_download_pretrained_from_url:
                mock_get_abs_path.return_value = "/path/to/model.pt"
                result = download_pretrained_from_s3(
                    location=self.s3_location, auth=self.s3_auth
                )

        self.assertEqual(result, "/path/to/model.pt")
        mock_download_pretrained_from_url.assert_not_called()
        mock_check_s3_model.assert_called_once_with(
            location=self.s3_location, download_dir=None
        )

    @patch(
        "marqo.inference.model_download.model_download.check_s3_model_already_exists"
    )
    @patch("marqo.inference.model_download.model_download.get_presigned_s3_url")
    def test_model_does_not_exist_locally(
        self, mock_get_presigned_url, mock_check_s3_model
    ):
        ...
        # note the cache file name is to come from the key rather than the URL
    ...
    @patch("marqo.inference.model_download.model_download.get_presigned_s3_url")
    def test_model_download_raises_403_error(
        self, mock_get_presigned_url, mock_check_s3_model
    ):
        mock_check_s3_model.return_value = False
        mock_get_presigned_url.return_value = "http://example.com/model.pt"
        ...
        with patch(
            ...
            mock_download_url.side_effect = urllib.error.HTTPError(
                url=None, code=403, msg=None, hdrs=None, fp=None
            )
...
class TestDownloadPretrainedFromURL(unittest.TestCase):
    ...
    def test_file_exists_locally(self, mock_isfile, mock_urlopen):
        ...
        with patch("builtins.open", unittest.mock.mock_open()) as mock_open:
            with patch(
                "marqo.inference.model_download.model_download.tqdm"
            ) as mock_tqdm:
                with patch(
                    "marqo.inference.model_download.model_download.ModelCache"
                ) as mock_cache:
    ...
    def test_file_does_not_exist_locally(self, mock_urlopen, mock_isfile):
        ...
        mock_open.assert_called_once_with(
            os.path.join(temp_cache_dir, "model.pt"), "wb"
        )

    @patch("os.path.isfile")
    @patch("urllib.request.urlopen")
    def test_file_does_not_exist_locally_custom_filename(
        ...
        with patch("builtins.open", unittest.mock.mock_open()) as mock_open:
            with patch(
                ...
                with patch(
                    ...
                    with tempfile.TemporaryDirectory() as temp_cache_dir:
                        mock_cache.clip_cache_path = temp_cache_dir
                        result = download_pretrained_from_url(
                            self.url, cache_file_name="unusual_model.pt"
                        )

        self.assertEqual(result, os.path.join(temp_cache_dir, "unusual_model.pt"))
        mock_isfile.assert_called_once()
        mock_urlopen.assert_called_once_with(self.url)
        mock_open.assert_called_once_with(
            os.path.join(temp_cache_dir, "unusual_model.pt"), "wb"
        )

    @patch("os.path.isfile")
    @patch("urllib.request.urlopen")
    def test_file_does_not_exist_locally_custom_cache_dir(
        self, mock_urlopen, mock_isfile
    ):
        mock_isfile.return_value = False
        mock_source = MagicMock()
            ...
            with patch(
                ...
                with patch(
                    ...
                    with tempfile.TemporaryDirectory() as temp_cache_dir:
                        ...
                        mock_cache.clip_cache_path = temp_cache_dir
                        result = download_pretrained_from_url(
                            self.url, cache_dir=custom_dir
                        )

        self.assertEqual(result, os.path.join(custom_dir, "model.pt"))
        mock_isfile.assert_called_once()
        mock_urlopen.assert_called_once_with(self.url)
        mock_open.assert_called_once_with(os.path.join(custom_dir, "model.pt"), "wb")

    @patch("os.path.isfile")
    @patch("urllib.request.urlopen")
    def test_file_does_not_exist_locally_custom_cache_path(
        self, mock_urlopen, mock_isfile
    ):
        mock_isfile.return_value = False
        mock_source = MagicMock()
                ...
                with patch(
                    ...
                    with tempfile.TemporaryDirectory() as temp_cache_dir:
                        ...
                        result = download_pretrained_from_url(
                            ...
                            cache_file_name="unusual_model.pt",
                        )

        self.assertEqual(result, os.path.join(custom_dir, "unusual_model.pt"))
        mock_isfile.assert_called_once()
        mock_urlopen.assert_called_once_with(self.url)
        mock_open.assert_called_once_with(
            os.path.join(custom_dir, "unusual_model.pt"), "wb"
        )
