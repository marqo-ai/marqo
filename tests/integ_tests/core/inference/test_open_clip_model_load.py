...
from marqo.inference.native_inference.embedding_models.open_clip_model import (
    OpenCLIPModel,
)
...
class TestOpenCLIPModelLoad(TestCase):
    ...
    def test_load_OpenCLIPModelFromCheckPointMethod_success(self):
        ...
        with patch(
            "marqo.inference.native_inference.embedding_models.open_clip_model.OpenCLIPModel._load_model_and_image_preprocessor_from_checkpoint",
            return_value=(MagicMock(), MagicMock()),
        ) as mock_load_method:
            with patch(
                "marqo.inference.native_inference.embedding_models.open_clip_model.OpenCLIPModel._load_tokenizer_from_checkpoint",
                return_value=MagicMock(),
            ) as mock_load_tokenizer:
                with patch.object(MagicMock(), "eval", return_value=None) as mock_eval:
                    model = OpenCLIPModel(
                        model_properties=model_properties, device="cpu"
                    )
                    model.load()
    ...
    def test_load_OpenCLIPModelFromCheckPointParameters_success(self):
        ...
        with patch(
            "marqo.inference.native_inference.embedding_models.open_clip_model.open_clip.create_model",
            return_value=MagicMock(),
        ) as mock_create_model:
            with patch(
                "marqo.inference.native_inference.embedding_models.open_clip_model.open_clip.get_tokenizer",
                return_value=MagicMock(),
            ) as mock_tokenizer:
                with patch(
                    "marqo.inference.native_inference.embedding_models.open_clip_model.download_model",
                    return_value="my_test_model.pt",
                ):
                    with patch.object(
                        MagicMock(), "eval", return_value=None
                    ) as mock_eval:
                        model = OpenCLIPModel(
                            model_properties=model_properties, device="cpu"
                        )
                        ...
                        self.assertEqual(
                            (0.48145466, 0.4578275, 0.40821073), preprocess_config.mean
                        )
                        self.assertEqual(
                            (0.26862954, 0.26130258, 0.27577711), preprocess_config.std
                        )
    ...
    def test_open_clip_load_fromHuggingFaceHub_success(self):
        ...
        with patch(
            "marqo.s2_inference.clip_utils.open_clip.create_model_and_transforms",
            return_value=(MagicMock(), MagicMock(), MagicMock()),
        ) as mock_create_model:
            with patch(
                "marqo.s2_inference.clip_utils.open_clip.get_tokenizer",
                return_value=MagicMock(),
            ) as mock_tokenizer:
                with patch.object(MagicMock(), "eval", return_value=None) as mock_eval:
                    model = OpenCLIPModel(
    ...
    def test_load_OpenCLIPModel_unsupported_image_preprocessor(self):
        ...
        self.assertIn(
            "permitted: 'SigLIP', 'OpenAI', 'OpenCLIP', 'CLIPA'", str(context.exception)
        )
    ...
    def test_load_OpenCLIPModel_from_local_path(self):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch.object(MagicMock(), "eval", return_value=None) as mock_eval:
                    with patch(
                        "marqo.inference.native_inference.embedding_models.open_clip_model.os.path.exists",
                        return_value=True,
                    ) as mock_path_exists:
                        model = OpenCLIPModel(
                            model_properties=model_properties, device="cpu"
    ...
    def test_load_OpenCLIPModel_with_auth_hf(self):
        ...
        model_properties = {
            "name": "ViT-B-16",
            "model_location": {
                "hf": {"repo_id": "my-hf-repo", "filename": "my-hf-filename.pt"},
                "authRequired": True,
            },
            "type": "open_clip",
            "dimensions": 768,
