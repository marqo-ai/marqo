...
class TestMonitoring(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        structured_index_request = cls.structured_marqo_index_request(
            ...
            text_preprocessing=TextPreProcessing(
                split_length=20, split_overlap=1, split_method=TextSplitMethod.Word
            ),
        )
        ...
        structured_index_request_multimodal = cls.structured_marqo_index_request(
            fields=[
                ...
                FieldRequest(name="img", type=FieldType.ImagePointer),
                FieldRequest(
                    name="price",
                    type=FieldType.Float,
                    features=[FieldFeature.ScoreModifier],
        ...
        for marqo_index in self.indexes_to_test:
            with self.subTest(f"{marqo_index.name} - {marqo_index.type.value}"):
                self.assertIndexStatsEqual(
                    MarqoIndexStats(
                        number_of_documents=0, number_of_vectors=0, backend=VespaStats()
                    ),
    ...
    def test_get_index_stats_docsWithTensorFields_successful(self):
        ...
        for marqo_index in self.indexes_to_test:
            with self.subTest(f"{marqo_index.name} - {marqo_index.type.value}"):
                ...
                self.assertIndexStatsEqual(
                    MarqoIndexStats(
                        number_of_documents=3, number_of_vectors=3, backend=VespaStats()
                    ),
                    self.monitoring.get_index_stats(marqo_index),
                )
    ...
    def test_get_index_stats_structuredMultimodalIndex_successful(self):
        ...
        self.add_documents(
            ...
            add_docs_params=AddDocsParams(
                docs=[
                    {"title": "2", "img": TestImageUrls.HIPPO_REALISTIC.value},
                    {"title": "2"},
                    {"desc": "2"},
                ],
                index_name=marqo_index.name,
        ...
        self.assertIndexStatsEqual(
            MarqoIndexStats(
                number_of_documents=3, number_of_vectors=4, backend=VespaStats()
            ),
            self.monitoring.get_index_stats(marqo_index),
        )
    ...
    def test_get_index_stats_docsWithoutTensorFields_successful(self):
        ...
        for marqo_index in self.indexes_to_test:
            with self.subTest(f"{marqo_index.name} - {marqo_index.type.value}"):
                ...
                self.assertIndexStatsEqual(
                    MarqoIndexStats(
                        number_of_documents=3, number_of_vectors=0, backend=VespaStats()
                    ),
                    self.monitoring.get_index_stats(marqo_index),
                )
    ...
    def test_get_index_stats_mixedDocs_successful(self):
        ...
        for marqo_index in self.indexes_to_test:
            with self.subTest(f"{marqo_index.name} - {marqo_index.type.value}"):
                ...
                self.assertIndexStatsEqual(
                    MarqoIndexStats(
                        number_of_documents=3, number_of_vectors=2, backend=VespaStats()
                    ),
                    self.monitoring.get_index_stats(marqo_index),
                )
    ...
    def test_get_index_stats_sequentialIndexingAndDeletion_successful(self):
        ...
        operations = [
            ...
            (
                ...
                MarqoIndexStats(
                    number_of_documents=6, number_of_vectors=4, backend=VespaStats()
                ),
            ),
            (
                ...
                ["1", "4"],
                MarqoIndexStats(
                    number_of_documents=4, number_of_vectors=3, backend=VespaStats()
                ),
            ),
            (
                "add",  # add 3 docs, 1 has a vector
                ...
                MarqoIndexStats(
                    number_of_documents=7, number_of_vectors=4, backend=VespaStats()
                ),
            ),
            (
        ...
        for marqo_index in self.indexes_to_test:
            with self.subTest(f"{marqo_index.name} - {marqo_index.type.value}"):
                for operation, docs, expected_stats in operations:
                    ...
                    self.assertIndexStatsEqual(
                        expected_stats, self.monitoring.get_index_stats(marqo_index)
                    )
        ...
        for marqo_index in self.indexes_to_test:
            with self.subTest(f"{marqo_index.name} - {marqo_index.type.value}"):
                self.add_documents(
                    ...
                    add_docs_params=AddDocsParams(
                        docs=[
                            {"title": "test " * number_of_words},
                            {"title": "2"},
                        ],  # 3 + 1 vectors expected
                        index_name=marqo_index.name,
                        device="cpu",
                        tensor_fields=["title"]
                        if isinstance(marqo_index, UnstructuredMarqoIndex)
                ...
                self.assertIndexStatsEqual(
                    MarqoIndexStats(
                        number_of_documents=2, number_of_vectors=4, backend=VespaStats()
                    ),
                    self.monitoring.get_index_stats(marqo_index),
                )
    ...
    @patch(
        "marqo.core.monitoring.monitoring.torch.cuda.is_available", return_value=True
    )
    @patch("marqo.core.monitoring.monitoring.torch.cuda.device_count", return_value=1)
    @patch(
        "marqo.core.monitoring.monitoring.torch.cuda.get_device_name",
        return_value="Tesla T4",
    )
    @patch(
        "marqo.core.monitoring.monitoring.torch.cuda.memory_allocated",
    ...
    def test_getCudaInfo_success_on_gpu_instance(self, *mocks):
        ...
        with patch(
            "marqo.core.monitoring.monitoring.torch.cuda.get_device_properties",
