...
from marqo.core.vespa_index.vespa_schema import (
    for_marqo_index_request as vespa_schema_factory,
)
...
class TestIndexManagement(MarqoTestCase):
    ...
    def test_bootstrap_vespa_should_successfully_bootstrap_a_new_vespa_application_package(
        self,
    ):
        ...
        with open(os.path.join(app, "marqo_config.json")) as f:
            self.assertEqual(
                json.loads(f'{{"version": "{version.get_version()}"}}'), json.load(f)
            )
    ...
    def test_bootstrap_vespa_should_skip_bootstrapping_if_already_bootstrapped_for_older_vespa_version(
        ...
        with mock.patch.object(
            httpx.Client, "post", side_effect=modified_post
        ) as mock_post:
    ...
    @patch("marqo.vespa.vespa_client.VespaClient.check_for_application_convergence")
    def test_bootstrap_vespa_should_skip_bootstrapping_if_already_bootstrapped(
        self, mock_check_convergence
    ):
        ...
        # verify the first boostrap call deploys the app to vespa
        with mock.patch.object(
            httpx.Client, "put", side_effect=modified_put
        ) as mock_post:
            self.assertTrue(self.index_management.bootstrap_vespa())
    ...
    def test_bootstrap_vespa_should_add_typeahead_schemas_for_existing_2_23_0_indexes(
        self,
    ):
        ...
        request = self.unstructured_marqo_index_request(
            name="test_index_230", marqo_version="2.23.0"
        )
        ...
        app._store.save_file(
            schema, "schemas", f"{index_with_230_version.schema_name}.sd"
        )
        ...
        app._store.save_file(
            app._marqo_config_store.get().json(), app._MARQO_CONFIG_FILE
        )
        app._deploy()
        ...
        self.assertEqual(
            "marqo__test_00index_00230_typeahead", saved_index.typeahead_schema_name
        )

        # Check that the typeahead schema file exists
        self._assert_file_exists(
            downloaded_app, "schemas", f"{saved_index.typeahead_schema_name}.sd"
        )
        ...
        typeahead_doc = services_xml.getroot().find(
            f'content/documents/document[@type="{saved_index.typeahead_schema_name}"]'
        )
    ...
    def test_boostrap_vespa_should_migrate_index_settings_from_existing_vespa_app(self):
        ...
        with open(os.path.join(app, "marqo_index_settings.json")) as f:
            ...
            self.assertEqual(
                existing_index_with_version.json(),
                json.dumps(index_settings[existing_index.name]),
            )
    ...
    def test_rollback_should_succeed(self):
        ...
        components_jar_folder = (
            Path(__file__).parent / "existing_vespa_app" / "components"
        )
        with mock.patch.object(
            VespaApplicationPackage, "_COMPONENTS_JAR_FOLDER", components_jar_folder
        ):
        ...
        backup_dir = tempfile.mkdtemp()
        with tarfile.open(
            os.path.join(rolled_back_version, "app_bak.tgz"), mode="r:gz"
        ) as tar:
            for member in tar.getmembers():
                tar.extract(member, path=backup_dir)
        ...
        for file in expected_backup_files:
            self._assert_files_equal(
                os.path.join(backup_dir, *file), os.path.join(latest_version, *file)
            )
    ...
    def test_rollback_should_fail_when_target_version_is_current_version(self):
        ...
        with self.assertRaisesStrict(ApplicationRollbackError) as e:
            self.index_management.rollback_vespa()
        self.assertIn(
            "The target version must be lower than the current one", str(e.exception)
        )

    def test_rollback_should_fail_when_target_version_does_not_match_backup_version(
        self,
    ):
        ...
        with mock.patch.object(version, "get_version", return_value="2.13.0"):
            ...
            with self.assertRaisesStrict(ApplicationRollbackError) as e:
                self.index_management.rollback_vespa()
            self.assertEqual(
                "Cannot rollback to 2.12.0, current Marqo version is 2.13.0",
                str(e.exception),
            )
    ...
    def test_rollback_should_fail_when_schemas_are_changed(self):
        ...
        with mock.patch.object(version, "get_version", return_value="2.10.0"):
            with self.assertRaisesStrict(ApplicationRollbackError) as e:
                self.index_management.rollback_vespa()
            self.assertEqual(
                "Aborting rollback. Reason: Indexes have been added or removed since last backup.",
                str(e.exception),
            )
    ...
    def test_rollback_should_fail_when_nodes_are_changed(self):
        ...
        container_nodes_element = application._service_xml._ensure_only_one(
            "container/nodes"
        )
        # add <jvm options="-Xms32M -Xmx128M"/> to container/nodes
        ET.SubElement(container_nodes_element, "jvm", {"options": "-Xms32M -Xmx128M"})
        application._store.save_file(
            application._service_xml.to_xml(), application._SERVICES_XML_FILE
        )
        application._deploy()
        ...
        with mock.patch.object(version, "get_version", return_value="2.10.0"):
            with self.assertRaisesStrict(ApplicationRollbackError) as e:
                self.index_management.rollback_vespa()
            self.assertEqual(
                "Aborting rollback. Reason: Vector store config has been changed since the last backup.",
                str(e.exception),
            )
    ...
    def _index_operations(self, index_management: IndexManagement):
        ...
        return [
            (
                "create single index",
                lambda: index_management.create_index(index_request_1),
            ),
            (
                "batch create indexes",
                lambda: index_management.batch_create_indexes(
                    [index_request_1, index_request_2]
                ),
            ),
            (
                "delete single index",
                lambda: index_management.delete_index_by_name(index_request_1.name),
            ),
            (
                "batch delete indexes",
                lambda: index_management.batch_delete_indexes_by_name(
                    [index_request_1.name, index_request_2.name]
                ),
            ),
        ]

    def test_index_operation_methods_should_raise_error_if_index_operation_is_disabled(
        self,
    ):
        index_management_without_zookeeper = IndexManagement(
            self.vespa_client, zookeeper_client=None
        )

        for test_case, index_operation in self._index_operations(
            index_management_without_zookeeper
        ):
            with self.subTest(test_case):
                with self.assertRaisesStrict(InternalError):
                    index_operation()

    def test_index_operation_methods_should_raise_error_if_marqo_is_not_bootstrapped(
        self,
    ):
    ...
    @patch("marqo.vespa.vespa_client.VespaClient.check_for_application_convergence")
    def test_index_operation_methods_should_check_convergence(
        self, mock_check_convergence
    ):
        for test_case, index_operation in self._index_operations(self.index_management):
            with self.subTest(test_case):
    ...
    def test_create_and_delete_index_should_succeed(self):
        ...
        app = self.vespa_client.download_application()
        self._assert_index_is_not_present(
            app, index.name, index.schema_name, index.typeahead_schema_name
        )
    ...
    def test_update_index_should_succeed(self):
        ...
        # update this index
        semi_structured_marqo_index = cast(
            SemiStructuredMarqoIndex, self.index_management.get_index(request.name)
        )
        ...
        app = self.vespa_client.download_application()
        self._assert_index_is_present(
            app, semi_structured_marqo_index, new_schema, expected_version=2
        )
    ...
    def test_update_index_should_fail_under_race_condition(self):
        ...
        self.assertIn(
            "Current version is 2, and cannot be upgraded to target version 2. "
            "Some other request might have changed the index. Please try again.",
            str(err.exception),
        )
    ...
    def test_update_index_should_fail_for_wrong_index_types(self):
        ...
        for request in [
            ...
            self.unstructured_marqo_index_request(
                model=Model(name="hf/e5-small"), marqo_version="2.12.0"
            ),
    ...
    def test_update_index_should_skip_if_nothing_to_update(self):
        ...
        change1 = semi_structured_marqo_index.copy(deep=True)
        change1.tensor_fields.extend(
            [
                self._tensor_field("title"),
                self._tensor_field("description"),
                self._tensor_field("tags"),
            ]
        )
        change1.lexical_fields.extend(
            [
                self._lexical_field("field1"),
                self._lexical_field("field2"),
                self._lexical_field("field3"),
            ]
        )
        ...
        change2.tensor_fields.extend(
            [self._tensor_field("description"), self._tensor_field("title")]
        )
        change2.lexical_fields.extend(
            [self._lexical_field("field2"), self._lexical_field("field1")]
        )
        ...
        @mock.patch(
            "marqo.core.index_management.vespa_application_package.VespaApplicationPackage.update_index_setting_and_schema",
            mock_update_index_and_schema,
        )
        def worker2():
            try:
                self.index_management.update_index(change2)
    ...
    def test_batch_create_and_delete_index_should_succeed(self):
        ...
        all_indexes = {
            index.name: index for index in self.index_management.get_all_indexes()
        }
        self.assertEqual(2, len(all_indexes))
        exclude_fields = {"model", "version"}
        for index in [index1, index2]:
            self.assertEqual(
                all_indexes[index.name].dict(exclude=exclude_fields),
                index.dict(exclude=exclude_fields),
            )

        self.index_management.batch_delete_indexes_by_name(
            [request1.name, request2.name]
        )

        app = self.vespa_client.download_application()
        self._assert_index_is_not_present(
            app, index1.name, index1.schema_name, index1.typeahead_schema_name
        )
        self._assert_index_is_not_present(
            app, index2.name, index2.schema_name, index2.typeahead_schema_name
        )
    ...
    @pytest.mark.skip(
        reason="This test case is just used to verify the optimistic locking mechanism works"
    )
    def test_race_condition(self):
        ...
        store1 = ApplicationPackageDeploymentSessionStore(
            content_base_url1, self.vespa_client
        )
        app1 = VespaApplicationPackage(store1)

        content_base_url2, prepare_url2 = self.vespa_client.create_deployment_session()
        store2 = ApplicationPackageDeploymentSessionStore(
            content_base_url2, self.vespa_client
        )
    ...
    def _assert_file_exists(self, *file_paths: str):
        self.assertTrue(
            os.path.exists(os.path.join(*file_paths)),
            f"File {'/'.join(file_paths[1:])} does not exist",
        )

    def _assert_file_does_not_exist(self, *file_paths: str):
        self.assertFalse(
            os.path.exists(os.path.join(*file_paths)),
            f"File {'/'.join(file_paths[1:])} exists",
        )
    ...
    def _assert_index_is_present(
        self,
        app,
        expected_index,
        expected_schema,
        expected_typeahead_schema=None,
        expected_version=1,
    ):
        ...
        exclude_fields = {"model", "version"}
        self.assertEqual(
            saved_index.dict(exclude=exclude_fields),
            expected_index.dict(exclude=exclude_fields),
        )
        ...
        if "text_chunk_prefix" in model_properties:
            self.assertEqual(
                saved_index.model.text_chunk_prefix,
                model_properties["text_chunk_prefix"],
            )
        if "text_query_prefix" in model_properties:
            self.assertEqual(
                saved_index.model.text_query_prefix,
                model_properties["text_query_prefix"],
            )
        ...
        doc = (
            ET.parse(os.path.join(app, "services.xml"))
            .getroot()
            .find(f'content/documents/document[@type="{schema_name}"]')
        )
    ...
    def _assert_index_is_not_present(
        self, app, index_name, schema_name, typeahead_schema_name
    ):
        with self.assertRaisesStrict(IndexNotFoundError):
        ...
        typeahead_schema_doc = (
            ET.parse(os.path.join(app, "services.xml"))
            .getroot()
            .find(f'content/documents/document[@type="{typeahead_schema_name}"]')
        )
    ...
    def _deploy_existing_app_package(self) -> MarqoIndex:
        _, index = vespa_schema_factory(
            self.unstructured_marqo_index_request(
                name="existing_index", marqo_version="2.10.0"
            )
        ).generate_schema()
    ...
    def _tensor_field(self, field_name: str):
        return TensorField(
            name=field_name,
            chunk_field_name=f"marqo__chunks_{field_name}",
