...
class TestSemiStructuredVespaSchema(MarqoTestCase):
    ...
    def test_semi_structured_index_schema_random_model(self):
        test_cases = [
            ...
            (
                ...
                ["text_field"],
                [],
                "semi_structured_vespa_index_schema_one_lexical_field.sd",
                [],
            ),
            (
                "one_tensor_field",
                [],
                ["tensor_field"],
                "semi_structured_vespa_index_schema_one_tensor_field.sd",
                [],
            ),
            (
                "one_string_array_field",
                [],
                [],
                "semi_structured_vespa_index_schema_one_string_array_field.sd",
                ["string_array_1"],
            ),
            (
                "one_lexical_one_tensor_field",
                ["text_field"],
                ["tensor_field"],
            ...
            (
                "one_lexical_one_tensor_field_one_string_array_field",
                ["text_field"],
                ["tensor_field"],
                "semi_structured_vespa_index_schema_one_lexical_one_tensor_one_string_array_field.sd",
                ["string_array_1"],
            ),
            (
                "multiple_lexical_tensor_fields",
                ["text_field1", "text_field2"],
                ["tensor_field1", "tensor_field2"],
                "semi_structured_vespa_index_schema_multiple_lexical_tensor_and_string_array_fields.sd",
        ...
        for test_case in test_cases:
            with self.subTest(msg=test_case[0]):
                ...
                _, index = SemiStructuredVespaSchema(
                    test_marqo_index_request
                ).generate_schema()
                marqo_index = self._populate_fields(
                    index, lexical_fields, tensor_fields, string_array_fields
                )
                generated_schema = SemiStructuredVespaSchema.generate_vespa_schema(
                    marqo_index
                )
    ...
    def test_semi_structured_index_schema_with_collapse_field(self):
        ...
        expected_schema = self._read_schema_from_file(
            "test_schemas/semi_structured_vespa_index_schema_multiple_fields_with_collapse_fields.sd"
        )
    ...
    def test_semi_structured_index_schema_with_pre_2_16(self):
        ...
        for test_case in test_cases:
            with self.subTest(msg=f"mocked_version_{test_case[0]}"):
                ...
                tensor_fields = test_case[2]
                expected_schema = self._read_schema_from_file(
                    f"test_schemas/pre_2_16/{test_case[3]}"
                )
                ...
                marqo_index = self._populate_fields(
                    index, lexical_fields, tensor_fields
                )
                generated_schema = SemiStructuredVespaSchema.generate_vespa_schema(
                    marqo_index
                )
    ...
    def _populate_fields(
        self, index, lexical_fields, tensor_fields, string_array_fields=None
    ):
        ...
        for tensor_field in tensor_fields:
            marqo_index.tensor_fields.append(
                TensorField(
                    name=tensor_field,
        ...
        if string_array_fields:
            for string_array_field in string_array_fields:
                marqo_index.string_array_fields.append(
                    StringArrayField(
