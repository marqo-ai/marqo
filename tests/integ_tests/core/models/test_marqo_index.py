...
from marqo.core.models.marqo_index import Field, FieldFeature, FieldType, MarqoIndex
...
class TestStructuredMarqoIndex(MarqoTestCase):
    def test_filterable_field_names_pre220(self):
        ...
        """
        versions = ["2.0.0", "2.0.1", "2.1.0", "2.1.5"]
        for version in versions:
            with self.subTest(version=version):
                ...
                self.assertEqual(marqo_index.filterable_fields_names, {"price", "tags"})
    ...
    def test_filterable_field_names_post220(self):
        ...
        index Marqo version >=2.2.0.
        """
        versions = ["2.2.0", "2.2.1", "2.3.0", "2.5.5"]
        for version in versions:
            with self.subTest(version=version):
                marqo_index = self.structured_marqo_index(
                    name="my_index",
                ...
                self.assertEqual(
                    marqo_index.filterable_fields_names, {"_id", "price", "tags"}
                )
    ...
    def test_deserialization_with_extra_fields(self):
        ...
        indexes = [
            ...
            self.unstructured_marqo_index(
                name="my_index_2", schema_name="my_index_2", marqo_version="2.12.0"
            ),
        ]
