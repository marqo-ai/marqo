...
from marqo.core.utils.vector_interpolation import Lerp, Nlerp, Slerp
...
class TestRecommender(MarqoTestCase):
    ...
    def setUpClass(cls):
        ...
        structured_text_index = cls.structured_marqo_index_request(
            ...
            fields=[
                ...
                FieldRequest(
                    name="content", type=FieldType.Text, features=[FieldFeature.Filter]
                ),
                FieldRequest(
                    name="tags",
    ...
    def setUp(self) -> None:
        ...
        self.recommender = Recommender(
            self.vespa_client, self.index_management, self.config.inference
        )
        ...
        for index in [self.unstructured_text_index, self.structured_text_index]:
            with self.subTest(type=index.type):
                ...
                with mock.patch.object(
                    Slerp, "interpolate", wraps=interpolate
                ) as mock_interpolate:
    ...
    def test_recommend_nlerp_success(self):
        for index in [self.unstructured_text_index, self.structured_text_index]:
            with self.subTest(type=index.type):
                ...
                with mock.patch.object(
                    Nlerp, "interpolate", wraps=interpolate
                ) as mock_interpolate:
                    res = self.recommender.recommend(
                        index_name=index.name,
                        documents=["1", "2"],
    ...
    def test_recommend_nlerpZeroMagnitudeVector_failure(self):
        ...
        for index in [self.unstructured_text_index, self.structured_text_index]:
            with self.subTest(type=index.type):
                ...
                def interpolate(vectors, weights):
                    return original_nlerp([[1, 0, 2, 0], [-1, 0, -2, 0]], [1, 1])
    ...
    def test_recommend_lerp_success(self):
        for index in [self.unstructured_text_index, self.structured_text_index]:
            with self.subTest(type=index.type):
                ...
                with mock.patch.object(
                    Lerp, "interpolate", wraps=interpolate
                ) as mock_interpolate:
                    res = self.recommender.recommend(
                        index_name=index.name,
                        documents=["1", "2"],
    ...
    def test_recommend_allDocsZeroWeight_failure(self):
        ...
        for index in [self.unstructured_text_index, self.structured_text_index]:
            with self.subTest(type=index.type):
                ...
                with self.assertRaisesStrict(InvalidArgumentError) as ex:
                    self.recommender.recommend(
                        index_name=index.name, documents={"1": 0, "2": 0, "3": 0}
                    )
                self.assertIn(
                    "No documents with non-zero weight provided", str(ex.exception)
                )
    ...
    def test_recommend_docsWithoutVectors_success(self):
        ...
        docs = [
            ...
            {"_id": "2", "title": "Cephalanthera longifolia", "content": "Content"},
        ]
        index = self.unstructured_text_index
        self.add_documents(
            self.config,
            add_docs_params=AddDocsParams(
                index_name=index.name, docs=docs, tensor_fields=["content"]
            ),
        )
    ...
    def test_recommend_emptyDocuments_failure(self):
        ...
        with self.assertRaisesStrict(InvalidArgumentError):
            self.recommender.recommend(
                index_name=self.unstructured_text_index.name, documents=None
            )

        with self.assertRaisesStrict(InvalidArgumentError):
            self.recommender.recommend(
                index_name=self.unstructured_text_index.name, documents=[]
            )
                ...
                with mock.patch.object(tensor_search, "search") as mock_search:
                    self.recommender.recommend(
                        ...
                        score_modifiers=ScoreModifierLists(
                            multiply_score_by=[
                                ScoreModifierOperator(field_name="title", weight=1)
                            ]
                        ),
                    )
        ...
        for index in [self.unstructured_text_index, self.structured_text_index]:
            with self.subTest(index_type=index.name):
                with (
                    ...
                    mock.patch(
                        "marqo.tensor_search.tensor_search.get_documents_by_ids"
                    ) as mock_get_docs,
                ):
        ...
        for index in [self.unstructured_text_index, self.structured_text_index]:
            with self.subTest(index_type=index.name):
                with (
                    ...
                    mock_get_docs.return_value.dict.return_value = {
                        "results": [
                            {"_id": "doc1", "_found": True, "_tensor_facets": []}
                        ]
                    }
        ...
        for index in [self.unstructured_text_index, self.structured_text_index]:
            with self.subTest(index_type=index.name):
                with (
                    ...
                    mock.patch(
                        "marqo.tensor_search.tensor_search.get_doc_vectors_per_tensor_field_by_ids"
                    ) as mock_get_doc_vectors,
                ):
                    mock_get_index.return_value = index
                    mock_get_doc_vectors.return_value = {
                        "doc1": {"field1": [[0.1, 0.2, 0.3]]}
                    }
    ...
    def test_recommend_all_documents_missing_with_allow_missing_documents_true_fails(
        self,
    ):
    ...
    def test_recommend_all_documents_missing_embeddings_with_allow_missing_embeddings_true_fails(
        self,
    ):
        ...
        docs = [
            {"_id": "doc1", "content": "Document 1 with only content"},
            {"_id": "doc2", "content": "Document 2 with only content"},
            {
                "_id": "doc3",
                "title": "Document 3",
            },
