...
from marqo.s2_inference.types import ImageType, List, Tuple
...
class TestImageUtils(unittest.TestCase):
    ...
    def test_load_rcnn_image(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            gt_size = img.size
            image, image_pt, original_size = load_rcnn_image(
                temp_file_name, size=scaled_size
            )
    ...
    def test_rescale_box(self):
        ...
        for gt, bb, orig_size, target_size in zip(
            boxes_gt, boxes, original_sizes, target_sizes
        ):
    ...
    def test_generate_boxes_overlap(self):
        ...
        for (
            ...
            assert bboxes[0] == (0, 0, image_size[0] // wn, image_size[1] // hn)
            assert within_a_pixel(
                bboxes[-1],
                (width * (wn - 1), height * (hn - 1), image_size[0], image_size[1]),
            )
