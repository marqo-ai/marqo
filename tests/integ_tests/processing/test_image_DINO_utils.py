...
class TestImageUtils(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.test_image = Image.open(
            requests.get(self.test_image_name, stream=True).raw
        )
        self.size = (224, 224)
    ...
    def test_load_model(self):
        model, tform = _load_DINO_model(
            arch="vit_small", device=self.device, patch_size=16, image_size=self.size
        )
    ...
    def test_dino_inference_no_device(self):
        try:
            ...
            attentions = DINO_inference(
                model=model, transform=tform, img=self.test_image, patch_size=16
            )
    ...
    def test_rescale_image(self):
        _img = np.array(self.test_image) * 0.9
