...
from marqo.s2_inference.processing.text import prefix_text_chunks, split_text
...
class TestPrefixTextChunks(unittest.TestCase):
    ...
    def test_prefix_text_chunks_empty_prefix(self):
        ...
        assert prefix_text_chunks(text_splits, "") == ["a", "b", "c", ""]

    def test_prefix_text_chunks_none(self):
        text_splits = ["a", "b", "c", ""]
        # None prefix should do nothing
        assert prefix_text_chunks(text_splits, None) == ["a", "b", "c", ""]
