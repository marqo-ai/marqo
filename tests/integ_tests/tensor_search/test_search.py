...
from marqo.tensor_search.enums import EnvVars, SearchMethod, TensorField
...
class TestVectorSearch(MarqoTestCase):
            ...
            docs=[
                {
                    ...
                    "other field": "baaadd efgh ",
                    "_id": "5678",
                    "finally": "some field efgh ",
                },
                {
                    "abc": "shouldn't really match ",
                    "other field": "Nope.....",
                    "_id": "1234",
                    "finally": "Random text here efgh ",
                },
            ],
        )
    ...
    @mock.patch.dict(
        os.environ, {**os.environ, **{"MARQO_MAX_SEARCHABLE_TENSOR_ATTRIBUTES": "2"}}
    )
    ...
    @mock.patch.dict(
        os.environ,
        {**os.environ, **{"MARQO_MAX_SEARCHABLE_TENSOR_ATTRIBUTES": f"{sys.maxsize}"}},
    )
    def test_search_with_no_searchable_attributes_but_max_searchable_attributes_env_set(
        self,
    ):
        with self.assertRaises(InvalidArgError):
            add_docs_caller(
    ...
    def test_vector_search_against_non_existent_index(self):
        try:
            tensor_search._vector_text_search(
                config=self.config,
                index_name="some-non-existent-index",
                result_count=5,
                query="some text...",
                device="cpu",
            )
    ...
    def test_vector_search_long_query_string(self):
        ...
        add_docs_caller(
            ...
            docs=[
                ...
                {
                    "_id": "abc12334",
                    "Title": "Grandma Jo's family recipe. ",
    ...
    def test_tricky_search(self):
        ...
        add_docs_caller(
            ...
            docs=[
                ...
                {"abc": "defgh", "this cat sat": "on the mat"},
            ],
            auto_refresh=True,
        )
    ...
    def test_filtering_list_case_lexical(self):
        ...
        res_exists = tensor_search.search(
            **{"filter": "my_list:tag1", **base_search_args}
        )

        res_not_exists = tensor_search.search(
            **{"filter": "my_list:tag55", **base_search_args}
        )

        res_other = tensor_search.search(
            **{"filter": "my_string:b", **base_search_args}
        )
    ...
    def test_filtering_list_case_image(self):
        ...
        res_img_2_img = tensor_search.search(
            **{"filter": "my_list:tag1", "text": hippo_img, **tensor_search_base_args}
        )
        assert res_img_2_img["hits"][0]["_id"] == "1235"
        assert len(res_img_2_img["hits"]) == 1
        assert res_img_2_img["hits"][0]["_highlights"] == {"img": hippo_img}

        res_img_2_img_none = tensor_search.search(
            **{
                "filter": "my_list:not_exist",
                "text": hippo_img,
                **tensor_search_base_args,
            }
        )
        ...
        res_txt_2_img = tensor_search.search(
            **{"filter": "my_list:tag1", "text": "some", **tensor_search_base_args}
        )
        assert res_txt_2_img["hits"][0]["_id"] == "1235"
        assert res_txt_2_img["hits"][0]["_highlights"] == {"abc": "some text"}
        assert len(res_txt_2_img["hits"]) == 1

        res_txt_2_img_none = tensor_search.search(
            **{"filter": "my_list:not_exist", "text": "some", **tensor_search_base_args}
        )
    ...
    def test_filtering(self):
        ...
        res_multi = tensor_search.search(
            ...
            text="some text",
            result_count=3,
            filter="an_int:[0 TO 30] OR my_bool:true",
            verbose=0,
        )
        ...
        res_complex = tensor_search.search(
            ...
            text="some text",
            result_count=3,
            filter="(an_int:[0 TO 30] and an_int:2) AND abc:(some text)",
            verbose=0,
        )
    ...
    def test_filter_spaced_fields(self):
        ...
        res_mult = tensor_search.search(
            ...
            index_name=self.index_name_1,
            text="",
            filter=r"other\ field:(Close match hehehe)",
        )
        ...
        get_res = tensor_search.get_document_by_id(
            config=self.config, index_name=self.index_name_1, document_id="344"
        )
    ...
    def test_set_device(self):
        ...
        DEFAULT_MODEL_DIMENSION = get_model_properties_from_registry(MlModel.bert)[
            "dimensions"
        ]
    ...
    def test_search_other_types_subsearch(self):
        add_docs_caller(
            ...
            index_name=self.index_name_1,
            auto_refresh=True,
            docs=[{"an_int": 1, "a_float": 1.2, "a_bool": True, "some_str": "blah"}],
        )
        index_object = index_meta_cache.get_index(
            self.index_management, self.default_text_index
        )
    ...
    def test_lexical_filtering(self):
        add_docs_caller(
            ...
            docs=[
                ...
                },
                {
                    "_id": "other doc",
                    "a_float": 0.66,
                    "bfield": "some text too",
                    "my_int": 5,
                    "fake_int": "234",
                    "fake_float": "1.23",
                    "gapped field_name": "gap",
                },
            ],
            auto_refresh=True,
        )
        ...
        pairs = [
            ...
            ("*:*", math.inf),
            ("my_int:5", "other doc"),
            ("my_int:[1 TO 10]", "other doc"),
            ("a_float:0.61", "123456"),
            ("field1:(other things)", "123456"),
            ("fake_int:234", "other doc"),
            ("fake_float:1.23", "other doc"),
            ("fake_float:[0 TO 2]", "other doc"),
            (r"gapped\ field_name:gap", "other doc"),
        ]
        ...
        for to_retrieve in [
            ...
            for method in ("LEXICAL", "TENSOR"):
                search_res = tensor_search.search(
                    ...
                    index_name=self.index_name_1,
                    text="Exact match hehehe",
                    attributes_to_retrieve=to_retrieve,
                    search_method=method,
                )
    ...
    def test_attributes_to_retrieve_non_existent(self):
        docs = {
            "5678": {
                "abc": "Exact a match hehehe",
                "other field": "baaadd",
                "Cool Field 1": "res res res",
                "_id": "5678",
            },
            "rgjknrgnj": {
                "Cool Field 1": "somewhata  match",
                "_id": "rgjknrgnj",
                "abc": "random a text",
                "other field": "Close match hehehe",
            },
            "9000": {
                "Cool Field 1": "somewhat a match",
                "_id": "9000",
                "other field": "weewowow",
            },
        }
        ...
        for to_retrieve in [
            ...
            ["other field", "non existing field name"],
        ]:
    ...
    def test_attributes_to_retrieve_and_searchable_attribs(self):
        docs = {
            ...
            "i_3": {"field_1": " a ", "_id": "i_3", "field_2": "a", "field_3": "a "},
        }
        add_docs_caller(
            config=self.config,
            index_name=self.index_name_1,
        ...
        for to_retrieve, to_search, expected_ids, expected_fields in [
            ...
            for method in ("TENSOR", "LEXICAL"):
                ...
                for res in search_res["hits"]:
                    relevant_fields = set(expected_fields).intersection(
                        set(docs[res["_id"]].keys())
                    )
                    assert (
                        set(
                            k
            ...
            for bad_attr in ["jknjhc", "", dict(), 1234, 1.245]:
                try:
                    tensor_search.search(
                        ...
                        index_name=self.index_name_1,
                        text="a",
                        attributes_to_retrieve=bad_attr,
                        search_method=method,
                    )
                    raise AssertionError
    ...
    def test_limit_results(self):
        ...
        add_docs_caller(
            ...
            index_name=self.index_name_1,
            docs=[
                {"Title": "a " + (" ".join(random.choices(population=vocab, k=25)))}
                for _ in range(2000)
            ],
            auto_refresh=False,
        )
    ...
    def test_limit_results_none(self):
        ...
        for search_method in (SearchMethod.LEXICAL, SearchMethod.TENSOR):
            for mock_environ in [dict(), {EnvVars.MARQO_MAX_RETRIEVABLE_DOCS: ""}]:
    ...
    def test_empty_searchable_attributes(self):
        ...
        docs = [
            {"field_a": 0, "field_b": 0, "field_c": 0},
            {"field_a": 1, "field_b": 1, "field_c": 1},
        ]
    ...
    def test_image_search_highlights(self):
        ...
        docs = [
            {"_id": "123", "image_field": url_1, "text_field": "some words here"},
            {"_id": "789", "image_field": url_2},
        ]
        add_docs_caller(
            config=self.config,
            auto_refresh=True,
        ...
        assert {hit["_highlights"]["image_field"] for hit in res["hits"]} == {
            url_2,
            url_1,
        }
    ...
    def test_multi_search(self):
        ...
        for query, expected_ordering in queries_expected_ordering:
            ...
            for hit_position, _ in enumerate(res["hits"]):
                assert (
                    res["hits"][hit_position]["_id"] == expected_ordering[hit_position]
                )

    def test_multi_search_images(self):
        docs = [
            {"loc a": TestImageUrls.HIPPO_REALISTIC.value, "_id": "realistic_hippo"},
            {"loc b": TestImageUrls.HIPPO_STATUE.value, "_id": "artefact_hippo"},
        ]
        image_index_config = {
        ...
        queries_expected_ordering = [
            (
                {"Nature photography": 2.0, "Artefact": -2},
                ["realistic_hippo", "artefact_hippo"],
            ),
            (
                {"Nature photography": -1.0, "Artefact": 1.0},
                ["artefact_hippo", "realistic_hippo"],
            ),
            (
                {"Nature photography": -1.5, "Artefact": 1.0, "hippo": 1.0},
                ["artefact_hippo", "realistic_hippo"],
            ),
            (
                {TestImageUrls.HIPPO_STATUE.value: -1.0, "blah": 1.0},
                ["realistic_hippo", "artefact_hippo"],
            ),
            (
                {
    ...
    def test_multi_search_check_vector(self):
        ...
        docs = [
            {"loc a": TestImageUrls.HIPPO_REALISTIC.value, "_id": "realistic_hippo"},
            {"loc a": TestImageUrls.HIPPO_STATUE.value, "_id": "artefact_hippo"},
        ]
        image_index_config = {
            IndexSettingsField.index_defaults: {
                IndexSettingsField.model: "ViT-B/16",
        ...
        for multi_query in multi_queries:
            ...
            def run() -> typing.List[float]:
                ...
                query_vec = query_dict["query"]["nested"]["query"]["knn"][
                    f"{TensorField.chunks}.{TensorField.marqo_knn_field}"
                ]["vector"]
                return query_vec
            ...
            if norm > 0:
                manually_combined /= np.linalg.norm(
                    manually_combined, axis=-1, keepdims=True
                )
    ...
    def test_multi_search_images_edge_cases(self):
        docs = [
            {"loc": TestImageUrls.HIPPO_REALISTIC.value, "_id": "realistic_hippo"},
            {"field_a": "Some text about a weird forest", "_id": "artefact_hippo"},
        ]
        image_index_config = {
            IndexSettingsField.index_defaults: {
                IndexSettingsField.model: "ViT-B/16",
        ...
        invalid_queries = [
            ...
            {123: 123},
            {"123": None},
            {"https://marqo-not-real.com/image_1.png": 3},
            set(),
        ]
        for q in invalid_queries:
            try:
                ...
                raise AssertionError
            except (InvalidArgError, BadRequestError):
                pass
    ...
    def test_image_search(self):
        ...
        doc_dict = {
            "realistic_hippo": {"loc": hippo_image, "_id": "realistic_hippo"},
            "artefact_hippo": {
    ...
    def test_zero_vectors_search(self):
        ...
        tensor_search.create_vector_index(
            config=self.config, index_name=index_name, index_settings=index_settings
        )
        ...
        @mock.patch(
            "marqo.tensor_search.tensor_search._create_dummy_query_for_zero_vector_search",
            mock_create_dummy_query,
        )
        @mock.patch(
            "marqo.tensor_search.tensor_search._vector_text_search_query_verbose",
    ...
    def test_vector_text_search_verbose(self):
        ...
        mock_generate_verbose_one_body.side_effect = (
            _generate_vector_text_search_query_for_verbose_one
        )

        @mock.patch(
            "marqo.tensor_search.tensor_search._generate_vector_text_search_query_for_verbose_one",
        ...
        def run():
            for verbose in [0, 1, 2]:
                ...
                if verbose == 0:
                    ...
                    assert (
                        "knn" in verbose_kwargs["body"][1]["query"]["nested"]["query"]
                    )
                    ...
                    if verbose == 1:
                        mock_generate_verbose_one_body.assert_called_once_with(
                            verbose_kwargs["body"]
                        )
                    elif verbose == 2:
