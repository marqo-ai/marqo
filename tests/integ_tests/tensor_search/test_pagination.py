...
from marqo.api.exceptions import IllegalRequestedDocCount
...
from marqo.core.models.marqo_index import FieldFeature, FieldType
...
class TestPagination(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.indexes = cls.create_indexes(
            [index_request_structured, index_request_unstructured]
        )
        ...
        for index in [self.index_structured, self.index_unstructured]:
            for _ in range(0, num_docs, batch_size):
                ...
                for i in range(batch_size):
                    ...
                    doc = {"_id": str(i), "title": title, "desc": "my description"}
                    docs.append(doc)
            ...
            for search_method in (SearchMethod.LEXICAL, SearchMethod.TENSOR):
                ...
                for page_size in [100, 200]:
                    with self.subTest(
                        ...
                        self.assertEqual(
                            len(full_search_results["hits"]),
                            len(paginated_search_results["hits"]),
                        )
                        for i in range(len(full_search_results["hits"])):
                            self.assertEqual(
                                full_search_results["hits"][i]["_id"],
                                paginated_search_results["hits"][i]["_id"],
                            )
                            self.assertEqual(
                                full_search_results["hits"][i]["_score"],
    ...
    def test_pagination_hybrid(self):
        ...
        for index in [self.index_structured, self.index_unstructured]:
            ...
            for i in range(num_docs):
                title = static_title + " ".join(
                    self.generate_unique_strings(i % batch_size)
                )  # Doc has 0 to 99 unique words
                doc = {"_id": str(i), "title": title, "desc": "my description"}
                docs.append(doc)
            ...
            for retrieval_method, ranking_method in test_cases:
                with self.subTest(
                    ...
                    for page_size in [10, 100, 200]:
                        with self.subTest(
                            f"Index: {index.type}, Page size: {page_size}"
                        ):
                            paginated_search_results = {"hits": []}
                            ...
                            for page_num in range(math.ceil(num_docs / page_size)):
                                ...
                                paginated_search_results["hits"].extend(
                                    page_res["hits"]
                                )
                ...
                for search_method in [SearchMethod.TENSOR, SearchMethod.LEXICAL]:
                    ...
                    for page_size in [max_limit]:
                        with self.subTest(
                            ...
                            self.assertEqual(
                                expected_count, len(paginated_search_results["hits"])
                            )

                    self.assertTrue(
                        max_offset in offsets_covered,
                        "Max offset not covered. Check test parameters",
                    )
        ...
        for index in [self.index_structured, self.index_unstructured]:
            for search_method in [SearchMethod.TENSOR, SearchMethod.LEXICAL]:
                for limit in [1001, 2000, 10000]:
                    with self.subTest(
                        f"Index: {index.type}, Search method: {search_method}, Limit: {limit}"
                    ):
        ...
        for index in [self.index_structured, self.index_unstructured]:
            for search_method in [SearchMethod.TENSOR, SearchMethod.LEXICAL]:
                for offset in [10001, 12000, 50000]:
                    with self.subTest(
                        f"Index: {index.type}, Search method: {search_method}, Offset: {offset}"
                    ):
                        with self.assertRaises(api_exceptions.IllegalRequestedDocCount):
                            tensor_search.search(
                                search_method=search_method,
    ...
    def test_pagination_multi_field(self):
        ...
        add_docs_caller(
            ...
            docs=[
                {
                    "field_1": "a " + (" ".join(random.choices(population=vocab, k=5))),
        ...
        for search_method in (SearchMethod.LEXICAL, SearchMethod.TENSOR):
            for doc_count in [1000]:
                ...
                for page_size in [5, 10, 100, 1000]:
                    ...
                    # Compare paginated to full results (length only for now)
                    assert len(full_search_results["hits"]) == len(
                        paginated_search_results["hits"]
                    )
    ...
    def test_pagination_empty_searchable_attributes(self):
        ...
        add_docs_caller(config=self.config, index_name=self.index_name_1, docs=docs)
