...
@unittest.skip(
    reason="temporarily skip due to inference interface change: image chunking not supported"
)
class TestImagePreprocessing(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.indexes = cls.create_indexes(
            [structured_image_index, unstructured_image_index]
        )
    ...
    def test_image_preprocess_search_highlights_format(self):
        ...
        for index_name in [self.structured_image_index, self.unstructured_image_index]:
            tensor_fields = (
                None if index_name == self.structured_image_index else ["image_field_1"]
            )
            ...
            with self.subTest(f"index_name = {index_name}"):
                ...
                self.assertIn(
                    "image_field_1", search_result["hits"][0]["_highlights"][0]
                )
                self.assertTrue(
                    isinstance(
                        search_result["hits"][0]["_highlights"][0]["image_field_1"], str
                    )
                )
                self.assertTrue(
                    isinstance(
                        eval(
                            search_result["hits"][0]["_highlights"][0]["image_field_1"]
                        ),
                        list,
                    )
                )
    ...
    def test_image_preprocess_get_documents_format(self):
        ...
        for index_name in [self.structured_image_index, self.unstructured_image_index]:
            ...
            with self.subTest(f"index_name = {index_name}"):
                ...
                get_doc_result = tensor_search.get_document_by_id(
                    config=self.config,
                    index_name=index_name,
                    document_id="1",
                    show_vectors=True,
                ...
                for tensor_facet in get_doc_result["_tensor_facets"]:
                    ...
                    self.assertTrue(
                        isinstance(eval(tensor_facet["image_field_1"]), list)
                    )
