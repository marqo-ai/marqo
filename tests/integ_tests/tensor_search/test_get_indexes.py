...
from marqo.tensor_search import constants, tensor_search
...
class TestGetIndexes(MarqoTestCase):
    ...
    def test_get_indexes(self):
        assert not _index_is_present(
            self.index_name_1, tensor_search.get_indexes(config=self.config)
        )
        assert not _index_is_present(
            self.index_name_2, tensor_search.get_indexes(config=self.config)
        )

        tensor_search.create_vector_index(
            config=self.config, index_name=self.index_name_1
        )
        assert _index_is_present(
            self.index_name_1, tensor_search.get_indexes(config=self.config)
        )
        assert not _index_is_present(
            self.index_name_2, tensor_search.get_indexes(config=self.config)
