...
class TestAddDocumentsUseExistingTensors(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.indexes = cls.create_indexes(
            [structured_index, structured_index_multimodal, unstructured_index]
        )
    ...
    def test_use_existing_tensor_no_change(self):
        ...
        """
        doc = {"text_field_1": "content 1", "_id": "1"}
        ...
        for index_name in [self.structured_index, self.unstructured_index]:
            tensor_fields = (
                None
                if index_name == self.structured_index
                else ["text_field_1", "text_field_2"]
            )
            with self.subTest(f"{index_name}"):
                with mock.patch.object(
                    s2_inference, "vectorise", side_effect=original_vectorise
                ) as mock_vectorise:
                    self.add_documents(
                        config=self.config,
                    ...
                    self.assertEqual(
                        1, mock_vectorise.call_count, "vectorise should be called once"
                    )
                ...
                with mock.patch.object(
                    ...
                    self.assertEqual(
                        0, mock_vectorise.call_count, "vectorise should not be called"
                    )

                    search_res = tensor_search.search(
                        config=self.config, index_name=index_name, text="content"
                    )
                    get_doc_res = tensor_search.get_document_by_id(
                        config=self.config,
                        index_name=index_name,
                        document_id="1",
                        show_vectors=True,
                    ...
                    self.assertEqual(
                        "content 1", get_doc_res["_tensor_facets"][0]["text_field_1"]
                    )

    def test_use_existing_tensor_new_fields(self):
        doc_1 = {"text_field_1": "content 1", "_id": "1"}

        doc_2 = {"text_field_2": "content 2", "_id": "1"}
        ...
        for index_name in [self.structured_index, self.unstructured_index]:
            ...
            with self.subTest(f"{index_name}"):
                ...
                with mock.patch.object(
                    ...
                    self.assertEqual(
                        1, mock_vectorise.call_count, "vectorise should not be called"
                    )
                    ...
                    self.assertEqual(1, len(get_doc_res["_tensor_facets"]))
                    self.assertEqual(
                        "content 2", get_doc_res["_tensor_facets"][0]["text_field_2"]
                    )
    ...
    def test_use_existing_tensor_multimodal_no_change(self):
        ...
        """
        doc = {"text_field_1": "content 1", "text_field_2": "content 2", "_id": "1"}
        ...
        for index_name in [self.structured_index_multimodal, self.unstructured_index]:
            tensor_fields = (
                None
                if index_name == self.structured_index_multimodal
                else ["multimodal_field"]
            )
            mappings = (
                None
                if index_name == self.structured_index
                    ...
                    "multimodal_field": {
                        ...
                        "weights": {"text_field_1": 0.5, "text_field_2": 0.8},
                    }
                }
            )
            with self.subTest(f"{index_name}"):
                ...
                with mock.patch.object(
                    ...
                    self.assertEqual(1, len(get_doc_res["_tensor_facets"]))
                    self.assertEqual(
                        '{"text_field_1": "content 1", "text_field_2": "content 2"}',
    ...
    def test_use_existing_tensor_multimodal_added(self):
        ...
        """
        doc_1 = {"_id": "1"}
        doc_2 = {"text_field_1": "content 1", "text_field_2": "content 2", "_id": "1"}
        ...
        for index_name in [self.structured_index_multimodal, self.unstructured_index]:
            tensor_fields_1 = (
                None if index_name == self.structured_index_multimodal else []
            )
            tensor_fields_2 = (
                None
                if index_name == self.structured_index_multimodal
            ...
            with self.subTest(f"{index_name}"):
                ...
                with mock.patch.object(
                    ...
                    self.assertEqual(
                        1, mock_vectorise.call_count, "vectorise should be called"
                    )
    ...
    def test_use_existing_tensor_multimodal_changed(self):
        ...
        Checks that the vectors are updated if a multimodal field is added
        """
        doc_1 = {"text_field_1": "content 1", "text_field_2": "content 2", "_id": "1"}
        doc_2 = {
        ...
        for index_name in [self.structured_index_multimodal, self.unstructured_index]:
            ...
            with self.subTest(f"{index_name}"):
                ...
                with mock.patch.object(
                    ...
                    self.assertEqual(
                        1, mock_vectorise.call_count, "vectorise should be called again"
                    )
                    ...
                    self.assertEqual("content 1", search_res["hits"][0]["text_field_1"])
                    self.assertEqual(
                        "content 2-updated", search_res["hits"][0]["text_field_2"]
                    )
                    self.assertEqual(1, len(get_doc_res["_tensor_facets"]))
                    self.assertEqual(
                        '{"text_field_1": "content 1", "text_field_2": "content 2-updated"}',
                        get_doc_res["_tensor_facets"][0]["multimodal_field"],
                    )
    ...
    def test_use_existing_tensors_resilience(self):
        ...
        d1 = {"title 1": "content 1", "desc 2": "content 2. blah blah blah"}
        # 1 valid ID doc:
    ...
    def test_use_existing_tensors_getting_non_tensorised(self):
        ...
        self.add_documents(
            ...
            add_docs_params=AddDocsParams(
                index_name=self.index_name_1,
                docs=[{"_id": "999", "non-tensor-field": "content 2. blah blah blah"}],
                auto_refresh=True,
                non_tensor_fields=["non-tensor-field"],
                device="cpu",
            ),
    ...
    def test_use_existing_tensors_check_meta_data(self):
        ...
        updated_doc = requests.get(
            url=f"{self.endpoint}/{self.index_name_1}/_doc/123", verify=False
        )
        ...
        for vector_field in ["title 1", "my new field", "modded field"]:
            ...
            for ch in chunks:
                if ch["__field_name"] == vector_field:
                    ...
                    assert (
                        ch["__field_content"] == use_existing_tensor_doc[vector_field]
                    )
    ...
    def test_use_existing_tensors_check_meta_data_mappings(self):
        ...
        tensor_search.index_meta_cache.refresh_index(
            config=self.config, index_name=self.index_name_1
        )

        index_info = tensor_search.get_index_info(
            config=self.config, index_name=self.index_name_1
        )
        ...
        text_fields = [
            ...
            "field_to_be_list",
            "new_field_list",
            "field_that_will_disappear",
        ]

        for text_field in text_fields:
            assert index_info.properties[text_field]["type"] == "text"
            assert (
                index_info.properties["__chunks"]["properties"][text_field]["type"]
                == "keyword"
            )

        # Only 1 tensor field
        assert (
            index_info.properties["__chunks"]["properties"][
        ...
        for field_name, os_type in [("fl", "float"), ("new_bool", "boolean")]:
            ...
            assert (
                index_info.properties["__chunks"]["properties"][field_name]["type"]
                == os_type
            )
    ...
    def test_use_existing_tensors_long_strings_and_images(self):
        ...
        @unittest.mock.patch(
            "PIL.Image.Image.close", mock_close
        )  # disable .close so we can access images in this test
        def run():
            use_existing_tensor_doc = {
                "txt_to_be_the_same": "some text to leave unchanged. I repeat, unchanged",
            ...
            artefact_pil_image = load_image_from_path(
                artefact_hippo_img, media_download_headers={}
            )
            ...
            updated_doc = requests.get(
                url=f"{self.endpoint}/{self.index_name_2}/_doc/123", verify=False
            )
            ...
            for vector_field in vector_img_fields:
                ...
                for ch in chunks:
                    if ch["__field_name"] == vector_field:
                        ...
                        assert (
                            ch["__field_content"]
                            == use_existing_tensor_doc[vector_field]
                        )
                    # Only 1 tensor field
                    assert isinstance(ch[TensorField.marqo_knn_field], list)
                assert found_vector_field
            ...
            real_txt_chunks = {
                (ch["__field_name"], ch["__field_content"])
                for ch in chunks
                if ch["__field_name"].startswith("txt")
            }
    ...
    def test_use_existing_tensors_all_data_types(self):
        ...
        for doc_arg in doc_args:
            ...
            d1 = tensor_search.get_documents_by_ids(
                config=self.config,
                index_name=self.index_name_1,
                document_ids=[doc["_id"] for doc in doc_arg],
                show_vectors=True,
            )
