            ...
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                ...
                self.assertEqual(
                    tensor_search.get_document_by_id(
                        config=self.config, index_name=index.name, document_id="123"
                    ),
                    {
                        "_id": "123",
                        "title1": "content 1",
        ...
        for index in self.indexes:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                with self.assertRaises(InvalidDocumentIdError):
                    a = tensor_search.get_document_by_id(
                        config=self.config, index_name=index.name, document_id=""
                    )
        ...
        for index in self.indexes:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                ...
                self.add_documents(
                    ...
                    add_docs_params=AddDocsParams(
                        index_name=index.name,
                        docs=[{"_id": "123", **dict(zip(keys, vals))}],
                        auto_refresh=True,
                        device="cpu",
                        tensor_fields=["title1", "desc2"]
                        if isinstance(index, UnstructuredMarqoIndex)
