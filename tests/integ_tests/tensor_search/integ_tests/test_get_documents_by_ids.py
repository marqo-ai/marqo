...
class TestGetDocuments(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        structured_text_index_with_random_model_request = (
            cls.structured_marqo_index_request(
                ...
                fields=[
                    ...
                    FieldRequest(
                        name="custom_vector_field", type=FieldType.CustomVector
                    ),
                ],
        ...
        unstructured_text_index_with_random_model_request = (
            cls.unstructured_marqo_index_request(
                model=Model(name="random"), marqo_version="2.12.0"
            )
        )
    ...
    def test_get_documents_by_ids_structured(self):
        ...
        docs = [
            {
                ...
                "int_map_field": {"a": 1},
                "float_field": 2.9,
                "float_array_field": [1.0, 2.0],
                "float_map_field": {"b": 2.9},
                "long_field": 10,
                "long_array_field": [10, 20],
                "long_map_field": {"a": 10},
                "double_field": 3.9,
                "double_array_field": [3.0, 5.0],
                "double_map_field": {"b": 5.9},
                "bool_field": True,
            ...
            },
            {
                "_id": "2",
                "title1": "content 2",
                "custom_vector_field": {"content": "a", "vector": [1.0] * 384},
            },
        ...
        for i in range(3):
            ...
            self.assertIn(
                ...
                res["results"][i][enums.TensorField.tensor_facets][0],
            )
    ...
    def test_get_documents_vectors_format(self):
        ...
        vals = [
            ("content 1", "content 2. blah blah blah", "123"),
        ...
        for index in self.indexes:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                self.add_documents(
                    ...
                    add_docs_params=AddDocsParams(
                        index_name=index.name,
                        docs=[dict(zip(k, v)) for k, v in zip(keys, vals)],
                        device="cpu",
                ...
                for i, retrieved_doc in enumerate(get_res):
                    ...
                    assert set(keys[i]).union({enums.TensorField.embedding}) - {
                        "_id"
                    } == functools.reduce(
        ...
        for index in self.indexes:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                ...
                id_reqs = [
                    (["123", "456"], [False, True]),
                    ([["456", "789"], [True, False]]),
                    ([["456", "789", "221"], [True, False, True]]),
                    ([["vkj", "456", "4891"], [False, True, False]]),
                ]
                for is_vector_shown in (True, False):
        ...
        for index in self.indexes:
            for show_vectors_option in (True, False):
                for document_ids, msg in test_cases:
                    with self.subTest(
                        f"Index type: {index.type}. Index name: {index.name}. Msg: {msg}"
                    ):
                        with self.assertRaises(InvalidArgError) as e:
                            ...
                            if not document_ids == []:
                                ...
                                self.assertIn(
                                    "Can't get empty collection of IDs!",
                                    str(e.exception),
                                )
                ...
                for document_ids, error_index, msg in test_cases:
                    with self.subTest(
                        ...
                        for i in error_index:
                            ...
                            self.assertIn(
                                "Document _id must be a string type!", item.message
                            )
    ...
    def test_get_documents_env_limit(self):
        for index in self.indexes:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                docs = [
                    {"title1": "a", "_id": uuid.uuid4().__str__()} for _ in range(2000)
                ]
                add_docs_batched(
                    config=self.config,
                    index_name=index.name,
                ...
                for max_doc in [0, 1, 2, 5, 10, 100, 1000]:
                    mock_environ = {
                        enums.EnvVars.MARQO_MAX_RETRIEVABLE_DOCS: str(max_doc)
                    }
                    ...
                    def run():
                        ...
                        with self.assertRaises(IllegalRequestedDocCount):
                            oversized_search = tensor_search.get_documents_by_ids(
                                ...
                                index_name=index.name,
                                document_ids=[
                                    docs[i]["_id"] for i in range(max_doc + 1)
                                ],
                            ).dict(exclude_none=True, by_alias=True)
                        with self.assertRaises(IllegalRequestedDocCount):
                            very_oversized_search = tensor_search.get_documents_by_ids(
                                ...
                                index_name=index.name,
                                document_ids=[
                                    docs[i]["_id"] for i in range(max_doc * 2)
                                ],
                            ).dict(exclude_none=True, by_alias=True)
                        return True
        ...
        with patch(
            ...
            for i in range(10):
                t = threading.Thread(
                    ...
                    kwargs={"document_ids": ["1", "2", "3"], "thread_id": i},
                )
                threads.append(t)
    ...
    def test_get_documents_concurrent_resilient_when_some_fail(self):
        ...
        def get_documents_thread(
            index_name, document_ids, thread_id, should_fail=False
        ):
            try:
                r = tensor_search.get_documents_by_ids(
                    config=self.config,
                ...
                if should_fail:
                    failed_results.append(
                        (thread_id, r, "Expected to fail but succeeded")
                    )
                else:
        ...
        self.assertEqual(
            ...
            5,
            f"Expected 5 successful results, got {len(successful_results)}",
        )

        # Verify that we got the expected number of failed results (exceptions)
        expected_failures = [
            exc for thread_id, exc, should_fail in all_exceptions if should_fail
        ]
        self.assertEqual(
            len(expected_failures),
            3,
            f"Expected 3 failures, got {len(expected_failures)}",
        )
        ...
        for exception in expected_failures:
            ...
            self.assertIn(
                "nonexistent_index_name_that_will_cause_error", str(exception)
            )

        # Verify that no unexpected failures occurred
        unexpected_failures = [
            exc for thread_id, exc, should_fail in all_exceptions if not should_fail
        ]
        self.assertEqual(
