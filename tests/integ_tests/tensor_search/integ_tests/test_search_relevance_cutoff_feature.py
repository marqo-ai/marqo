...
class TestSearchRelevanceCutoffFeatureSort1Field(MarqoTestCase):
    ...
    def setUpClass(cls):
        ...
        test_sort1field_docs = [
            ...
            {"_id": "3", "content": ' '.join([f"content{i}" for i in range(10)]), "sort_field_1": 3},
            # tie value for relevance tiebreak
            {"_id": "4", "content": ' '.join([f"content{i}" for i in range(7)]), "sort_field_1": 3},
            # tie value for relevance tiebreak
    ...
    def test_simple_sort_with_default_settings(self):
        ...
        for _ in range(10):
            ...
            # Relevance cutoff is not implemented in Vespa, so it is same as the relevance order.
            self.assertEqual([
                "3", "7", "5", "4", "2", "9", "1", "6", "8", "0"
            ], ids)