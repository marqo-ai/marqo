...
class TestAddDocumentsSemiStructuredAddFields(MarqoTestCase):
    ...
    def test_add_documents_should_successfully_add_new_tensor_fields(self):
        ...
        updated_index = cast(
            SemiStructuredMarqoIndex,
            self.config.index_management.get_index(self.text_index_1),
        )
    ...
    def test_add_documents_should_use_existing_tensors_from_the_same_doc(self):
        ...
        with mock.patch("marqo.s2_inference.s2_inference.vectorise") as mock_vectorise:
            doc2 = self._add_and_get_doc(
                self.text_index_1, "123", ["title"], use_existing_tensors=True
            )
    ...
    def test_add_documents_should_add_string_fields_as_lexical_fields(self):
        ...
        updated_index = cast(
            SemiStructuredMarqoIndex,
            self.config.index_management.get_index(self.text_index_2),
        )
    ...
    def test_add_documents_should_add_custom_vector_field_content_as_lexical_fields(
        self,
    ):
        self.add_documents(
            config=self.config,
        ...
        updated_index = cast(
            SemiStructuredMarqoIndex,
            self.config.index_management.get_index(self.text_index_3),
        )
        self.assertIn("custom_vector_field", updated_index.field_map.keys())
        self.assertIn(
            "marqo__lexical_custom_vector_field", updated_index.lexical_field_map.keys()
        )
    ...
    def test_add_documents_should_add_image_field_as_lexical_fields(self):
        ...
        self.assertIn(
            "marqo__lexical_image_field", updated_index.lexical_field_map.keys()
        )
    ...
    def test_add_documents_should_add_multimodal_subfield_as_lexical_fields(self):
        ...
        self.assertNotIn(
            "marqo__lexical_combo_field", updated_index.lexical_field_map.keys()
        )

    def test_add_documents_should_allow_the_same_field_to_have_different_types_in_different_batches(
        self,
    ):
        ...
        self.config.document.add_documents(
            AddDocsParams(
                ...
                device="cpu",
                tensor_fields=["universal_tensor_field"],
                mappings={
        ...
        self.config.document.add_documents(
            AddDocsParams(
                ...
                device="cpu",
                tensor_fields=["universal_tensor_field"],
                mappings={"universal_tensor_field": {"type": "custom_vector"}},
            )
        )
    ...
    def test_add_documents_should_raise_error_when_field_count_exceeds_limit(self):
        field_count_config = SemiStructuredFieldCountConfig(
            max_lexical_field_count=6,
            max_tensor_field_count=5,
            max_string_array_field_count=5,
        )
        self.config.document.add_documents(
            AddDocsParams(
        ...
        with self.assertRaises(TooManyFieldsError) as err:
            self.config.document.add_documents(
                AddDocsParams(
                    index_name=self.text_index_6,
                    docs=[{"_id": "2", "tensor_field6": "content 6"}],
                    tensor_fields=["tensor_field6"],
                ),
                field_count_config=field_count_config,
            )
        self.assertIn(
            "has 5 tensor fields. Your request to add tensor_field6 as a tensor field is "
            "rejected since it exceeds the limit of 5. Please set a larger limit in "
            "MARQO_MAX_TENSOR_FIELD_COUNT_UNSTRUCTURED environment variable.",
            str(err.exception),
        )
        ...
        self.assertIn(
            "has 6 lexical fields. Your request to add lexical_field7 as a lexical field is "
            "rejected since it exceeds the limit of 6. Please set a larger limit in "
            "MARQO_MAX_LEXICAL_FIELD_COUNT_UNSTRUCTURED environment variable.",
            str(err2.exception),
        )
        ...
        with self.assertRaises(TooManyFieldsError) as err3:
            ...
            self.assertIn(
                "has 5 string array fields. Your request to add string_array_field7 as a string array field is "
                "rejected since it exceeds the limit of 5. Please set a larger limit in "
                "MARQO_MAX_STRING_ARRAY_FIELD_COUNT_UNSTRUCTURED environment variable.",
                str(err3.exception),
            )
