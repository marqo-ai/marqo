...
class TestAddDocumentsStructured(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        index_request_1 = cls.structured_marqo_index_request(
            fields=[
                ...
                FieldRequest(
                    name="in_stock", type=FieldType.Bool, features=[FieldFeature.Filter]
                ),
                FieldRequest(
                    name="int_field_1",
                    type=FieldType.Int,
            ...
            with self.subTest(desc):
                ...
                self.assertEqual(
                    ...
                    tensor_search.get_document_by_id(
                        config=self.config, index_name=index_name, document_id="123"
                    ),
                )
    ...
    def test_boolean_field(self):
        ...
        test_cases = [
            ("True", {"_id": "123", "in_stock": True}),
            ("False", {"_id": "124", "in_stock": False}),
            (
            ...
            for test_case in test_cases:
                with self.subTest(test_case[0] + " - " + desc):
                    self.add_documents(
                        config=self.config,
                        add_docs_params=AddDocsParams(
                            index_name=index_name, docs=[test_case[1]], device="cpu"
                        ),
                    )
                    self.assertEqual(
                        test_case[1],
    ...
    def test_add_documents_dupe_ids(self):
        ...
        self.add_documents(
            ...
            add_docs_params=AddDocsParams(
                index_name=self.index_name_1,
                docs=[{"_id": "2", "title": "doc 123"}],
                device="cpu",
            ),
        )

        expected_doc = {"_id": "2", "title": "doc 123", "_tensor_facets": tensor_facets}
    ...
    def test_add_documents_validation(self):
        ...
        bad_doc_args = [
            ...
            ],  # dict for non-combination field
            [
                {
                    "_id": "to_fail_123",
                    "title": {"abc": "234"},
                ...
                with self.subTest(
                    ...
                    add_res = self.add_documents(
                        ...
                        add_docs_params=AddDocsParams(
                            index_name=self.index_name_1,
                            docs=bad_doc_arg,
                            use_existing_tensors=use_existing_tensors_flag,
                            device="cpu",
                        ),
                    ).dict(exclude_none=True, by_alias=True)
                    assert add_res["errors"] is True
    ...
    def test_add_documents_list_success(self):
        good_docs = [[{"_id": "to_fail_123", "tags": ["wow", "this", "is"]}]]
        for bad_doc_arg in good_docs:
            add_res = self.add_documents(
                config=self.config,
                add_docs_params=AddDocsParams(
                    index_name=self.index_name_1, docs=bad_doc_arg, device="cpu"
                ),
            ).dict(exclude_none=True, by_alias=True)
    ...
    def test_add_documents_list_data_type_validation(self):
        ...
        for bad_doc_arg in bad_doc_args:
            with self.subTest(bad_doc_arg):
                ...
                assert all(
                    [
                        "All list elements must be of the same type and that type must be int, float or string"
                        in item["message"]
                        for item in add_res["items"]
                    ]
                )
    ...
    def test_add_documents_empty(self):
        ...
        try:
            self.add_documents(
                config=self.config,
                add_docs_params=AddDocsParams(
                    index_name=self.index_name_1, docs=[], device="cpu"
                ),
            )
    ...
    def test_add_documents_id_image_url(self):
        ...
        """
        docs = [{"_id": TestImageUrls.HIPPO_REALISTIC.value, "title": "wow"}]

        with mock.patch("PIL.Image.open") as mock_image_open:
            self.add_documents(
                config=self.config,
                add_docs_params=AddDocsParams(
                    index_name=self.index_name_img_no_chunking,
                    docs=docs,
                    device="cpu",
                ),
            )
    ...
    def test_add_documents_resilient_doc_validation(self):
        docs_results = [
            ...
            (
                [
                    {"__chunks": "bad"},
                    {"_id": "1511", "__vector_a": "some content"},
                    {"_id": "cool"},
            ...
            (
                [{123: "bad", "_id": "12345"}, {"_id": "cool"}],
                [("12345", 400), ("cool", 200)],
            ),
            (
                [{None: "bad", "_id": "12345"}, {"_id": "cool"}],
                [("12345", 400), ("cool", 200)],
            ),
            # handle bad content
            (
                [{"title": None, "_id": "12345"}, {"_id": "cool"}],
                [(None, 400), ("cool", 200)],
            ),
            (
                [{"tags": [1, 2, "3", 4], "_id": "12345"}, {"_id": "cool"}],
                [("12345", 400), ("cool", 200)],
            ),
            (
                [{"title": ("cat", "dog"), "_id": "12345"}, {"_id": "cool"}],
                [("12345", 400), ("cool", 200)],
            ),
            (
                [{"title": set(), "_id": "12345"}, {"_id": "cool"}],
                [(None, 400), ("cool", 200)],
            ),
            (
                [{"title": dict(), "_id": "12345"}, {"_id": "cool"}],
                [(None, 400), ("cool", 200)],
            ),
            # handle bad _ids
            (
                [{"bad": "hehehe", "_id": 12345}, {"_id": "cool"}],
                [(None, 400), ("cool", 200)],
            ),
            (
                [
                    {"bad": "hehehe", "_id": 12345},
                    {"_id": "cool"},
                    {"bad": "hehehe", "_id": None},
                    {"title": "yep"},
                    {"_id": (1, 2), "efgh": "abc"},
                    {"_id": 1.234, "cool": "wowowow"},
                ],
                [
                    (None, 400),
                    ("cool", 200),
    ...
    def test_add_document_with_tensor_fields(self):
        docs_ = [
            {
                ...
                "desc": "Alice grew up in Houston, Texas.",
            }
        ]
        self.add_documents(
            config=self.config,
        ...
        resp = tensor_search.get_document_by_id(
            config=self.config,
            index_name=self.index_name_1,
            document_id="789",
            show_vectors=True,
    ...
    def test_doc_too_large(self):
        ...
        def run():
            ...
            assert "doc_too_large" == items[0]["code"] and (
                "doc_too_large" == items[0]["code"]
            )
    ...
    def test_various_image_count(self):
        ...
        def _check_get_docs(doc_count, title_value):
            ...
            for d in get_res["results"]:
                ...
                assert {"_embedding", "location", "title"} == functools.reduce(
                    lambda x, y: x.union(y),
                    [list(facet.keys()) for facet in d["_tensor_facets"]],
                    set(),
                )
        ...
        for c in doc_counts:
            ...
            res1 = self.add_documents(
                ...
                add_docs_params=AddDocsParams(
                    docs=[
                        {"_id": str(doc_num), "location": hippo_url, "title": "blah"}
    ...
    def test_add_documents_nonImageContentForAnImageField(self):
        ...
        for item in r.items:
            ...
            # modality mismatch
            self.assertIn(
                "is not a local file, a valid url, or a base64-encoded image",
                item.message,
            )
