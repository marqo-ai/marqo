        ...
        mock_vespa_client_query.return_value = QueryResult(
            **{'root': {
                ...
                'children': [{'id': 'index:content_default/0/c81e728d5f3b597225351eac',
                              ...
                              'fields': {
                                  'matchfeatures': {'closest(marqo__embeddings)': {'type': 'tensor<float>(p{})',
                                                                                   'cells': {'1': 1.0}}},
    ...
    def test_search_incomplete_response_processed_correctly(self):
        ...
        self.add_documents(
            config=self.config, add_docs_params=AddDocsParams(
                index_name=self.default_text_index, docs=docs, tensor_fields=["text_field_1"]
            )
        )
        ...
        # mock the VespaClient.query method to return real response and modify the response
            def wrapper(*args, **kwargs):
                ...
                del response.hits[0].fields['marqo__int_fields']
                return response

            mock_query.side_effect = wrapper
            result = tensor_search.search(config=self.config, text="sample text", index_name=self.default_text_index, search_method=SearchMethod.LEXICAL)
