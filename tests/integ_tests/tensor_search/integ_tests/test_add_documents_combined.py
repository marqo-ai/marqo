...
from marqo.tensor_search import add_docs, tensor_search
from marqo.tensor_search.models.preprocessors_model import Preprocessors
from tests.integ_tests.marqo_test import (
    MarqoTestCase,
    TestAudioUrls,
    TestImageUrls,
    TestVideoUrls,
...
class TestAddDocumentsCombined(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        structured_image_index_request = cls.structured_marqo_index_request(
            ...
            fields=[
                ...
                FieldRequest(
                    ...
                    dependent_fields={"image_field_1": 1.0, "text_field_1": 0.0},
                ),
            ],
        ...
        structured_image_index_request_unnormalized = (
            cls.structured_marqo_index_request(
                name="structured_image_index_unnormalised"
        ...
        semi_structured_languagebind_index_request = (
            cls.unstructured_marqo_index_request(
                name="unstructured_languagebind_index"
                + str(uuid.uuid4()).replace("-", ""),
                model=Model(name="LanguageBind/Video_V1.5_FT_Audio_FT_Image"),
                treat_urls_and_pointers_as_images=True,
        ...
        unstructured_image_index_request_unnormalized = (
            cls.unstructured_marqo_index_request(
                name="unstructured_image_index_unnormalised"
                + str(uuid.uuid4()).replace("-", ""),
                model=Model(name="open_clip/ViT-B-32/laion2b_s34b_b79k"),
                normalize_embeddings=False,
                distance_metric=DistanceMetric.DotProduct,
        ...
        cls.structured_languagebind_index_name = (
            structured_languagebind_index_request.name
        )
        cls.semi_structured_marqo_index_name = semi_structured_image_index_request.name
        cls.semi_structured_languagebind_index_name = (
            semi_structured_languagebind_index_request.name
        )
        cls.structured_image_index_unnormalized_name = (
            structured_image_index_request_unnormalized.name
        )
        cls.structured_text_index_unnormalized_name = (
            structured_text_index_request_unnormalized.name
        )

        cls.unstructured_marqo_index_name = unstructured_image_index_request.name
        cls.unstructured_languagebind_index_name = (
            unstructured_languagebind_index_request.name
        )
        cls.unstructured_image_index_unnormalized_name = (
            unstructured_image_index_request_unnormalized.name
        )
        cls.unstructured_text_index_unnormalized_name = (
            unstructured_text_index_request_unnormalized.name
        )
    ...
    def test_add_documents_with_truncated_image(self):
        ...
        truncated_image_url = (
            "https://marqo-assets.s3.amazonaws.com/tests/images/truncated_image.jpg"
        )
        ...
        for index_name in [
            ...
            self.semi_structured_marqo_index_name,
            self.unstructured_marqo_index_name,
        ]:
            tensor_fields = (
                ["image_field_1", "text_field_1"]
                if index_name != self.structured_marqo_index_name
                else None
            )
            with self.subTest(
                f"test add documents with truncated image for {index_name}"
            ):
    ...
    def test_add_multimodal_single_documents(self):
        ...
        documents = [
            ...
            {"image_field_2": TestImageUrls.HIPPO_REALISTIC_LARGE.value, "_id": "3"},
            {
        ...
        for index_name in [
            self.structured_languagebind_index_name,
            self.semi_structured_languagebind_index_name,
            ...
            with self.subTest(index_name):
                res = self.add_documents(
                    ...
                    add_docs_params=AddDocsParams(
                        ...
                        if index_name != self.structured_languagebind_index_name
                        else None,
                    ),
                )
                print(res)
                ...
                for i, doc in enumerate(get_res["results"]):
                    ...
                    if i in [1, 3, 4]:
                        self.assertEqual(
                            len(tensor_facets),
                            1,
                            f"Document {i} should have 1 tensor facet",
                        )
                    elif i == 2:
                        # print(tensor_facets)
                        self.assertEqual(
                            len(tensor_facets),
                            10,
                            "Document 2 should have 10 tensor facets",
                        )
                    ...
                    for facet in tensor_facets:
                        ...
                        self.assertEqual(
                            len(embedding),
                            768,
                            f"Embedding length should be 768 for document {i}",
                        )
                        self.assertNotIn(
                            embedding,
                            embeddings,
                            f"Duplicate embedding found in document {i}",
                        )
    ...
    def test_imageRepoHandleThreadHandleError_successfully(self):
        ...
        documents = [{"image_field_1": TestImageUrls.HIPPO_REALISTIC.value, "_id": "1"}]
        ...
        for index_name in [
            ...
            with self.subTest(f"{index_name}-{error}"):
                with patch(
                    "marqo.s2_inference.clip_utils.requests.get", side_effect=error
                    ...
                    with self.assertRaises(Exception) as e:
                        ...
                        self.assertIn(
                            "Unexpected error during image download", str(e.exception)
                        )
    ...
    def test_image_url_is_embedded_as_image_not_text(self):
        ...
        """
        docs = [{"_id": "1", "image_field_1": TestImageUrls.IMAGE2.value}]
        for index_name in [
            self.structured_marqo_index_name,
            self.semi_structured_marqo_index_name,
            self.unstructured_marqo_index_name,
            ...
            with self.subTest(index_name):
                res = self.add_documents(
                    config=self.config,
                    add_docs_params=AddDocsParams(
                        index_name=index_name, docs=docs, tensor_fields=tensor_fields
                    ),
                )
    ...
    def test_imageDownloadWithoutPreprocessor(self):
        ...
        assert isinstance(
            media_repo["https://google.com/my_dog.png"], PIL.UnidentifiedImageError
        )
    ...
    def test_imageDownloadWithPreprocessor(self):
        ...
        add_docs.threaded_download_and_preprocess_content(
            ...
            preprocessors=Preprocessors(
                **{"image": lambda x: torch.randn(3, 224, 224)}
            ),
            device="cpu",
            marqo_index_type=IndexType.Unstructured,
            marqo_index_model=Model(name="test", properties={}),
    ...
    def test_image_download_timeout(self):
        ...
        def run():
            ...
            add_docs.threaded_download_and_preprocess_content(
                allocated_docs=[
                    {"Title": "frog", "Desc": "blah"},
                    {"Title": "Dog", "Loc": "https://google.com/my_dog.png"},
                ],
                media_repo=media_repo,
        ...
        docs_results = [
            (
                [
                    ...
                    {"_id": "789", "image_field_1": TestImageUrls.HIPPO_STATUE.value},
                    {
                        "_id": "456",
                        "image_field_1": "https://www.marqo.ai/this/image/doesnt/exist.png",
                    },
                ],
            ...
            (
                [
                    ...
                    {
                        "_id": "111",
                        "image_field_1": "https://www.marqo.ai/this/image/doesnt/exist2.png",
                    },
                ],
            ...
            (
                [
                    {
                        "_id": "505",
                        "image_field_1": "https://www.marqo.ai/this/image/doesnt/exist3.png",
                    },
                    {
                        "_id": "456",
                        "image_field_1": "https://www.marqo.ai/this/image/doesnt/exist.png",
                ...
                for docs, expected_results in docs_results:
                    with self.subTest(f"{expected_results} - {index_name}"):
                        ...
                        for i, res_dict in enumerate(add_res["items"]):
                            self.assertEqual(
                                expected_results[i][0], res_dict["_id"], res_dict
                            )
                            self.assertEqual(
                                expected_results[i][1], res_dict["status"], res_dict
                            )
    ...
    def test_threaded_download_images_non_tensor_field(self):
        ...
        examples = [
            (
                [{"field_1": bad_url, "field_2": good_url}],
                {bad_url: PIL.UnidentifiedImageError, good_url: types.ImageType},
            ),
            ([{"nt_1": bad_url, "nt_2": good_url}], {}),
            (
                [{"field_1": bad_url, "nt_1": good_url}],
                {
                    bad_url: PIL.UnidentifiedImageError,
                },
            ),
            ([{"nt_2": bad_url, "field_2": good_url}], {good_url: types.ImageType}),
        ]
        ...
        for index_name in [
            ...
            with self.subTest(index_name):
                ...
                for i in range(1, 4):
                    ...
                    self.assertIn(
                        "Document _id must be a string", r["items"][i]["error"]
                    )
    ...
    def test_imageIndexEmbeddingsUnnormalised(self):
        ...
        documents = [{"image_field_1": TestImageUrls.HIPPO_REALISTIC.value, "_id": "1"}]
        for index_name in [
            self.unstructured_image_index_unnormalized_name,
            self.structured_image_index_unnormalized_name,
        ]:
            tensor_fields = (
                ["image_field_1"]
                if index_name == self.unstructured_image_index_unnormalized_name
                else None
            )
            with self.subTest(index_name):
                res = self.add_documents(
    ...
    def test_imageIndexEmbeddingsNormalised(self):
        ...
        for index_name in [
            ...
            tensor_fields = (
                ["image_field_1"]
                if index_name == self.unstructured_marqo_index_name
                else None
            )
            with self.subTest(index_name):
                res = self.add_documents(
    ...
    def test_textIndexEmbeddingsUnnormalized(self):
        ...
        documents = [{"text_field_1": "This is a test text", "_id": "1"}]
        for index_name in [
            self.unstructured_text_index_unnormalized_name,
            self.structured_text_index_unnormalized_name,
        ]:
            tensor_fields = (
                ["text_field_1"]
                if index_name == self.unstructured_text_index_unnormalized_name
                else None
            )
            with self.subTest(index_name):
                res = self.add_documents(
    ...
    def test_add_private_images_proper_error_returned(self):
        ...
        for index_name in test_indexes:
            tensor_fields = (
                ["multimodal_field", "my_combination_field"]
                if (index_name == self.unstructured_marqo_index_name)
                else None
            )
            mappings = {
    ...
    def test_add_private_images_success(self):
        ...
        for index_name in test_indexes:
            tensor_fields = (
                ["image_field_1", "multimodal_field"]
                if (index_name == self.unstructured_marqo_index_name)
                else None
            )
            mappings = {
            ...
            with self.subTest(index_name):
                res = self.add_documents(
                    ...
                    add_docs_params=AddDocsParams(
                        ...
                        tensor_fields=tensor_fields,
                        media_download_headers={
                            "marqo_media_header": "media_header_test_key"
                        },
                        mappings=mappings,
                    ),
                )
...
class TestLanguageBindModelAddDocumentCombined(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.unstructured_language_bind_index_name = (
            unstructured_language_bind_index.name
        )
        cls.unstructured_custom_language_bind_index_name = (
            unstructured_custom_language_bind_index.name
        )
    ...
    def test_language_bind_model_can_add_all_media_modalities(self):
        ...
        for index_name in [
            self.structured_language_bind_index_name,
            self.unstructured_language_bind_index_name,
        ]:
            tensor_fields = (
                [
                    "text_field_1",
                ...
                if index_name == self.unstructured_language_bind_index_name
                else None
            )
            with self.subTest(index_name):
                res = self.add_documents(
    ...
    @patch.dict(
        "os.environ",
        {
            "MARQO_MAX_ADD_DOCS_VIDEO_AUDIO_FILE_SIZE": "2097152",
        },
    )
    def test_video_size_limit_in_batch(self):
        ...
        for index in [
            ...
            with self.subTest(f"Testing video size limit for index {index}"):
                tensor_fields = (
                    ["video_field_1", "text_field_1"]
                    if "unstructured" in index
                    else None
                )
                ...
                result = self.add_documents(
                    config=self.config,
                    add_docs_params=AddDocsParams(
                        index_name=index, docs=test_docs, tensor_fields=tensor_fields
                    ),
                ).dict(exclude_none=True, by_alias=True)
                ...
                self.assertTrue(
                    result["errors"]
                )  # Should have errors due to second document
                ...
                self.assertIn(
                    "exceeds the maximum allowed size", result["items"][1]["error"]
                )

                # Verify the first document was actually added
                get_result = tensor_search.get_documents_by_ids(
                    config=self.config, index_name=index, document_ids=["1"]
                ).dict(exclude_none=True, by_alias=True)
            ...
            for index in [
                ...
                with self.subTest(f"{index} - {audio_format}"):
                    self.clear_index_by_schema_name(
                        schema_name=self.index_management.get_index(
                            index_name=index
                        ).schema_name
                    )
                    self.assertEqual(
                    ...
                    document = {"audio_field_1": test_case, "_id": "1"}
                    ...
                    res = self.add_documents(
                        ...
                        add_docs_params=AddDocsParams(
                            index_name=index,
                            docs=[document],
                            tensor_fields=["audio_field_1"]
                            if index == self.unstructured_language_bind_index_name
                            else None,
                        ),
                    )
        ...
        for test_case, audio_format in test_cases:
            for index in [
                ...
                with self.subTest(f"{index} - {audio_format}"):
                    ...
                    document = {"video_field_1": test_case, "_id": "1"}
    ...
    def test_proper_error_is_raised_when_adding_documents_with_unsupported(self):
        ...
        self.assertIn(
            "The model does not support the requested modality.", str(cm.exception)
        )
