...
class TestAddDocumentsSemiStructured(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        image_index_with_random_model = cls.unstructured_marqo_index_request(
            model=Model(name="random"), treat_urls_and_pointers_as_images=True
        )
    ...
    def test_add_documents_validation(self):
        ...
        for use_existing_tensors_flag in (True, False):
            for bad_doc_arg in bad_doc_args:
                with self.subTest(
                    msg=f"{bad_doc_arg} - use_existing_tensors={use_existing_tensors_flag}"
                ):
                    add_res = self.add_documents(
                        config=self.config,
                        add_docs_params=AddDocsParams(
    ...
    def test_add_documents_list_data_type_validation(self):
        ...
        for bad_doc_arg in bad_doc_args:
            with self.subTest(bad_doc_arg):
                ...
                assert all(
                    [
                        "Unstructured Marqo index only supports string lists."
                        in item["message"]
                        for item in add_res["items"]
                    ]
                )
    ...
    def test_add_documents_empty(self):
        ...
        try:
            self.add_documents(
                config=self.config,
                add_docs_params=AddDocsParams(
                    index_name=self.default_text_index, docs=[], device="cpu"
                ),
            )
    ...
    def test_bad_tensor_fields(self):
        test_cases = [
            (
                {"tensor_fields": None},
                "tensor_fields must be explicitly provided",
                "None as tensor fields",
            ),
            ({}, "tensor_fields must be explicitly provided", "No tensor fields"),
            (
                {"tensor_fields": ["_id", "some"]},
                "`_id` field cannot be a tensor field",
                "_id can't be a tensor field",
            ),
        ]
    ...
    def test_supported_large_integer_and_float_number(self):
        ...
        test_case = [
            ...
            (
                {"double_field_1": 1e10 + 0.123249357987123},
                False,
    ...
    def test_duplicate_ids_behaviour(self):
        ...
        test_cases = [
            (
                [
                    {"_id": "1", "text_field": "test 1"},
                    {"_id": "1", "text_field": "test 2"},
                ],
                1,
                "Normal case",
            ),
    ...
    def test_a_text_index_will_treat_a_url_as_text(self):
        ...
        self.add_documents(
            ...
            add_docs_params=AddDocsParams(
                index_name=self.default_text_index,
                docs=[
                    {"_id": "1", "title": invalid_url, "non_tensor_field": valid_url}
                ],
                device="cpu",
                tensor_fields=["title"],
            ),
    ...
    def _assert_field_types(self, vespa_fields, field_names, field_types, id):
        for field_name, field_type in zip(field_names, field_types):
            self.assertEqual(
                vespa_fields["marqo__field_types"][field_name],
                field_type.value,
    ...
    def test_original_document_has_correct_field_types(self):
        ...
        for doc in [self.doc, self.doc2, self.doc3]:
            ...
            raw_vespa_doc = self.config.vespa_client.get_document(
                id, self.config.index_management.get_index(self.index.name).schema_name
            )
            ...
            field_types = [
                MarqoFieldTypes.INT_MAP,
                MarqoFieldTypes.INT_MAP,
                MarqoFieldTypes.FLOAT_MAP,
                MarqoFieldTypes.FLOAT_MAP,
                MarqoFieldTypes.BOOL,
                MarqoFieldTypes.STRING,
                MarqoFieldTypes.STRING,
                MarqoFieldTypes.INT,
                MarqoFieldTypes.FLOAT,
                MarqoFieldTypes.BOOL,
                MarqoFieldTypes.TENSOR,
                MarqoFieldTypes.TENSOR,
                MarqoFieldTypes.TENSOR,
                MarqoFieldTypes.TENSOR,
            ]
            ...
            if id in ["1", "2"]:
                self._assert_field_types(
                    ...
                    [MarqoFieldTypes.STRING_ARRAY, MarqoFieldTypes.STRING_ARRAY],
                    id,
                )
            if id in ["2", "3"]:
            ...
            if id == "2":
                self._assert_field_types(
                    vespa_fields, ["lexical_field"], [MarqoFieldTypes.STRING], id
                )
