...
class TestStemmingIntegration(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        index_requests = [
            cls.stemming_index,
            cls.v216_unstructured_index,
            cls.v215_unstructured_index,
        ]
    ...
    def populate_index(self):
        ...
        res = self.add_documents(config=self.config, add_docs_params=add_docs_params)
    ...
    def test_stemming_search(self):
        ...
        for query, fields, expected_ids, description in cases:
            for search_method, hybrid_params, search_description in search_configs:
                with self.subTest(
                    f"{search_method} search for '{query}' in {fields}: {description}"
                ):
                    ...
                    self.assertEqual(
                        set(expected_ids),
                        actual_ids,
                        f"Failed for query '{query}' in fields {fields}",
                    )
    ...
    def test_stemming_invalid_value_error(self):
        ...
        with self.assertRaises(Exception) as cm:
            self.add_documents(config=self.config, add_docs_params=add_docs_params)
    ...
    def test_stemming_no_language(self):
        ...
        docs = [
            {"_id": "no_lang_1", "content": "apples"},
            {"_id": "no_lang_2", "content": "apple"},
        ]
        ...
        self.assertFalse(
            res.errors, "Should not have errors when adding documents without language"
        )
        ...
        self.assertEqual(
            {"no_lang_1"}, apples_ids, "Should find document with 'running'"
        )
    ...
    def test_stemming_not_specified(self):
        ...
        mappings1 = {
            "text_field": {"type": "text_field", "language": "en", "stemming": "none"}
        }
        ...
        mappings2 = {
            "text_field": {"type": "text_field", "language": "en"}
        }  # No stemming specified
        ...
        self.assertEqual(
            {"doc1"},
            apples_ids,
            "Should find first document with exact match 'running'",
        )
        self.assertEqual(
            {"doc2"}, apple_ids, "Should find second document with exact match 'runs'"
        )
    ...
    def test_vespa_schema_contains_language_and_stemming(self):
        ...
        mappings = {
            "schema_field": {"type": "text_field", "language": "en", "stemming": "best"}
        }
        ...
        marqo_index = self.config.vespa_client.get_index_setting_by_name(
            self.stemming_index.name
        )
        self.assertIsNotNone(
            marqo_index, "Should be able to retrieve the index settings"
        )
        ...
        try:
            schema_content = self.config.vespa_client.get_text_content(
                content_base_url, "/schemas/", schema_file_name
            )
        ...
        for line in schema_content.split("\n"):
            if "field marqo__lexical_schema_field" in line and "type string" in line:
        ...
        self.assertTrue(
            schema_field_found,
            "Should find the schema_field in the generated Vespa schema",
        )
    ...
    def test_v216_unstructured_index_supports_stemming_mapping(self):
        ...
        docs = [
            {
                "_id": "v216_doc1",
                "title": "nacionalmente",
                "content": "German word example",
            },
            {
                "_id": "v216_doc2",
                "title": "andere Wörter",
                "content": "Other German words",
            },
        ...
        self.assertFalse(
            response.errors,
            "Failed to add documents with stemming mapping to v2.16 index",
        )
        ...
        self.assertGreater(
            len(result["hits"]), 0, "Expected hits for stemmed search on v2.16 index"
        )
        hit_ids = [hit["_id"] for hit in result["hits"]]
        self.assertIn(
            "v216_doc1",
            hit_ids,
            "Expected v216_doc1 in search results for stemmed query",
        )
