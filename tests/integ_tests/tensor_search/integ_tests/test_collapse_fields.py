...
from marqo.tensor_search.models.sort_by_model import SortByField, SortByModel
...
class TestCollapseFields(MarqoTestCase):
    ...
    def test_add_documents_mixed_batch_with_collapse_field_errors(self):
        ...
        docs = [
            ...
            {
                "_id": "invalid2",
                "title": "Invalid document - wrong type",
                "parent_id": 456,
            },
        ]
        ...
        self.assertIn(
            "Document missing required field 'parent_id'", failed_items[0].message
        )
        self.assertIn(
            "Field 'parent_id' must be of type string", failed_items[1].message
        )
        ...
        valid_docs = tensor_search.get_documents_by_ids(
            config=self.config,
    ...
    def test_partial_update_of_collapse_field_does_not_work(self):
        ...
        self.assertIn(
            "parent_id of type str does not exist in the original document. "
    ...
    def test_search_with_invalid_collapse_field_raises_error(self):
        ...
        self.assertIn(
            "Field 'non_existent_field' is not configured as a collapse field for this index",
            str(cm.exception),
        )
    ...
    def test_search_with_valid_collapse_field_succeeds(self):
        ...
        for retrieval_method, ranking_method in test_cases:
            with self.subTest(
                ...
                self.assertEqual(
                    set(range(5)), set([hit["group"] for hit in res["hits"]])
                )
    ...
    def test_filter(self):
        ...
        for retrieval_method, ranking_method in test_cases:
            with self.subTest(
                ...
                self.assertEqual(
                    3, len(res["hits"])
                )  # there's only 5 groups, so at most 5 results
        ...
        docs = [
            {
                ...
                "rating": int(g + 1)
                if g % 2 == 0
                else float(g + 1.5),  # mix of float and int
                "color": colors[i % 5],
            }
            for i in range(10)
            for g in range(5)
    ...
    @pytest.mark.skip_for_multinode(
        "Pagination result is not consistent across different Vespa infrastructures"
    )
    def test_pagination(self):
        ...
        for retrieval_method, ranking_method in test_cases:
            with self.subTest(
                ...
                page_1_res_groups = set(
                    [hit["parent_id"] for hit in page_1_res["hits"]]
                )
                ...
                page_2_res_groups = set(
                    [hit["parent_id"] for hit in page_2_res["hits"]]
                )
    ...
    def test_sort_by(self):
        ...
        self.assertListEqual(
            ["group_2", "group_1", "group_0"], [hit["parent_id"] for hit in res["hits"]]
        )
    ...
    def test_score_modifiers(self):
        ...
        score_modifiers = ScoreModifierLists(
            multiply_score_by=[ScoreModifierOperator(field_name="rating", weight=1)]
        )
            ...
            with self.subTest(
                ...
                res = tensor_search.search(
                    ...
                    hybrid_parameters=HybridParameters(
                        ...
                        if ranking_method in [RankingMethod.Tensor, RankingMethod.RRF]
                        else None,
                        scoreModifiersLexical=score_modifiers
                        if ranking_method != RankingMethod.Tensor
                        or retrieval_method != RetrievalMethod.Tensor
                        else None,
                    ),
                    result_count=6,
                    collapse_field_name="parent_id",
    ...
    def test_filter_by_collapse_field(self):
        ...
        self.assertEqual(
            set([f"doc1{i:02}" for i in range(5)]),
            set([hit["_id"] for hit in hybrid_res["hits"]]),
        )
        ...
        self.assertEqual(
            set([f"doc1{i:02}" for i in range(5)]),
            set([hit["_id"] for hit in lexical_res["hits"]]),
        )
