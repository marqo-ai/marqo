...
from marqo.tensor_search import delete_docs, enums, tensor_search
from marqo.tensor_search.models.delete_docs_objects import (
    MqDeleteDocsRequest,
    MqDeleteDocsResponse,
)
...
class TestDeleteDocuments(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        unstructured_text_index_with_random_model_request = (
            cls.unstructured_marqo_index_request(model=Model(name="random"))
        )
    ...
    def setUp(self) -> None:
        ...
        self.device_patcher = patch.dict(
            ...
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                self.add_documents(
                    ...
                    add_docs_params=AddDocsParams(
                        ...
                        docs=[
                            {
                                "field1": "cat dog sat mat",
                                "field2": "Australia contains Sydney",
                            },
                ...
                count0_res = self.monitoring.get_index_stats_by_name(
                    index.name
                ).number_of_documents
                ...
                count1_res = self.monitoring.get_index_stats_by_name(
                    index.name
                ).number_of_documents

                marqo.tensor_search.tensor_search.delete_documents(
                    config=self.config, index_name=index.name, doc_ids=["455", "at-at"]
                )

                count_post_delete = self.monitoring.get_index_stats_by_name(
                    index.name
                ).number_of_documents
    ...
    def test_delete_docs_format(self):
        for index in self.indexes:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                ...
                res = marqo.tensor_search.tensor_search.delete_documents(
                    config=self.config, doc_ids=["5678", "491"], index_name=index.name
                )
                ...
                self.assertEqual(
                    res["details"]["deletedDocuments"], 2
                )  # 491 is counted in deletedDocuments count, even if it doesn't exist
    ...
    def test_delete_multiple_documents(self):
        for index in self.indexes:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                ...
                remaining_document = tensor_search.get_document_by_id(
                    config=self.config, index_name=index.name, document_id="doc_id_2"
                )
        ...
        for index in self.indexes:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                ...
                tensor_search.delete_documents(
                    config=self.config,
                    index_name=index.name,
                    doc_ids=["unique_id_1", "unique_id_2"],
                )
        ...
        for index in self.indexes:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                response = tensor_search.delete_documents(
                    config=self.config,
                    index_name=index.name,
                    doc_ids=["non_existent_id"],
                )

                self.assertEqual(
                    response["details"]["deletedDocuments"], 1
                )  # Vespa still returns a 200, so the doc is counted as deleted
        ...
        for index in self.indexes:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                ...
                response = tensor_search.delete_documents(
                    config=self.config,
                    index_name=index.name,
                    doc_ids=["doc_id_1", "invalid_id"],
                )

                self.assertEqual(
                    response["details"]["deletedDocuments"], 2
                )  # Vespa still returns a 200, so the doc is counted as deleted
                self.assertEqual(response["details"]["receivedDocumentIds"], 2)
                self.assertEqual(response["status"], "succeeded")
                self.assertEqual(response["index_name"], index.name)
...
class TestDeleteDocumentsEndpoint(MarqoTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.mock_result_list = [
            {"delete": {"_id": "1", "result": "deleted", "status": 200}},
            {"delete": {"_id": "2", "result": "deleted", "status": 200}},
    ...
    def test_format_delete_docs_response(self):
        for index in self.indexes:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                ...
                self.assertEqual(
                    formatted_response["details"]["receivedDocumentIds"], 3
                )
                self.assertEqual(formatted_response["details"]["deletedDocuments"], 3)
                self.assertEqual(formatted_response["duration"], "PT5.0S")
                self.assertEqual(
                    formatted_response["startedAt"], "2023-04-17T00:00:00Z"
                )
                self.assertEqual(
                    formatted_response["finishedAt"], "2023-04-17T00:00:05Z"
                )
        ...
        for index in self.indexes:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                request = MqDeleteDocsRequest(
                    index_name=index.name,
                    schema_name=index.name,
                    document_ids=["1", "2", "3"],
                )

                with patch(
                    "marqo.tensor_search.delete_docs.delete_documents_vespa"
                ) as mock_delete_documents_vespa:
                    ...
                    mock_delete_documents_vespa.assert_called_once_with(
                        config=config_copy, deletion_instruction=request
                    )
    ...
    def test_delete_documents_vespa(self):
        ...
        for index in self.indexes:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                ...
                with patch(
                    "marqo.vespa.vespa_client.VespaClient.delete_batch"
                ) as mock_delete_batch:
                    mock_delete_batch.return_value = DeleteBatchResponse(
                        responses=[
                            DeleteBatchDocumentResponse(
                                status=200,
                                pathId="/document/v1/mytestindex1/mytestindex1/docid/1",
                                id="id:mytestindex1:mytestindex1::1",
                                message=None,
                            ),
                            DeleteBatchDocumentResponse(
                                status=200,
                                pathId="/document/v1/mytestindex1/mytestindex1/docid/2",
                                id="id:mytestindex1:mytestindex1::2",
                                message=None,
                            ),
                            DeleteBatchDocumentResponse(
                                status=200,
                                pathId="/document/v1/mytestindex1/mytestindex1/docid/3",
                                id="id:mytestindex1:mytestindex1::3",
                                message=None,
                            ),
                        ],
                        errors=False,
                    ...
                    self.assertEqual(
                        ...
                        [item["delete"] for item in self.mock_result_list],
                    )
    ...
    def test_format_delete_docs_response_valid_input(self):
        for index in self.indexes:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                ...
                formatted_response = delete_docs.format_delete_docs_response(
                    mq_delete_res
                )
                expected_response = {
                    "index_name": "test_index",
    ...
    def test_max_doc_delete_limit(self):
        ...
        def run():
            ...
            for index in self.indexes:
                with self.subTest(
                    f"Index type: {index.type}. Index name: {index.name}"
                ):
                    add_docs_batched(
                        config=self.config,
                        index_name=index.name,
                    ...
                    try:
                        tensor_search.delete_documents(
                            config=self.config, index_name=index.name, doc_ids=doc_ids
                        )
                        raise AssertionError
                    except api_exceptions.InvalidArgError:
                        pass
                    # under the limit
                    ...
                    self.assertEqual(
                        update_res["details"]["receivedDocumentIds"],
                        update_res["details"]["deletedDocuments"],
                    )
    ...
    def test_max_doc_delete_default_limit(self):
        ...
        def run():
            self.assertEqual(
                default_limit,
                tensor_search.utils.read_env_vars_and_defaults_ints(
