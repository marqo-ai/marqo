...
from marqo.vespa.models.query_result import Child, Root, RootFields
...
class TestEmbed(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.mock_query_result = QueryResult(
            root=Root(
                ...
                children=[
                    Child(
                        id="index:content_default/0/674f3c2ce3ae7b71e3e805be",
                        relevance=0.5975484726281337,
                        ...
                        fields={
                            ...
                            "sddocname": "a06babea28d0f408ebb0e7be130f86f40",
                            "marqo__id": "5678",
                            "marqo__strings": ["Exact match hehehe efgh "],
                            "marqo__long_string_fields": {
                                "abc": "Exact match hehehe efgh "
                            },
                            "marqo__short_string_fields": {},
                            "marqo__chunks": ["abc::Exact match hehehe efgh"],
                        },
                    )
                ],
                fields=RootFields(total_count=1),
            ),
        )
        ...
        unstructured_image_index_with_test_prefix = (
            cls.unstructured_marqo_index_request(
                model=Model(name="test_prefix"), treat_urls_and_pointers_as_images=True
            )
        )
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                for expected_device in expected_devices:
                    with patch.dict(
                        os.environ, {"MARQO_BEST_AVAILABLE_DEVICE": expected_device}
                    ):
                        with patch(
                            "marqo.tensor_search.tensor_search.run_vectorise_pipeline"
                        ) as mock_vectorise:
                        ...
                        # Check the result
                        self.assertEqual(
                            embed_res["content"], ["This is a test document"]
                        )
    ...
    def test_embed_image_url_as_image_not_text(self):
        ...
        for index in [
            self.unstructured_image_index_request,
            self.structured_image_index_request,
        ]:
            with self.subTest(index=index.type):
                embed_res = embed(
                    marqo_config=self.config,
                    index_name=index.name,
                    embedding_request=EmbedRequest(content=[image_url]),
                    device="cpu",
                )
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                embed_res = embed(
                    marqo_config=self.config,
                    index_name=index.name,
                    embedding_request=EmbedRequest(content=content),
                    device=None,
                )
                # print(embed_res)
                ...
                expected = [
                    0.0298048947006464,
                    0.05226955562829971,
                    -0.0038126774597913027,
                    0.061151087284088135,
                    -0.013925471343100071,
                    0.060153547674417496,
                    -0.0031225811690092087,
                ]
            ...
            with self.subTest(index=index.type):
                add_docs_res = self.add_documents(
                    ...
                    add_docs_params=AddDocsParams(
                        index_name=index.name,
                        docs=[{"_id": "0", "text_field_1": "I am the GOAT."}],
                        device="cpu",
                ...
                get_docs_embedding = get_docs_res[enums.TensorField.tensor_facets][0][
                ...
                embed_res = embed(
                    marqo_config=self.config,
                    index_name=index.name,
                    embedding_request=EmbedRequest(content=["I am the GOAT."]),
                    device="cpu",
                )
                ...
                self.assertTrue(
                    np.allclose(embed_res["embeddings"][0], get_docs_embedding)
                )
    ...
    def test_embed_equivalent_to_search_text(self):
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                search_query_embedding = vespa_query_kwargs["query_features"][
                ...
                self.assertEqual(embed_res["content"], ["I am the GOAT."])
                self.assertTrue(
                    np.allclose(embed_res["embeddings"][0], search_query_embedding)
                )
    ...
    def test_embed_with_media_download_headers_and_model_auth(self):
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                @unittest.mock.patch(
                    "marqo.s2_inference.s2_inference.vectorise", mock_vectorise
                )
                def run():
                    embed_res = embed(
                        marqo_config=self.config,
                ...
                self.assertEqual(
                    vectorise_kwargs["media_download_headers"],
                    {"Authorization": "my secret key"},
                )
                self.assertEqual(
                    vectorise_kwargs["model_auth"],
                    ModelAuth(
                        s3=S3Auth(
                            aws_access_key_id="12345",
                            aws_secret_access_key="this-is-a-secret",
    ...
    def test_embed_equivalent_to_search_weighted_dict(self):
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                self.assertEqual(
                    embed_res["content"],
                    [{"I am the GOATest of all time.": 0.7, image_url: 0.3}],
                )
                self.assertTrue(
                    np.allclose(embed_res["embeddings"][0], search_query_embedding)
                )
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                sample_content_list = [
                    ...
                    "GOAT #2",
                    image_url,
                    {"The inner GOAT.": 0.65, image_url: 0.42},
                ]
                ...
                for item in sample_content_list:
                    ...
                    vespa_query_kwargs = call_args[0].kwargs
                    search_query_embeddings.append(
                ...
                embed_res = embed(
                    marqo_config=self.config,
                    index_name=index.name,
                    embedding_request=EmbedRequest(content=sample_content_list),
                    device="cpu",
                )
                ...
                for i in range(len(sample_content_list)):
                    self.assertTrue(
                        np.allclose(
                            embed_res["embeddings"][i],
                            search_query_embeddings[i],
                            atol=1e-6,
                        )
                    )
            ...
            with self.subTest(index=index.type):
                with self.assertRaises(ValidationError) as e:
                    embed_res = embed(
                        marqo_config=self.config,
                        index_name=index.name,
                        embedding_request=EmbedRequest(content=[]),
                        device="cpu",
                    )
    ...
    def test_embed_invalid_content_type_fails(self):
        test_cases = [
            (
                {"key": "not a number"},
                "not a valid float",
            ),  # dict with wrong typed value
            (
                [{"key": "value"}],
                "not a valid float",
            ),  # list of dict with wrong typed value
        ]
        for index in [
            self.unstructured_default_text_index,
            self.structured_default_text_index,
    ...
    def test_embed_prefix_content_type(self):
        ...
        """
        for index in [
            self.unstructured_image_index_with_test_prefix,
            self.structured_image_index_with_test_prefix,
        ]:
            with self.subTest(index=index.type):
                ...
                embed_res_prefix_query = embed(
                    ...
                    index_name=index.name,
                    embedding_request=EmbedRequest(
                        content=["I am the GOAT."], content_type="query"
                    ),
                    device="cpu",
                )
                ...
                # Assert vectors are equal
                self.assertEqual(
                    embed_res_hardcoded["content"], ["test query: I am the GOAT."]
                )
                self.assertTrue(
                    np.allclose(
                        embed_res_hardcoded["embeddings"][0],
                        embed_res_prefix_query["embeddings"][0],
                    )
                )
                ...
                embed_res_hardcoded = embed(
                    ...
                    index_name=index.name,
                    embedding_request=EmbedRequest(
                        content=["test passage: I am the GOAT."], content_type=None
                    ),
                    device="cpu",
                )
                ...
                embed_res_prefix_query = embed(
                    ...
                    index_name=index.name,
                    embedding_request=EmbedRequest(
                        content=["I am the GOAT."], content_type="document"
                    ),
                    device="cpu",
                )
                ...
                # Assert vectors are equal
                self.assertEqual(
                    embed_res_hardcoded["content"], ["test passage: I am the GOAT."]
                )
                self.assertTrue(
                    np.allclose(
                        embed_res_hardcoded["embeddings"][0],
    ...
    def test_embed_private_image_proper_error_raised(self):
        ...
        for index_name in [
            self.unstructured_default_image_index.name,
            self.structured_default_image_index.name,
        ]:
                ...
                with self.subTest(f"{index_name} - {msg}"):
                    with self.assertRaises(InvalidArgError) as e:
                        embed_res = embed(
                            ...
                            embedding_request=EmbedRequest(content=test_content),
                            device="cpu",
                        )
