...
from marqo.tensor_search.models.api_models import CustomVectorQuery, ScoreModifierLists
...
class TestHybridSearch(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        semi_structured_default_image_index = cls.unstructured_marqo_index_request(
            ...
            ),  # Used to be ViT-B/32 in old structured tests
        ...
        semi_structured_index_with_no_model = cls.unstructured_marqo_index_request(
            model=Model(
                name="no_model",
                properties={"dimensions": 16, "type": "no_model"},
                custom=True,
            ),
        ...
        semi_structured_text_index_2_14 = cls.unstructured_marqo_index_request(
            model=Model(name="hf/all-MiniLM-L6-v2"), marqo_version="2.14.0"
        )

        # Legacy UNSTRUCTURED indexes
        unstructured_default_text_index = cls.unstructured_marqo_index_request(
            model=Model(name="hf/all-MiniLM-L6-v2"), marqo_version="2.12.0"
        )
        ...
        structured_index_empty = cls.structured_marqo_index_request(
            model=Model(name="hf/all-MiniLM-L6-v2"), fields=[], tensor_fields=[]
        )

        structured_index_2_9 = cls.structured_marqo_index_request(
            marqo_version="2.9.0", fields=[], tensor_fields=[]
        )
    ...
    def test_hybrid_search_calls_correct_vespa_query(self):
        ...
        for index in [
            self.structured_text_index_score_modifiers,
            self.semi_structured_default_text_index,
            ...
            with self.subTest(index=index.name):
                ...
                @unittest.mock.patch(
                    "marqo.vespa.vespa_client.VespaClient.query",
                    mock_vespa_client_query,
                )
                def run():
                    res = tensor_search.search(
                        ...
                        hybrid_parameters=HybridParameters(
                            ...
                            if isinstance(
                                index, (StructuredMarqoIndex, SemiStructuredMarqoIndex)
                            )
                            else None,
                ...
                self.assertIn(
                    "PLACEHOLDER. WILL NOT BE USED IN HYBRID SEARCH.",
                    vespa_query_kwargs["yql"],
                )
                self.assertEqual(
                    vespa_query_kwargs["marqo__hybrid.retrievalMethod"],
                    RetrievalMethod.Disjunction,
                )
                self.assertEqual(
                    vespa_query_kwargs["marqo__hybrid.rankingMethod"], RankingMethod.RRF
                )
                ...
                self.assertEqual(
                    vespa_query_kwargs["ranking"], "hybrid_custom_searcher"
                )
                self.assertEqual(
                    vespa_query_kwargs["marqo__ranking.lexical.lexical"], "bm25"
                )
                self.assertEqual(
                    vespa_query_kwargs["marqo__ranking.tensor.tensor"],
                    "embedding_similarity",
                )
                self.assertEqual(
                    vespa_query_kwargs["marqo__ranking.lexical.tensor"],
                    "hybrid_bm25_then_embedding_similarity",
                )
                self.assertEqual(
                    vespa_query_kwargs["marqo__ranking.tensor.lexical"],
                    "hybrid_embedding_similarity_then_bm25",
                )

                self.assertEqual(
                    vespa_query_kwargs["query_features"]["marqo__mult_weights_lexical"],
                    {"mult_field_1": 1.0, "mult_field_2": 1.0},
                )
                self.assertEqual(
                    vespa_query_kwargs["query_features"]["marqo__add_weights_lexical"],
                    {},
                )
                self.assertEqual(
                    vespa_query_kwargs["query_features"]["marqo__mult_weights_tensor"],
                    {"mult_field_1": 1.0},
                )
                self.assertEqual(
                    vespa_query_kwargs["query_features"]["marqo__add_weights_tensor"],
                    {"add_field_1": 1.0},
                )
                ...
                if isinstance(index, (StructuredMarqoIndex, SemiStructuredMarqoIndex)):
                    ...
                    self.assertEqual(
                        vespa_query_kwargs["query_features"][
                            "marqo__fields_to_rank_lexical"
                        ],
                        {
                            "marqo__lexical_text_field_1": 1,
                            "marqo__lexical_text_field_2": 1,
                        },
                    )
                    self.assertEqual(
                        vespa_query_kwargs["query_features"][
                        ...
                        {
                            "marqo__embeddings_text_field_2": 1,
                            "marqo__embeddings_text_field_3": 1,
                        },
                    )
                    # global rerankDepth & score modifiers are not available for legacy unstructured
                    self.assertEqual(
                        vespa_query_kwargs["marqo__hybrid.rerankDepthGlobal"], 3
                    )
    ...
    def test_hybrid_search_structured_with_custom_vector_query(self):
        ...
        with self.subTest("Custom vector query, with content, no context"):
            ...
            @unittest.mock.patch(
                "marqo.vespa.vespa_client.VespaClient.query", mock_vespa_client_query
            )
            def run():
                res = tensor_search.search(
                    config=self.config,
                    ...
                    text=CustomVectorQuery(
                        customVector=CustomVectorQuery.CustomVector(
                            content="sample", vector=sample_vector
                        )
                    ),
                    search_method="HYBRID",
                    hybrid_parameters=HybridParameters(
            ...
            self.assertIn(
                'marqo__lexical_text_field_1 contains "sample"',
                vespa_query_kwargs["marqo__yql.lexical"],
            )
            self.assertEqual(
                vespa_query_kwargs["query_features"]["marqo__query_embedding"],
                sample_vector,
            )
            self.assertIn("hits", res)
        ...
        with self.subTest("Custom vector query, with context, with content"):
                    ...
                    context=SearchContext(
                        **{
                            "tensor": [
                                {
                                    "vector": [
                                        i * 2 for i in sample_vector
                                    ],  # Double the sample vector
                                    "weight": 1,
                                }
                            ],
                        }
            ...
            self.assertEqual(
                vespa_query_kwargs["query_features"]["marqo__query_embedding"],
                [i * 1.5 for i in sample_vector],
            )  # Should average the query & context vectors
            self.assertIn("hits", res)
        ...
        with self.subTest("Custom vector query, no content, no context, tensor/tensor"):
            ...
            def run():
                res = tensor_search.search(
                    ...
                    text=CustomVectorQuery(
                        customVector=CustomVectorQuery.CustomVector(
                            content=None, vector=sample_vector
                        )
                    ),
                    search_method="HYBRID",
                    hybrid_parameters=HybridParameters(
    ...
    def test_hybrid_search_semi_structured_with_custom_vector_query(self):
        ...
        with self.subTest("Custom vector query, with content, no context"):
            ...
            # Lexical yql has content text, but Tensor uses the sample vector
            self.assertIn(
                'default contains "sample"', vespa_query_kwargs["marqo__yql.lexical"]
            )
            self.assertEqual(
                vespa_query_kwargs["query_features"]["marqo__query_embedding"],
                sample_vector,
        ...
        for index in [
            ...
            with self.subTest(index=type(index)):
                ...
                for i in range(len(hybrid_res["hits"])):
                    self.assertEqual(
                        hybrid_res["hits"][i]["_id"], lexical_res["hits"][i]["_id"]
                    )
                    self.assertEqual(
                        hybrid_res["hits"][i]["_lexical_score"],
                        lexical_res["hits"][i]["_score"],
                    )
    ...
    def test_hybrid_search_disjunction_rrf_one_alpha_same_as_tensor(self):
        ...
        for index in [
            ...
            with self.subTest(index=index.name):
                ...
                for i in range(len(hybrid_res["hits"])):
                    self.assertEqual(
                        hybrid_res["hits"][i]["_id"], tensor_res["hits"][i]["_id"]
                    )
                    self.assertEqual(
                        hybrid_res["hits"][i]["_tensor_score"],
                        tensor_res["hits"][i]["_score"],
                    )
        ...
        for index in [
            ...
            with self.subTest(index=index.name):
                ...
                with self.subTest("retrieval: disjunction, ranking: rrf"):
                    ...
                    self.assertEqual(
                        len(hybrid_res["hits"]), 3
                    )  # Only 3 documents have text_field_2 at all
                    self.assertEqual(
                        hybrid_res["hits"][0]["_id"], "doc12"
                    )  # puppies puppies in text field 2
                ...
                with self.subTest("retrieval: lexical, ranking: tensor"):
                    ...
                    self.assertIn("hits", hybrid_res)
                    self.assertEqual(
                        len(hybrid_res["hits"]), 1
                    )  # Only 1 document has puppies in text_field_2. Lexical retrieval will only get this one.
                ...
                with self.subTest("retrieval: tensor, ranking: lexical"):
                    ...
                    self.assertEqual(
                        len(hybrid_res["hits"]), 3
                    )  # Only 3 documents have text field 2. Tensor retrieval will get them all.
                    self.assertEqual(hybrid_res["hits"][0]["_id"], "doc12")
                    # doc11 and doc13 has score 0, so their order is non-deterministic
                    self.assertSetEqual(
                        {"doc11", "doc13"},
                        {hit["_id"] for hit in hybrid_res["hits"][1:]},
                    )
        ...
        for index in [
            ...
            with self.subTest(index=type(index)):
                ...
                with self.subTest("retrieval: lexical, ranking: tensor"):
                    ...
                    self.assertIn("hits", hybrid_res)
                    self.assertEqual(
                        hybrid_res["hits"][0]["_id"], "doc9"
                    )  # highest score (score*10*3)
                    self.assertEqual(hybrid_res["hits"][0]["_score"], 30.0)
                    self.assertEqual(
                        hybrid_res["hits"][1]["_id"], "doc8"
                    )  # (score*10*2)
                    self.assertEqual(hybrid_res["hits"][1]["_score"], 20.0)
                    self.assertEqual(
                        hybrid_res["hits"][2]["_id"], "doc7"
                    )  # (score + 5*1)
                    ...
                    self.assertEqual(
                        hybrid_res["hits"][-1]["_id"], "doc10"
                    )  # lowest score (score*-10*3)
                ...
                with self.subTest("retrieval: tensor, ranking: lexical"):
                    ...
                    self.assertAlmostEqual(
                        hybrid_res["hits"][0]["_score"], base_lexical_score * 10 * 3
                    )
                    self.assertEqual(
                        hybrid_res["hits"][1]["_id"], "doc8"
                    )  # second highest score (score*10*2)
                    self.assertAlmostEqual(
                        hybrid_res["hits"][1]["_score"], base_lexical_score * 10 * 2
                    )
                    self.assertEqual(
                        hybrid_res["hits"][2]["_id"], "doc7"
                    )  # third highest score (score + 2*1)
                    self.assertAlmostEqual(
                        hybrid_res["hits"][2]["_score"], base_lexical_score + 2 * 1
                    )
                    self.assertEqual(
                        hybrid_res["hits"][3]["_id"], "doc6"
                    )  # ORIGINAL SCORE
                    self.assertEqual(
                        hybrid_res["hits"][-1]["_id"], "doc10"
                    )  # lowest score (score*-10*3)
                    self.assertAlmostEqual(
                        hybrid_res["hits"][-1]["_score"], base_lexical_score * -10 * 3
                    )
            ...
            with self.subTest(index=type(index)):
                ...
                hybrid_res = tensor_search.search(
                    ...
                    score_modifiers=ScoreModifierLists(
                        ...
                        add_to_score=[{"field_name": "add_field_1", "weight": 5}],
                    ),
                    hybrid_parameters=HybridParameters(
                        retrievalMethod=RetrievalMethod.Disjunction,
                        rankingMethod=RankingMethod.RRF,
                ...
                self.assertAlmostEqual(
                    hybrid_res["hits"][0]["_lexical_score"],
                    base_lexical_score * 1000 * 3,
                )
                self.assertAlmostEqual(
                    hybrid_res["hits"][0]["_tensor_score"], base_tensor_score * 1000 * 3
                )
                self.assertAlmostEqual(
                    hybrid_res["hits"][0]["_score"],
                    unmodified_rrf_scores["doc9"] * 1000 * 3,
                )

                self.assertEqual(hybrid_res["hits"][1]["_id"], "doc8")  # (score*10*2)
                self.assertAlmostEqual(
                    hybrid_res["hits"][1]["_lexical_score"],
                    base_lexical_score * 1000 * 2,
                )
                self.assertAlmostEqual(
                    hybrid_res["hits"][1]["_tensor_score"], base_tensor_score * 1000 * 2
                )
                self.assertAlmostEqual(
                    hybrid_res["hits"][1]["_score"],
                    unmodified_rrf_scores["doc8"] * 1000 * 2,
                )

                self.assertEqual(hybrid_res["hits"][2]["_id"], "doc7")  # (score + 5*1)
                self.assertAlmostEqual(
                    hybrid_res["hits"][2]["_lexical_score"], base_lexical_score + 5 * 1
                )
                self.assertAlmostEqual(
                    hybrid_res["hits"][2]["_tensor_score"], base_tensor_score + 5 * 1
                )
                self.assertAlmostEqual(
                    hybrid_res["hits"][2]["_score"],
                    unmodified_rrf_scores["doc7"] + 5 * 1,
                )
                ...
                self.assertAlmostEqual(
                    hybrid_res["hits"][-1]["_tensor_score"],
                    base_tensor_score * -1000 * 3,
                )
                self.assertAlmostEqual(
                    hybrid_res["hits"][-1]["_score"],
                    unmodified_rrf_scores["doc10"] * -1000 * 3,
                )
    ...
    def test_hybrid_search_global_score_modifiers(self):
        ...
        for index in [
            ...
            with self.subTest(index=type(index)):
                ...
                self.assertEqual(
                    modified_res["hits"][0]["_id"], "doc9"
                )  # highest score (score*1000*3)
                self.assertAlmostEqual(
                    modified_res["hits"][0]["_score"],
                    unmodified_scores["doc9"] * 1000 * 3,
                )

                self.assertEqual(
                    modified_res["hits"][1]["_id"], "doc8"
                )  # (score*1000*2)
                self.assertAlmostEqual(
                    modified_res["hits"][1]["_score"],
                    unmodified_scores["doc8"] * 1000 * 2,
                )

                self.assertEqual(
                    modified_res["hits"][2]["_id"], "doc7"
                )  # (score + 5*1)
                self.assertAlmostEqual(
                    modified_res["hits"][2]["_score"], unmodified_scores["doc7"] + 5 * 1
                )

                self.assertEqual(modified_res["hits"][3]["_id"], "doc6")  # (score)
                self.assertEqual(
                    modified_res["hits"][3]["_score"], unmodified_scores["doc6"]
                )

                self.assertEqual(
                    modified_res["hits"][-1]["_id"], "doc10"
                )  # lowest score (score*-1000*3)
                self.assertAlmostEqual(
                    modified_res["hits"][-1]["_score"],
                    unmodified_scores["doc10"] * -1000 * 3,
                )
                ...
                with self.subTest("Only multiply_score_by"):
                    ...
                    self.assertSetEqual(
                        {"doc6", "doc7"},
                        {hit["_id"] for hit in modified_res["hits"][2:4]},
                    )
    ...
    def test_hybrid_search_global_score_modifiers_with_rerank_depth(self):
        ...
        for index in [
            ...
            with self.subTest(index=type(index)):
                ...
                self.add_documents(
                    ...
                    add_docs_params=AddDocsParams(
                        ...
                        docs=[
                            {
                                "_id": "tensor1",
                                "text_tensor_only": "dog",
                                "add_field_1": 1.0,
                                "mult_field_1": 1.0,
                            },  # tensor HIGH score, no lexical match
                            {
                                "_id": "tensor2",
                                "text_tensor_only": "something completely unrelated. garbage.",
                                "add_field_1": 2.0,
                                "mult_field_1": 2.0,
                            },  # tensor LOW score, no lexical match
                            {
                                "_id": "lexical1",
                                "text_lexical_only": "dogs dogs",
                                "add_field_1": -2.0,
                                "mult_field_1": -2.0,
                            },  # lexical HIGH score, no tensor
                            {
                                "_id": "lexical2",
                                "text_lexical_only": "dogs",
                                "add_field_1": -1.0,
                                "mult_field_1": -1.0,
                            },  # lexical LOWER score, no tensor
                            {
                                "_id": "both1",
                                "text_field_1": "dogs dogs",
                                "add_field_1": 0.0001,
                            },  # both tensor and lexical, HIGHEST rank in each list.
                        ],
                ...
                self.assertEqual(
                    set([hit["_id"] for hit in unmodified_res["hits"][1:3]]),
                    {"tensor1", "lexical1"},
                )
                self.assertEqual(
                    set([hit["_id"] for hit in unmodified_res["hits"][3:5]]),
                    {"tensor2", "lexical2"},
                )

                with self.subTest("Case 1: limit == rerankDepth == result.size()"):
                    modified_res = tensor_search.search(
                        config=self.config,
                        index_name=index.name,
                        text="dogs",
                    ...
                    # Order with score modifiers: tensor2, tensor1, both1, lexical2, lexical1
                    self.assertEqual(
                        ["tensor2", "tensor1", "both1", "lexical2", "lexical1"],
                        [hit["_id"] for hit in modified_res["hits"]],
                    )
                    # Assert scores are all correctly modified
                    self.assertAlmostEqual(
                        modified_res["hits"][0]["_score"],
                        2 * unmodified_scores["tensor2"] + 2,
                    )
                    self.assertAlmostEqual(
                        modified_res["hits"][1]["_score"],
                        1 * unmodified_scores["tensor1"] + 1,
                    )
                    self.assertAlmostEqual(
                        modified_res["hits"][2]["_score"],
                        unmodified_scores["both1"] + 0.0001,
                    )
                    self.assertAlmostEqual(
                        modified_res["hits"][3]["_score"],
                        -1 * unmodified_scores["lexical2"] - 1,
                    )
                    self.assertAlmostEqual(
                        modified_res["hits"][4]["_score"],
                        -2 * unmodified_scores["lexical1"] - 2,
                    )

                with self.subTest("Case 2: limit < rerankDepth < hits.size()"):
                ...
                with self.subTest("Case 3: limit == rerankDepth < hits.size()"):
                    ...
                    self.assertEqual(
                        ["tensor1", "both1", "lexical1"],
                        [hit["_id"] for hit in modified_res["hits"]],
                    )

                with self.subTest("Case 4: limit < hits.size() < rerankDepth"):
                    ...
                    self.assertEqual(
                        ["tensor2", "tensor1", "both1", "lexical2"],
                        [hit["_id"] for hit in modified_res["hits"]],
                    )

                with self.subTest("Case 5: rerankDepth < hits.size() < limit"):
                    ...
                    self.assertIn(
                        [hit["_id"] for hit in modified_res["hits"]], possible_results
                    )
                    ...
                    for hit in modified_res["hits"]:
                        if hit["_id"] == "tensor1":
                            self.assertEqual(
                                hit["_score"], 1 * unmodified_scores[hit["_id"]] + 1
                            )
                        elif hit["_id"] == "both1":
                            self.assertAlmostEqual(
                                hit["_score"], unmodified_scores[hit["_id"]] + 0.0001
                            )
                        elif hit["_id"] in ["tensor2", "lexical2"]:
                            self.assertEqual(
                                hit["_score"], unmodified_scores[hit["_id"]]
                            )
                        elif hit["_id"] == "lexical1":
                            self.assertEqual(
                                hit["_score"], -2 * unmodified_scores[hit["_id"]] - 2
                            )
        ...
        for index in [
            ...
            with self.subTest(index=type(index)):
                ...
                for retrieval_method, ranking_method in test_cases:
                    with self.subTest(
                        retrieval=retrieval_method, ranking=ranking_method
                    ):
                        hybrid_res = tensor_search.search(
                            config=self.config,
                            index_name=index.name,
                        ...
                        for i in range(len(hybrid_res["hits"])):
                            self.assertEqual(
                                hybrid_res["hits"][i]["_id"], base_res["hits"][i]["_id"]
                            )
                            self.assertEqual(
                                hybrid_res["hits"][i]["_score"],
                                base_res["hits"][i]["_score"],
                            )
    ...
    def test_hybrid_search_with_images(self):
        ...
        for index in [
            ...
            self.semi_structured_default_image_index,
            ...
            with self.subTest(index=index.name):
                ...
                self.add_documents(
                    ...
                    add_docs_params=AddDocsParams(
                        ...
                        docs=[
                            ...
                            {
                                "_id": "hippo text low relevance",
                                "text_field_1": "hippo text text random",
                            },
                ...
                with self.subTest("disjunction text search"):
                    ...
                    self.assertEqual(
                        hybrid_res["hits"][1]["_id"], "hippo text low relevance"
                    )
    ...
    def test_hybrid_search_structured_opposite_retrieval_and_ranking(self):
        ...
        # Reference results
        tensor_res_all_docs = (
            tensor_search.search(  # To get tensor scores of every doc, for reference
                config=self.config,
                index_name=self.structured_text_index_score_modifiers.name,
                text="dogs",
                search_method="TENSOR",
        ...
        with self.subTest(
            retrievalMethod=RetrievalMethod.Lexical, rankingMethod=RankingMethod.Tensor
        ):
            hybrid_res = tensor_search.search(
                config=self.config,
                index_name=self.structured_text_index_score_modifiers.name,
            ...
            for hybrid_hit in hybrid_res["hits"]:
                tensor_hit = next(
                    doc
                    for doc in tensor_res_all_docs["hits"]
                    if doc["_id"] == hybrid_hit["_id"]
                )
                self.assertEqual(hybrid_hit["_score"], tensor_hit["_score"])

        # Tensor retrieval with Lexical ranking
        with self.subTest(
            retrievalMethod=RetrievalMethod.Tensor, rankingMethod=RankingMethod.Lexical
        ):
            hybrid_res = tensor_search.search(
                config=self.config,
                index_name=self.structured_text_index_score_modifiers.name,
            ...
            for hybrid_hit in hybrid_res["hits"]:
                if hybrid_hit["_score"] > 0:
                    ...
                    self.assertNotIn(
                        hybrid_hit["_id"], [doc["_id"] for doc in lexical_res["hits"]]
                    )
    ...
    def test_hybrid_search_invalid_parameters_fails(self):
        test_cases = [
            (
                {"alpha": 0.6, "rankingMethod": "tensor"},
                "can only be defined for 'rrf'",
            ),
            (
                {"rrfK": 61, "rankingMethod": "normalize_linear"},
                "can only be defined for 'rrf'",
            ),
            (
                {
            ...
            ({"rrfK": -1}, "greater than or equal to 0"),
            (
                {"retrievalMethod": "disjunction", "rankingMethod": "lexical"},
                "rankingMethod must be: rrf",
            ),
            (
                {"retrievalMethod": "tensor", "rankingMethod": "rrf"},
                "rankingMethod must be: tensor or lexical",
            ),
            (
                {"retrievalMethod": "lexical", "rankingMethod": "rrf"},
                "rankingMethod must be: tensor or lexical",
            ),
        ...
        for index in [
            ...
            with self.subTest(index=type(index)):
                with self.subTest("searchable_attributes active"):
                    ...
                    self.assertIn(
                        "'searchableAttributes' cannot be used for hybrid",
                        str(e.exception),
                    )
    ...
    def test_hybrid_search_score_modifiers_old_version_fails(self):
        ...
        for index in [
            self.unstructured_default_text_index,
            self.semi_structured_text_index_2_14,
                ...
                with self.assertRaises(core_exceptions.UnsupportedFeatureError) as e:
                    tensor_search.search(
                        ...
                        score_modifiers=ScoreModifierLists(
                            ...
                            add_to_score=[{"field_name": "add_field_1", "weight": 1.0}],
                        ),
                    )
                self.assertIn(
                    "global score modifiers is only supported for "
                    "Marqo indexes created with Marqo 2.15.0",
                    str(e.exception),
                )
    ...
    def test_hybrid_search_rerank_depth_old_version_fails(self):
        ...
        for index in [
            ...
            with self.subTest(index=type(index)):
                ...
                self.assertIn(
                    "'rerankDepth' search parameter is only supported for indexes created "
                    "with Marqo version 2.15.0",
                    str(e.exception),
                )
    ...
    def test_hybrid_search_score_modifiers_wrong_ranking_method_fails(self):
        ...
        with self.subTest(
            "score_modifiers for structured/semi-structured but not RRF ranking"
        ):
            for index in [
                self.structured_text_index_score_modifiers,
                self.semi_structured_default_text_index,
    ...
    def test_hybrid_search_structured_invalid_fields_fails(self):
        ...
        for retrieval_method, ranking_method in test_cases:
            with self.subTest(retrieval=retrieval_method, ranking=ranking_method):
                ...
                self.assertIn(
                    "has no lexically searchable field add_field_1", str(e.exception)
                )
    ...
    def test_hybrid_search_structured_index_has_no_hybrid_rank_profile_fails(self):
        ...
        with self.subTest("hybrid search"):
            ...
            self.assertIn(
                "either has no tensor fields or no lexically searchable fields",
                str(cm.exception),
            )
    ...
    def test_hybrid_search_none_query_wrong_retrieval_or_ranking_fails(self):
        ...
        for index in [
            self.structured_index_with_no_model,
            self.unstructured_index_with_no_model,
        ]:
            with self.subTest(index=index.name):
                for retrieval_method, ranking_method in test_cases:
                    with self.subTest(
                        ...
                        self.assertIn(
                            "retrievalMethod and rankingMethod are both 'tensor'",
                            str(e.exception),
                        )

    def test_hybrid_search_none_query_wrong_retrieval_or_ranking_fails_for_disjunction(
        self,
    ):
        """
        ...
        for index in [
            ...
            with self.subTest(index_name=index.name):
                ...
                _ = self.add_documents(
                    config=self.config, add_docs_params=add_docs_params
                )
                ...
                r = tensor_search.search(
                    ...
                    context=SearchContext(
                        **{
                            "tensor": [{"vector": custom_vector, "weight": 1}],
                        }
                    ),
                )
    ...
    def test_hybrid_search_query_tensor_none_with_context_vectors_passes(self):
        ...
        for index in [
            self.structured_index_with_no_model,
            self.semi_structured_index_with_no_model,
        ]:
            with self.subTest(index_name=index.name):
                add_docs_params = AddDocsParams(
                    index_name=index.name,
    ...
    def test_hybrid_search_query_tensor_none_with_context_docs_passes(self):
        ...
        docs = [
            {"_id": "1", "text_field_1": "Some content 1"},
            {"_id": "2", "text_field_1": "Some content 2"},
        ]
        ...
        for index in [
            ...
            with self.subTest(index_name=index.name):
                ...
                r = tensor_search.search(
                    ...
                    context=SearchContext(
                        documents=SearchContextDocuments(ids={"1": 1})
                    ),
                )
    ...
    def test_hybrid_search_unstructured_with_searchable_attributes_fails(self):
        ...
        self.assertIn(
            "does not support `searchableAttributesTensor` or `searchableAttributesLexical`",
            str(e.exception),
        )
    ...
    def test_hybrid_with_two_errors_returns_both(self):
        ...
        for result_dict, vespa_httpx_code, should_be_timeout in test_cases:
            with self.subTest(
                ...
                vespa_httpx_code=vespa_httpx_code,
                    ...
                    for index in [
                        ...
                        with self.subTest(index=type(index)):
                            with self.assertRaises(
                                vespa_exceptions.VespaStatusError
                            ) as e:
                                tensor_search.search(
                                    text="dogs",
                                    config=self.config,
                            ...
                            self.assertEqual(
                                should_be_timeout,
                                isinstance(
                                    e.exception, vespa_exceptions.VespaTimeoutError
                                ),
                            )
    ...
    def test_hybrid_search_unstructured_with_2_10_fails(self):
        ...
        self.assertIn(
            "only supported for Marqo unstructured indexes created with Marqo 2.11.0 or later",
            str(e.exception),
        )
    ...
    def test_hybrid_search_structured_with_2_9_fails(self):
        ...
        self.assertIn(
            "only supported for Marqo structured indexes created with Marqo 2.10.0 or later",
            str(e.exception),
        )
    ...
    def test_correct_error_is_raised_if_rerank_depth_is_provided_on_old_marqo_version(
        self,
    ):
        """
    ...
    def test_rerank_depth_tensor_hybrid_search(self):
        ...
        docs = [
            {"_id": f"doc_{i}", "text_field_1": f"sample text {i}"} for i in range(10)
        ]
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                tensor_queries_list = [
                    ...
                    [
                        {"dogs": 1.0, "cats": 0.5, "birds": 0.25},
                        {"dogs": -1, "cats": -0.5, "birds": -0.25},
                    ],
                ]
                    ...
                    with self.subTest(tensor_query=tensor_query):
                        ...
                        self.assertIsNone(
                            next(
                                (
                                    doc
                                    for doc in tensor_res_reverse["hits"]
                                    if doc["_id"] == top_hit["_id"]
                                ),
                                None,
                            )
                        )
    ...
    def test_different_retrieval_and_ranking_combinations_with_weighted_queries(self):
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                with self.subTest(
                    "Lexical retrieval, Tensor ranking - opposite weights should provide opposite results"
                ):
                    res = tensor_search.search(
                        **base_parameters,
                        hybrid_parameters=HybridParameters(
                ...
                with self.subTest(
                    "tensor retrieval, lexical ranking - opposite weights result should not be included"
                ):
                    res = tensor_search.search(
                        **base_parameters,
                        hybrid_parameters=HybridParameters(
                    ...
                    assert not any(
                        r["_id"]
                        for r in reverse_res["hits"]
                        if r["_id"] == res["hits"][0]["_id"]
                    )

                with self.subTest(
                    "tensor retrieval, tensor ranking - opposite weights result should not be included"
                ):
                    res = tensor_search.search(
                        **base_parameters,
                        hybrid_parameters=HybridParameters(
                ...
                with self.subTest(
                    "disjunction retrieval, RRF ranking - opposite weights should provide opposite results"
                ):
                    res = tensor_search.search(
                        **base_parameters,
                        hybrid_parameters=HybridParameters(
    ...
    def test_query_tensor_as_string_equivalent_to_single_query(self):
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                self.assertEqual(
                    [hit["_id"] for hit in str_res["hits"]],
                    [hit["_id"] for hit in dict_res["hits"]],
                    "String queryTensor should behave like single-entry dict",
                )

    def test_query_tensor_as_multi_values_dict_produces_different_results_to_single_value_dict(
        self,
    ):
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                single_query = {"dogs": 1.0}

                multi_query = {"dogs": 1.0, "cats": 0.5}
                ...
                for res in single_res["hits"]:
                    res_with_same_id = next(
                        (r for r in multi_res["hits"] if r["_id"] == res["_id"]), None
                    )
                    if res_with_same_id:
    ...
    def test_tensor_query_provided_for_lexical_retrieval_lexical_ranking_raises_error(
        self,
    ):
    ...
    def test_lexical_query_provided_for_tensor_retrieval_tensor_ranking_raises_error(
        self,
    ):
            ...
            with self.subTest(index=index):
                ...
                self.add_documents(
                    ...
                    add_docs_params=AddDocsParams(
                        ...
                        docs=[doc],
                        tensor_fields=["text_field_1"]
                        if isinstance(index, SemiStructuredMarqoIndex)
                        else None,
                    ),
                )
