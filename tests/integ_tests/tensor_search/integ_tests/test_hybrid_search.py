...
class TestHybridSearch(MarqoTestCase):
    ...
    def test_single_facet(self):
        """
        Test that single facet requests work as expected.
        """
        def assert_dict_almost_equal(self, d1, d2, places=1):
            ...
            for key in d1:
                if isinstance(d1[key], dict):
                    assert_dict_almost_equal(self, d1[key], d2[key], places)
        ...
        def test_facet_query(retrieval_method, ranking_method, facets, expected_lexical_facets, expected_other_facets):
            res = tensor_search.search(
            ...
            if retrieval_method == RetrievalMethod.Lexical:
                ...
                assert_dict_almost_equal(self, res["facets"], expected_lexical_facets)
            else:
                assert_dict_almost_equal(self, res["facets"], expected_other_facets)

        self.add_documents(
            config=self.config,
            add_docs_params=AddDocsParams(
                index_name=self.semi_structured_default_text_index.name,
                docs=EXAMPLE_FASHION_DOCUMENTS,
        ...
        for retrieval_method, ranking_method in (
            ...
            with self.subTest(retrieval_method=retrieval_method, ranking_method=ranking_method):
                with self.subTest("Single facet: Default configuration"):
                ...
                with self.subTest("Single facet: Max Results"):
                ...
                with self.subTest("Single facet: Max Results Override per field works"):
                ...
                with self.subTest("Single facet: Max Results 1 with ascending order will return lowest result"):
                ...
                with self.subTest("Single facet: Number field"):
                ...
                with self.subTest("Single facet: Number field with ranges"):
    ...
    def test_single_number_facets_int(self):
        """ This test validates int type for field 'price' """
    ...
    def test_facets_filter_term_exclusions(self):
        """
        Test that facets filter term exclusions work as expected.
        """
