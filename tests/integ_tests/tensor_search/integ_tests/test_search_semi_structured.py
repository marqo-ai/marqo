...
class TestSearchSemiStructured(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        image_index_with_random_model = cls.unstructured_marqo_index_request(
            model=Model(name="random/small"), treat_urls_and_pointers_as_images=True
        )
    ...
    def test_lexical_search_no_highlights_format(self):
        ...
        self.add_documents(
            config=self.config,
            add_docs_params=AddDocsParams(
                index_name=self.default_text_index.name, docs=docs, tensor_fields=[]
            ),
        )
    ...
    def test_filter_maps(self):
        ...
        docs = [
            {
                ...
                "metadata": {"score": 0.9, "rank": 10},
            },
            {
                "_id": "doc2",
                "title": "Just Your Average Doc",
                "metadata": {"score": 0.5, "rank": 20},
            },
            {
                "_id": "doc3",
                "title": "Another Document",
                "metadata": {"score": 0.7, "rank": 15},
            },
        ]
        ...
        for search_method in [SearchMethod.LEXICAL, SearchMethod.TENSOR]:
            for filter_string, expected in test_cases:
                with self.subTest(
                    f"search_method={search_method}, filter={filter_string}"
                ):
                    search_res = tensor_search.search(
                        config=self.config,
                        index_name=self.default_text_index.name,
