...
class TestSearch(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        structured_default_text_index = cls.structured_marqo_index_request(
            ...
            fields=[
                ...
                FieldRequest(
                    name="custom_vector_field_1",
                    type=FieldType.CustomVector,
                    features=[FieldFeature.Filter],
                ),
                FieldRequest(
                    name="multimodal_field_1",
    ...
    def setUp(self) -> None:
        ...
        self.device_patcher = mock.patch.dict(
            os.environ,
            {"MARQO_BEST_AVAILABLE_DEVICE": "cpu", "MARQO_MAX_CPU_MODEL_MEMORY": "15"},
        )
    ...
    def test_search_video(self):
        documents = [
            ...
            {
                "text_field_1": "hello there padawan. Today you will begin your training to be a Jedi",
                "_id": "4",
            },
        ]
        for index in [
            self.unstructured_languagebind_index,
            ...
            with self.subTest(index=index.type):
                ...
                self.assertEqual(
                    len(results["hits"]), 3
                )  # 3 documents should be returned (limit=3)
                self.assertEqual(
                    results["hits"][0]["_id"], "1"
                )  # The video document should be the top result
                self.assertGreater(
                    results["hits"][0]["_score"], results["hits"][1]["_score"]
    ...
    def test_search_audio(self):
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                self.assertEqual(
                    results["hits"][0]["_id"], "2"
                )  # The audio document should be the top result
                self.assertGreater(
                    results["hits"][0]["_score"], results["hits"][1]["_score"]
                )  # Audio should have higher score
            ...
            with self.subTest(type=index.type):
                ...
                if isinstance(index, StructuredMarqoIndex):
                    test_cases += [
                        ...
                        (
                            "list_field_1 in (tag2, random)",
                            0,
                            None,
                            False,
                        ),  # incomplete match for "tag2 some"
                        ("text_field_3 in (b, c)", 1, "5678", True),
                        (
                            "text_field_3 in (a, c, (b but wrong))",
                            0,
                            None,
                            False,
                        ),  # incomplete match for "b"
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                res = self.add_documents(
                    ...
                    add_docs_params=AddDocsParams(
                        ...
                        docs=[
                            ...
                            {
                                "_id": "in1",
                                "text_field_1": "random1",
                                "int_field_1": 100,
                                "text_field_7": "multimodal red herring",
                            },
                            {
                                "_id": "in2",
                                "text_field_1": "blahblah",
                                "int_field_1": 200,
                                "long_field_1": 300,
                        ...
                        mappings={
                            ...
                            "multimodal_field_1": {
                                ...
                                "weights": {"text_field_7": 0.1, "text_field_8": 0.1},
                            },
                        }
                        if isinstance(index, UnstructuredMarqoIndex)
                        else None,
                ...
                test_cases = [
                    ...
                    (
                        "bool_field_1:some_value",
                        0,
                        None,
                    ),  # no hits for bool_field_1 not boolean
                    ("int_field_1:[0 TO 30] OR bool_field_1:true", 2, None),
                    (
                        "(int_field_1:[0 TO 30] AND int_field_1:2) AND text_field_1:(some text)",
                        1,
                        ["1234"],
                    ),
                    (
                        ...
                        1,
                        ["1232"],
                    ),  # string field with boolean-like value
                ]
                ...
                if isinstance(index, StructuredMarqoIndex):
                    test_cases += [
                        ...
                        (
                            "custom_vector_field_1 in ((custom vector text!))",
                            1,
                            ["in2"],
                        ),
                        # multimodal subfield in
                        (
                            "text_field_7 in ((multimodal correct)) AND text_field_8 in ((multimodal correct))",
                            1,
                            ["in2"],
                        ),
                        # in with AND
                ...
                for filter_string, expected_hits, expected_ids in test_cases:
                    with self.subTest(
                        ...
                        if expected_ids:
                            self.assertEqual(
                                set(expected_ids), {hit["_id"] for hit in res["hits"]}
                            )
    ...
    def test_filter_unstructured_index_in_keyword_fails(self):
        ...
        for case in test_cases:
            with self.subTest(case=case):
                ...
                self.assertIn(
                    "'IN' filter keyword is not yet supported for unstructured",
                    str(cm.exception),
                )
            ...
            with self.subTest(index=index.type):
                ...
                if isinstance(index, StructuredMarqoIndex):
                    test_cases += [
                        ...
                        (
                            "_id in (1, doc1) OR _id:doc5",
                            3,
                            ["1", "doc1", "doc5"],
                        ),  # combine in and equality
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                test_cases = [
                    ...
                    (
                        "(float_field_1:[0 TO 1]) AND (text_field_1:(some text))",
                        1,
                        ["344"],
                    ),
                ]
                ...
                if isinstance(index, StructuredMarqoIndex):
                    test_cases += [
                        (
                            "text_field_2 in ((Close match hehehe), (something else))",
                            2,
                            ["1234", "1233"],
                        ),
                        (
                ...
                for filter_string, expected_hits, expected_ids in test_cases:
                    with self.subTest(
                        ...
                        for expected_id in expected_ids:
                            self.assertIn(
                                expected_id, [hit["_id"] for hit in res["hits"]]
                            )
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                if isinstance(index, UnstructuredMarqoIndex):
                    ...
                    test_cases += (
                        unstructured_eq_test_cases + unstructured_range_test_cases
                    )
                ...
                for filter_string, expected_ids in test_cases:
                    with self.subTest(
                        f"filter_string={filter_string}, expected_ids={expected_ids}"
                    ):
                        res = tensor_search.search(
                            config=self.config,
                            index_name=index.name,
        ...
        for index in [self.structured_default_text_index]:
            with self.subTest(index=index.type):
                ...
                self.add_documents(
                    ...
                    add_docs_params=AddDocsParams(
                        ...
                        + [
                            ...
                            {
                                "_id": f"doc with {char}",
                                "text_field_1": f"som{char}e text",
                            }
                ...
                test_cases = [
                    ...
                    (
                        "text_field_1 in ((som\\\\e text), (hello))",
                        ["doc5"],
                    ),  # \ is escaped
                    ...
                    (
                        f"text_field_1 in ((som\\{char}e text), (hello))",
                        [f"doc with {char}"],
                    )
                    for char in MARQO_FILTER_STRING_SPECIAL_CHARS
                ]
                ...
                for filter_string in bad_filter_strings:
                    with self.subTest(f"filter_string={filter_string}"):
                        with self.assertRaises(
                            core_exceptions.FilterStringParsingError
                        ):
                            tensor_search.search(
                                config=self.config,
                                index_name=index.name,
    ...
    def test_filtering_in_with_wrong_type(self):
        ...
        for index in [self.structured_default_text_index]:
            with self.subTest(index=index.type):
                ...
                bad_filter_strings = [
                    (
                        "int_field_1 IN (1,2,not_int)",
                        "'not_int', which is not of type 'int'",
                    ),
                ...
                for filter_string, error_message in bad_filter_strings:
                    with self.subTest(f"filter_string={filter_string}"):
                        with self.assertRaises(
                            core_exceptions.InvalidDataTypeError
                        ) as cm:
                            tensor_search.search(
                                config=self.config,
                                index_name=index.name,
        ...
        for index in [
            ...
            with self.subTest(index=index):
                with mock.patch(
                    ...
                    tensor_search.search(
                        text="some text", index_name=index.name, config=self.config
                    )
    ...
    def test_LexicalSearchResultsScore(self):
        ...
        for index in [
            ...
            with self.subTest(msg=index.type):
                ...
                self.assertTrue(
                    0 < res["hits"][0]["_score"], f"score: {res['hits'][0]['_score']}"
                )

                # Test search with text_field_2
                res = tensor_search.search(
                    text="field_2_document_1",
                    config=self.config,
                    index_name=index.name,
                    search_method=SearchMethod.LEXICAL,
                    result_count=10,
    ...
    def test_get_lexical_search_term(self):
        ...
        structured_vespa_index = StructuredVespaIndex(
            self.structured_default_text_index
        )
        unstructured_vespa_index = UnstructuredVespaIndex(
            self.unstructured_default_text_index
        )
    ...
    def test_lexical_search_DoesNotErrorWithEscapedCharacters(self):
        ...
        docs_list = [
            ...
            {"_id": "doc5", "text_field_1": "back\\slash"},
            {
                "_id": "doc6",
                "text_field_1": "\\",
            },  # This token alone cannot be searched for some reason.
        ...
        test_cases = [
            ...
            (
                '\\"escaped\\"',
                ["doc4", "red_herring_4"],
            ),  # Vespa tokenizer removes " so both docs are retrieved
            (
                "escaped",
                ["doc4", "red_herring_4"],
            ),  # Vespa tokenizer removes " so both docs are retrieved
            ...
            (
                '\\\\\\"literalbackslashthenquote',
                ["doc9"],
            ),  # escaped backslash before escaped double quote
            (
                "\\word",
                ["red_herring_0"],
            ),  # backslash to escape normal character (removed)
    ...
    def test_search_private_image_return_proper_error(self):
        ...
        test_queries_list = [
            ...
            (
                {
                    "https://d2k91vq0avo7lq.cloudfront.net/ai_hippo_realistic_small": 1,
                    "test": 1,
                },
    ...
    def test_video_size_limit(self):
        ...
        with mock.patch(
            "marqo.inference.native_inference.local_inference.NativeInferenceLocal.vectorise"
        ) as mock_vectorise:
            mock_vectorise.side_effect = MediaExceedsMaxSizeError(
        ...
        test_cases = [
            ...
            CustomVectorQuery(
                customVector=CustomVectorQuery.CustomVector(
                    content="hello", vector=[1 for _ in range(384)]
                )
            ),
        ]
                        ...
                        for approx_threshold in approximate_threshold_values:
                            with self.subTest(approximate_threshold=approx_threshold):
                                ...
                                with mock.patch.object(
                                    ...
                                    self.assertEqual(
                                        call_kwargs[key_name], approx_threshold
                                    )
...
biased_text_strategy = st.lists(biased_char_strategy, min_size=0, max_size=100).map(
    "".join
)
...
class TestSearchFuzz(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        for index in [
            cls.unstructured_default_text_index,
            cls.structured_default_text_index,
        ]:
            cls.add_documents(
                config=cls.config,
                add_docs_params=AddDocsParams(
    ...
    @unittest.skip(
        reason="temporarily skip. to come back and add coverage for equality, range, and IN filters"
    )
    ...
    def test_fuzz_filter_all_characters(self, filter: str):
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                res = tensor_search.search(
                    text="", config=self.config, index_name=index.name, filter=filter
                )
                self.assertIn("hits", res)
