...
from marqo.api.exceptions import IllegalRequestedDocCount, InvalidArgError
...
from marqo.tensor_search.models.score_modifiers_object import (
    ScoreModifierLists,
...
class TestSearchWithContext(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.structured_default_text_index = cls.indexes[
            0
        ]  # Use the structured index we created
        cls.unstructured_default_text_index = cls.indexes[
            1
        ]  # Use the unstructured index we created
        cls.legacy_unstructured_default_text_index = cls.indexes[
            2
        ]  # Use the legacy unstructured index we created
    ...
    def _populate_index_orchids(self, index):
        ...
        docs = [
            {
                "_id": "orchid1",
                "text_field_1": "Anacamptis laxiflora is a species of orchid found in wet meadows with alkaline soil.",
                "tags": ["flower", "orchid"],
            },
            {
                "_id": "orchid2",
                "text_field_1": "Cephalanthera longifolia reaches on average 20-60 centimetres in height and is a type of orchid.",
                "tags": ["flower", "orchid"],
            },
            {
                "_id": "orchid3",
                "text_field_1": "Anacamptis morio subsp. longicornu is a subspecies of orchid found in the Mediterranean region.",
                "tags": ["flower", "orchid"],
            },
            {
                "_id": "flower1",
                "text_field_1": "Red rose is a popular flower known for its beauty and fragrance.",
                "tags": ["flower", "rose"],
            },
            {
                "_id": "continent1",
                "text_field_1": "Europe is a continent located entirely in the Northern Hemisphere and mostly in the Eastern Hemisphere.",
                "tags": ["continent"],
            },
            {
                "_id": "continent2",
                "text_field_1": "Asia is Earth's largest and most populous continent, located primarily in the Eastern and Northern Hemispheres.",
                "tags": ["continent"],
            },
            {
                "_id": "continent3",
                "text_field_1": "Africa is the world's second-largest and second-most populous continent, after Asia in both cases.",
                "tags": ["continent"],
            },
        ]
    ...
    def test_search(self):
        for index_name in [
            self.structured_index_basic,
            self.unstructured_index_basic,
    ...
    def test_search_with_incorrect_tensor_dimension(self):
        for index_name in [self.structured_index_basic, self.unstructured_index_basic]:
            with self.subTest(msg=index_name):
                ...
                self.assertIn(
                    "does not match the expected dimension", str(e.exception.message)
                )
    ...
    def test_search_with_incorrect_query_format(self):
        for index_name in [self.structured_index_basic, self.unstructured_index_basic]:
            with self.subTest(msg=index_name):
                ...
                self.assertIn(
                    "This is not supported as the context only works when the query is a dictionary.",
    ...
    def test_search_score(self):
        ...
        for index_name in [self.structured_index_basic, self.unstructured_index_basic]:
            tensor_fields = (
                ["text_field_1"]
                if index_name == self.unstructured_index_basic
                else None
            )
            self.add_documents(
                config=self.config,
            ...
            with self.subTest(msg=index_name):
                ...
                res_1 = tensor_search.search(
                    config=self.config, index_name=index_name, text=query
                )
                res_2 = tensor_search.search(
                    config=self.config,
                    index_name=index_name,
                    text=query,
                    context=SearchContext(
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                docs = [
                    {
                        "_id": "doc1",
                        "text_field_1": "machine learning algorithms and artificial intelligence",
                    },
                    {
                        "_id": "doc2",
                        "text_field_1": "deep neural networks for computer vision tasks",
                    },
                    {
                        "_id": "doc3",
                        "text_field_1": "natural language processing and text generation",
                    },
                    {
                        "_id": "doc4",
                    ...
                    {
                        "_id": "doc5",
                        "text_field_1": "statistical models for data analysis",
                    },
                    {
                        "_id": "doc6",
                        "text_field_1": "clustering algorithms for unsupervised learning",
                    },
                ...
                with self.subTest(
                    exclude_input_documents=False, tensor_fields=["text_field_1"]
                ):
                    ...
                    self.assertEqual(
                        result_ids,
                        ["doc3", "doc1", "doc4", "doc2", "doc6", "doc7", "doc5"],
                    )

                with self.subTest(
                    exclude_input_documents=True, tensor_fields=["text_field_1"]
                ):
                    # Create search context with only documents
                    search_context = SearchContext(
                        documents=SearchContextDocuments(
                ...
                # Using tensor fields
                with self.subTest(
                    excludeInputDocuments=False, tensor_fields=["tensor_field_2"]
                ):
                    # Create search context with only documents
                    search_context = SearchContext(
                        documents=SearchContextDocuments(
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                with self.subTest("With context documents"):
                    ...
                    result_ids = [
                        hit["_id"] for hit in results_with_context_docs["hits"]
                    ]
            ...
            with self.subTest(index=index.type):
                ...
                search_context = SearchContext(
                    documents=SearchContextDocuments(
                        ...
                        parameters=SearchContextDocumentsParameters(
                            tensorFields=["text_field_1"], excludeInputDocuments=False
                        ),
                    )
                )
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                docs = [
                    {
                        "_id": "doc1",
                        "text_field_1": "Test document one about artificial intelligence",
                    },
                    {
                        "_id": "doc2",
                        "text_field_1": "Test document two about artificial intelligence",
                    },
                    {
                        "_id": "doc3",
                        "text_field_1": "Test document three about artificial intelligence",
                    },
                    {
                        "_id": "doc4",
                        "text_field_1": "Test document four about neural networks",
                    },
                    {
                        "_id": "doc5",
                        "text_field_1": "Test document five about machine learning",
                    },
                ]
                ...
                search_context = SearchContext(
                    documents=SearchContextDocuments(
                        ...
                        parameters=SearchContextDocumentsParameters(
                            tensorFields=["text_field_1"], excludeInputDocuments=True
                        ),
                    )
                )
        ...
        for index in [
            ...
            with self.subTest(index=index.type):
                ...
                docs = [
                    {
                        "_id": "doc1",
                        "text_field_1": "Machine learning",
                        "score_field": 10,
                    },
                    {"_id": "doc2", "text_field_1": "Deep learning", "score_field": 5},
                    {
                        "_id": "doc3",
                        "text_field_1": "Neural networks",
                        "score_field": 1,
                    },
                ]
                ...
                score_modifiers = ScoreModifierLists(
                    multiply_score_by=[
                        ScoreModifierOperator(field_name="score_field", weight=1.0)
                    ]
                )
        ...
        for index in [self.structured_default_text_index]:
            with self.subTest(index=index.type):
                ...
                if isinstance(index, StructuredMarqoIndex):
                    from marqo.core.structured_vespa_index import (
                        common as structured_common,
                    )
                    ...
                    from marqo.core.unstructured_vespa_index import (
                        common as unstructured_common,
                    )

                    expected_id_field = unstructured_common.VESPA_FIELD_ID
                    expected_embedding_field_1 = (
                        unstructured_common.VESPA_DOC_EMBEDDINGS
                    )
                    expected_embedding_field_2 = (
                        unstructured_common.VESPA_DOC_EMBEDDINGS
                    )
                ...
                def mock_get_batch(*args, **kwargs):
                    ...
                    vespa_requests.append({"args": args, "kwargs": kwargs})
                    return original_get_batch(*args, **kwargs)

                # Mock the vespa client to capture requests
                with mock.patch.object(
                    self.config.vespa_client, "get_batch", side_effect=mock_get_batch
                ):
                    ...
                    with self.subTest(tensor_field="text_field_1_only"):
                        ...
                        self.assertGreater(
                            len(vespa_requests), 0, "get_batch should have been called"
                        )
                        ...
                        if isinstance(index, StructuredMarqoIndex):
                            ...
                            self.assertNotIn(
                                expected_embedding_field_2, requested_fields
                            )
                        else:
                    ...
                    with self.subTest(tensor_field="direct_vector_retrieval"):
                        ...
                        for doc_id in ["doc1", "doc2"]:
                            ...
                            if isinstance(index, StructuredMarqoIndex):
                                ...
                                self.assertGreater(
                                    len(doc_embeddings["marqo__embeddings"]), 0
                                )
                    ...
                    with self.subTest(tensor_field="comparison_with_full_retrieval"):
                        ...
                        if "_tensor_facets" in full_doc_response:
                            for facet in full_doc_response["_tensor_facets"]:
                                if "text_field_1" in facet:
                                    full_doc_text_field_1_vectors.append(
                                        facet["_embedding"]
                                    )
                        ...
                        # Compare the vectors
                        if (
                            "doc1" in optimized_vectors
                            and "text_field_1" in optimized_vectors["doc1"]
                        ):
                            optimized_text_field_1_vectors = optimized_vectors["doc1"][
                                "text_field_1"
                            ]

                            # Verify that the vectors match
                            self.assertEqual(
                                len(full_doc_text_field_1_vectors),
                                len(optimized_text_field_1_vectors),
                            )
                            for i, (full_vector, optimized_vector) in enumerate(
                                zip(
                                    full_doc_text_field_1_vectors,
                                    optimized_text_field_1_vectors,
                                )
                            ):
                                self.assertEqual(
                                    len(full_vector), len(optimized_vector)
                                )
                                # Compare vectors with some tolerance for floating point precision
                                for j, (full_val, opt_val) in enumerate(
                                    zip(full_vector, optimized_vector)
                                ):
                    ...
                    with self.subTest(tensor_field="all_fields"):
                        ...
                        if isinstance(index, StructuredMarqoIndex):
                            ...
                            get_batch_call = vespa_requests[0]
                            requested_fields = get_batch_call["kwargs"].get(
                                "fields", []
                            )
                            self.assertIn(expected_embedding_field_1, requested_fields)
    ...
    def test_search_with_context_documents_max_search_context_docs_env_var(self):
        ...
        self.add_documents(
            config=self.config,
            add_docs_params=AddDocsParams(
                index_name=index.name, docs=docs, tensor_fields=None
            ),
        )
        ...
        with self.subTest("default_limit_exceeded"):
            search_context = SearchContext(
                documents=SearchContextDocuments(
                    ids={
                        f"doc_{i}": 1.0 for i in range(11)
                    },  # 11 documents > default limit of 10
                    parameters=SearchContextDocumentsParameters(
                        tensorFields=["text_field_1"], excludeInputDocuments=False
                    ),
                )
        ...
        with self.subTest("increased_limit_passes"):
            with mock.patch.dict(
                os.environ, {EnvVars.MARQO_MAX_SEARCH_CONTEXT_DOCS: "11"}
            ):
                search_context = SearchContext(
                    documents=SearchContextDocuments(
                        ids={
                            f"doc_{i}": 1.0 for i in range(11)
                        },  # 11 documents = new limit of 11
                        parameters=SearchContextDocumentsParameters(
                            tensorFields=["text_field_1"], excludeInputDocuments=False
                        ),
                    )
        ...
        with self.subTest("increased_limit_still_enforced"):
            with mock.patch.dict(
                ...
                search_context = SearchContext(
                    documents=SearchContextDocuments(
                        ids={
                            f"doc_{i}": 1.0 for i in range(12)
                        },  # 12 documents > new limit of 11
                        parameters=SearchContextDocumentsParameters(
                            tensorFields=["text_field_1"], excludeInputDocuments=False
                        ),
                    )
    ...
    def test_search_with_context_documents_fails_for_legacy_unstructured_index(self):
        ...
        context_documents = SearchContextDocuments(ids={"orchid1": 1.0, "orchid2": 1.0})
        ...
        self.assertIn(
            "Search context is not supported for unstructured indexes created with Marqo version",
            str(cm.exception),
        )
    ...
    def test_context_documents_all_zero_weights_error(self):
        ...
        for index in [
            ...
            with self.subTest(type=index.type):
                ...
                self.assertIn(
                    "No documents with non-zero weight provided", str(cm.exception)
                )
    ...
    def test_context_documents_uses_different_tensor_fields(self):
        ...
        self.add_documents(
            config=self.config,
            add_docs_params=AddDocsParams(index_name=index.name, docs=docs),
        )
        ...
        results_field2 = tensor_search.search(
            ...
            context=SearchContext(
                documents=SearchContextDocuments(
                    ids={"context_doc": 1.0},
                    parameters=SearchContextDocumentsParameters(
                        tensorFields=["text_field_2"], excludeInputDocuments=True
                    ),
                )
            ),
            result_count=2,
            ...
            for search_method in ["TENSOR", "HYBRID"]:
                ...
                with self.subTest(f"index={index.type}, search_method={search_method}"):
                    ...
                    docs = [
                        {
                            "_id": "doc1",
                            "text_field_1": "Machine learning and artificial intelligence",
                        },
                        {
                            "_id": "doc2",
                            "text_field_1": "Deep learning neural networks",
                        },
                    ...
                    search_context = SearchContext(
                        documents=SearchContextDocuments(
                            ids={
                                "doc1": 2.0,
                                "non_existent_doc": 1.0,
                                "doc2": 1.5,
                                "another_missing_doc": 0.5,
                            },
                            parameters=SearchContextDocumentsParameters(
                                tensorFields=["text_field_1"],
                                excludeInputDocuments=False,
        ...
        for index in [
            ...
            for search_method in ["TENSOR", "HYBRID"]:
                ...
                with self.subTest(f"index={index.type}, search_method={search_method}"):
                    ...
                    self.assertIn(
                        "Marqo could not collect any vectors from the search query",
                        str(e.exception),
                    )

    def test_search_with_context_documents_allow_missing_both_parameters_for_different_hybrid_parameters(
        self,
    ):
        """Test that search works when both allowMissingDocuments=True and allowMissingEmbeddings=True with mixed scenarios,
        ...
        for index in [
            ...
            for hybrid_parameters in hybrid_parameters_test_cases:
                with self.subTest(
                    f"index={index.type}, hybrid_parameters={hybrid_parameters_test_cases}"
                ):
                    # Add some documents
                    docs = [
                        {
    ...
    def test_search_with_context_documents_raise_vector_collect_errors_with_disjunction(
        self,
    ):
        """Test that search works when both allowMissingDocuments=True and allowMissingEmbeddings=True with mixed scenarios,
        with different hybrid parameters"""
        for index in [
            ...
            for hybrid_parameters in hybrid_parameters_test_cases:
                with self.subTest(
                    ...
                    self.assertIn(
                        "Please check the provided query, context (if any), "
                        "or queryTensor(for Hybrid search)",
                        str(e.exception),
                    )

    def test_search_with_context_if_context_vectors_exist_documents_can_have_no_embeddings(
        self,
    ):
