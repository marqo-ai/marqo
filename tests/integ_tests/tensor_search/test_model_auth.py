...
from marqo.tensor_search.models.private_models import (
    HfAuth,
    ModelAuth,
    ModelLocation,
    S3Auth,
    ...
    def test_after_downloading_search_doesnt_redownload(self):
        ...
        with mock.patch("urllib.request.urlopen", mock_req):
            res = tensor_search.search(
                config=self.config, index_name=self.index_name_1, text="hi"
            )
            assert "hits" in res
            mock_req.assert_not_called()
...
class TestModelAuthOpenCLIP(MarqoTestCase):
    ...
    def test_model_auth_hf(self):
        ...
        tensor_search.create_vector_index(
            config=self.config, index_name=self.index_name_1, index_settings=hf_settings
        )
        ...
        with unittest.mock.patch(
            "open_clip.create_model_and_transforms", mock_open_clip_creat_model
        ):
            with unittest.mock.patch(
    ...
    def test_model_auth_s3_search(self):
        ...
        model_abs_path = get_s3_model_absolute_cache_path(
            S3Location(Key=s3_object_key, Bucket=s3_bucket)
        )
        ...
        s3_settings["index_defaults"]["model_properties"] = model_properties
        tensor_search.create_vector_index(
            config=self.config, index_name=self.index_name_1, index_settings=s3_settings
        )
        ...
        mock_s3_client.generate_presigned_url.assert_called_with(
            "get_object", Params={"Bucket": "your-bucket-name", "Key": s3_object_key}
        )
        mock_boto3_client.assert_called_once_with(
            "s3",
            aws_access_key_id=fake_access_key_id,
    ...
    def test_model_auth_mismatch_param_s3_ix(self):
        ...
        with unittest.mock.patch("boto3.client", return_value=mock_s3_client):
            with self.assertRaises(BadRequestError) as cm:
                ...
                self.assertIn(
                    "s3 authorisation information is required", str(cm.exception)
                )
    ...
    def test_model_loads_from_all_add_docs_derivatives(self):
        ...
        mock_s3_client.generate_presigned_url.return_value = (
            "https://some_non_existent_model.pt"
        )
                ...
                with unittest.mock.patch(
                    ...
                    self.add_documents(
                        ...
                        add_docs_params=AddDocsParams(
                            ...
                            auto_refresh=True,
                            docs=[{"Title": "something {i} good"} for i in range(20)],
                            device="cpu",
                        ),
                    )
        mock_download_pretrained_from_url.assert_called_once_with(
            url="https://some_non_existent_model.pt",
            cache_dir=None,
            cache_file_name="secret_model.pt",
        )
        mock_s3_client.generate_presigned_url.assert_called_with(
            "get_object", Params={"Bucket": "your-bucket-name", "Key": s3_object_key}
        )
    ...
    def test_model_loads_from_multi_search(self):
        ...
        with unittest.mock.patch(
            "marqo.s2_inference.s2_inference.vectorise", side_effect=fake_vectorise
        ) as mock_vectorise:
            model_auth = ModelAuth(
                s3=S3Auth(
                    aws_access_key_id=fake_access_key_id,
            ...
            res = tensor_search.search(
                ...
                text={TestImageUrls.HIPPO_REALISTIC.value: 0.3, "my text": -1.3},
            )
            assert "hits" in res
    ...
    def test_no_creds_error(self):
        ...
        with unittest.mock.patch("boto3.client", return_value=mock_s3_client):
            ...
            self.assertIn(
                "s3 authorisation information is required", str(cm2.exception)
            )
    ...
    def test_bad_creds_error_s3(self):
        ...
        self.assertIn(
            "403 error when trying to retrieve model from s3", str(cm.exception)
        )
        ...
        self.assertIn(
            "403 error when trying to retrieve model from s3", str(cm2.exception)
        )
    ...
    def test_non_existent_hf_location(self):
        ...
        self.assertIn(
            "Could not find the specified Hugging Face model repository.",
            str(cm.exception),
        )
        ...
        for bulk_search_query in [
            BulkSearchQuery(
                queries=[
                    ...
                    BulkSearchQueryEntity(
                        index=self.index_name_1,
                        q={"random text": 0.5, "other_text": -0.3},
...
class TestModelAuthlLoadForHFModelBasic(MarqoTestCase):
    ...
    def test_1_load_model_from_hf_zip_file_with_auth_search(self):
        ...
        with unittest.mock.patch(
            "transformers.AutoModel.from_pretrained", mock_automodel_from_pretrained
        ):
            with unittest.mock.patch(
                ...
                with unittest.mock.patch(
                    ...
                    with unittest.mock.patch(
                        "marqo.core.inference.embedding_models.hugging_face_model.HuggingFaceModel."
        ...
        assert (
            mock_automodel_from_pretrained.call_args_list[0][0][0]
            == "cache/path/to/model/"
        ...
        assert (
            mock_extract_huggingface_archive.call_args_list[0][0][0]
    ...
    def test_2_load_model_from_hf_zip_file_without_auth_search(self):
        ...
        assert (
            mock_autotokenizer_from_pretrained.call_args_list[0][0][0]
            == "cache/path/to/model/"
        ), "Expected call not found"
    ...
    def test_3_load_model_from_s3_zip_file_with_auth_search(self):
        ...
        _delete_file(
            os.path.join(ModelCache.hf_cache_path, os.path.basename(s3_object_key))
        )
    ...
    def test_4_load_model_from_public_url_zip_file_search(self):
        ...
        mock_extract_huggingface_archive = mock.MagicMock(
            side_effect=HuggingFaceModel.extract_huggingface_archive
        )
        mock_automodel_from_pretrained = mock.MagicMock(
            side_effect=AutoModel.from_pretrained
        )
        mock_download = mock.MagicMock(side_effect=download_pretrained_from_url)

        with mock.patch(
            "transformers.AutoModel.from_pretrained", new=mock_automodel_from_pretrained
        ):
            with mock.patch(
                "marqo.s2_inference.processing.custom_clip_utils.download_pretrained_from_url",
                new=mock_download,
                ...
                with mock.patch(
                    "marqo.core.inference.embedding_models.hugging_face_model.HuggingFaceModel."
                    "extract_huggingface_archive",
                    new=mock_extract_huggingface_archive,
                ):
                    res = tensor_search.search(
        ...
        assert len(mock_extract_huggingface_archive.call_args_list) == 1
        assert mock_extract_huggingface_archive.call_args_list[0][0][0] == (
            ModelCache.hf_cache_path + os.path.basename(public_url)
        )
        ...
        assert len(mock_automodel_from_pretrained.call_args_list) == 1
        assert mock_automodel_from_pretrained.call_args_list[0][0][
            0
        ] == ModelCache.hf_cache_path + os.path.basename(public_url).replace(".zip", "")
    ...
    def test_2_load_model_from_hf_zip_file_without_auth_add_documents(self):
        ...
        mock_hf_hub_download.assert_called_once_with(
            repo_id=hf_repo_name, filename=hf_object, cache_dir=ModelCache.hf_cache_path
        )
    ...
    def test_61_load_model_from_public_hf_repo_without_auth_using_repo_id_add_documents(
        self,
    ):
        public_repo_name = "sentence-transformers/all-MiniLM-L6-v1"
        hf_token = "some-secret-token"
    ...
    def test_62_load_model_from_public_hf_repo_without_auth_using_name_add_documents(
        self,
    ):
        public_repo_name = "sentence-transformers/all-MiniLM-L6-v1"
    ...
    def test_invalid_hf_location(self):
        ...
        invalid_model_properties_list = [
            ...
            },
            {
                ...
                "model_location": {
                    "auth_required": True,
                    "hf": {"name": 3, "filename": "random again"},
                },
                "type": "hf",
            },
        ]
        for invalid_model_properties in invalid_model_properties_list:
            try:
                model_location = ModelLocation(
                    **invalid_model_properties["model_location"]
                )
                raise AssertionError
            except (InvalidArgError, ValidationError):
                pass
    ...
    def test_load_model_from_private_hf_repo_with_redirect(self):
        ...
        assert len(mock_automodel_from_pretrained.call_args_list) == 1
        assert (
            mock_automodel_from_pretrained.call_args_list[0][0][0] == private_repo_name
        )
        mock_autotokenizer_from_pretrained.call_args_list[0][1][
            "use_auth_token"
        ] == hf_token
...
class TestS3ModelAuthlLoadForHFModelVariants(MarqoTestCase):
    ...
    def test_model_loads_from_all_add_docs_derivatives(self):
        ...
        mock_download_pretrained_from_url.assert_called_once_with(
            url=public_url,
            cache_dir=ModelCache.hf_cache_path,
            cache_file_name="secret_model.zip",
        )
        mock_s3_client.generate_presigned_url.assert_called_with(
            "get_object", Params={"Bucket": "your-bucket-name", "Key": s3_object_key}
        )
    ...
    def test_model_loads_from_multi_search(self):
        ...
        with unittest.mock.patch(
            "marqo.s2_inference.s2_inference.vectorise", side_effect=fake_vectorise_384
        ) as mock_vectorise:
            model_auth = ModelAuth(
                s3=S3Auth(
                    aws_access_key_id=fake_access_key_id,
