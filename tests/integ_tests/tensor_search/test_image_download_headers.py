    ...
    def test_img_download_search(self):
        ...
        self.add_documents(
            ...
            add_docs_params=AddDocsParams(
                index_name=self.index_name_1,
                docs=[{"_id": "1", "image": self.real_img_url}],
                auto_refresh=True,
        ...
        with mock.patch("requests.get", mock_get):
            with mock.patch(
                "marqo._httprequests.ALLOWED_OPERATIONS",
                {mock_get, requests.post, requests.put},
            ):
                ...
                search_res = tensor_search._vector_text_search(
                    ...
                    index_name=self.index_name_1,
                    result_count=1,
                    query=self.real_img_url,
                    media_download_headers=media_download_headers,
                    device="cpu",
                )
                # Check if the image URL was called at least once with the correct headers
                image_url_called = any(
                    call_args[0] == self.real_img_url
                    and call_kwargs.get("headers", None) == media_download_headers
