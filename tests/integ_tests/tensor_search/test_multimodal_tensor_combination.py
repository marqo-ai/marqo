...
class TestMultimodalTensorCombination(MarqoTestCase):
    def get_specific_field_facet(self, index_name, document_id, field):
        doc_facets = tensor_search.get_document_by_id(
            config=self.config,
    ...
    def setUpClass(cls):
        ...
        structured_random_multimodal_index = cls.structured_marqo_index_request(
            ...
            fields=[
                ...
                FieldRequest(
                    name="my_multimodal_field_0",
                    type=FieldType.MultimodalCombination,
                    dependent_fields={"text_field": 0.5, "image_field": 0.8},
                ),
                FieldRequest(
                    name="my_multimodal_field_1",
                    type=FieldType.MultimodalCombination,
                    dependent_fields={"text_field": 0.5, "image_field": -1},
                ),
                FieldRequest(
                    name="my_multimodal_field_2",
                    type=FieldType.MultimodalCombination,
                ...
                FieldRequest(
                    name="bad_multimodal_field",
                    type=FieldType.MultimodalCombination,
                    dependent_fields={
                        "bad_test_text_field": 0.5,
                        "bad_field_float": 0.8,
                        "bad_field_int": 0.8,
                        "bad_field_bool": 0.8,
                        "bad_field_list": 0.8,
        ...
        structured_multimodal_index = cls.structured_marqo_index_request(
            ...
            fields=[
                ...
                FieldRequest(
                    name="zero_weight_image_field", type=FieldType.ImagePointer
                ),
                FieldRequest(
                    name="zero_weight_combo_text_image",
                    type=FieldType.MultimodalCombination,
                    dependent_fields={
                        "zero_weight_text_field": 1,
                        "zero_weight_image_field": 0,
                    },
                ),  # Note only image_field has weight 0, just for this example.
            ],
        ...
        cls.indexes = cls.create_indexes(
            [
                unstructured_random_multimodal_index,
    ...
    def test_add_documents_with_one_multimodal_fields(self):
        for index in [
            self.unstructured_random_multimodal_index,
            self.structured_random_multimodal_index,
        ]:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                ...
                mappings = {
                    "combo_text_image": {
                        ...
                        "weights": {"text_field": 0.5, "image_field": 0.8},
                    }
                }
                ...
                added_doc = tensor_search.get_document_by_id(
                    config=self.config,
                    index_name=index.name,
                    document_id="1",
                    show_vectors=True,
                ...
                relevant_facets = self.get_relevant_tensor_facets(
                    doc=added_doc, prefix="combo_text_image"
                )
        ...
        for index in [
            ...
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                ...
                mappings = {
                    ...
                    "my_multimodal_field_1": {
                        "type": "multimodal_combination",
                        "weights": {"text_field": 0.5, "image_field": -1},
                    },
                    "my_multimodal_field_2": {
                        "type": "multimodal_combination",
                        "weights": {"Title": 0.5, "image_field": 0},
                    },
                }
                ...
                relevant_facets = self.get_relevant_tensor_facets(
                    doc=added_doc, prefix="my_multimodal_field"
                )
            ...
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                ...
                test_cases = [
                    (
                        True,
                        True,
                        "show_vectors = True, should return multimodal_params",
                    ),
                    (
                        False,
                        False,
                        "show_vectors = False, should not return multimodal_params",
                    ),
                ]
                ...
                for show_vectors, is_return_multimodal_params, msg in test_cases:
                    with self.subTest(msg):
                        ...
                        if isinstance(index, UnstructuredMarqoIndex):
                            self.assertEqual(
                                is_return_multimodal_params,
                                "multimodal_params" in returned_doc,
                            )
            ...
            (
                {
                    "multimodal_fields_0": {
                        "type": "multimodal_combination",
                        "weights": {"text_field": 0.5, "image_field": 0.3},
                    }
                },
                ["multimodal_fields_0"],
                3,
            ...
            (
                {
                    ...
                    "multimodal_fields_1": {
                        "type": "multimodal_combination",
                        "weights": {"content": 0.5, "image_field": 0.3},
                    },
                },
                ["multimodal_fields_0", "multimodal_fields_1"],
                3,
                4,
            ),
        ...
        for (
            ...
            with self.subTest(f"{mappings}, {tensor_fields}"):
                self.add_documents(
                    config=self.config,
                    add_docs_params=AddDocsParams(
                        index_name=self.unstructured_random_multimodal_index.name,
                        docs=doc,
                        mappings=mappings,
                        device="cpu",
                ...
                res = self.monitoring.get_index_stats_by_name(
                    index_name=self.unstructured_random_multimodal_index.name
                )
    ...
    def test_multimodal_field_bad_field_content(self):
        ...
        for index in [
            ...
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                test_cases = [
                    (
                        {"bad_test_text_field": "test", "bad_field_float": 2.4},
                        "received bad_field_float:2.4",
                        "Invalid type <class 'float'> for tensor field bad_field_float",
                    ),
                    (
                        {"bad_test_text_field": "test", "bad_field_int": 1},
                        "received bad_field_int:1",
                        "Invalid type <class 'int'> for tensor field bad_field_int",
                    ),
                    (
                        {"bad_test_text_field": "test", "bad_field_bool": True},
                        "received bad_field_bool:True",
                        "Invalid type <class 'bool'> for tensor field bad_field_bool",
                    ),
                    (
                        {
                            "bad_test_text_field": "test",
                            "bad_field_list": ["123", "23"],
                        },
                        f"received bad_field_list:{['123', '23']}",
                        "Invalid type <class 'list'> for tensor field bad_field_list",
                    ),
                    (
                        {
                            "bad_test_text_field": "test",
                            "bad_field_img": "https://a-void-image.jpg",
                        },
                        "Could not process the media file found",
                        "Could not process the media file found",
                    ),
                    (
                        {"bad_multimodal_field": "test"},
                        "Document and mappings object have conflicting fields",
                        "Field bad_multimodal_field is a multimodal combination field and cannot be assigned a value.",
                    ),
                ]
                ...
                for (
                    document,
                    error_msg_unstructured,
                    error_msg_structured,
                ) in test_cases:
                    error_msg = (
                        error_msg_unstructured
                        if isinstance(index, UnstructuredMarqoIndex)
                        else error_msg_structured
                    )
                    with self.subTest(error_msg):
                        with mock.patch(
                            "marqo.s2_inference.s2_inference.vectorise"
                        ) as mock_vectorise:
                            res = self.add_documents(
                                ...
                                add_docs_params=AddDocsParams(
                                    ...
                                    device="cpu",
                                    tensor_fields=["bad_multimodal_field"]
                                    if isinstance(index, UnstructuredMarqoIndex)
                                    else None,
                                ),
                            )
    ...
    def test_multimodal_tensor_combination_score(self):
        for index in [
            self.unstructured_multimodal_index,
            self.structured_multimodal_index,
        ]:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                ...
                assert (score_3 >= min(score_1, score_2)) and (
                    score_3 <= max(score_1, score_2)
                )

    def test_multimodal_tensor_combination_tensor_value(self):
        for index in [
            self.unstructured_unnormalized_multimodal_index,
            self.structured_unnormalized_multimodal_index,
        ]:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                res = self.add_documents(
                    config=self.config,
                ...
                self.add_documents(
                    ...
                    add_docs_params=AddDocsParams(
                        ...
                        docs=[
                            ...
                            {"image_field_3": TestImageUrls.IMAGE1.value, "_id": "3"},
                            {"image_field_4": TestImageUrls.IMAGE2.value, "_id": "4"},
                        ],
                        tensor_fields=[
                            "text_field_3",
                            "text_field_4",
                ...
                combo_tensor_1 = self.get_specific_field_facet(
                    index.name, "c1", "combo_text_image"
                )
                combo_tensor_2 = self.get_specific_field_facet(
                    index.name, "c2", "combo_text_image"
                )
                combo_tensor_3 = self.get_specific_field_facet(
                    index.name, "c3", "combo_text_image"
                )
                combo_tensor_4 = self.get_specific_field_facet(
                    index.name, "c4", "combo_text_image"
                )

                text_tensor_1 = self.get_specific_field_facet(
                    index.name, "1", "text_field_3"
                )
                text_tensor_2 = self.get_specific_field_facet(
                    index.name, "2", "text_field_4"
                )
                image_tensor_1 = self.get_specific_field_facet(
                    index.name, "3", "image_field_3"
                )
                image_tensor_2 = self.get_specific_field_facet(
                    index.name, "4", "image_field_4"
                )
                ...
                def get_score(document):
                    ...
                    self.add_documents(
                        ...
                        add_docs_params=AddDocsParams(
                            ...
                            tensor_fields=["zero_weight_combo_text_image"]
                            if isinstance(index, UnstructuredMarqoIndex)
                            else None,
                        ),
                    )
            ...
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                ...
                def run():
                    ...
                    assert tensor_search.get_document_by_id(
                        config=self.config, index_name=index.name, document_id="111"
                    )
                    # Ensure that vectorise is only called twice
                    assert len(mock_vectorise.call_args_list) == 2

                    text_content = [
                        f"A rider is riding a horse jumping over the barrier_{i}."
                        for i in range(1, 5)
                    ]

                    real_text_content = [
                        call_kwargs["content"]
                        for call_args, call_kwargs in mock_vectorise.call_args_list
                    ][0]
    ...
    def test_batched_vectorise_call_infer_image_is_false(self):
        ...
        """
        for index in [
            self.unstructured_random_text_index,
            self.structured_random_text_index,
        ]:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
    ...
    def test_concurrent_image_downloading(self):
        ...
        @mock.patch(
            "marqo.s2_inference.clip_utils.load_image_from_path",
            mock_load_image_from_path,
        )
        def run():
            res = self.add_documents(
                config=self.config,
    ...
    def test_lexical_search_on_multimodal_combination(self):
        ...
        self.add_documents(
            config=self.config,
            add_docs_params=AddDocsParams(
                index_name=self.unstructured_multimodal_index.name,
                docs=[
                    {
    ...
    def test_search_with_filtering_and_infer_image_false(self):
        ...
        res_exist_0 = tensor_search.search(
            index_name=self.unstructured_random_multimodal_index.name,
            config=self.config,
            text="",
            filter="filter_field:test_this_0",
        )

        assert res_exist_0["hits"][0]["_id"] == "0"

        res_exist_2 = tensor_search.search(
            index_name=self.unstructured_random_multimodal_index.name,
            config=self.config,
            text="",
            filter="filter_field:test_this_2",
        )

        assert res_exist_2["hits"][0]["_id"] == "2"

        res_nonexist_1 = tensor_search.search(
            index_name=self.unstructured_random_multimodal_index.name,
            config=self.config,
            text="",
            filter="filter_field:test_this_5",
        )
    ...
    def test_multimodal_combination_chunks(self):
        ...
        for index in [self.unstructured_random_multimodal_index]:
            with self.subTest(f"Index type: {index.type}. Index name: {index.name}"):
                ...
                self.assertIn(
                    "combo_text_image", doc_w_facets[TensorField.tensor_facets][0]
                )

                self.assertEqual(
                    doc_w_facets[TensorField.tensor_facets][0]["combo_text_image"],
                    json.dumps(
                        {
                            "text_field": "marqo is good",
                            "image_field": TestImageUrls.IMAGE4.value,
