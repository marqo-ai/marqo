...
class ApiTests(MarqoTestCase):
    ...
    def test_add_or_replace_documents_tensor_fields(self):
        with mock.patch(
            "marqo.core.document.document.Document.add_documents"
    ...
    def test_custom_search_limit(self):
        ...
        for custom_limit in custom_limits:
            with patch.dict(
                ...
                self.assertIn(
                    f"result limit must be less than or equal to the "
    ...
    def test_custom_search_offset(self):
        ...
        for custom_offset in custom_offsets:
            with patch.dict(
                ...
                self.assertEqual(response.status_code, 400)
                self.assertIn(
                    f"The search result offset must be less than or equal "
...
class ValidationApiTests(MarqoTestCase):
    ...
    def test_schema_validation_defaultDisabled(self):
        ...
        """
        data = {"type": "structured", "allFields": [], "tensorFields": []}
    ...
    def test_ops_api_200(self):
        ...
        with patch.dict("os.environ", {EnvVars.MARQO_ENABLE_OPS_API: "TRUE"}):
            ...
            response = self.client.post(f"/validate/index/{index_name}", json=data)
            self.assertEqual(
                response.json(), {"validated": True, "index": "test-index"}
            )
...
class TestApiCustomEnvVars(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        structured_index_request = cls.structured_marqo_index_request(
            fields=[
                FieldRequest(
                    name="field1", type=FieldType.Text, features=["lexical_search"]
                ),
        ...
        cls.indexes = cls.create_indexes(
            [unstructured_index_request, structured_index_request]
        )
    ...
    def test_search_timeout_short_timer_fails(self):
        ...
        with mock.patch.dict(
            os.environ, {"VESPA_SEARCH_TIMEOUT_MS": "1", "MARQO_MODE": "COMBINED"}
        ):
                ...
                for index in [self.unstructured_index, self.structured_index]:
                    with self.subTest(index=index.name):
                        res = self.client.post(
                            "/indexes/" + index.name + "/search?device=cpu",
                            json={"q": "irrelevant", "searchMethod": "TENSOR"},
                        )
            ...
            with self.subTest(search_method="HYBRID"):
                for index in [self.unstructured_index, self.structured_index]:
                    with self.subTest(index=index.name):
                        res = self.client.post(
                            "/indexes/" + index.name + "/search?device=cpu",
                            json={"q": "irrelevant", "searchMethod": "HYBRID"},
                        )
                        # The search request must timeout, since the timeout is set to 1ms
                        self.assertEqual(res.status_code, 504)
                        self.assertEqual(res.json()["code"], "vector_store_timeout")
    ...
    def test_inference_cache_caches_query_string(self):
        with mock.patch.dict(
            ...
            with patch.object(
                inference.delegate, "vectorise", wraps=inference.delegate.vectorise
            ) as mock_vectorise:
                with TestClient(api.app) as client:
                    for index in [self.unstructured_index, self.structured_index]:
                        with self.subTest(index=index.name):
                            ...
                            client.post(
                                "/indexes/" + index.name + "/search?telemetry=true",
...
class TestApiErrors(MarqoTestCase):
    ...
    def test_index_not_found_error(self):
        ...
        assert (
            ...
            and index_name in response.json()["message"]
        )

    def test_index_already_exists(self):
        response = self.client.post(
            "/indexes/" + self.structured_index.name,
        ...
        assert (
            ...
            and self.structured_index.name in response.json()["message"]
        )

    def test_invalid_field_name(self):
        # use attributesToRetrieve on a non-existent field
        response = self.client.post(
            "/indexes/" + self.structured_index.name + "/search?device=cpu",
    ...
    def test_invalid_data_type(self):
        ...
        response = self.client.post(
            "/indexes/" + self.structured_index.name + "/documents?device=cpu",
            json={"documents": [{"field2": 123}]},
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_invalid_argument_error(self):
        ...
        response = self.client.post(
            "/indexes/" + self.random_index_name(),
            json={
                "type": "structured",
    ...
    def test_approximate_threshold_with_lexical_method_error(self):
        ...
        response = self.client.post(
            "/indexes/" + self.structured_index.name + "/search?device=cpu",
            json={"q": "test", "searchMethod": "LEXICAL", "approximateThreshold": 0.5},
        )
    ...
    def test_approximate_threshold_outside_range_error(self):
        ...
        for threshold in invalid_thresholds:
            with self.subTest(threshold=threshold):
                ...
                self.assertEqual(response.json()["detail"][0]["type"], "value_error")
                self.assertIn(
                    f"'approximateThreshold' must be between 0 and 1, got {threshold}",
                    response.json()["detail"][0]["msg"],
                )
    ...
    def test_create_index_snake_case_fails(self):
        ...
        test_cases_fail = [
            ...
            (
                ...
                "ef_construction",
                "Snake case within a dict is invalid",
            ),
            (
                {
                    "type": "unstructured",
                ...
                "ef_construction",
                "Snake case within a dict is invalid, unstructured index",
            ),
        ]
        test_cases_pass = [
            (
        ...
        for test_case, field, test_name in test_cases_fail:
            with self.subTest(test_name):
                response = self.client.post("/indexes/my_index", json=test_case)
        ...
        for test_case, test_name in test_cases_pass:
            with self.subTest(test_name):
                ...
                response = self.client.post(f"/indexes/{index_name}", json=test_case)
    ...
    def test_invalid_structured_index_field_type(self):
        ...
        for test_case, test_name in test_cases:
            ...
            with self.subTest(test_name):
                index_name = "a" + str(uuid.uuid4()).replace("-", "")
                response = self.client.post(
                    f"/indexes/{index_name}", json=test_settings
                )
                self.assertEqual(response.status_code, 422)
    ...
    def test_log_stack_trace_for_core_exceptions(self):
        ...
        with patch("marqo.api.route.logger.error") as mock_logger_error:
            with patch(
                "marqo.core.index_management.index_management.IndexManagement.create_index",
    ...
    def test_log_stack_trace_for_base_exceptions_invalid_arg(self):
        ...
        with patch("marqo.api.route.logger.error") as mock_logger_error:
            with patch(
                "marqo.tensor_search.tensor_search.search", side_effect=raised_error
            ):
                response = self.client.post(
                    "/indexes/test_index/search", json={"q": "test"}
                )
            mock_logger_error.assert_called_once()
    ...
    def test_log_stack_trace_for_base_exceptions_internal(self):
        ...
        with patch("marqo.api.route.logger.error") as mock_logger_error:
            with patch(
                "marqo.tensor_search.tensor_search.get_document_by_id",
                side_effect=raised_error,
            ):
                response = self.client.get("/indexes/test_index/documents/1")
            mock_logger_error.assert_called_once()
    ...
    def test_handle_pydantic_v1_validation_errors(self):
        ...
        error = pydantic.v1.ValidationError(
            errors=[ErrorWrapper(ValueError("some message"), loc="doc")],
            model=SearchQuery,
        )
        with patch("marqo.tensor_search.tensor_search.search", side_effect=error):
            response = self.client.post(
    ...
    def test_handle_pydantic_v2_validation_errors(self):
        ...
        error = pydantic.ValidationError.from_exception_data(
            ...
            line_errors=[
                InitErrorDetails(
                    type=PydanticCustomError("type1", "some message"),
                    loc=("doc",),
                    input=...,
                )
            ],
