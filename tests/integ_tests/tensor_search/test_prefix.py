...
class TestPrefix(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        unstructured_index_e5 = cls.unstructured_marqo_index_request(
            model=Model(name="hf/e5-small"), treat_urls_and_pointers_as_images=True
        )
        ...
        for index in [self.unstructured_index_1, self.structured_text_index]:
            with self.subTest(index=index.type):
                ...
                self.add_documents(
                    ...
                    add_docs_params=AddDocsParams(
                        index_name=index.name,
                        docs=[{"_id": "doc_c", "text": "PREFIX: hello"}],
                        auto_refresh=True,
                        tensor_fields=["text"]
                        if isinstance(index, UnstructuredMarqoIndex)
                        else None,
    ...
    def test_prefix_text_chunks_e5(self):
        ...
        for index in [self.unstructured_index_e5]:
            with self.subTest(index=index.type):
                ...
                self.assertTrue(
                    np.allclose(
                        embed_res_document_prefix["embeddings"][0],
                        retrieved_doc_a["_tensor_facets"][0]["_embedding"],
                    )
                )
                ...
                self.assertTrue(
                    np.allclose(
                        embed_res_no_prefix["embeddings"][0],
                        retrieved_doc_c["_tensor_facets"][0]["_embedding"],
                    )
                )
    ...
    def test_prefix_multimodal(self):
        ...
        for index in [self.unstructured_index_multimodal]:
            with self.subTest(index=index.type):
                ...
                self.assertTrue(
                    np.allclose(
                        res["results"][0]["_tensor_facets"][0]["_embedding"],
            ...
            with self.subTest(index=index.type):
                ...
                queries = [
                    BulkSearchQueryEntity(
                        q="hello", text_query_prefix="PREFIX: ", index=index
                    )
                ]
                ...
                queries = [
                    BulkSearchQueryEntity(
                        q={"text query": 0.5, TestImageUrls.HIPPO_REALISTIC.value: 0.5},
    ...
    def test_determine_text_chunk_prefix(self):
        ...
        with self.subTest("All prefixes on (request level chosen)"):
            result = self.unstructured_index_with_override.model.get_text_chunk_prefix(
                "request-level"
            )
        ...
        with self.subTest("Request and model default on (request level chosen)"):
            result = (
                self.unstructured_index_with_model_default.model.get_text_chunk_prefix(
                    "request-level"
                )
            )
            self.assertEqual(result, "request-level")
        ...
        self.add_documents(
            config=self.config,
            add_docs_params=AddDocsParams(
                index_name=self.unstructured_index_with_override.name,
                docs=[{"_id": "doc_a", "text": "hello"}],
                auto_refresh=True,
                tensor_fields=["text"]
                if isinstance(
                    self.unstructured_index_with_override, UnstructuredMarqoIndex
                )
                else None,
            ),
        )
        ...
        with self.subTest(
            "Embeddings are equal between overriden doc and direct embed"
        ):
            self.assertTrue(
                np.allclose(
                    embed_res["embeddings"][0],
    ...
    def test_backward_compatibility_no_prefix(self):
        ...
        mock_old_marqo_index = UnstructuredMarqoIndex(
            ...
            model=Model(
                name="hf/e5-small", text_chunk_prefix=None, text_query_prefix=None
            ),
            ...
            text_preprocessing=TextPreProcessing(
                splitLength=6, splitOverlap=1, splitMethod=TextSplitMethod.Character
            ),
            ...
            hnsw_config=HnswConfig(ef_construction=100, m=42),
            marqo_version="0.0.1",
            created_at=1,
            updated_at=1,
        )
