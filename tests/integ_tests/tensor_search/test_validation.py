...
from marqo.tensor_search import enums, validation
...
class TestValidation(unittest.TestCase):
    ...
    def test_validate_str_against_enum_case_senstive(self):
        try:
            validation.validate_str_against_enum(
                "banana", self.SimpleEnum, case_sensitive=True
            )
    ...
    def test_validate_str_against_enum_case_insensitive(self):
        assert "banana" == validation.validate_str_against_enum(
            "banana", self.SimpleEnum, case_sensitive=False
        )
    ...
    def test_validate_field_name_good(self):
        assert "some random fieldname" == validation.validate_field_name(
            "some random fieldname"
        )
            ...
            for bad_content in bad_field_content:
                try:
                    validation.validate_field_content(
                        bad_content, is_non_tensor_field=non_tensor_field
                    )
                    raise AssertionError
                except InvalidArgError:
                    pass
    ...
    def test_validate_field_content_good(self):
        good_field_content = [123, "heehee", 12.4, False]
        for non_tensor_field in (True, False):
            for good_content in good_field_content:
                assert good_content == validation.validate_field_content(
                    good_content, is_non_tensor_field=non_tensor_field
                )

    def test_validate_field_content_list(self):
        good_field_content = [[], [""], ["abc", "efg", "123"], ["", ""]]
        for good_content in good_field_content:
            assert good_content == validation.validate_field_content(
                good_content, is_non_tensor_field=True
            )
        ...
        for good_content in good_field_content:
            ...
            try:
                validation.validate_field_content(
                    good_content, is_non_tensor_field=False
                )
                raise AssertionError
            except InvalidArgError:
                pass
    ...
    def test_validate_id_good(self):
        bad_ids = [{123}, [], None, {"abw": "cjnk"}, 1234]
    ...
    def test_boost_validation_illegal(self):
        ...
        for search_method in ("TENSOR", "LEXICAL", "OTHER"):
            for bad_boost in bad_boosts:
                try:
                    validation.validate_boost(
                        boost=bad_boost, search_method=search_method
                    )
                    raise AssertionError
    ...
    def test_boost_validation_good_boost_bad_method(self):
        ...
        for search_method in ("", "LEXICAL", "OTHER"):
            for good_boost in good_boosts:
                try:
                    validation.validate_boost(
                        boost=good_boost, search_method=search_method
                    )
                    raise AssertionError
                except (InvalidArgError, InvalidFieldNameError):
                    pass
    ...
    def test_boost_validation_good_boosts(self):
        ...
        for good_boost in good_boosts:
            assert good_boost == validation.validate_boost(
                boost=good_boost, search_method="TENSOR"
            )

    def test_boost_validation_None_ok(self):
        for search_method in ("", "LEXICAL", "OTHER", "TENSOR"):
            assert None is validation.validate_boost(
                boost=None, search_method=search_method
            )
...
class TestValidateSearchableAttributes(unittest.TestCase):
    ...
    def test_search_method_not_tensor(self):
        validation.validate_searchable_attributes(
            self.searchable_attributes, search_method=enums.SearchMethod.LEXICAL
        )

    def test_maximum_searchable_attributes_not_set(self):
        validation.validate_searchable_attributes(
            self.searchable_attributes, search_method=enums.SearchMethod.TENSOR
        )

    @patch.dict(
        "os.environ", {**os.environ, **{"MARQO_MAX_SEARCHABLE_TENSOR_ATTRIBUTES": "1"}}
    )
    def test_searchable_attributes_is_none_max_value_set_raise_invalid_arg_error(self):
        try:
            validation.validate_searchable_attributes(
                searchable_attributes=None, search_method=enums.SearchMethod.TENSOR
            )
            raise AssertionError(
                "'searchable_attributes' is None, but MARQO_MAX_SEARCHABLE_TENSOR_ATTRIBUTES is set"
            )
    ...
    @patch.dict(
        "os.environ", {**os.environ, **{"MARQO_MAX_SEARCHABLE_TENSOR_ATTRIBUTES": "6"}}
    )
...
class TestValidateIndexSettings(unittest.TestCase):
    ...
    def test_validate_mappings(self):
        mappings = [
            ...
            },
            {
                ...
                "other_field": {
                    "type": "multimodal_combination",
                    "weights": {"some_text": 0.7, "bugs": 200},
                },
            },
            {},
            {" ": {"type": "multimodal_combination", "weights": {"some_text": -2}}},
            {
                "abcd ": {
                    "type": "multimodal_combination",
                    "weights": {"some_text": -4.6, "other_text": 22},
                }
            },
            {"abcd ": {"type": "multimodal_combination", "weights": {}}},
            {
                "abcd ": {
                    "type": "multimodal_combination",
                    "weights": {
    ...
    def test_validate_mappings_invalid(self):
        mappings = [
            ...
            # Field with no type
            {"my_combination_field": {"weights": {"some_text": 0.5}}},
            # Empty mapping
            {"empty field": {}},
            {
                "my_combination_field": {
            ...
            {"my_custom_vector": {"type": "custom_vector", "extra_field": "blah"}},
            # Custom vector with extra field and multimodal
            {
                "my_custom_vector": {"type": "custom_vector", "extra_field_2": "blah"},
                "abcd": {
                    "type": "multimodal_combination",
                    "weights": {"some_text": -4.6, "other_text": 22},
                },
    ...
    def test_validate_multimodal_combination_mappings_object(self):
        mappings = [
            {"type": "multimodal_combination", "weights": {"some_text": 0.5}},
            {"type": "multimodal_combination", "weights": {"some_text": -2}},
            {
                "type": "multimodal_combination",
                "weights": {"some_text": -4.6, "other_text": 22},
    ...
    def test_invalid_custom_vector_mappings_object(self):
        mappings = [
            # Extra field
            (
                {"type": "custom_vector", "extra_field": "blah"},
    ...
    def test_validate_dict(self):
        ...
        """
        test_mappings = {"my_custom_vector": {"type": "custom_vector"}}
        ...
        assert validation.validate_dict(
            field="my_custom_vector",
            field_content=obj,
            is_non_tensor_field=False,
            mappings=test_mappings,
        ...
        invalid_custom_vector_objects = [
            ...
            (
                {
                    "content": "custom content is here!!",
                    "vector": [1.0 for _ in range(index_model_dimensions - 1)]
                    + ["NOT A FLOAT"],
                },
        ...
        with self.subTest("No index model dimensions"):
            ...
            self.assertIn(
                "none is not an allowed value", str(cm.exception.args[0][0].exc)
            )
        ...
        with self.subTest("No index model dimensions"):
            ...
            self.assertIn(
                "value is not a valid integer", str(cm.exception.args[0][0].exc)
            )
...
class TestValidateDeleteDocsRequest(unittest.TestCase):
    ...
    def test_valid_delete_request(self):
        ...
        result = validation.validate_delete_docs_request(
            delete_request, self.max_delete_docs_count
        )
    ...
    def test_empty_document_ids(self):
        delete_request = MqDeleteDocsRequest(
            index_name="my_index", schema_name="my__00index", document_ids=[]
        )
    ...
    def test_document_ids_not_sequence(self):
        delete_request = MqDeleteDocsRequest(
            index_name="my_index", schema_name="my__00index", document_ids="id1"
        )
        with self.assertRaises(InvalidArgError):
            validation.validate_delete_docs_request(
                delete_request, self.max_delete_docs_count
