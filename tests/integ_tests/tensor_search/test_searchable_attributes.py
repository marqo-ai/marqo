...
class TestSearchableAttributes(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.indexes = cls.create_indexes(
            [structured_text_index, semi_structured_text_index]
        )
    ...
    def _add_documents(self, index_name):
        self.add_documents(
            ...
            add_docs_params=AddDocsParams(
                ...
                tensor_fields=["text_field_1", "text_field_2", "text_field_3"]
                if index_name != self.structured_text_index
                else None,
            ),
        )
            ...
            with self.subTest(index_name=index_name):
                ...
                for search_method in ["TENSOR", "LEXICAL"]:
                    for (
                        ...
                        with self.subTest(
                            f"{search_method}-{searchable_attributes}-{filter_string}"
                        ):
                            res = tensor_search.search(
                                config=self.config,
                                index_name=index_name,
    ...
    def test_searchable_attributes_behaves_the_same_way_for_different_types_of_indexes(
        self,
    ):
        for index_name in [self.structured_text_index, self.semi_structured_text_index]:
            self._add_documents(index_name)
