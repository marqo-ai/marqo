...
class TestSearchRegression(MarqoTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.indexes = cls.create_indexes(
            [structured_text_index_score_modifiers, unstructured_text_index]
        )
            ...
            with self.subTest(index=type(index)):
                ...
                for search_method in [SearchMethod.TENSOR, SearchMethod.LEXICAL]:
                    with self.subTest(search_method=search_method):
                        ...
                        self.assertEqual(
                            len(search_res["hits"]),
                            len(expected_results[search_method]),
                        )
                        for i in range(len(search_res["hits"])):
                            ...
                            if (search_res["hits"][i]["_id"] in same_score_group) and (
                                search_method == SearchMethod.LEXICAL
                            ):
                                self.assertIn(
                                    expected_results[search_method][i]["_id"],
                                    same_score_group,
                                )
                            else:
                                self.assertEqual(
                                    search_res["hits"][i]["_id"],
                                    expected_results[search_method][i]["_id"],
                                )
                            self.assertTrue(
                                np.allclose(
                                    search_res["hits"][i]["_score"],
                                    expected_results[search_method][i]["_score"],
                                    atol=1e-6,
                                ),
    ...
    def test_document_vectors_match_2_9(self):
        ...
        for index in [
            ...
            with self.subTest(index=index.name):
                ...
                # Get document
                fetched_doc = tensor_search.get_document_by_id(
                    self.config, index.name, "doc10", show_vectors=True
                )
                self.assertTrue(
                    np.allclose(
                        fetched_doc["_tensor_facets"][0]["_embedding"],
