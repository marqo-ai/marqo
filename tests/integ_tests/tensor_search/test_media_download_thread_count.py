...
class AddDocsParams:
    def __init__(
        self, media_download_thread_count=None, image_download_thread_count=None
    ):
...
class TestDetermineThreadCount(unittest.TestCase):
    ...
    def test_both_thread_counts_set_in_params_non_languagebind_model(self):
        ...
        marqo_index = MarqoIndex(model_type="other")
        add_docs_params = AddDocsParams(
            media_download_thread_count=8, image_download_thread_count=15
        )
        result = _determine_thread_count(marqo_index, add_docs_params)
        self.assertEqual(result, 8)

    def test_media_thread_count_in_env_image_thread_count_in_params_non_languagebind_model(
        self,
    ):
    ...
    def test_media_thread_count_in_params_image_thread_count_in_env_non_languagebind_model(
        self,
    ):
    ...
    def test_media_thread_count_equals_default_image_thread_count_set_non_languagebind_model(
        self,
    ):
        """Test when media_download_thread_count equals default and image_download_thread_count is set for non-languagebind model."""
        marqo_index = MarqoIndex(model_type="other")
        add_docs_params = AddDocsParams(
            media_download_thread_count=5, image_download_thread_count=15
        )
        result = _determine_thread_count(marqo_index, add_docs_params)
        self.assertEqual(result, 15)
