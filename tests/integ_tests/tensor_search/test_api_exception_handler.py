...
from marqo.tensor_search.api import (
...
class TestBaseExceptionHandler(MarqoTestCase):
    ...
    def test_base_exception_handler_base_errors(self, mock_api_exception_handler):
        with self.subTest("Base error: Internal Error"):
            marqo_base_exception_handler(
                self.normal_request,
                base_exceptions.InternalError(self.generic_error_message),
            )
            assert isinstance(
                mock_api_exception_handler.call_args_list[-1][0][1],
                api_exceptions.InternalError,
            )  # 500
    ...
    def test_base_exception_handler_core_errors(self, mock_api_exception_handler):
        ...
        with self.subTest("Core error: Parsing Error"):
            marqo_base_exception_handler(
                self.normal_request,
                core_exceptions.ParsingError(self.generic_error_message),
            )
            assert isinstance(
                mock_api_exception_handler.call_args_list[-1][0][1],
                api_exceptions.MarqoWebError,
            )  # 500
    ...
    def test_base_exception_handler_vespa_errors(self, mock_api_exception_handler):
        with self.subTest("Vespa error: Vespa Error"):
            marqo_base_exception_handler(
                self.normal_request,
                vespa_exceptions.VespaError(self.generic_error_message),
            )
            assert isinstance(
                mock_api_exception_handler.call_args_list[-1][0][1],
                api_exceptions.MarqoWebError,
    ...
    def test_base_exception_handler_unhandled_error(self, mock_api_exception_handler):
        ...
        marqo_base_exception_handler(
            self.normal_request,
            base_exceptions.MarqoError("This should not be propagated."),
        )
    ...
    async def test_validation_exception_handler_CorrectResponseBody(self):
        ...
        response = await api_validation_exception_handler(
