...
from marqo.s2_inference.clip_utils import encode_url
...
class TestImageDownloading(TestCase):
    def test_encode_url_HandleDifferentUrlsCorrectly(self):
        test_cases = [
            ...
            ("http://example.com/你好世界", "http://example.com/%E4%BD%A0%E5%A5%BD%E4%B8%96%E7%95%8C", "URL with non-ASCII characters"),
            ("http://example.com/test?name=John Doe&age=30", "http://example.com/test?name=John%20Doe&age=30", "URL with query parameters"),
            ...
            ("http://example.com/test url%20example", "http://example.com/test%20url%20example", "URL with mixed encoded and unencoded characters"),
            ("http://例子.测试", "http://%E4%BE%8B%E5%AD%90.%E6%B5%8B%E8%AF%95", "URL with unicode characters in the domain"),
        ...
        for url, expected, msg in test_cases:
