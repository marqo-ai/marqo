...
class ModelCache:
    ...
    onnx_cache_path = os.environ.get(
        "ONNX_SAVE_PATH", f"{utils.get_marqo_root_from_env()}/cache/models_onnx/"
    )
    torch_cache_path = os.getenv(
        "SENTENCE_TRANSFORMERS_HOME", f"{utils.get_marqo_root_from_env()}/cache/models/"
    )
    clip_cache_path = os.getenv(
        "CLIP_SAVE_PATH", f"{utils.get_marqo_root_from_env()}/cache/clip/"
    )
    # The hf_cache_path is managed by the hf_hub_download function
    hf_cache_path = os.getenv(
        "HF_SAVE_PATH", f"{utils.get_marqo_root_from_env()}/cache/hf/"
    )

    languagebind_cache_path = os.getenv(
        "LANGUAGEBIND_CACHE_PATH",
        f"{utils.get_marqo_root_from_env()}/cache/languagebind/",
    )
