...
from marqo.s2_inference.reranking.cross_encoders import ReRankerOwl, ReRankerText
...
def rerank_search_results(
    ...
    overwrite_original_scores_highlights: bool = True,
) -> None:
    ...
    if not _check_searchable_fields_in_results(
        search_results=search_result, searchable_fields=searchable_attributes
    ):
    ...
    if "owl" in model_name.lower():
        ...
        if searchable_attributes in (None, [], (), ""):
            raise RerankerError(
                f"found searchable_attributes={searchable_attributes} but expected list of strings for {model_name}"
            )

        if len(searchable_attributes) > 1:
            logger.info(
                f"currently only a single attribute can be reranked over for {model_name}. taking the first field {[searchable_attributes[0]]} from {searchable_attributes}"
            )
            searchable_attributes = [searchable_attributes[0]]

        try:
            reranker = ReRankerOwl(
                model_name=model_name, device=device, image_size=(240, 240)
            )
            reranker.rerank(
                query=query,
                results=search_result,
                image_attributes=searchable_attributes,
            )
        ...
        try:
            reranker = ReRankerText(
                model_name=model_name, device=device, num_highlights=num_highlights
            )
            reranker.rerank(
                query=query,
                results=search_result,
...
def _check_searchable_fields_in_results(
    search_results: Dict, searchable_fields: List[str] = None
) -> bool:
    """
