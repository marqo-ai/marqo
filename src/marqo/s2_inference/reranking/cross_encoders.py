...
class FormattedResults:
    ...
    def __init__(
        self,
        results: Dict,
        highlights_field: str = ResultsFields.highlights,
    ...
    def results_to_df(self) -> None:
        ...
        if self.searchable_fields is not None and isinstance(
            self.searchable_fields, list
        ):
            self.results[ResultsFields.hits] = [
                r
                for r in self.results[ResultsFields.hits]
                if all(s in r for s in self.searchable_fields)
            ]
        ...
        def _get_highlights(content):
            if content == []:
                ...
                if len(_key) > 1:
                    logger.warning(
                        f"found more than 1 highlight. found {_key}. keeping the first only..."
                    )
                return content[_key[0]]

        if self.highlights_field in self.results_df:
            self.results_df[self.highlights_field] = self.results_df[
                self.highlights_field
            ].apply(_get_highlights)
    ...
    def _get_searchable_columns(self) -> None:
        ...
        self.searchable_fields = [
            field
            for field in self.results_df.columns.tolist()
            if not field.startswith("_")
        ]

    @staticmethod
    def format_for_model(
        results_df: pd.DataFrame, searchable_fields: List[str], query: str = None
    ):
        ...
        for field in searchable_fields:
            _inputs_df = results_df[
                [field, ResultsFields.reranked_id, ResultsFields.original_score]
            ]
...
class ReRanker:
    ...
    def format_results(
        self, results: Dict, query: str = None, searchable_fields: List[str] = None
    ):
        ...
        """
        self.results = results
        self.formatted_results = FormattedResults(
            self.results, searchable_fields=searchable_fields
        )

    @staticmethod
    def _prepare_inputs(
        inputs_df: pd.DataFrame,
        query_column: str = Columns.query,
        content_column: str = Columns.field_content,
    ) -> List[List[str]]:
    ...
    def get_reranked_results(
        self,
        score_column: str = ResultsFields.reranker_score,
        ...
        for i, group in self.inputs_df.groupby(ResultsFields.reranked_id):
            group = group.sort_values(score_column, ascending=False).head(
                self.num_highlights
            )
        ...
        for result in self.results[ResultsFields.hits]:
            ...
            if self.num_highlights == 1:
                _content = [
                    {_df[Columns.original_field_name]: _df[highlight_content_column]}
                ]
            else:
                _content = [
                    {row[Columns.original_field_name]: row[highlight_content_column]}
                    for _, row in _df.iterrows()
                ]

            result[ResultsFields.highlights_reranked] = _content

        self.results[ResultsFields.hits] = sorted(
            self.results[ResultsFields.hits],
            key=lambda x: x[ResultsFields.reranker_score],
            reverse=True,
        )
...
class ReRankerText(ReRanker):
    ...
    def __init__(
        ...
        if self.split_params is not None and isinstance(
            self.split_params, (dict, defaultdict)
        ):
    ...
    def load_model(self) -> None:
        self.model = load_sbert_cross_encoder_model(
            model_name=self.model_name, device=self.device, max_length=self.max_length
        )["model"]
    ...
    def explode_nested_content_field(self, inputs_df: pd.DataFrame) -> pd.DataFrame:
        ...
        _func = partial(
            text_processor.split_text,
            split_length=self.split_length,
            split_overlap=self.split_overlap,
            split_by=self.split_method,
        )
        inputs_df = inputs_df.merge(
            inputs_df[Columns.field_content].apply(_func).explode(),
            left_index=True,
            right_index=True,
        )
        inputs_df[Columns.field_content_original] = inputs_df[
            Columns.field_content + "_x"
        ]
    ...
    def rerank(
        self, query: str, results: Dict, searchable_attributes: List[str] = None
    ) -> None:
        ...
        self.inputs_df = self.formatted_results.format_for_model(
            self.formatted_results.results_df, self.searchable_attributes, query=query
        )
        ...
        if self.split_params is not None:
            ...
            logger.info(
                f"chunking field content, went from length {_n} to {len(self.inputs_df)}"
            )
        ...
        if not _verify_model_inputs(self.model_inputs):
            raise RuntimeError(
                f"incorrect model inputs, expected list of lists but recevied {type(self.model_inputs)} and {type(self.model_inputs[0])}"
            )
        ...
        self.inputs_df[ResultsFields.hybrid_score_multiply] = np.clip(
            self.inputs_df[ResultsFields.original_score], 1e-3, np.inf
        ) * np.clip(self.inputs_df[ResultsFields.reranker_score], 1e-3, np.inf)
        self.inputs_df[ResultsFields.hybrid_score_add] = (
            self.inputs_df[ResultsFields.original_score]
            + self.inputs_df[ResultsFields.reranker_score]
        )
...
class ReRankerOwl(ReRanker):
    ...
    def __init__(self, model_name: str, device: str, image_size: Tuple):
        ...
        if self.model_name not in self._model_map:
            raise RerankerNameError(
                f"could not find model_name={self.model_name} in mappings {list(self._model_map.keys())}"
            )
    ...
    def load_model(self):
        ...
        logger.info(
            f"loading model={self._remapped_name} from input name={self.model_name} to device {self.device}"
        )
    ...
    @staticmethod
    def load_images(
        content: List[str], size: Tuple[int]
    ) -> Tuple[ImageType, List[Tuple]]:
    ...
    def rerank(
        self, query: str, results: Dict, image_attributes: List, num_highlights: int = 1
    ):
        ...
        # first stage of formatting converts results dict to dataframe
        self.inputs_df = self.formatted_results.format_for_model(
            self.formatted_results.results_df, self.image_attributes, query=query
        )
        ...
        # try:
        self.images, self.original_sizes = self.load_images(
            image_names, self.image_size
        )
        ...
        for image, content, _query, orig_size in zip(
            self.images, image_names, queries, self.original_sizes
        ):
            self.processed_inputs = _process_owl_inputs(
                self.processor, _query, image
            ).to(self.device)
            owl_results = _predict_owl(
                self.model,
            ...
            boxes, scores, identifier = (
                _keep_top_k(boxes, k=num_highlights),
                _keep_top_k(scores, k=num_highlights),
                _keep_top_k(identifier, k=num_highlights),
            )

            # just keep top from each image
            _bo += [
                list(
                    image_processor.rescale_box(
                        b.detach().cpu().tolist(), self.image_size, orig_size
                    )
                )
                for b in boxes
            ]
        ...
        self.boxes_scores_df = pd.DataFrame(
            zip(
                self.boxes.detach().cpu().tolist(),
                self.scores.detach().cpu().tolist(),
                self.identifier,
                self.boxes_original,
            ),
            columns=[
                Columns.bbox,
                ResultsFields.reranker_score,
                Columns.field_content,
                Columns.bbox_original,
            ],
        )

        # now merge back with original - filed_content is the image pointer and can be used as the identifier
        self.inputs_df = self.boxes_scores_df.merge(
            self.inputs_df,
            left_on=Columns.field_content,
            right_on=Columns.field_content,
        )
