...
class SBERT_ONNX(object):
    ...
    def _get_paths(self) -> None:
        ...
        if self.onnx_model_name is None:
            self.onnx_model_name = (
                f"{os.path.basename(self.model_name_or_path.replace('/', '_'))}.onnx"
            )

        self.export_model_name = os.path.join(
            self.onnx_folder, f"{self.onnx_model_name}"
        )
    ...
    def _get_onnx_provider(self) -> None:
        ...
        logger.info(
            f"device:{self.device} and available providers {self.onnxproviders}"
        )
        if self.device == "cpu":
    ...
    @staticmethod
    def mean_pooling(
        token_embeddings: FloatTensor, attention_mask: FloatTensor
    ) -> FloatTensor:
    ...
    def encode(
        self, sentences: Union[str, List[str]], normalize: bool = True, **kwargs
    ) -> FloatTensor:
        ...
        result = self.mean_pooling(
            token_embeddings=torch.FloatTensor(ort_outputs[0]),
