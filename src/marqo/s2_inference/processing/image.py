...
from marqo.s2_inference.types import ImageType, List, Literal, Tuple, Union, ndarray
...
def chunk_image(
    ...
    device: str,
    method: Literal[
        "simple", "overlap", "frcnn", "marqo-yolo", "yolox", "dino-v1", "dino-v2"
    ],
    size=get_default_size(),
) -> Tuple[List[ImageType], List[float]]:
    """_summary_
    ...
    if method in [None, "none", "", "None", " "]:
        if isinstance(image, str):
            ...
            raise TypeError(
                f"only pointers to an image or a PIL image are allowed. received {type(image)}"
            )
    ...
    if method == "simple":
        ...
        patch = PatchifySimple(size=size, hn=hn, wn=wn, overlap=True)
...
class PatchifySimple:
    ...
    def infer(self, image: Union[str, ImageType]):
        ...
        self.bboxes_simple = generate_boxes(
            self.size, self.hn, self.wn, overlap=self.overlap
        )
    ...
    def process(self):
        ...
        self.bboxes_orig = [
            rescale_box(bb, self.size, self.original_size) for bb in self.bboxes
        ]
...
class PatchifyModel:
        ...
        with _load_model_lock:
            if model_cache_key not in get_available_models():
                ...
                get_available_models()[model_cache_key] = {
                    AvailableModelsKey.model: (self.model, self.preprocess),
                    AvailableModelsKey.most_recently_used_time: datetime.datetime.now(),
                }

            else:
                self.model, self.preprocess = get_available_models()[model_cache_key][
                    AvailableModelsKey.model
                ]

    def _load_image(self, image):
        self.image, self.image_pt, self.original_size = load_rcnn_image(
    ...
    def _filter_bb(self):
        ...
        if self.filter_bb:
            ...
            self.boxes_xyxy = [
                bb for ind, bb in enumerate(self.boxes_xyxy) if ind in self.inds
            ]
            if len(self.scores) == self.n_prefilter:
                self.scores = [
                    bb for ind, bb in enumerate(self.scores) if ind in self.inds
                ]
    ...
    def _replace_small_bb(self):
        ...
        if self.replace_small:
            if len(self.boxes_xyxy):
                self.boxes_xyxy = replace_small_boxes(
                    self.boxes_xyxy,
                    min_area=self.min_area_replace,
                    new_size=self.new_size,
                )
                self.boxes_xyxy = clip_boxes(
                    self.boxes_xyxy, 0, 0, self.size[0], self.size[1]
                )
    ...
    def _nms_bb(self):
        ...
        if self.nms:
            if len(self.boxes_xyxy) > 1:
                logger.debug(
                    f"doing nms for {len(self.boxes_xyxy)} {self.n_postfilter} boxes..."
                )
                self.scores_pt = torch.tensor(self.scores, dtype=torch.float32)

                self.inds = torchvision.ops.nms(
                    torch.tensor(self.boxes_xyxy, dtype=torch.float32),
                    self.scores_pt.squeeze(),
                    self.iou_thresh,
                )
    ...
    def _keep_top_k_sorted(self):
        ...
        if len(self.scores) > self.top_k_scores:
            self.inds = np.argsort(np.array(self.scores).squeeze())[::-1][
                : self.top_k_scores
            ]
            self.boxes_xyxy = [self.boxes_xyxy[ind] for ind in self.inds]
            self.scores = [self.scores[ind] for ind in self.inds]
...
class PatchifyViT(PatchifyModel):
    ...
    def infer(self, image):
        ...
        self.attentions = DINO_inference(
            self.model, self.preprocess, self.image, self.patch_size, device=self.device
        )

        self.attentions_processed = self._process_attention(
            self.attentions, method=self.attention_method
        )
    ...
    def _process_attention(
        attentions: ndarray, method: Literal["abs", "pos"]
    ) -> List[ndarray]:
...
class PatchifyYolox(PatchifyModel):
    ...
    def infer(self, image):
        ...
        self.results, self.ratio = _infer_yolox(
            session=self.model,
            preprocess=self.preprocess,
            opencv_image=self.image_cv,
            input_shape=self.input_shape,
        )

        self.boxes_xyxy, self.scores = _process_yolox(
            output=self.results, ratio=self.ratio, size=self.input_shape
        )
        if isinstance(self.scores, (np.ndarray, np.generic)):
            self.scores = self.scores.tolist()
