...
from marqo.s2_inference.types import FloatTensor, ImageType, List, Tuple, Union, ndarray
...
def _keep_topk(
    boxes_xyxy: Union[ndarray, FloatTensor], k: int = 10
) -> Union[ndarray, FloatTensor]:
...
def load_rcnn_image(
    image_name: str, size: Tuple = (320, 320)
) -> Tuple[ImageType, FloatTensor, Tuple[int, int]]:
    ...
    if isinstance(image_name, ImageType):
        ...
        raise TypeError(
            f"received {type(image_name)} but expected a string or PIL image"
        )
...
def calc_area(
    bboxes: Union[List[List], FloatTensor, ndarray],
...
def filter_boxes(
    bboxes: Union[FloatTensor, ndarray],
...
def rescale_box(
    box: Union[List[float], ndarray, FloatTensor], from_size: Tuple, to_size: Tuple
) -> List[float]:
...
def generate_boxes(
    image_size: Tuple[int, int], hn: int, wn: int, overlap: bool = False
) -> List[Tuple]:
...
def replace_small_boxes(
    boxes: Union[List[List[float]], List[Tuple[float]], ndarray],
    min_area: float = 40 * 40,
    new_size: Tuple = (100, 100),
) -> List[Tuple]:
...
def clip_boxes(
    boxes: Union[List, ndarray], xmin: int, ymin: int, xmax: int, ymax: int
) -> List[Tuple]:
...
def patchify_image(
    image: ImageType, bboxes: Union[List[float], FloatTensor, ndarray]
) -> List[ImageType]:
...
def _process_patch_method(method: str) -> Tuple[str, dict]:
    ...
    try:
        ...
        raise ChunkerMethodProcessError(
            message=f"could not pass parameters for string {query} from full path {method}"
        )
