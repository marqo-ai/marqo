...
from marqo.s2_inference.types import Dict, Tuple, ndarray
...
def preprocess_yolox(
    img: ndarray, input_size: Tuple, swap: Tuple = (2, 0, 1)
) -> Tuple[ndarray, float]:
...
def load_yolox_onnx(
...
) -> Tuple[onnxruntime.InferenceSession, preprocess_yolox]:
    ...
    session = onnxruntime.InferenceSession(
        model_name, sess_options=sess_options, providers=[fast_onnxprovider]
    )
    session.disable_fallback()
    logger.info(
        f"using yolox onnx provider:{session.get_providers()} after requesting {fast_onnxprovider}"
    )
...
def demo_postprocess(
    outputs: ndarray, img_size: Tuple[int, int], p6: bool = False
) -> ndarray:
...
def _process_yolox(
    output: ndarray, ratio: float, size: Tuple = (384, 384)
