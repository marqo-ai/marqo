...
from marqo.s2_inference.types import FloatTensor, ImageType, List, Union, ndarray
...
def sentence_to_hash(sentence):
    ...
    if isinstance(sentence, ImageType):
        ...
        if isinstance(pixel_data[0], int):
            ...
            image_average = functools.reduce(lambda x, y: x + y, pixel_data) / len(
                pixel_data
            )
        else:
            ...
            image_average = functools.reduce(lambda x, y: x + y, pixel_averages) / len(
                pixel_data
            )
        return (
            int(hashlib.sha256(str(image_average).encode("utf-8")).hexdigest(), 16)
            % 10**8
        )
    else:
...
class Random(Model):
    ...
    def encode(
        ...
        if self.embedding_dimension is None or self.embedding_dimension == 0:
            raise ValueError(
                "invalid embedding dimension size. check the model registry is correct"
            )
