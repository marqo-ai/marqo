...
from marqo.s2_inference.errors import ImageDownloadError, InvalidModelPropertiesError
from marqo.s2_inference.types import *
from marqo.s2_inference.types import Modality
from marqo.tensor_search.enums import EnvVars, InferenceParams, ModelProperties
...
def _get_transform(
    n_px: int, image_mean: List[float] = None, image_std: List[float] = None
) -> torch.Tensor:
...
def format_and_load_CLIP_images(
    images: List[Union[str, ndarray, ImageType]], media_download_headers: dict
) -> List[ImageType]:
...
def load_image_from_path(
    ...
    if os.path.isfile(image_path):
        ...
        raise UnidentifiedImageError(
            f"Input str of {image_path} is not a local file or a valid url. "
...
class CLIP:
    ...
    def _download_from_repo(self):
        ...
        model_location = ModelLocation(
            **self.model_properties[ModelProperties.model_location]
        )
    ...
    def load(self) -> None:
        ...
        if path is None and not model_location_presence:
            ...
            logger.info(
                "Detecting custom clip model path. We use generic clip model loading."
            )
            ...
            if model_location_presence:
                ...
                raise InvalidModelPropertiesError(
                    f"Marqo can not load the custom clip model."
    ...
    def custom_clip_load(self):
        ...
        logger.info(
            f"The name of the custom clip model is {self.model_name}. We use openai clip load"
        )
        # We must load the model into CPU then transfer it to the desired device, always
    ...
    def _preprocess_images(
        self,
        images: Union[str, ImageType, List[Union[str, ImageType, Tensor]], Tensor],
        ...
        if isinstance(images, list):
            image_input: List[Union[ImageType, Tensor]] = format_and_load_CLIP_images(
                images, media_download_headers
            )
        else:
            image_input: List[Union[ImageType, Tensor]] = [
                format_and_load_CLIP_image(images, media_download_headers)
            ]

        image_input_processed: Tensor = torch.stack(
            [
                self.preprocess(_img).to(self.device)
    ...
    def encode_image(
        ...
        """
        self.image_input_processed: Tensor = self._preprocess_images(
            images, media_download_headers
        )
        with torch.no_grad():
        ...
        **kwargs,
        ...
        if infer and _is_image(inputs):
            ...
            if default == "text":
                ...
                raise UnidentifiedImageError(
                    f"expected default='image' or default='text' but received {default}"
                )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.device.startswith("cuda"):
            logger.warning(
                f"The fp16 clip model `{self.model_type} is loaded with device `{self.device}`."
...
class MULTILINGUAL_CLIP(CLIP):
    def __init__(
        self,
        model_type: str = "multilingual-clip/ViT-L/14",
        device: str = None,
        embedding_dim: int = None,
        truncate: bool = True,
        **kwargs,
        ...
        if not device:
            raise InternalError(
                "`device` is required for loading MULTILINGUAL CLIP models!"
            )
    ...
    def load(self) -> None:
        if self.visual_name.startswith("openai/"):
            ...
            self.visual_model, self.preprocess = clip.load(
                name=clip_name,
                device="cpu",
                jit=False,
                download_root=ModelCache.clip_cache_path,
            ...
            self.visual_model, _, self.preprocess = (
                open_clip.create_model_and_transforms(
                    model_name=clip_name.split("/")[0],
                    pretrained=clip_name.split("/")[1],
                    device=self.device,
                )
            )
            self.visual_model = self.visual_model.visual

        self.textual_model = pt_multilingual_clip.MultilingualCLIP.from_pretrained(
            self.textual_name, self.device
        )
    ...
    def encode_image(
        self,
        images: Union[str, ImageType, List[Union[str, ImageType]]],
        normalize=True,
        media_download_headers: Optional[dict] = None,
    ) -> FloatTensor:
        ...
        self.image_input_processed = torch.stack(
            [self.preprocess(_img).to(self.device) for _img in image_input]
        )
