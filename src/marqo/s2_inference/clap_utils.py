...
def _is_audio(inputs: Union[str, List[Union[str, ImageType, ndarray]]]) -> bool:
    ...
    if isinstance(thing, str):
        ...
        if os.path.isfile(thing):
            ...
            raise ValueError(f"local file [{thing}] extension {extension} does not match allowed file types of {_allowed}")
        else:
    ...
    # if it is an array, then it is an image
        ...
        raise ValueError(f"expected type Image or str for inputs but received type {type(thing)}")



def format_and_load_CLAP_audio(audio: Union[str, ndarray], download_headers: dict) -> np.ndarray:
    ...
    if isinstance(audio, str):
        ...
        raise ValueError(f"input of type {type(audio)} did not match allowed types of str, np.ndarray")

    return audio_data

def format_and_load_CLAP_audios(audios: Union[str, np.ndarray, List[Union[str, np.ndarray]]], download_headers: Optional[Dict] = None) -> np.ndarray:
    if not isinstance(audios, list):
        raise TypeError(f"expected list but received {type(audios)}")

    results = []
    for image in audios:
        results.append(format_and_load_CLAP_audio(image, download_headers))
    
    return results

def load_audio_from_path(audio_path: str, download_headers: dict, timeout=15,
    ...
    if os.path.isfile(audio_path):
        ...
        try:
            if metrics_obj is not None:
                metrics_obj.start(f"image_download.{audio_path}")

            with requests.get(audio_path, stream=True, timeout=timeout, headers=download_headers) as resp:
                if not resp.ok:
                    raise ValueError(
                        f"image url `{audio_path}` returned {resp.status_code}. Reason: {resp.reason}")

                audio, _ = librosa.load(io.BytesIO(resp.content), sr=SR)

            if metrics_obj is not None:
                metrics_obj.stop(f"image_download.{audio_path}")

        except (requests.exceptions.ConnectTimeout, requests.exceptions.ConnectionError,
                requests.exceptions.RequestException
                ) as e:
            raise ValueError(
                f"image url `{audio_path}` is unreachable, perhaps due to timeout. "
                f"Timeout threshold is set to {timeout} seconds."
                f"\nConnection error type: `{e.__class__.__name__}`")
    else:
        raise ValueError(f"input str of `{audio_path}` is not a local file or a valid url")
...
class CLAP:
    ...
    def load(self) -> None:
        ...
        if path is None and not model_location_presence:
            self.model = ClapModel.from_pretrained(self.model_name).eval().to(self.device)
        ...
        **kwargs
        ...
        if infer and _is_audio(inputs):
            ...
            if default == 'text':
                ...
                raise ValueError(f"expected default='audio' or default='text' but received {default}")

        if is_image:
            logger.debug('image')
            image_download_headers = kwargs.get("image_download_headers", dict())
            return self.encode_audio(inputs, normalize=normalize, download_headers=image_download_headers)
        else:
            logger.debug('text')
