...
from marqo.core.unstructured_vespa_index.common import (
    MARQO_DOC_MULTIMODAL_PARAMS,
    MARQO_DOC_MULTIMODAL_PARAMS_WEIGHTS,
)
...
class SemiStructuredVespaDocumentFields(MarqoBaseModelV2):
    ...
    short_string_fields: Dict[str, str] = Field(
        default_factory=dict, alias=common.SHORT_STRINGS_FIELDS
    )
    ...
    int_fields: Union[Dict[str, int], List[Dict]] = Field(
        default_factory=dict, alias=common.INT_FIELDS
    )
    float_fields: Union[Dict[str, float], List[Dict]] = Field(
        default_factory=dict, alias=common.FLOAT_FIELDS
    )
    score_modifiers_fields: Dict[str, Any] = Field(
        default_factory=dict, alias=common.SCORE_MODIFIERS
    )
    vespa_multimodal_params: Dict[str, str] = Field(
        default_factory=dict, alias=common.VESPA_DOC_MULTIMODAL_PARAMS
    )

    # metadata fields
    version_uuid: Optional[str] = Field(
        default=None, alias=common.VESPA_DOC_VERSION_UUID
    )
    field_types: Dict[str, str] = Field(
        default_factory=dict, alias=common.VESPA_DOC_FIELD_TYPES
    )
    vector_counts: int = Field(default=0, alias=common.FIELD_VECTOR_COUNT)

    # Only in search result
    match_features: Dict[str, Any] = Field(
        default_factory=dict, alias=common.VESPA_DOC_MATCH_FEATURES
    )
    raw_tensor_score: Optional[float] = Field(
        default=None, alias=common.VESPA_DOC_HYBRID_RAW_TENSOR_SCORE
    )
    raw_lexical_score: Optional[float] = Field(
        default=None, alias=common.VESPA_DOC_HYBRID_RAW_LEXICAL_SCORE
    )
...
class SemiStructuredVespaDocument(MarqoBaseModelV2):
    ...
    id: str
    fixed_fields: SemiStructuredVespaDocumentFields = Field(
        default_factory=SemiStructuredVespaDocumentFields
    )
    ...
    @classmethod
    def from_vespa_document(
        cls, document: Dict, marqo_index: SemiStructuredMarqoIndex
        ...
        string_array_field_map = (
            marqo_index.string_array_field_name_to_string_array_field_map
        )
    ...
    def from_marqo_document(
        ...
        instance = cls(
            ...
            fixed_fields=SemiStructuredVespaDocumentFields(
                marqo__id=doc_id,
                version_uuid=generate_uuid_str()
        ...
        instance.fixed_fields.vespa_multimodal_params = document.get(
            common.MARQO_DOC_MULTIMODAL_PARAMS, {}
        )
    ...
    def _process_regular_fields(
        ...
        for field_name, field_content in document.items():
            if field_name in [
                index_constants.MARQO_DOC_ID,
                constants.MARQO_DOC_TENSORS,
            ]:
                continue
            try:
                cls._handle_field_content(
                    field_name, field_content, instance, marqo_index
                )
    ...
    @classmethod
    def _handle_field_content(
        cls,
        field_name: str,
        field_content: Union[str, bool, list, int, float, dict],
        instance,
        marqo_index: SemiStructuredMarqoIndex,
    ):
        ...
        elif isinstance(field_content, list) and all(
    ...
    def _handle_string_field(
        ...
        if field_name not in marqo_index.field_map:
            raise MarqoDocumentParsingError(
                f"Field {field_name} is not in index {marqo_index.name}"
            )
    ...
    def _handle_string_array_field(
        cls, field_name: str, field_content: List[str], instance
    ):
        if instance.index_supports_partial_updates:
            instance.string_array_fields[field_name] = field_content
            instance.fixed_fields.field_types[field_name] = (
                MarqoFieldTypes.STRING_ARRAY.value
            )
        else:
            instance.fixed_fields.string_arrays.extend(
                [f"{field_name}::{element}" for element in field_content]
            )

    @classmethod
    def _handle_numeric_field(
        cls, field_name: str, field_content: Union[int, float], instance
    ):
        if isinstance(field_content, int):
            instance.fixed_fields.int_fields[field_name] = field_content
            if instance.index_supports_partial_updates:
                instance.fixed_fields.field_types[field_name] = (
                    MarqoFieldTypes.INT.value
                )
        else:  # float
    ...
    def _handle_dict_field(
        cls, field_name: str, field_content: Dict[str, Union[int, float]], instance
    ):
        for k, v in field_content.items():
            ...
            if isinstance(v, int):
                ...
                if instance.index_supports_partial_updates:
                    instance.fixed_fields.field_types[field_key] = (
                        MarqoFieldTypes.INT_MAP.value
                    )
                    instance.fixed_fields.field_types[field_name] = (
                        MarqoFieldTypes.INT_MAP.value
                    )  # Marking the overall dict field as a int_map_entry field as well
                ...
                if instance.index_supports_partial_updates:
                    ...
                    instance.fixed_fields.field_types[field_name] = (
                        MarqoFieldTypes.FLOAT_MAP.value
                    )  # Marking the overall dict field as a float_map_entry field as well

    @classmethod
    def _process_tensor_fields(
        cls, document: Dict, instance, marqo_index: SemiStructuredMarqoIndex
    ):
        """Process tensor fields in the document"""
        vector_count = 0
        if constants.MARQO_DOC_TENSORS in document:
            for marqo_tensor_field, tensor_value in document[
                constants.MARQO_DOC_TENSORS
            ].items():
                if instance.index_supports_partial_updates:
                    instance.fixed_fields.field_types[marqo_tensor_field] = (
                        MarqoFieldTypes.TENSOR.value
                    )  # Set field_types as tensor for tensor fields

                    multimodal_params = document.get(MARQO_DOC_MULTIMODAL_PARAMS)
                    if (
                        multimodal_params is not None
                        and multimodal_params.get(marqo_tensor_field) is not None
                    ):  # Set field_types as tensor for sub-fields of multimodal combo fields
                        try:
                            multimodal_params = json.loads(
                                document.get(MARQO_DOC_MULTIMODAL_PARAMS).get(
                                    marqo_tensor_field
                                )
                            )
                            multimodal_combo_sub_fields = multimodal_params.get(
                                MARQO_DOC_MULTIMODAL_PARAMS_WEIGHTS
                            ).keys()
                            for sub_field in multimodal_combo_sub_fields:
                                instance.fixed_fields.field_types[sub_field] = (
                                    MarqoFieldTypes.TENSOR.value
                                )
                        except json.JSONDecodeError as e:
                            raise MarqoDocumentParsingError(
                                f"Error parsing multimodal params for field {marqo_tensor_field}: {str(e)}"
                            )
    ...
    def to_vespa_document(self) -> Dict[str, Any]:
        ...
        vespa_fields = {
            **{
                k: v
                for k, v in self.fixed_fields.model_dump(
        ...
        if self.index_supports_partial_updates:
            if self.string_array_fields is not None:
                for (
                    string_array_key,
                    string_array_value,
                ) in self.string_array_fields.items():
    ...
    def to_marqo_document(
        self, marqo_index: SemiStructuredMarqoIndex
    ) -> Dict[str, Any]:
        """
        ...
        if self.index_supports_partial_updates and self.string_array_fields:
            ...
            for string_array in self.fixed_fields.string_arrays:
                string_array_key, string_array_value = string_array.split(
                    "::", 1
                )  # String_array_key will be string in this case, and string_array_value will be a single string in this case.
        ...
        marqo_document.update(
            {k: bool(v) for k, v in self.fixed_fields.bool_fields.items()}
        )
        ...
        for field_name, field_content in self.tensor_fields.items():
            ...
            if field_name == tensor_field.chunk_field_name:
                ...
                try:
                    ...
                    ] = list(field_content["blocks"].values())
        ...
        if self.fixed_fields.vespa_multimodal_params:
            ...
            for (
                multimodal_field_name,
                serialized_multimodal_params,
            ) in self.fixed_fields.vespa_multimodal_params.items():
                marqo_document[common.MARQO_DOC_MULTIMODAL_PARAMS][
                    multimodal_field_name
                ] = json.loads(serialized_multimodal_params)

        # Hybrid search raw scores
        if self.fixed_fields.raw_tensor_score is not None:
            marqo_document[index_constants.MARQO_DOC_HYBRID_TENSOR_SCORE] = (
                self.fixed_fields.raw_tensor_score
            )
        if self.fixed_fields.raw_lexical_score is not None:
            marqo_document[index_constants.MARQO_DOC_HYBRID_LEXICAL_SCORE] = (
                self.fixed_fields.raw_lexical_score
            )

        return marqo_document

    @classmethod
    def _verify_marqo_tensor_field_name(
        cls, field_name: str, marqo_index: SemiStructuredMarqoIndex
    ):
        tensor_field_map = marqo_index.tensor_field_map
        if field_name not in tensor_field_map:
            raise InvalidFieldNameError(
                f"Invalid tensor field name {field_name} for index {marqo_index.name}. "
