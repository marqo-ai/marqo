    ...
    def validate_model(cls, values):
        ...
        for tensor_field in tensor_field_names:
            if tensor_field not in field_names:
                raise ValueError(
                    f"Tensor field '{tensor_field}' is not a defined field. "
        ...
        multimodal_fields = [
            field
            for field in fields
            if field.type == marqo_index.FieldType.MultimodalCombination
        ]
        custom_vector_fields = [
            field
            for field in fields
            if field.type == marqo_index.FieldType.CustomVector
        ]
        ...
        for field in must_be_tensor_fields:
            if field.name not in tensor_field_names:
                raise ValueError(
                    f"Field '{field.name}' has type '{field.type.value}' and must be a tensor field."
                )
