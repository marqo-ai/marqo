    ...
    def validate_properties(cls, values):
        ...
        if values.get("alpha") is None:
            ...
            if values.get("rankingMethod") not in fusion_ranking_methods:
                raise ValueError(
                    "'alpha' can only be defined for 'rrf' ranking method"
                )  # TODO: Re-add normalize linear
        ...
        if values.get("searchableAttributesLexical") is not None:
            if not (
                values.get("retrievalMethod")
                in [RetrievalMethod.Lexical, RetrievalMethod.Disjunction]
                or values.get("rankingMethod") == RankingMethod.Lexical
            ):
                raise ValueError(
        ...
        if values.get("searchableAttributesTensor") is not None:
            if not (
                values.get("retrievalMethod")
                in [RetrievalMethod.Tensor, RetrievalMethod.Disjunction]
                or values.get("rankingMethod") == RankingMethod.Tensor
            ):
                raise ValueError(
        ...
        if values.get("scoreModifiersLexical") is not None:
            if not (
                values.get("rankingMethod")
                in [RankingMethod.Lexical, RankingMethod.RRF]
                or values.get("retrievalMethod") == RetrievalMethod.Lexical
            ):
                raise ValueError(
                    "'scoreModifiersLexical' can only be defined for 'lexical', 'rrf' ranking methods or "
                    "'lexical' retrieval method."
                )  # TODO: re-add normalize_linear

        # score_modifiers_tensor can only be defined for Tensor, RRF, NormalizeLinear
        if values.get("scoreModifiersTensor") is not None:
            if values.get("rankingMethod") not in [
                RankingMethod.Tensor,
                RankingMethod.RRF,
            ]:
                raise ValueError(
                    "'scoreModifiersTensor' can only be defined for 'tensor', 'rrf', ranking methods"
                )  # TODO: re-add normalize_linear
        ...
        if values.get("retrievalMethod") in [
            ...
            if values.get("rankingMethod") not in [
                ...
                raise ValueError(
                    "For retrievalMethod: tensor or lexical, rankingMethod must be: tensor or lexical"
                )
