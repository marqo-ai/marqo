...
from marqo.base_model import ImmutableBaseModelV2, ImmutableStrictBaseModelV2
...
class TypeaheadRequest(ImmutableStrictBaseModelV2):
    ...
    limit: int = Field(
        default=10, gt=0, description="Maximum number of suggestions to return"
    )
    ...
    bm25_weight: Optional[float] = Field(
        default=None, alias="bm25Weight", description="Weight for BM25 score in ranking"
    )
...
class TypeaheadSuggestion(ImmutableStrictBaseModelV2):
    ...
    score: float = Field(
        ..., alias="_score", description="Relevance score for the suggestion"
    )
...
class TypeaheadResponse(ImmutableStrictBaseModelV2):
    ...
    suggestions: List[TypeaheadSuggestion] = Field(
...
class TypeaheadIndexingRequest(ImmutableStrictBaseModelV2):
    ...
    def validate_queries_batch_size(cls, queries):
        ...
        max_queries = read_env_vars_and_defaults_ints(
...
class TypeaheadIndexingResponse(ImmutableStrictBaseModelV2):
    ...
    errors: List[TypeaheadIndexingError] = Field(
        default_factory=list, description="Index Errors"
    )
    processing_time_ms: float = Field(
        alias="processingTimeMs", description="Processing time in milliseconds"
    )


class TypeaheadStatsResponse(ImmutableStrictBaseModelV2):
    indexed_queries: int = Field(
        alias="indexedQueries", description="Number of indexed queries"
    )
...
class TypeaheadQuery(ImmutableBaseModelV2):
    ...
    query_words: List[str] = Field(
        default_factory=list,
        alias="queryWords",
        description="The normalised query splits into words",
    )
    query_index: str = Field(
        default="",
        alias="queryIndex",
        description="Substrings of the query used for lexical matching",
    )
    popularity: float = Field(default=0.0, description="Popularity score")
    metadata: Dict[str, float] = Field(
        default_factory=dict, description="Additional metadata"
    ...
    last_updated_at: Optional[int] = Field(
        default=None, alias="lastUpdatedAt", description="Last updated timestamp"
    )
