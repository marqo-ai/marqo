...
class RangeConfiguration(StrictBaseModel):
    class Config:
        allow_population_by_field_name = (
            False  # disable ability to pass from_ or to_, only alias
        )
...
class FieldFacetsConfiguration(StrictBaseModel):
    class Config:
        allow_population_by_field_name = (
            False  # disable ability to pass max_results, only alias
        )
    ...
    def validate_ranges_overlap(cls, ranges):
        if ranges:
            ...
            sorted_ranges = sorted(
                ranges,
                key=lambda x: (x.from_ if x.from_ is not None else float("-inf")),
            )
            ...
            for i in range(len(sorted_ranges) - 1):
                ...
                if current.to_ is None:
                    raise ValueError(
                        "Open-ended ranges (missing 'to' value) will overlap with subsequent ranges"
                    )

                # If next_range.from_ is None, it extends from -infinity and will overlap with any previous range
                if next_range.from_ is None:
                    raise ValueError(
                        "Open-ended ranges (missing 'from' value) will overlap with previous ranges"
                    )
...
class FacetsParameters(StrictBaseModel):
    class Config:
        allow_population_by_field_name = (
            False  # disable ability to pass max_depth or max_results, only alias
        )
