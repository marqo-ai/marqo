...
class StructuredVespaSchema(VespaSchema):
    ...
    def _generate_document_section(
        self, schema_name: str
    ) -> (List[str], StructuredMarqoIndex):
        """
        ...
        for tensor_field_request in self._index_request.fields:
            ...
            if FieldFeature.LexicalSearch in tensor_field_request.features:
                ...
                document.append("indexing: index | summary")
            ...
            if (
                FieldFeature.LexicalSearch not in tensor_field_request.features
                and FieldFeature.Filter not in tensor_field_request.features
            ):
        ...
        # score modifiers
        if any(
            FieldFeature.ScoreModifier in f.features for f in self._index_request.fields
        ):
            document.append(
        ...
        for tensor_field_request in self._index_request.tensor_fields:
            ...
            embedding_field_name = (
                f"{self._FIELD_EMBEDDING_PREFIX}{tensor_field_request}"
            )
            document.append(f"field {chunks_field_name} type array<string> {{")
            document.append("indexing: attribute | summary")
            document.append("}")
            document.append(
                f"field {embedding_field_name} type tensor<float>(p{{}}, x[{model_dim}]) {{"
            )
            ...
            document.append(
                f"neighbors-to-explore-at-insert: {self._index_request.hnsw_config.ef_construction}"
            )
            document.append("}}")
            document.append("}")
        ...
        # vector count field
        document.append(
            f"field {common.FIELD_VECTOR_COUNT} type int {{ indexing: attribute | summary }}"
        )
    ...
    def _generate_max_similarity_expression(
        self, tensor_fields: List[TensorField]
    ) -> str:
        """
    ...
    def _generate_rank_profiles(self, marqo_index: StructuredMarqoIndex) -> List[str]:
        ...
        bm25_sum_expression = " + ".join(
            [
                f"if (query({field.lexical_field_name}) > 0, bm25({field.lexical_field_name}), 0)"
                for field in lexical_fields
            ]
        )
        bm25_avg_denominator_sum = " + ".join(
            [f"bm25({field.lexical_field_name})" for field in lexical_fields]
        )
        bm25_avg_expression = (
            f"({bm25_sum_expression}) / max(1, "
            + " + ".join(
                [
                    f"if (query({field.lexical_field_name}) > 0, 1, 0)"
                    for field in lexical_fields
                ]
            )
            + ")"
        ...
        embedding_similarity_expression = self._generate_max_similarity_expression(
            tensor_fields
        )
        ...
        embedding_match_features_expression = [
            f"match-features inherits {common.RANK_PROFILE_BASE} {{"
        ]
        for field in marqo_index.tensor_fields:
            embedding_match_features_expression.append(
                f"closest({field.embeddings_field_name})"
            )
            embedding_match_features_expression.append(
                f"distance(field, {field.embeddings_field_name})"
            )
        embedding_match_features_expression.append("}")

        # Base rank profile
        rank_profiles.append(
            f"rank-profile {common.RANK_PROFILE_BASE} inherits default {{"
        )
        ...
        rank_profiles.append(
            f"query({common.QUERY_INPUT_EMBEDDING}) tensor<float>(x[{model_dim}])"
        )
        ...
        rank_profiles.append(
            f"   expression: mult_modifier(query({constants.QUERY_INPUT_SCORE_MODIFIERS_MULT_WEIGHTS_GLOBAL}))"
        )
        rank_profiles.append("}")

        rank_profiles.append("function global_add_modifier() {")
        rank_profiles.append(
            f"   expression: add_modifier(query({constants.QUERY_INPUT_SCORE_MODIFIERS_ADD_WEIGHTS_GLOBAL}))"
        )
        rank_profiles.append("}")
        ...
        if lexical_fields:
            rank_profiles.append(
                f"rank-profile {common.RANK_PROFILE_BM25} inherits {common.RANK_PROFILE_BASE} {{"
            )
        ...
        if tensor_fields:
            rank_profiles.append(
                f"rank-profile {common.RANK_PROFILE_EMBEDDING_SIMILARITY} "
            ...
            rank_profiles.append("}")
            rank_profiles.extend(
                embedding_match_features_expression
            )  # This is a multi-line list, so we extend
            rank_profiles.append("}")

        # Hybrid search
        if lexical_fields and tensor_fields:
            rank_profiles.append(
                f"rank-profile {common.RANK_PROFILE_HYBRID_CUSTOM_SEARCHER} inherits default {{"
            )
            ...
            rank_profiles.append(
                f"query({common.QUERY_INPUT_HYBRID_FIELDS_TO_RANK_LEXICAL}) tensor<int8>(p{{}})"
            )
            rank_profiles.append(
                f"query({common.QUERY_INPUT_HYBRID_FIELDS_TO_RANK_TENSOR}) tensor<int8>(p{{}})"
            )
            rank_profiles.append(
                f"query({constants.QUERY_INPUT_SCORE_MODIFIERS_MULT_WEIGHTS_LEXICAL}) tensor<double>(p{{}})"
            )
    ...
    def _generate_default_fieldset(
        self, marqo_index: StructuredMarqoIndex
    ) -> List[str]:
    ...
    def _generate_summaries(self, marqo_index: StructuredMarqoIndex) -> List[str]:
        ...
        non_vector_summary_fields.append(f"summary {common.FIELD_ID} type string {{ }}")
