...
class StructuredAddDocumentsHandler(AddDocumentsHandler):
    ...
    def _create_tensor_fields_container(self) -> TensorFieldsContainer:
        multimodal_combo_fields = {
            field.name: field.dependent_fields
            for field in self.marqo_index.field_map_by_type[
        ...
        if self.add_docs_params.mappings:
            ...
            for field_name, mapping in self.add_docs_params.mappings.items():
                if (
                    mapping.get("type", None) == FieldType.MultimodalCombination
        ...
        return TensorFieldsContainer(
            ...
            custom_vector_fields=[
                field.name
                for field in self.marqo_index.field_map_by_type[FieldType.CustomVector]
            ],
            multimodal_combo_fields=multimodal_combo_fields,
            should_normalise_custom_vector=self.should_normalise_custom_vector,
        )

    def _validate_add_docs_params(
        self, add_docs_params: AddDocsParams, marqo_index: StructuredMarqoIndex
    ):
        if add_docs_params.tensor_fields is not None:
            raise InvalidArgumentError(
                "Cannot specify 'tensorFields' when adding documents to a "
    ...
    def _infer_modality(self, tensor_field: TensorField) -> Modality:
        ...
        if tensor_field.field_type == FieldType.Text:
            ...
            raise AddDocumentsError(
                f"Error processing {tensor_field.field_name}, tensor field type "
    ...
    def _validate_field(self, field_name: str, field_content: Any) -> None:
        try:
            ...
            if field_name not in self.marqo_index.field_map:
                raise AddDocumentsError(
                    f"Field {field_name} is not a valid field for structured index "
            ...
            if field_type == FieldType.MultimodalCombination:
                raise AddDocumentsError(
                    f"Field {field_name} is a multimodal combination field and cannot "
            ...
            validation.validate_field_content(
                field_content=field_content, is_non_tensor_field=not is_tensor_field
            )
