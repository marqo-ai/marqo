...
class Embed:
    ...
    def embed_content(
        ...
        temp_config = config.Config(
            vespa_client=self.vespa_client, inference=self.inference
        )
        ...
        marqo_index = index_meta_cache.get_index(
            index_management=temp_config.index_management, index_name=index_name
        )
        ...
        if isinstance(content, List):
            ...
            raise base_exceptions.InternalError(
                f"Content type {type(content)} is not supported for embed endpoint."
            )
        ...
        if content_type == EmbedContentType.Query:
            ...
            marqo_index.model.text_query_prefix is None
            or marqo_index.model.text_chunk_prefix is None
        ):
            prefix = ""
        else:
            # use [item.value for item in list(EmbedContentType)], but formatted not as a list
            raise ValueError(
                f"Invalid content_type: {content_type}. Must be {', '.join([item.value for item in list(EmbedContentType)])}, or None."
            )
        ...
        with RequestMetricsStore.for_request().time(
            "embed.vector_inference_full_pipeline"
        ):
            qidx_to_vectors: Dict[Qidx, List[float]] = (
                tensor_search.run_vectorise_pipeline(temp_config, queries, device)
            )
        ...
        logger.debug(
            f"embed request completed with total processing time: {(time_taken):.3f}s."
        )
