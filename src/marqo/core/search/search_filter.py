...
class Modifier(Node, ABC):
    def __init__(self, modified: Union[Term, Operator], raw: str):
        ...
        if isinstance(modified, Operator) and modified.right is None:
            raise ValueError(
                "A modifier can only be applied to terms and expressions "
                "(operator with both left and right)"
            )
...
class RangeTerm(Term):
    def __init__(
        self,
        field: str,
        lower: Optional[RangeLimit],
        upper: Optional[RangeLimit],
        raw: str,
    ):
        super().__init__(field, raw)
        self.lower = lower
    ...
    def __repr__(self):
        return (
            f"{self.__class__.__name__}({repr(self.field)}, {repr(self.lower)}, {repr(self.upper)}, "
            f"{repr(self.raw)})"
        )
...
class InTerm(Term):
    ...
    def __eq__(self, other):
        return (
            ...
            set(self.value_list) == set(other.value_list)
            and self.raw == other.raw
        )
...
class MarqoFilterStringParser:
    ...
    def _join_term_value(
        self, term_value: Union[List[str], List[List[str]]]
    ) -> Union[str, List[str]]:
        """
    ...
    def parse(self, filter_string: str) -> SearchFilter:
        ...
        escape = False
        read_space_until = (
            None  # ignore space until reaching the value of this variable
        )
        ...
        while i < len(filter_string):
            ...
            if self._read_term_value and (
                c not in [" ", ")"]
                or read_space_until != None
                or self._in_term_value_after_comma
                or escape
            ):
                if self._reached_term_end:
                    self._error(
                        f"Expected end of term, but found '{c}'", filter_string, i
                    )
                ...
                if escape:
                    ...
                    c == "," and self._term_type == MarqoFilterStringParser._TermType.In
                ):
                    ...
                    if c == read_space_until:
                        ...
                        len(self._get_current_term_value()) == 0
                        and c == "("
                        and not read_space_until
                    ):  # start of term value
                        read_space_until = ")"
                    elif (
                        len(self._get_current_term_value()) == 0
                        ...
                        if self._term_type != MarqoFilterStringParser._TermType.In:
                            ...
                            self._error(
                                "Unexpected [ after IN operator.", filter_string, i
                            )
                    else:
                        self._append_to_term_value(c)
            ...
            if escape:
                ...
                if (
                    self._term_type == MarqoFilterStringParser._TermType.In
                    and not self._reached_term_end
                ):
                    self._current_token.append(c)
                    self._current_raw_token.append(c)
                    self._reached_term_end = True
                ...
                if self._term_divider_is_IN(i, filter_string):
                    ...
                    not self._reached_term_end
                    and self._term_type == MarqoFilterStringParser._TermType.In
                ):
                    # Found UNGROUPED white space in IN term list. This should be an error.
                    self._error(
                        "Unexpected white space in IN term list", filter_string, i
                    )
                else:
        ...
        if token == "AND":
            ...
            if not self._is_expression(prev):
                if not isinstance(prev, Node):
                    ...
                    self._error(
                        "Expected term or expression, but found AND",
                        filter_string,
                        pos - len(raw_token),
                    )
            ...
            if not self._is_expression(prev):
                if not isinstance(prev, Node):
                    ...
                    # Consecutive operators or operator after modifier
                    self._error(
                        "Expected term or expression, but found OR",
                        filter_string,
                        pos - len(raw_token),
                    )
            ...
            if not (
                self._is_start_of_expression(prev)
                or self._is_modifier(prev)
                or self._is_operator(prev)
            ):
                # Term after expression
                self._error(
                    f"Unexpected term '{token}'. Expected an operator",
                    filter_string,
                    pos - len(raw_token),
                )
            ...
            if term_type == self._TermType.Equality:
                ...
                try:
                    ...
                    self._error(
                        f"Cannot parse range term '{token}': {str(e)}",
                        filter_string,
                        pos,
                    )
    ...
    def _merge_expression(
        self, stack: List[Union[str, Node]], filter_string: str, pos: int
    ):
        ...
        while len(stack) > 1:
            ...
            if prev == "(":
                ...
                if not (
                    len(stack) == 0 or stack[-1] == "(" or self._is_operator(stack[-1])
                ):
                    self._error(
            ...
            if not isinstance(prev, Operator):
                # Expected to be unreachable
                self._error(
                    f"Unexpected term {prev.raw} in expression ending at position {pos}"
                )
    ...
    def _merge_stack(self, stack: List[Union[str, Node]], filter_string: str):
        ...
        if not self._is_expression(last):
            ...
            self._error(
                ...
                filter_string,
                len(filter_string) - len(str(last.raw)),
            )
    ...
    def _is_expression(self, node: Node):
        return (
            isinstance(node, Term)
            or isinstance(node, Operator)
            and node.right is not None
            or isinstance(node, Modifier)
            and node.modified is not None
        )
    ...
    def _error(
        self, msg, filter_string: Optional[str] = None, pos: Optional[int] = None
    ):
