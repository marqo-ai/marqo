...
slow_query_threshold_ms = float(
    utils.read_env_vars_and_defaults(EnvVars.MARQO_SLOW_QUERY_THRESHOLD_MS)
)
log_query_details = (
    utils.read_env_vars_and_defaults(EnvVars.MARQO_LOG_QUERY_DETAILS).upper() == "TRUE"
)
log_query_max_length = int(
    utils.read_env_vars_and_defaults(EnvVars.MARQO_LOG_QUERY_MAX_LENGTH)
)
...
class QueryLogger:
    ...
    def sanitised_query(self) -> dict:
        ...
        * Truncating long query string
        """
        query_dict = self.search_query.dict(
            by_alias=True, exclude_none=True, skip_defaults=True, exclude=SECRET_FIELDS
        )
        ...
        if isinstance(q, CustomVectorQuery):
            if q.customVector.content:
                query_dict["q"]["customVector"]["content"] = _truncate_if_long(
                    q.customVector.content
                )
            ...
            if self.search_query.hybridParameters.queryTensor:
                query_dict["hybridParameters"]["queryTensor"] = (
                    _sanitise_str_or_dict_query(
                        self.search_query.hybridParameters.queryTensor
                    )
                )
            if self.search_query.hybridParameters.queryLexical:
                query_dict["hybridParameters"]["queryLexical"] = (
                    _sanitise_str_or_dict_query(
        ...
        if self.search_query.context and self.search_query.context.tensor:
            sanitised_context_tensor = [
                {"vector": [], "weight": tensor.weight}
                for tensor in self.search_query.context.tensor
            ]
    ...
    def log_error_query(self, error_message: str):
        if log_query_details:
            marqo_query_logger.error(
                f"Failed search query: Error: {error_message}. Query: {self.sanitised_query}"
            )
    ...
    def log_slow_query(self, elapsed_time_ms: float):
        if (
            log_query_details
            and not self.error_logged
            and elapsed_time_ms > slow_query_threshold_ms
        ):
            marqo_query_logger.warning(
                f"Slow search query detected: {elapsed_time_ms:.1f}ms. "
