...
from marqo.tensor_search.tensor_search import (
    gather_documents_from_response,
    logger,
...
class HybridSearch:
    def search(
        ...
        if (
            isinstance(marqo_index, StructuredMarqoIndex)
            and marqo_index_version
            ...
            isinstance(marqo_index, UnstructuredMarqoIndex)
            and marqo_index_version
            < constants.MARQO_UNSTRUCTURED_HYBRID_SEARCH_MINIMUM_VERSION
        ):
            raise core_exceptions.UnsupportedFeatureError(
        ...
        if (
            ...
            and marqo_index_version
            < constants.MARQO_GLOBAL_SCORE_MODIFIERS_MINIMUM_VERSION
        ):
            raise core_exceptions.UnsupportedFeatureError(
        ...
        if (
            isinstance(marqo_index, UnstructuredMarqoIndex)
            and not isinstance(marqo_index, SemiStructuredMarqoIndex)
            and (
                hybrid_parameters.searchableAttributesTensor is not None
                or hybrid_parameters.searchableAttributesLexical is not None
            )
        ):
            raise core_exceptions.UnsupportedFeatureError(
                "Hybrid search for unstructured indexes currently does not support `searchableAttributesTensor` or "
                "`searchableAttributesLexical`. Please set these attributes to None."
            )

        if facets is not None and not isinstance(marqo_index, SemiStructuredMarqoIndex):
            raise core_exceptions.UnsupportedFeatureError(
                "Facets are only supported for unstructured indexes"
            )
        if track_total_hits is not None and not isinstance(
            marqo_index, SemiStructuredMarqoIndex
        ):
            raise core_exceptions.UnsupportedFeatureError(
                "trackTotalHits is only supported for unstructured indexes"
            )

        if query is not None and (
            hybrid_parameters.queryLexical is not None
            or hybrid_parameters.queryTensor is not None
        ):
            raise ValueError(
        ...
        if sort_by and (
            marqo_index_version < constants.MARQO_SORT_BY_MINIMUM_VERSION
            or not marqo_index.type == IndexType.SemiStructured
        ):
            raise core_exceptions.UnsupportedFeatureError(
        ...
        if query is None:
            ...
            if tensor_query is not None:
                if (
                    ...
                    and hybrid_parameters.rankingMethod == RankingMethod.Lexical
                ):
            ...
            if lexical_query is not None:
                if (
                    hybrid_parameters.retrievalMethod == RetrievalMethod.Tensor
                    and hybrid_parameters.rankingMethod == RankingMethod.Tensor
                ):
                    raise core_exceptions.InvalidArgumentError(
        ...
        if isinstance(query, CustomVectorQuery):
            ...
            if context is None:
                ...
                context.tensor.append(
                    SearchContextTensor(vector=tensor_query, weight=1)
                )
            ...
            if not (
                hybrid_parameters.retrievalMethod.upper() == SearchMethod.TENSOR
                and hybrid_parameters.rankingMethod.upper() == SearchMethod.TENSOR
            ):
                raise core_exceptions.InvalidArgumentError(
        ...
        if hybrid_parameters.retrievalMethod in [
            RetrievalMethod.Tensor,
            RetrievalMethod.Disjunction,
        ] or hybrid_parameters.rankingMethod in [
            RankingMethod.Tensor,
            RankingMethod.RRF,
        ]:
            with RequestMetricsStore.for_request().time(
                "search.hybrid.vector_inference_full_pipeline"
            ):
        ...
        if query_text_search:
            (required_terms, optional_terms) = utils.parse_lexical_query(
                query_text_search
            )
        else:
            required_terms = []
        ...
        marqo_query = MarqoHybridQuery(
            ...
            if hybrid_parameters.scoreModifiersLexical is not None
            else None,
            score_modifiers_tensor=hybrid_parameters.scoreModifiersTensor.to_marqo_score_modifiers()
            if hybrid_parameters.scoreModifiersTensor is not None
            else None,
        ...
        total_preprocess_time = RequestMetricsStore.for_request().stop(
            "search.hybrid.processing_before_vespa"
        )
        logger.debug(
        ...
        with RequestMetricsStore.for_request().time(
            ...
            try:
                ...
                if (
                    f"No profile named '{RANK_PROFILE_HYBRID_CUSTOM_SEARCHER}'"
                    in e.message
                ):
                    raise core_exceptions.InvalidArgumentError(
        ...
        if facets is not None or track_total_hits is not None:
            if isinstance(vespa_index, SemiStructuredVespaIndex):
                gathered_results.update(
                    vespa_index.gather_facets_from_response(responses, facets)
                )
            if facets is not None:
                for facet_field_name, facet_field_parameters in facets.fields.items():
                    ...
                    if (
                        facet_field_name not in gathered_results["facets"]
                        and facet_field_parameters.type == "array"
                    ):
                        gathered_results.get("facets", {}).update(
                            {facet_field_name: {}}
                        )
        ...
        if sort_by is not None:
            if (
                responses.root.fields.marqo_fields is None
                or responses.root.fields.marqo_fields.sort_candidates is None
            ):  # pragma: no cover
                raise core_exceptions.InternalError(
                    "'sortBy' feature is enabled, but Vespa did not return sortCandidates in the response "
                )
            gathered_results["_sortCandidates"] = (
                responses.root.fields.marqo_fields.sort_candidates
            )
        ...
        if relevance_cutoff is not None:
            if (
                ...
                raise core_exceptions.InternalError(
                    "'relevanceCutoff' feature is enabled, but Vespa did not return relevantCandidates or "
                    "probeCandidates in the response "
                )
            gathered_results["_relevantCandidates"] = (
                responses.root.fields.marqo_fields.relevant_candidates
            )
            gathered_results["_probeCandidates"] = (
                responses.root.fields.marqo_fields.probe_candidates
            )
