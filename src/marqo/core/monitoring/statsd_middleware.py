...
from marqo.core.monitoring.statsd_client import get_client
...
class StatsDMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        statsd.increment("requests.completed", tags={"status_code": status_tag})
        ...
        if _DOCS_RE.fullmatch(request.url.path):
            ...
            if request.method in {"POST", "PATCH", "GET"}:
                ...
                for hdr in ("x-count-success", "x-count-failure", "x-count-error"):
                    if hdr in lowered:
                        try:
                            statsd.increment(hdr, int(lowered[hdr]), tags={"method": request.method})
                        except ValueError:
