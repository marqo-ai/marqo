...
def determine_text_prefix(
    request_level_prefix: str,
    marqo_index: MarqoIndex,
    prefix_type: DeterminePrefixContentType,
) -> str:
    """
    ...
    if (
        prefix_type == DeterminePrefixContentType.TextQuery
        and marqo_index.override_text_query_prefix is not None
    ):
        return marqo_index.override_text_query_prefix
    elif (
        prefix_type == DeterminePrefixContentType.TextChunk
        and marqo_index.override_text_chunk_prefix is not None
    ):
    ...
    try:
        model_properties = (
            marqo_index.model.properties if marqo_index.model is not None else None
        )
        if model_properties is None and marqo_index.model is not None:
            model_properties = get_model_properties_from_registry(
                marqo_index.model.name
            )
        ...
        if model_properties is not None:
            ...
            raise ValueError(
                f"Model properties not found for model: {marqo_index.model.name}"
            )
