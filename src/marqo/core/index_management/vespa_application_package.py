...
class ServicesXml:
    ...
    def _ensure_only_one(self, xml_path: str) -> ET.Element:
        ...
        if len(elements) > 1:
            raise InternalError(
                f"Multiple {xml_path} elements found in services.xml. Only one is allowed"
            )
        if len(elements) == 0:
    ...
    def add_schema(self, name: str) -> None:
        if self._documents.find(f'document[@type="{name}"]') is not None:
            logger.warning(
                f"Schema {name} already exists in services.xml, nothing to add"
            )
        else:
    ...
    def remove_schema(self, name: str) -> None:
        ...
        if not docs:
            logger.warning(
                f"Schema {name} does not exist in services.xml, nothing to remove"
            )
        else:
            for doc in docs:
    ...
    def _should_preserve_container_element(self, element):
        ...
        if (
            ...
            == "com.yahoo.document.restapi.document-operation-executor"
        ):
            return True
    ...
    def _config_index_setting_components(self):
        ...
        index_setting_handler = self._add_component(
            container_elements, "handler", "ai.marqo.index.IndexSettingRequestHandler"
        )
        ...
        index_setting_component = self._add_component(
            container_elements, "component", "ai.marqo.index.IndexSettings"
        )
        config_element = ET.SubElement(index_setting_component, "config")
        config_element.set("name", "ai.marqo.index.index-settings")
        ET.SubElement(
            config_element, "indexSettingsFile"
        ).text = "marqo_index_settings.json"
        ET.SubElement(
            config_element, "indexSettingsHistoryFile"
        ).text = "marqo_index_settings_history.json"
    ...
    def compare_element(self, other: "ServicesXml", xml_path: str) -> bool:
        ...
        elements_self = sorted(
            [
                ET.tostring(normalize(elem), encoding="unicode")
                for elem in self._root.findall(xml_path)
            ]
        )
        elements_other = sorted(
            [
                ET.tostring(normalize(elem), encoding="unicode")
                for elem in other._root.findall(xml_path)
            ]
        )

        return len(elements_self) == len(elements_other) and all(
            x == y for x, y in zip(elements_self, elements_other)
        )
...
class IndexSettingStore:
    ...
    def __init__(self, index_settings_json: str, index_settings_history_json: str):
        self._index_settings: Dict[str, MarqoIndex] = {
            key: MarqoIndex.parse_obj(value)
            for key, value in json.loads(index_settings_json).items()
        }
    ...
    def to_json(self) -> (str, str):
        ...
        json_str = json.dumps(
            {
                key: json.loads(index.json())
                for key, index in self._index_settings.items()
            }
        )
        history_str = json.dumps(
            {
                key: [json.loads(index.json()) for index in history]
                for key, history in self._index_settings_history.items()
            }
        )
    ...
    def save_index_setting(self, index_setting: MarqoIndex) -> None:
        ...
        if name in self._index_settings:
            ...
            if current_version + 1 != target_version:
                raise OperationConflictError(
                    f"Conflict in version detected while saving index {name}. "
        ...
        self._index_settings[name] = index_setting.copy(
            deep=True, update={"version": target_version}
        )

    def delete_index_setting(self, index_setting_name: str) -> None:
        if index_setting_name not in self._index_settings:
            logger.warning(
                f"Index setting {index_setting_name} does not exist, nothing to delete"
            )
        else:
    ...
    def get_index(self, index_setting_name: str) -> MarqoIndex:
        return (
            self._index_settings[index_setting_name]
            if index_setting_name in self._index_settings
            else None
        )

    def _move_to_history(self, index_setting_name):
        if index_setting_name in self._index_settings_history:
            self._index_settings_history[index_setting_name].insert(
                0, self._index_settings[index_setting_name]
            )
            self._index_settings_history[index_setting_name] = (
                self._index_settings_history[index_setting_name][
...
class MarqoConfigStore:
    ...
    def __init__(self, marqo_config_json: str) -> None:
        self._config = (
            MarqoConfig.parse_obj(json.loads(marqo_config_json))
            if marqo_config_json
            else None
        )
...
class VespaAppBackup:
    ...
    def __repr__(self):
        return (
            f"<VespaAppBackup files_to_rollback={[os.path.join(*paths) for paths in self._files_to_rollback]} "
...
class VespaApplicationStore(ABC):
    ...
    def __init__(
        ...
        deploy_timeout: int,
        wait_for_convergence_timeout: int,
    ):
    ...
    def save_file(
        ...
        *paths: str,
        backup: Optional[VespaAppBackup] = None,
    ) -> None:
        """
...
class VespaApplicationFileStore(VespaApplicationStore):
    ...
    def remove_file(self, *paths: str, backup: Optional[VespaAppBackup] = None) -> None:
        ...
        if not os.path.exists(path):
            logger.warning(
                f"{path} does not exist in application package, nothing to delete"
            )
        else:
            if backup is not None:
                backup.backup_file(self.read_binary_file(*paths), *paths)
    ...
    def deploy_application(self) -> None:
        self._vespa_client.deploy_application(
            self._app_root_path, timeout=self._deploy_timeout
        )
        self._vespa_client.wait_for_application_convergence(
            timeout=self._wait_for_convergence_timeout
        )
...
class ApplicationPackageDeploymentSessionStore(VespaApplicationStore):
    ...
    def __init__(
        ...
        super().__init__(vespa_client, deploy_timeout, wait_for_convergence_timeout)
        self._content_base_url, self._prepare_url = (
            vespa_client.create_deployment_session(check_for_application_convergence)
        )
    ...
    def deploy_application(self) -> None:
        prepare_response = self._vespa_client.prepare(
            self._prepare_url, timeout=self._deploy_timeout
        )
        # TODO handle prepare configChangeActions
        # https://docs.vespa.ai/en/reference/deploy-rest-api-v2.html#prepare-session
        self._vespa_client.activate(
            prepare_response["activate"], timeout=self._deploy_timeout
        )
        self._vespa_client.wait_for_application_convergence(
            timeout=self._wait_for_convergence_timeout
        )
...
class VespaApplicationPackage:
    ...
    def __init__(self, store: VespaApplicationStore):
        ...
        self._service_xml = ServicesXml(
            self._store.read_text_file(self._SERVICES_XML_FILE)
        )
        self._marqo_config_store = MarqoConfigStore(
            self._store.read_text_file(self._MARQO_CONFIG_FILE)
        )
    ...
    def bootstrap(
        self, to_version: str, existing_index_settings: Optional[List[MarqoIndex]]
    ) -> None:
        """
        ...
        if not self._store.file_exists(self._MARQO_INDEX_SETTINGS_FILE):
            ...
            if existing_index_settings:
                logger.debug(
                    f"Migrating existing index settings {[index.json for index in existing_index_settings]}"
                )
                for index in existing_index_settings:
                    self._index_setting_store.save_index_setting(index)

            logger.debug(
                f"Persisting index settings: {self._index_setting_store.to_json()}"
            )
        ...
        self._store.save_file(
            self._service_xml.to_xml(), self._SERVICES_XML_FILE, backup=backup
        )

        logger.debug(
            f"Persisting marqo config: {self._marqo_config_store.get().json()}"
        )
        self._store.save_file(
            self._marqo_config_store.get().json(),
    ...
    def rollback(self, marqo_version: str) -> None:
        ...
        marqo_sem_version = semver.VersionInfo.parse(
            marqo_version, optional_minor_and_patch=True
        )
        app_sem_version = semver.VersionInfo.parse(
            vespa_app_version, optional_minor_and_patch=True
        )
        if marqo_sem_version >= app_sem_version:
            raise ApplicationRollbackError(
                f"Cannot rollback from ${app_sem_version} to ${marqo_sem_version}. "
                f"The target version must be lower than the current one."
            )

        if not self._store.file_exists(self._BACKUP_FILE):
            raise ApplicationRollbackError(
                f"{self._BACKUP_FILE} does not exist in current session, failed to rollback"
            )

        old_backup = VespaAppBackup(self._store.read_binary_file(self._BACKUP_FILE))
        logger.debug(f"Old backup {old_backup}")

        marqo_config = MarqoConfigStore(
            old_backup.read_text_file(self._MARQO_CONFIG_FILE)
        ).get()
        ...
        logger.info(
            f"Rolling the vector store back from {vespa_app_version} to {marqo_version}"
        )
    ...
    def batch_add_index_setting_and_schema(
        self, indexes: List[Tuple[str, str, MarqoIndex]]
    ) -> None:
        for schema, typeahead_schema, index in indexes:
            ...
            if index.typeahead_schema_name:
                self._store.save_file(
                    typeahead_schema, "schemas", f"{index.typeahead_schema_name}.sd"
                )
    ...
    def update_index_setting_and_schema(self, index: MarqoIndex, schema: str) -> None:
        ...
        self._index_setting_store.save_index_setting(
            index.copy(update={"version": version})
        )
        self._persist_index_settings()
        self._deploy()
    ...
    def _persist_index_settings(self):
        index_setting_json, index_setting_history_json = (
            self._index_setting_store.to_json()
        )
        self._store.save_file(index_setting_json, self._MARQO_INDEX_SETTINGS_FILE)
        self._store.save_file(
            index_setting_history_json, self._MARQO_INDEX_SETTINGS_HISTORY_FILE
        )
    ...
    def _add_missing_typeahead_schemas(self) -> None:
        ...
        for _, index in self._index_setting_store._index_settings.items():
            if (
                index.typeahead_schema_name is not None
                or index.parsed_marqo_version() < MARQO_TYPEAHEAD_SCHEMA_MINIMUM_VERSION
            ):
                # already has typeahead schema or the version is too old
                continue

            # Generate typeahead schema
            typeahead_schema, updated_index = TypeaheadVespaSchema(
                index
            ).generate_schema()
            logger.debug(
            ...
            self._store.save_file(
                typeahead_schema, "schemas", f"{updated_index.typeahead_schema_name}.sd"
            )
            # Add to services.xml
    ...
    def _configure_query_profiles(self, backup: VespaAppBackup) -> None:
        ...
        self._store.save_file(
            content, "search", "query-profiles", "default.xml", backup=backup
        )
    ...
    def _validate_services_xml_for_rollback(self, services_xml_backup: str) -> None:
        ...
        elements_to_check = [
            ...
            (
                "content/documents",
                "Indexes have been added or removed since last backup.",
            ),
        ...
        for xml_path, error_message in elements_to_check:
            if not self._service_xml.compare_element(services_xml_old, xml_path):
                logger.debug(
                    f"{error_message} services.xml in backup: {services_xml_old} vs. "
                    f"current service.xml: {self._service_xml}"
                )
                raise ApplicationRollbackError(
                    f"Aborting rollback. Reason: {error_message}"
                )
