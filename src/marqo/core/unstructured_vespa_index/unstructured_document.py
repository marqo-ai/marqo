...
from marqo.core.exceptions import MarqoDocumentParsingError, VespaDocumentParsingError
...
class UnstructuredVespaDocumentFields(MarqoBaseModel):
    ...
    long_string_fields: Dict[str, str] = Field(
        default_factory=dict, alias=unstructured_common.LONGS_STRINGS_FIELDS
    )
    short_string_fields: Dict[str, str] = Field(
        default_factory=dict, alias=unstructured_common.SHORT_STRINGS_FIELDS
    )
    string_arrays: List[str] = Field(
        default_factory=list, alias=unstructured_common.STRING_ARRAY
    )
    int_fields: Dict[str, int] = Field(
        default_factory=dict, alias=unstructured_common.INT_FIELDS
    )
    bool_fields: Dict[str, int] = Field(
        default_factory=dict, alias=unstructured_common.BOOL_FIELDS
    )
    float_fields: Dict[str, float] = Field(
        default_factory=dict, alias=unstructured_common.FLOAT_FIELDS
    )
    score_modifiers_fields: Dict[str, Any] = Field(
        default_factory=dict, alias=unstructured_common.SCORE_MODIFIERS
    )
    vespa_chunks: List[str] = Field(
        default_factory=list, alias=unstructured_common.VESPA_DOC_CHUNKS
    )
    vespa_embeddings: Dict[str, Any] = Field(
        default_factory=dict, alias=unstructured_common.VESPA_DOC_EMBEDDINGS
    )
    vespa_multimodal_params: Dict[str, str] = Field(
        default_factory=str, alias=unstructured_common.VESPA_DOC_MULTIMODAL_PARAMS
    )
    vector_counts: int = Field(default=0, alias=unstructured_common.FIELD_VECTOR_COUNT)

    match_features: Dict[str, Any] = Field(
        default_factory=dict, alias=unstructured_common.VESPA_DOC_MATCH_FEATURES
    )
    ...
    def extract_highlights(self) -> List[Dict[str, str]]:
        ...
        if (
            self.match_features[f"closest({unstructured_common.VESPA_DOC_EMBEDDINGS})"][
                "cells"
            ]
            == {}
        ):
            ...
            if not self.vespa_chunks:
                raise VespaDocumentParsingError(
                    f"Document {self.fields.marqo__id} does not have any chunks. "
            ...
            try:
                chunk_index: int = int(
                    list(
                        self.match_features[
                            f"closest({unstructured_common.VESPA_DOC_EMBEDDINGS})"
                        ]["cells"].keys()
                    )[0]
                )
            except KeyError:
                raise VespaDocumentParsingError(
                    "No match features found in the document"
                )
...
class UnstructuredVespaDocument(MarqoBaseModel):
    ...
    id: str
    fields: UnstructuredVespaDocumentFields = Field(
        default_factory=UnstructuredVespaDocumentFields
    )
    ...
    def from_vespa_document(cls, document: Dict) -> "UnstructuredVespaDocument":
        ...
        raw_lexical_score = (
            fields.pop(unstructured_common.VESPA_DOC_HYBRID_RAW_LEXICAL_SCORE)
            if unstructured_common.VESPA_DOC_HYBRID_RAW_LEXICAL_SCORE in fields
            else None
        )
        raw_tensor_score = (
            fields.pop(unstructured_common.VESPA_DOC_HYBRID_RAW_TENSOR_SCORE)
            if unstructured_common.VESPA_DOC_HYBRID_RAW_TENSOR_SCORE in fields
            else None
        )

        return cls(
            id=document[cls._VESPA_DOC_ID],
    ...
    def from_marqo_document(
        ...
        if index_constants.MARQO_DOC_ID not in document:
            raise MarqoDocumentParsingError(
                f"Unstructured Marqo document does not have a "
        ...
        instance = cls(
            id=doc_id, fields=UnstructuredVespaDocumentFields(marqo__id=doc_id)
        )

        for key, value in document.items():
            if key in [
                index_constants.MARQO_DOC_EMBEDDINGS,
                index_constants.MARQO_DOC_CHUNKS,
            ...
            if isinstance(value, str):
                ...
                instance.fields.string_arrays.extend(
                    [f"{key}::{element}" for element in value]
                )
                ...
                raise MarqoDocumentParsingError(
                    f"In document {doc_id}, field {key} has an "
                    f"unsupported type {type(value)} which has not been "
                    f"validated in advance."
                )

        instance.fields.vespa_multimodal_params = document.get(
            unstructured_common.MARQO_DOC_MULTIMODAL_PARAMS, {}
        )
        instance.fields.vespa_embeddings = document.get(
            index_constants.MARQO_DOC_EMBEDDINGS, {}
        )
        instance.fields.vespa_chunks = document.get(
            index_constants.MARQO_DOC_CHUNKS, []
        )
    ...
    def to_vespa_document(self) -> Dict[str, Any]:
        ...
        vespa_fields = {
            k: v
            for k, v in self.fields.dict(exclude_none=True, by_alias=True).items()
            if v or v == 0
        }
    ...
    def to_marqo_document(self, return_highlights: bool = False) -> Dict[str, Any]:
        ...
        if self.fields.vespa_chunks and self.fields.vespa_embeddings:
            try:
                ...
                raise VespaDocumentParsingError(
                    f"Can not parsing embeddings for document "
            ...
            if not len(self.fields.vespa_chunks) == len(embeddings_list):
                raise VespaDocumentParsingError(
                    f"Number of chunks and embeddings do not match "
            ...
            for chunk, embedding in zip(self.fields.vespa_chunks, embeddings_list):
                if "::" not in chunk:
                    raise VespaDocumentParsingError(
                        f"Chunk {chunk} does not have a field_name::content format"
                    )
        ...
        if self.fields.vespa_multimodal_params:
            ...
            ) in self.fields.vespa_multimodal_params.items():
                marqo_document[unstructured_common.MARQO_DOC_MULTIMODAL_PARAMS][
                    multimodal_field_name
