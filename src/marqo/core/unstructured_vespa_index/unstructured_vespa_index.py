...
class UnstructuredVespaIndex(VespaIndex):
    ...
    def to_vespa_partial_document(
        ...
        raise NotImplementedError(
            "Partial document update is not supported for unstructured indexes. This"
            "function should not be called."
        )

    def to_vespa_document(self, marqo_document: Dict[str, Any]) -> Dict[str, Any]:
        unstructured_document: UnstructuredVespaDocument = (
            UnstructuredVespaDocument.from_marqo_document(
                marqo_document, self._marqo_index.filter_string_max_length
            )
        )
    ...
    def to_marqo_document(
        ...
        unstructured_document: UnstructuredVespaDocument = (
            UnstructuredVespaDocument.from_vespa_document(vespa_document)
        )
        return unstructured_document.to_marqo_document(
    ...
    def to_vespa_query(self, marqo_query: MarqoQuery) -> Dict[str, Any]:
        if marqo_query.searchable_attributes is not None:
            raise errors.InvalidArgError(
                "Searchable attributes are not supported for unstructured indexes created "
        ...
        ):  # TODO: Rethink structure so order of checking doesn't matter
    ...
    def _to_vespa_tensor_query(self, marqo_query: MarqoTensorQuery) -> Dict[str, Any]:
        ...
        summary = (
            unstructured_common.SUMMARY_ALL_VECTOR
        ...
        if self._marqo_index_version < self._HYBRID_SEARCH_MINIMUM_VERSION:
            ranking = (
                unstructured_common.RANK_PROFILE_EMBEDDING_SIMILARITY_MODIFIERS_2_9
                if score_modifiers
        ...
        if self._marqo_index_version >= self._HYBRID_SEARCH_MINIMUM_VERSION:
            ...
            query_input_embedding_parameter = (
                unstructured_common.QUERY_INPUT_EMBEDDING_2_10
            )

        query_inputs = {query_input_embedding_parameter: marqo_query.vector_query}
    ...
    def _get_tensor_search_term(self, marqo_query: MarqoTensorQuery) -> str:
        ...
        additional_hits = max(
            marqo_query.ef_search - (marqo_query.limit + marqo_query.offset), 0
        )
    ...
    def _get_filter_term(cls, marqo_query: MarqoQuery) -> Optional[str]:
        ...
        def generate_equality_filter_string(node: search_filter.EqualityTerm) -> str:
            ...
            if node.value.lower() in cls._FILTER_STRING_BOOL_VALUES:
                ...
                bool_filter_string = (
                    f"({unstructured_common.BOOL_FIELDS} contains "
            ...
            short_string_filter_string = (
                f"({unstructured_common.SHORT_STRINGS_FIELDS} "
            ...
            string_array_filter_string = (
                f"({unstructured_common.STRING_ARRAY} contains "
        ...
        def generate_range_filter_string(node: search_filter.RangeTerm) -> str:
            ...
            float_field_string = (
                f"({unstructured_common.FLOAT_FIELDS} contains "
                f'sameElement(key contains "{node.field}", {bound}))'
            )

            int_field_string = (
                f"({unstructured_common.INT_FIELDS} contains "
                f'sameElement(key contains "{node.field}", {bound}))'
            )
    ...
    def _to_vespa_lexical_query(self, marqo_query: MarqoLexicalQuery) -> Dict[str, Any]:
        ...
        if self._marqo_index_version < self._HYBRID_SEARCH_MINIMUM_VERSION:
            ranking = (
                unstructured_common.RANK_PROFILE_BM25_MODIFIERS_2_9
                if score_modifiers
