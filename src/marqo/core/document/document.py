...
from marqo.core.structured_vespa_index.structured_add_document_handler import (
    StructuredAddDocumentsHandler,
)
...
class Document:
    ...
    def add_documents(
        ...
        if isinstance(marqo_index, StructuredMarqoIndex):
            add_docs_handler = StructuredAddDocumentsHandler(
                marqo_index, add_docs_params, self.vespa_client, self.inference
            )
        elif isinstance(marqo_index, SemiStructuredMarqoIndex):
            add_docs_handler = SemiStructuredAddDocumentsHandler(
                marqo_index,
                add_docs_params,
                self.vespa_client,
            ...
            add_docs_handler = UnstructuredAddDocumentsHandler(
                marqo_index, add_docs_params, self.vespa_client, self.inference
            )
        else:
    ...
    def delete_all_docs(self, marqo_index) -> int:
        ...
        res: DeleteAllDocumentsResponse = self.vespa_client.delete_all_docs(
            marqo_index.schema_name
        )
        return res.document_count

    def partial_update_documents_by_index_name(
        self, index_name, partial_documents: List[Dict]
    ) -> MarqoUpdateDocumentsResponse:
    ...
    def partial_update_documents(
        self, partial_documents: List[Dict], marqo_index
    ) -> MarqoUpdateDocumentsResponse:
        ...
        if marqo_index.type is IndexType.Unstructured:
            raise UnsupportedFeatureError(
                "Partial document update is not supported for unstructured indexes. "
        ...
        partial_documents, documents_that_contain_maps = self.process_documents(
            partial_documents,
            unsuccessful_docs,
            is_index_semi_structured=marqo_index.type is IndexType.SemiStructured,
        )
        ...
        if (
            marqo_index.type is IndexType.SemiStructured and documents_that_contain_maps
        ):  # Only retrieve the document back if the partial update request contains maps and the index is semi-structured
            get_batch_response = self.vespa_client.get_batch(
                ids=list(documents_that_contain_maps.keys()),
                fields=[
                    VESPA_FIELD_ID,
                    INT_FIELDS,
                    FLOAT_FIELDS,
            ...
            for resp in responses:
                if resp.document:
                    existing_vespa_documents[resp.document.fields[VESPA_FIELD_ID]] = (
                        resp.document.dict()
                    )
                else:
                    id = self.extract_document_id_from_vespa_id(
                        resp
                    )  # Extract the document id from the Vespa response. Vespa response will contain the document id even though the document was not found.
                    unsuccessful_docs.append(
                        (
                            documents_that_contain_maps.get(id),
                            MarqoUpdateDocumentsItem(
                                id=id,
                                status=int(api_exceptions.BadRequestError.status_code),
                                error="Marqo vector store couldn't update the document. Please see: "
        ...
        for index, doc in enumerate(partial_documents):
            if (
                marqo_index.type
                is IndexType.SemiStructured  # Only have this check in place for SemiStructured indexes
                and documents_that_contain_maps_but_dont_exist_in_vespa
                and doc.get(MARQO_DOC_ID, "")
                in documents_that_contain_maps_but_dont_exist_in_vespa
            ):
                continue
            try:
                vespa_document = VespaDocument(
                    **vespa_index.to_vespa_partial_document(
                        doc,
                        existing_vespa_documents.get(doc.get(MARQO_DOC_ID, ""), None),
                    )
                )
        ...
        with RequestMetricsStore.for_request().time("partial_update.vespa._bulk"):
            vespa_res: UpdateDocumentsBatchResponse = (
                self.vespa_client.update_documents_batch(
                    vespa_documents,
                    marqo_index.schema_name,
        ...
        with RequestMetricsStore.for_request().time("partial_update.postprocess"):
            result = self._translate_update_document_response(
                vespa_res, unsuccessful_docs, marqo_index.name, start_time
            )

        return result

    def _translate_update_document_response(
        self,
        responses: UpdateDocumentsBatchResponse,
        ...
        if responses is not None:
            for resp in responses.responses:
                ...
                new_item = MarqoUpdateDocumentsItem(
                    id=doc_id, status=status, message=message, error=message
                )
        ...
        return MarqoUpdateDocumentsResponse(
            errors=errors,
    ...
    def process_documents(
        ...
        documents: List[Dict],
        unsuccessful_docs: List[Tuple[int, MarqoUpdateDocumentsItem]],
        is_index_semi_structured=False,
        ...
        for i in range(len(documents) - 1, -1, -1):
            ...
            try:
                ...
                if doc_id is not None and doc_id in doc_ids:
                    logger.debug(
                        f"Duplicate document ID {doc_id} found, keeping the latest"
                    )
                    continue
                ...
                if is_index_semi_structured:
                    for field_name, field_value in doc.items():
                        if isinstance(field_value, dict):
                            if (
                                len(field_value) == 0
                            ):  # If the dictionary is empty, get back the document so that we can update the doc with an empty dictionary (i.e remove the map from the doc).
    ...
    def translate_add_documents_response(
        self,
        responses: Optional[FeedBatchResponse],
        index_name: str,
        unsuccessful_docs: List,
        ...
        if responses is not None:
            for resp in responses.responses:
                ...
                new_item = MarqoAddDocumentsItem(
                    id=doc_id, status=status, message=message
                )
