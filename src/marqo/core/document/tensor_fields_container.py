...
class TensorFieldContent(BaseModel):
    ...
    content_chunks: Optional[Union[List[str], List[Image]]] = None
    ...
    def chunk(self, chunkers: Dict[FieldType, Chunker]) -> List[str]:
        if self.field_type not in chunkers or self.is_resolved:
            return []

        chunker = chunkers[self.field_type]

        if ((self.is_tensor_field or self._is_audio_or_video()) and
                not self.chunks):  # if self.chunks has value, it's populated from existing tensor
            self.chunks, self.content_chunks = chunker(self.field_content, single_chunk=False)
        ...
        start_index_of_chunks_not_vectorised = 0 if not self.embeddings else len(self.embeddings) - 1
        return self.content_chunks[start_index_of_chunks_not_vectorised:]

    def vectorise(self, vectorisers: Dict[FieldType, Vectoriser]) -> None:
        if self.field_type not in vectorisers or self.is_resolved:
            return

        vectoriser = vectorisers[self.field_type]

        if not self.embeddings:
            self.embeddings = vectoriser(self.content_chunks)
        elif len(self.embeddings) < len(self.content_chunks):
            content_chunks_not_vectorised = self.content_chunks[len(self.embeddings) - 1:]
            self.embeddings.extend(vectoriser(content_chunks_not_vectorised))
