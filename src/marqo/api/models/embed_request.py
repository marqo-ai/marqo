...
class EmbedRequest(MarqoBaseModel):
    ...
    imageDownloadHeaders: Optional[Dict] = Field(
        default=None, alias="image_download_headers"
    )
    mediaDownloadHeaders: Optional[Dict] = None
    modelAuth: Optional[ModelAuth] = None
    content_type: Optional[EmbedContentType] = Field(
        default=EmbedContentType.Query, alias="contentType"
    )
    ...
    def validate_content(cls, value):
        ...
        if isinstance(value, str) or isinstance(value, dict):
            ...
            raise ValueError(
                "Embed content should be a string, a dictionary, or a list of strings or dictionaries"
            )
            ...
            if isinstance(item, str):
                ...
                for key in item:
                    if not isinstance(key, str):
                        raise ValueError(
                            "Keys in dictionary content should all be strings"
                        )
                    if not isinstance(item[key], float):
                        raise ValueError(
                            "Values in dictionary content should all be floats"
                        )
            else:
                raise ValueError(
                    "Embed content should be a string, a dictionary, or a list of strings or dictionaries"
