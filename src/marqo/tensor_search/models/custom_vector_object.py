...
class CustomVector(StrictBaseModel):
    ...
    def validate_custom_vector(cls, values):
        ...
        if extra_fields:
            raise ValueError(
                f"Custom vector field can only contain fields 'vector' and 'content'. Received unexpected extra fields: {extra_fields}."
            )
        ...
        if "vector" in dict_data:
            ...
            if not isinstance(vector, List):
                raise ValueError(
                    f"Custom vector field 'vector' must be a list, but given vector is of type {type(vector)}"
                )
            ...
            raise ValueError(
                f"Cannot create custom_vector field missing 'vector', which is a required key. "
        ...
        if dimension and len(vector) != dimension:
            raise ValueError(
                f"Custom vector must have dimension of {dimension}, but given vector is of length {len(vector)}"
            )
