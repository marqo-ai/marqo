...
class ScoreModifierLists(BaseModel):
    ...
    def __init__(self, **data):
        try:
            ...
            raise ScoreModifierValidationError(
                modifier=data, message=json.dumps(e.errors())
            )
    ...
    def at_least_one_must_be_provided(cls, v, values, field):
        ...
        if (
            field.name == "add_to_score"
            and v is None
            and values.get("multiply_score_by") is None
        ):
            raise ScoreModifierValidationError(
                modifier=values,
    ...
    def to_marqo_score_modifiers(self) -> List[ScoreModifier]:
        ...
        mult = (
            [
                ScoreModifier(
                    field=x.field_name, weight=x.weight, type=ScoreModifierType.Multiply
                )
                for x in self.multiply_score_by
            ]
            if self.multiply_score_by is not None
            else []
        )
        add = (
            [
                ScoreModifier(
                    field=x.field_name, weight=x.weight, type=ScoreModifierType.Add
                )
                for x in self.add_to_score
            ]
