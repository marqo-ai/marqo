...
class ScoreModifierOperator(BaseModel):
    ...
    def to_painless_script_and_params(self, i: int, operation: Literal["multiply_score_by", "add_to_score"]) -> Tuple[str, Dict]:
        ...
        if operation == "multiply_score_by":
            ...
            if (doc.containsKey('__chunks.{self.field_name}')) {{
                if (doc['__chunks.{self.field_name}'].size() > 0 &&
                    ...
                    _score = _score * doc['__chunks.{self.field_name}'].value * params.multiplier_weight_{i};
                }}
            }}
            """, {f"multiplier_weight_{i}": self.weight}
            ...
            if (doc.containsKey('__chunks.{self.field_name}')) {{     
                if (doc['__chunks.{self.field_name}'].size() > 0 &&
                    (doc['__chunks.{self.field_name}'].value instanceof java.lang.Number)) {{
                    additive = additive + doc['__chunks.{self.field_name}'].value * params.add_weight_{i};
                }}
            }}
            """, {f"add_weight_{i}": self.weight}
...
class ScoreModifier(BaseModel):
    ...
    def to_script_score(self) -> dict:
        ...
        if self.multiply_score_by:
            mult, mult_params = map(list, zip(*([x.to_painless_script_and_params(i, "multiply_score_by") for i, x in
