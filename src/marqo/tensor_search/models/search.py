...
Qidx = NewType(
    "Qidx", int
)  # Indicates the position of a search query in a bulk search request
JHash = NewType(
    "JHash", int
)  # hash of a VectoriseJob. Used for quick access of VectorisedJobs
...
class VectorisedJobs(BaseModel):
    ...
    def groupby_key(self) -> JHash:
        return VectorisedJobs.get_groupby_key(
            self.model_name,
    ...
    def get_groupby_key(
        ...
        return JHash(
            hash(model_name)
            + hash(json.dumps(model_properties, sort_keys=True))
            + hash(device)
            + hash(normalize_embeddings)
            + hash(modality)
    ...
    def add_content(self, content: List[Union[str, List[str]]]) -> VectorisedJobPointer:
        ...
        return VectorisedJobPointer(
            job_hash=self.groupby_key(), start_idx=start_idx, end_idx=len(self.content)
        )
...
class SearchContextDocumentsParameters(BaseModel):
    ...
    def check_tensor_fields_not_empty(cls, v):
        if v == []:
            raise ValueError(
                "context document tensorFields parameter must be non-empty list."
...
class SearchContextDocuments(BaseModel):
    ...
    parameters: Optional[SearchContextDocumentsParameters] = (
    ...
    def check_ids_not_empty(cls, v):
        if not v:
            raise ValueError(
                'context["documents"]["ids"] must be present and a non-empty dict of '
...
class SearchContext(BaseModel):
    tensor: Optional[List[SearchContextTensor]] = Field(min_items=1, max_items=64)
    ...
    def validate_at_least_one_context_exists(cls, values):
        ...
        if tensor is None and documents is None:
            raise ValueError(
                "At least 1 form of context (tensor or documents) must be provided"
            )
