...
class SearchQuery(BaseMarqoModel):
    ...
    imageDownloadHeaders: Optional[Dict] = Field(
        default_factory=None, alias="image_download_headers"
    )
    ...
    relevance_cutoff: Optional[RelevanceCutoffModel] = Field(
        default=None, alias="relevanceCutoff"
    )
    interpolationMethod: Optional[InterpolationMethod] = None
    collapse_fields: Optional[List[SearchCollapseField]] = Field(
        default=None, alias="collapseFields"
    )
    ...
    def _validate_image_download_headers_and_media_download_headers(cls, values):
        ...
        if image_download_headers and media_download_headers:
            raise ValueError(
                "Cannot set both imageDownloadHeaders(image_download_headers) and mediaDownloadHeaders. "
        ...
        if query is None:
            if search_method == SearchMethod.LEXICAL:
                ...
                if context is None and (
                    not hybrid_parameters
                    or (
                        hybrid_parameters.queryTensor is None
                        and hybrid_parameters.queryLexical is None
                    )
                ):
                    raise ValueError(
            ...
            if search_method == SearchMethod.HYBRID:
                if cls is SearchQuery:
                    ...
                    # parameters
                    if hybrid_parameters is not None and (
                        hybrid_parameters.queryTensor is not None
                        or hybrid_parameters.queryLexical is not None
                    ):
                        raise ValueError(
    ...
    def validate_hybrid_parameters_only_for_hybrid_search(cls, values):
        ...
        if (
            ...
            and search_method.upper() != SearchMethod.HYBRID
        ):
            raise ValueError(
                f"Hybrid parameters can only be provided for 'HYBRID' search. "
    ...
    def validate_rerank_depth(cls, values):
        ...
        if rerank_depth is not None:
            ...
            if (
                ...
                raise ValueError(
                    "'rerankDepth' is currently only supported for 'HYBRID' search with the 'RRF' rankingMethod."
                )
            if rerank_depth < 0:
                raise ValueError("rerankDepth cannot be negative.")
        if (
            hybrid_parameters
            and hybrid_parameters.rerankDepthTensor
            and hybrid_parameters.rerankDepthTensor < 0
        ):
            raise ValueError("rerankDepthTensor cannot be negative.")
    ...
    def validate_search_method(cls, value):
        return validation.validate_str_against_enum(
            value=value, enum_class=SearchMethod, case_sensitive=False
        )
    ...
    def validate_facets_only_for_hybrid_search(cls, values):
        ...
        if facets is not None and search_method.upper() != SearchMethod.HYBRID:
            raise ValueError(
                f"Facets can only be provided for 'HYBRID' search. "
                f"Search method is {search_method}."
            )
        return values
    ...
    def validate_facet_exclude_terms_in_filter(cls, values):
        ...
        if not filter_str:
            if any(field.exclude_terms for field in facets.fields.values()):
                raise ValueError(
                    "Exclude terms can only be used when a filter string is provided."
                )
            return values
        ...
        for field_name, field_params in facets.fields.items():
            ...
            missing_terms = [
                term
                for term in field_params.exclude_terms
    ...
    def validate_get_total_hits_only_for_hybrid_search(cls, values):
        ...
        if track_total_hits and search_method.upper() != SearchMethod.HYBRID:
            raise ValueError(
                f"trackTotalHits can only be provided for 'HYBRID' search. "
                f"Search method is {search_method}."
            )
        return values
    ...
    def validate_approximate_threshold(cls, values):
        ...
        if approximate_threshold is not None:
            ...
            if approximate_threshold < 0 or approximate_threshold > 1:
                raise ValueError(
                    f"'approximateThreshold' must be between 0 and 1, got {approximate_threshold}."
                )
    ...
    def _validate_relevance_cutoff_only_works_for_hybrid_search(cls, values):
        ...
        search_method = values.get("searchMethod")
        if (
            relevance_cutoff is not None
            and search_method.upper() != SearchMethod.HYBRID
        ):
            raise ValueError(
                f"relevanceCutoff can only be provided for 'HYBRID' search, but "
    ...
    def _validate_sort_by_only_works_for_hybrid_search(cls, values):
        ...
        if sort_by is not None and search_method.upper() != SearchMethod.HYBRID:
            raise ValueError(
                f"sortBy can only be provided for 'HYBRID' search, but "
                f"received search method {search_method}"
            )
        return values
    ...
    def _validate_sort_by_cannot_be_used_with_global_score_modifiers(cls, values):
        ...
        if sort_by is not None and score_modifiers is not None:
            raise ValueError(
                "'sortBy' cannot be used with 'scoreModifiers'(global score modifiers) in hybrid search "
    ...
    @root_validator(pre=False)
    def _validate_context_documents_not_supported_for_lexical_lexical_hybrid_search(
        cls, values
    ):
        ...
        if (
            context is not None
            and context.documents is not None
            and search_method == SearchMethod.HYBRID
            and hybrid_parameters is not None
        ):
    ...
    def _validate_and_set_sort_by_min_sort_candidates_parameters(cls, values):
        ...
        if sort_by.min_sort_candidates is None:
            ...
            if sort_by.min_sort_candidates < (
                values.get("offset") + values.get("limit")
            ):
                raise ValueError(
    ...
    def validate_collapse_fields_only_for_hybrid_search(cls, values):
        ...
        if collapse_fields is not None and search_method.upper() != SearchMethod.HYBRID:
            raise ValueError(
                f"collapseFields can only be provided for 'HYBRID' search. "
                f"Search method is {search_method}."
            )
        return values
