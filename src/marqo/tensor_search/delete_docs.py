...
from marqo.tensor_search import enums, utils, validation
...
def format_delete_docs_response(marqo_response: MqDeleteDocsResponse) -> dict:
    ...
    return {
        "index_name": marqo_response.index_name,
        "status": marqo_response.status_string,
        ...
        "duration": utils.create_duration_string(
            marqo_response.deletion_end - marqo_response.deletion_start
        ),
...
def delete_documents(config: Config, del_request: MqDeleteDocsRequest) -> dict:
    ...
    validation.validate_delete_docs_request(
        ...
        max_delete_docs_count=utils.read_env_vars_and_defaults_ints(
            enums.EnvVars.MARQO_MAX_DELETE_DOCS_COUNT
        ),
    )

    if config.backend == enums.SearchDb.vespa:
        del_response: MqDeleteDocsResponse = delete_documents_vespa(
            config=config, deletion_instruction=del_request
        )
    else:
        raise RuntimeError(
            f"Config set to use unknown backend `{config.backend}`. "
...
def delete_documents_vespa(
    config: Config, deletion_instruction: MqDeleteDocsRequest
) -> MqDeleteDocsResponse:
    """Deletes documents"""
    t0 = datetime.datetime.utcnow()
    responses = config.vespa_client.delete_batch(
        deletion_instruction.document_ids, deletion_instruction.schema_name
    )
