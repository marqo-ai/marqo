...
from marqo.core.inference.api import Inference, Modality, InferenceRequest, ModelConfig, ModelError, InferenceError, \
...
def vectorise_jobs(inference: Inference, jobs: List[VectorisedJobs]) -> Dict[JHash, Dict[str, List[float]]]:
    ...
    for v in jobs:
        ...
        try:
            # FIXME modality is inferred and saved when adding text query prefix, why do we infer it again here?
            ...
            inference_request = InferenceRequest(
                ...
                preprocessing_config=_get_preprocessing_config(modality, v.media_download_headers)
            )

            inference_result = inference.vectorise(inference_request)

            # TODO there shouldn't be any InferenceErrorModel in the response, check this.

            # TODO check the inference result has the same size as the content

            vectors = [per_content_result[0][1] for per_content_result in inference_result.result]
