...
from marqo.core.models.marqo_get_documents_by_id_response import (
...
from marqo.core.models.marqo_query import MarqoLexicalQuery, MarqoTensorQuery
from marqo.core.structured_vespa_index import common as structured_common
from marqo.core.structured_vespa_index.common import (
    RANK_PROFILE_BM25,
    RANK_PROFILE_EMBEDDING_SIMILARITY,
)
...
from marqo.tensor_search import delete_docs, index_meta_cache, utils, validation
from marqo.tensor_search.enums import Device, EnvVars, SearchMethod, TensorField
...
def _get_marqo_documents_by_ids(
    ...
    return [
        vespa_index.to_marqo_document(response.document.dict())
...
def get_documents_by_ids(
    config: Config,
    index_name: str,
    document_ids: typing.Collection[str],
    ...
    if not isinstance(document_ids, typing.Collection):
        raise api_exceptions.InvalidArgError(
            "Get documents must be passed a collection of IDs!"
        )
    if len(document_ids) <= 0:
        raise api_exceptions.InvalidArgError("Can't get empty collection of IDs!")

    max_docs_limit = utils.read_env_vars_and_defaults(
    ...
    if len(validated_ids) == 0:  # Can only happen when ignore_invalid_ids is True
        return MarqoGetDocumentsByIdsResponse(
            errors=True, results=[i[1] for i in unsuccessful_docs]
        )

    marqo_index = _get_latest_index(config, index_name)
    with RequestMetricsStore.for_request().time("get_documents.vespa"):
        batch_get = config.vespa_client.get_batch(
            validated_ids, marqo_index.schema_name
        )
    vespa_index = vespa_index_factory(marqo_index)
    ...
    for response in batch_get.responses:
        if response.status == 200:
            ...
            results.append({TensorField.found: True, **marqo_document})
        else:
            document = config.document
            status, message = document.vespa_client.translate_vespa_document_response(
                response.status, None
            )
            results.append(
                MarqoGetDocumentsByIdsItem(
                    id=_get_id_from_vespa_id(response.id),
                    status=status,
                    found=False,
                    message=message,
                )
...
def _get_latest_index(config: Config, index_name: str) -> MarqoIndex:
    ...
    marqo_index = index_meta_cache.get_index(
        index_management=config.index_management, index_name=index_name
    )
    ...
    for tensor_field in marqo_doc_tensors:
        ...
        for i in range(len(chunks)):
            tensor_facets.append(
                {tensor_field: chunks[i], TensorField.embedding: embeddings[i]}
            )
...
def rerank_query(
    ...
    result: Dict[str, Any],
    reranker: Union[str, Dict],
    device: str,
    num_highlights: int,
):
    if query.searchableAttributes is None:
        raise api_exceptions.InvalidArgError(
            "searchable_attributes cannot be None when re-ranking. Specify which fields to search and rerank over."
        )
    try:
...
def search(
    config: Config,
    index_name: str,
    text: Optional[Union[str, dict, CustomVectorQuery]],
    result_count: int = 3,
    offset: int = 0,
    ...
    if offset < 0:
        raise api_exceptions.IllegalRequestedDocCount(
            "search result offset cannot be less than 0!"
        )
    ...
    max_search_offset = utils.read_env_vars_and_defaults(
        EnvVars.MARQO_MAX_SEARCH_OFFSET
    )
    max_search_context_docs = utils.read_env_vars_and_defaults(
        EnvVars.MARQO_MAX_SEARCH_CONTEXT_DOCS
    )

    check_upper = (
        True if max_docs_limit is None else result_count + offset <= int(max_docs_limit)
    )
    check_limit = (
        True if max_search_limit is None else result_count <= int(max_search_limit)
    )
    check_offset = (
        True if max_search_offset is None else offset <= int(max_search_offset)
    )
    if not check_upper:
        upper_bound_explanation = (
            "The search result limit + offset must be less than or equal to the "
    ...
    validation.validate_context(
        context=context, query=text, search_method=search_method
    )
    validation.validate_boost(boost=boost, search_method=search_method)
    validation.validate_searchable_attributes(
        searchable_attributes=searchable_attributes, search_method=search_method
    )
    if searchable_attributes is not None:
        [
            validation.validate_field_name(attribute)
    ...
    if collapse_field_name is not None:
        ...
        if (
            marqo_index_version < constants.MARQO_COLLAPSE_FIELDS_MINIMUM_VERSION
            or not isinstance(marqo_index, SemiStructuredMarqoIndex)
        ):
            index_type = (
                "structured"
        ...
        if not marqo_index.is_collapse_field(collapse_field_name):
            raise api_exceptions.InvalidArgError(
                f"Field '{collapse_field_name}' is not configured as a collapse field "
                f"for this index"
            )
    ...
    if search_method.upper() in {SearchMethod.TENSOR, SearchMethod.HYBRID}:
        ...
        if context is not None and context.documents is not None:
            ...
            if context.documents.parameters.exclude_input_documents:
                filter = config.recommender.get_exclusion_filter(
                    marqo_index, list(context.documents.ids.keys()), filter
                )
        ...
        if ef_search is not None:
            raise errors.InvalidArgError(
                "efSearch is not a valid argument for lexical search"
            )
        if approximate is not None:
            raise errors.InvalidArgError(
                "approximate is not a valid argument for lexical search"
            )
        ...
        raise api_exceptions.InvalidArgError(
            f"Search called with unknown search method: {search_method}"
        )
    ...
    logger.debug(
        f"search ({search_method.lower()}) completed with total processing time: {(time_taken):.3f}s."
    )
...
def _lexical_search(
    ...
    marqo_query = MarqoLexicalQuery(
        ...
        searchable_attributes=searchable_attributes,
        attributes_to_retrieve=attributes_to_retrieve,
        score_modifiers=score_modifiers.to_marqo_score_modifiers()
        if score_modifiers
        else None,
        language=language,
    )
    ...
    total_preprocess_time = RequestMetricsStore.for_request().stop(
        "search.lexical.processing_before_vespa"
    )
    logger.debug(
        f"search (lexical) pre-processing: took {(total_preprocess_time):.3f}ms to process query."
    )

    with RequestMetricsStore.for_request().time(
        "search.lexical.vespa", lambda t: logger.debug(f"Vespa search: took {t:.3f}ms")
    ):
        try:
            ...
            if (
                f"does not contain requested rank profile '{RANK_PROFILE_BM25}'"
                in e.message
            ):
    ...
    gathered_docs = gather_documents_from_response(
        responses, marqo_index, False, attributes_to_retrieve
    )
    ...
    total_postprocess_time = RequestMetricsStore.for_request().stop(
        "search.lexical.postprocess"
    )
    logger.debug(
...
def construct_vector_input_batches(
    query: Optional[Union[str, Dict]], media_download_headers: Optional[Dict] = None
) -> QueryContentCollector:
...
def gather_documents_from_response(
    ...
    if (
        marqo_index.type in [IndexType.Unstructured, IndexType.SemiStructured]
        and attributes_to_retrieve is not None
    ):
        ...
        attributes_to_retrieve_set = set(attributes_to_retrieve).union(
            metadata_fields_to_retrieve
        )
    else:
    ...
    for doc in response.hits:
        if doc.id.startswith(
            "group:facet:"
        ):  # Not an actual document id but group's id returned by vespa
            continue
        marqo_doc = vespa_index.to_marqo_document(
            dict(doc), return_highlights=highlights
        )
...
def select_attributes(
    marqo_doc: Dict[str, Any], attributes_to_retrieve_set: Set[str]
) -> Dict[str, Any]:
    """
    ...
    return {
        k: v
        for k, v in marqo_doc.items()
        if k in attributes_to_retrieve_set
        or "." in k
        and k.split(".", maxsplit=1)[0] in attributes_to_retrieve_set
    }


def assign_query_to_vector_job(
    q: BulkSearchQueryEntity,
    jobs: Dict[JHash, VectorisedJobs],
    grouped_content: QueryContentCollector,
    index_info: MarqoIndex,
    device: str,
) -> List[VectorisedJobPointer]:
    """
    ...
    for i, list_of_queries_by_modalities in enumerate(content_lists_by_modality):
        if len(list_of_queries_by_modalities) > 0:
            content: List[str] = [
                query.content for query in list_of_queries_by_modalities
            ]
...
def create_vector_jobs(
    queries: List[BulkSearchQueryEntity], config: Config, device: str
) -> Tuple[Dict[Qidx, List[VectorisedJobPointer]], Dict[JHash, VectorisedJobs]]:
    """
    For each query:
    ...
    for i, q in enumerate(queries):
        ...
        to_be_vectorised: QueryContentCollector = construct_vector_input_batches(
            q.q, q.mediaDownloadHeaders
        )
        qidx_to_job[i] = assign_query_to_vector_job(
            q, jobs, to_be_vectorised, q.index, device
        )
...
def _get_preprocessing_config(
    modality: Modality, media_download_headers: Optional[Dict[str, str]]
):
    """
    ...
    if modality == Modality.TEXT:
        return (
            TextPreprocessingConfig()
        )  # the prefix has been added to the query, so we don't need to specify it here
...
def vectorise_jobs(
    inference: Inference, jobs: List[VectorisedJobs]
) -> Dict[JHash, Dict[str, List[float]]]:
    ...
    for v in jobs:
        ...
        try:
            inference_request = InferenceRequest(
                ...
                preprocessing_config=_get_preprocessing_config(
                    v.modality, v.media_download_headers
                ),
            )
            ...
            if len(inference_result.result) != len(v.content):
                raise InternalError(
                    f"Inference result contains embeddings for {len(inference_result.result)} "
                    f"query items, but {len(v.content)} is expected"
                )
            individual_errors = [
                f"{v.content[index]}: {r.error_message}"
                for index, r in enumerate(inference_result.result)
                if isinstance(r, InferenceErrorModel)
            ]
            if individual_errors:
                raise InternalError(
                    f"Individual errors returned when vectorising query string: {individual_errors}"
                )
            chunked_contents = [
                (v.content[index], len(chunks))
                for index, chunks in enumerate(inference_result.result)
                if len(chunks) > 1
            ]
            if chunked_contents:
                raise InternalError(
                    f"Tensor query string should not be chunked but some "
                    f"query items have multiple chunks: {chunked_contents}"
                )

            # The per_content_result format is [('chunk', np.array())]
            vectors = [
                per_content_result[0][1].tolist()
                for per_content_result in inference_result.result
            ]
            ...
            raise api_exceptions.InvalidArgError(
                message=f"Error vectorising content: {v.content}. Message: {e.message}"
            ) from e
    return result
...
def get_query_vectors_from_jobs(
    ...
    qidx_to_job: Dict[Qidx, List[VectorisedJobPointer]],
    ...
    for qidx, ptrs in qidx_to_job.items():
        ...
        if isinstance(q.q, dict) or q.q is None:
            ...
            if ordered_queries:
                ...
                collected_weights = [
                    weight for _, weight, _ in vectorised_ordered_queries
                ]
            ...
            if interpolation_method is None:
                interpolation_method = (
                    config.recommender.get_default_interpolation_method(
                        q.index, context_documents
                    )
                )

            if context_documents:
                with RequestMetricsStore.for_request().time(
                    "search.vectorise.get_doc_vectors_from_ids"
                ):
            ...
            with RequestMetricsStore.for_request().time(
                "search.vectorise.interpolate_vectors"
            ):
                if collected_vectors:
                    merged_vector = vector_interpolation.interpolate(
                        vectors=collected_vectors, weights=collected_weights
                    )
        ...
        if not result[qidx]:
            raise api_exceptions.InvalidArgError(
                "Marqo could not collect any vectors from the search query but the retrieval or ranking method requires "
                "at least one valid vector. "
                "Please check the provided query, context (if any), or queryTensor(for Hybrid search) "
            )
...
def get_content_vector(
    ...
    not_found_error = RuntimeError(
        f"get_content_vector(): could not find corresponding vector for content `{content}`"
    )
...
def add_prefix_to_queries(
    queries: List[BulkSearchQueryEntity],
) -> List[BulkSearchQueryEntity]:
    """
...
def run_vectorise_pipeline(
    ...
    vector_jobs_tuple: Tuple[
        Dict[Qidx, List[VectorisedJobPointer]], Dict[JHash, VectorisedJobs]
    ] = create_vector_jobs(prefixed_queries, config, device)
    ...
    with RequestMetricsStore.for_request().time(
        "search.vector.inference.vectorise_jobs"
    ):
        job_ptr_to_vectors: Dict[JHash, Dict[str, List[float]]] = vectorise_jobs(
            config.inference, list(jobs.values())
        )
...
def _vector_text_search(
    config: Config,
    marqo_index: MarqoIndex,
    query: Optional[Union[str, dict, CustomVectorQuery]],
    result_count: int = 5,
    offset: int = 0,
    ef_search: Optional[int] = None,
    ...
    if isinstance(query, CustomVectorQuery):
        if context is None:
            ...
            context.tensor.append(
                SearchContextTensor(vector=query.customVector.vector, weight=1)
            )
        query = None
    ...
    with RequestMetricsStore.for_request().time(
        ...
        qidx_to_vectors: Dict[Qidx, List[float]] = run_vectorise_pipeline(
            config, queries, device, interpolation_method
        )
    ...
    total_preprocess_time = RequestMetricsStore.for_request().stop(
        "search.vector.processing_before_vespa"
    )
    logger.debug(
    ...
    with RequestMetricsStore.for_request().time(
        "search.vector.vespa", lambda t: logger.debug(f"Vespa search: took {t:.3f}ms")
    ):
        try:
            responses = config.vespa_client.query(**vespa_query)
        except VespaStatusError as e:
    ...
    if not approximate and (
        responses.root.coverage.coverage < 100
        or responses.root.coverage.degraded is not None
    ):
    ...
    gathered_docs = gather_documents_from_response(
        responses, marqo_index, highlights, attributes_to_retrieve
    )

    if boost is not None:
        raise api_exceptions.MarqoWebError(
            "Boosting is not currently supported with Vespa"
        )

    total_postprocess_time = RequestMetricsStore.for_request().stop(
        "search.vector.postprocess"
    )
    logger.debug(
...
def get_loaded_models() -> dict:
    ...
    for ix in available_models:
        if isinstance(ix, str):
            message["models"].append(
                {"model_name": ix.split("||")[0], "model_device": ix.split("||")[-1]}
            )
    return message
...
def get_embedding_field_names(
    marqo_index: MarqoIndex, tensor_field_names: Optional[List[str]] = None
    ...
    if marqo_index.type in {IndexType.Structured, IndexType.SemiStructured}:
        ...
        if hasattr(marqo_index, "tensor_fields"):
                ...
                for tf_name in tensor_field_names:
                    if tf_name in index_tensor_field_names:
                        ...
                        requested_tensor_fields.append(
                            next(
                                tf
                                for tf in marqo_index.tensor_fields
                                if tf.name == tf_name
                            )
                        )
                    else:
            ...
            return (
                [tf.name for tf in requested_tensor_fields],
...
def get_doc_vectors_per_tensor_field_by_ids(
    ...
    viable_tensor_fields, embedding_fields = get_embedding_field_names(
    ...
    with RequestMetricsStore.for_request().time("get_document_vectors.vespa"):
        batch_get = config.vespa_client.get_batch(
            document_ids, marqo_index.schema_name, fields=fields_to_retrieve
        )
            ...
            for tf_idx in range(len(viable_tensor_fields)):
                ...
                if retrieved_embedding_field_name in raw_response_dict:
                    try:
                        ...
                        result[doc_id][marqo_tensor_field_name] = list(
                            raw_response_dict[retrieved_embedding_field_name][
                                "blocks"
                            ].values()
                        )
