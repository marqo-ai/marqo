...
class Device(ImmutableBaseModel):
    ...
    def cuda(cls, device_id, name, total_memory) -> "Device":
        return Device(
            id=device_id, name=name, type=DeviceType.CUDA, total_memory=total_memory
        )
...
class DeviceManager:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Found devices {self.devices}. Best available device set to: {self._best_available_device}."
        )

    def pick_and_validate_device(self, device: Optional[str] = None) -> str:
        if device is None:
            logger.debug(
                f"Device is not provided, pick the default device `{self._best_available_device}`"
            )
        ...
        raise DeviceError(
            f"`{device}` is not a valid device. Valid devices are {self.devices}"
        )
    ...
    def cuda_device_health_check(self) -> None:
        ...
        for device in self.cuda_devices:
            ...
            try:
                ...
                logger.debug(
                    f"CUDA device {device} with total memory {device.total_memory}. "
                ...
                if "out of memory" in str(e).lower():
                    logger.error(
                        f"CUDA device {device} is out of memory. Original error: {str(e)}. "
                        f"Memory stats: {str(memory_stats)}."
                    )
                    allocated_mem = (
                        memory_stats.get("allocated_bytes.all.current", None)
                        if memory_stats
                        else None
                    )
                    reserved_mem = (
                        memory_stats.get("reserved_bytes.all.current", None)
                        if memory_stats
                        else None
                    )
                    oom_errors.append(
                        f"CUDA device {device} is out of memory (reserved: {reserved_mem}, "
                        f"allocated: {allocated_mem}, total: {device.total_memory})"
                    )
                else:
                    # Log out a warning message when encounter other transient errors.
                    logger.error(
                        f"Encountered issue inspecting CUDA device {device}: {str(e)}"
                    )
