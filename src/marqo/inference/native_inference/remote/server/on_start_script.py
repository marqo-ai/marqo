...
from marqo.api import (
    exceptions,
)  # TODO EnvVarError and StartupSanityCheckError will need to be replicated here
...
from marqo.tensor_search import constants, utils
...
class CacheModels:
    ...
    def __init__(self, config: Config):
        ...
        warmed_models = utils.read_env_vars_and_defaults(
        ...
        self.logger.info(
            f"pre-loading {self.models} onto devices={self.default_devices}"
        )
    ...
    def run(self):
        ...
        for model in self.models:
            ...
            for device in self.default_devices:
                ...
                for n in range(N):
                    t0 = time.time()
                    _ = self._preload_model(
                        model=model, content=test_string, device=device
                    )
                    t1 = time.time()
                    t += t1 - t0
    ...
    def _preload_model(self, model, content, device):
        ...
        _ = self.config.local_inference.vectorise(
            InferenceRequest(
...
class CachePatchModels:
    ...
    def __init__(self):
        ...
        warmed_models = utils.read_env_vars_and_defaults(
            EnvVars.MARQO_PATCH_MODELS_TO_PRELOAD
        )
        if warmed_models is None:
            self.models = []
        elif isinstance(warmed_models, str):
        ...
        self.default_devices = (
            ["cpu"] if not torch.cuda.is_available() else ["cpu", "cuda"]
        )
    ...
    def run(self):
        ...
        test_image_pil = Image.fromarray(
            test_image.numpy().astype("uint8").transpose(1, 2, 0)
            ...
            for device in self.default_devices:
                ...
                with self.lock:
                    try:
                        # Warm it up
                        chunks = chunk_image(
                            test_image_pil, device=device, method=model
                        )
                        ...
                        self.logger.error(
                            f"Failed to prewarm model: {model} on device: {device}. Error: {e}"
                        )
...
class SetEnableVideoGPUAcceleration:
    ...
    def run(self):
        ...
        env_value = utils.read_env_vars_and_defaults(
            EnvVars.MARQO_ENABLE_VIDEO_GPU_ACCELERATION
        )
        if env_value is None:
            try:
                ...
                self.logger.debug(
                    f"Failed to use GPU acceleration for video processing. We will disable it. "
                    f"Original error message: {e}"
                )
                os.environ[EnvVars.MARQO_ENABLE_VIDEO_GPU_ACCELERATION] = "FALSE"
                logger.info("Video processing with GPU acceleration is disabled")
        elif env_value == "TRUE":
            self._check_video_gpu_acceleration_availability()
            logger.info("Video processing with GPU acceleration is enabled")
    ...
    def _check_video_gpu_acceleration_availability(self):
        ...
        ffmpeg_command_gpu_check = [
            ...
            "-f",
            "lavfi",  # Input format is a lavfi (FFmpeg's built-in filter)
            "-i",
            "nullsrc=s=200x100",  # Generate a blank video source of 200x100 resolution
            "-vframes",
            "1",  # Process only 1 frame
            "-c:v",
            "h264_nvenc",  # Use NVENC encoder
            "-f",
            "null",  # Output to null (discard the output)
