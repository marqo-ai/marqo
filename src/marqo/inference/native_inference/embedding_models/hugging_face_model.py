...
class HuggingFaceModel(AbstractEmbeddingModel):
    ...
    def _build_model_properties(
        ...
        if (
            self._model_flags.trust_remote_code
            or self._tokenizer_flags.trust_remote_code
        ):
    ...
    def _load_necessary_components(self):
        ...
        if self.model_properties.name:
            ...
            self.model_properties.model_location
            and self.model_properties.model_location.s3
        ):
            self._model, self._tokenizer = self._load_from_zip_file()
        elif (
            self.model_properties.model_location
            and self.model_properties.model_location.hf
        ):
            if self.model_properties.model_location.hf.filename:
                self._model, self._tokenizer = self._load_from_zip_file()
            else:
                self._model, self._tokenizer = (
                    self._load_from_private_hugging_face_repo()
                )
        else:
            raise InternalError(
                f"Invalid model properties: {self.model_properties}. Marqo can not load the model via "
    ...
    def _load_from_zip_file(self) -> Tuple:
        ...
        try:
            model = AutoModel.from_pretrained(
                model_dir, **self._model_flags.dict(exclude_none=True)
            )
            tokenizer = AutoTokenizer.from_pretrained(
