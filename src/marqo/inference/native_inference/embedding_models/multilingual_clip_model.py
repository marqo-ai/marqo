...
class MultilingualCLIPTokenizerWrapper:
    ...
    def __call__(
        self, inputs: list[str], padding: bool = True, return_tensors: str = "pt"
    ):
    ...
    def tokenize(
        ...
        return self.tokenizer(
            inputs, padding=padding, return_tensors=return_tensors
        ).to(self.device)
...
class MultilingualCLIPModel(AbstractCLIPModel):
    ...
    def _load_necessary_components(self):
        ...
        self._visual_model, _, self.image_preprocessor = (
    ...
    def _check_loaded_components(self):
        for component in [
            ...
            if component is None:
                raise InternalError(
                    f"The model component {component.__name__} is not loaded correctly."
                )
    ...
    def encode_image(
        self, images: List[Tensor], normalize: bool = True
    ) -> List[ndarray]:
        """
        Args:
            inputs:
