...
def get_presigned_s3_url(location: S3Location, auth: Optional[S3Auth] = None):
    ...
    try:
        return s3_client.generate_presigned_url(
            "get_object", Params=location.dict(exclude_unset=True)
        )
...
def get_s3_model_absolute_cache_path(
    location: S3Location, download_dir: Optional[str] = None
) -> str:
    ...
    cache_dir = os.path.expanduser(
        download_dir if download_dir is not None else ModelCache.clip_cache_path
    )
    return os.path.join(cache_dir, get_s3_model_cache_filename(location))


def check_s3_model_already_exists(
    location: S3Location, download_dir: Optional[str] = None
) -> bool:
